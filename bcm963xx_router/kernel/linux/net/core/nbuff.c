...
/*
 *------------------------------------------------------------------------------
 * Test whether SKB layout is as expected
 *------------------------------------------------------------------------------
 */
int skb_layout_test(int tail_offset, int end_offset, int head_offset)
{
#undef SKBOFFSETOF
#define SKBOFFSETOF(member)    ((int)&((struct sk_buff*)0)->member)

    if ( (SKBOFFSETOF(tail) == tail_offset) && 
         (SKBOFFSETOF(end)  == end_offset) && 
         (SKBOFFSETOF(head) == head_offset) )
        return 1;
    return 0;
}
                          ...
                          uint32_t len, uint32_t alignMask)
{
    ...
    else
    {
        ...
        {
#if 0
            printk("ERROR : FKB has no headroom (fkb_p 0x%08X, fkb_p->data 0x%08X)\n",
        ...
        *data_pp = fkb_data_p;
...
#if defined(CC_NBUFF_FLUSH_OPTIMIZATION)
        {
            ...
            fkb_p->dirty_p = _TO_DPTR_(tail_p);
        }
#endif
    }
