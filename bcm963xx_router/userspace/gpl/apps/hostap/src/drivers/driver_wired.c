...
#else 
//patch by yun
static u8 pae_group_addr[ETH_ALEN] =
{ 0x00, 0x24, 0x1d, 0x30, 0x37, 0x25 };
#endif
...
static void * wpa_driver_wired_init(void *ctx, const char *ifname)
{
	...
	int flags;
        ...
        wpa_printf(MSG_DEBUG, "--wpa_driver_wired_init");
        ...
        if (f)
        {
           ...
           len = strlen(line);
           wpa_printf(MSG_DEBUG, "--line=%s len=%d", line, len);
           if(len >= 17)
           { 
              sscanf(line, "%02x:%02x:%02x:%02x:%02x:%02x", &addr[0], &addr[1], &addr[2], &addr[3], &addr[4], &addr[5]);
              for (i=0; i<6; i++)
                   wpa_printf(MSG_DEBUG, "%x:", addr[i]);

              for (i=0; i<6; i++)
              {
                   pae_group_addr[i] = addr[i];
                   wpa_printf(MSG_DEBUG, "%x:", pae_group_addr[i]);
              }
           }
...
typedef struct mac_tb_s{
        char mac[24];
}mac_tb_t;

mac_tb_t global_mac_tb[64] = {{"\0"}};
static int global_mac_id = 0;

static int wpa_driver_wired_set_flags(void *priv, const u8 *addr, int total_flags, int flags_or, int flags_and)
{
        char cmd[1024] = "\0";
        int i; 
        char mac[24] = "\0";
        ...
        if (flags_or &WPA_STA_AUTHORIZED){
                ...
                for (i = 0; i < global_mac_id; i++){
                        if (!strcmp(global_mac_tb[i].mac, mac)){
                ...
                for(i = 0; i < global_mac_id; i++){
                        if (strlen(global_mac_tb[i].mac) < 1){
                                sprintf(global_mac_tb[global_mac_id ++].mac, "%s", mac);
                                sprintf(cmd, "ebtables -I INPUT 1 -s %s -j ACCEPT", mac);
                                system(cmd);
                                return 0;
                        }
                }
                ...
                if (global_mac_id >= 64){
                        printf("may extend the mac_tb length \n");
                        return 0;
                }
        ...
        if (!(flags_and &WPA_STA_AUTHORIZED)){
                printf("DROP MAC %s", mac);
                
                for (i = 0; i < global_mac_id; i++){
                        if (!strcmp(global_mac_tb[i].mac, mac)){
                                memset(global_mac_tb[i].mac, 0, sizeof(global_mac_tb[i].mac));
                                
                                sprintf(cmd, "ebtables -D INPUT  -s %s -j ACCEPT", mac);
                                system(cmd);
                                printf("DROP EXIST\n");
                                return 0;
                        }
                }
                printf("DROP NONEXIST\n");
        }
        return 0;
}

static int wpa_driver_wired_deauthenticate(void *priv, const u8 *own_addr, const u8 *addr, int reason_code)
{
        char cmd[1024] = "\0";
        char mac[24] = "\0";
        int i = 0;
...
const struct wpa_driver_ops wpa_driver_wired_ops = {
	...
	.hapd_send_eapol = wired_send_eapol,
        .sta_set_flags = wpa_driver_wired_set_flags,
        .sta_deauth = wpa_driver_wired_deauthenticate,
