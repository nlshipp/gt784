...
extern void sendEventMessage(UBOOL8 assigned, const char *ip, const char *mask, const char *gateway, const char *nameserver,  unsigned int lease_time);
#else
...
#if defined(AEI_VDSL_CUSTOMER_TELUS)
	  sendEventMessage(TRUE, local_ip, subnet_ip, router_ip, dns_ip, lease_time);
#else
