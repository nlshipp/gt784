...
#endif
{
    ...
    //ignore the signal of another process group.
    setpgrp();
#endif
	...
	act_getDhcpLeasesFromScratchPad();
#endif
	...
	while(1) { /* loop until universe collapses */
		...
		if (FD_ISSET(msg_fd, &rfds)) {
            ...
            case CMS_MSG_QOS_DHCP_OPT77_COMMAND:
               ...
               break;
#ifdef ACTION_TEC_DHCP_LEASE	
			case CMS_MSG_SET_LEASE_TO_SCRATCHPAD:
...
//brcm end
 		...
 		case DHCPREQUEST:
			...
			struct in_addr ttemp;
			if (cur_iface->reservedIPAddress != NULL)
			{
				if (requested)
				{
					ttemp.s_addr = requested_align;
					if (strstr(cur_iface->reservedIPAddress, inet_ntoa(ttemp)))
					{
						sendNAK(&packet);
						break;	
					}
				}		
					ttemp.s_addr = packet.ciaddr;
					if (strstr(cur_iface->reservedIPAddress, inet_ntoa(ttemp)))
					{
						sendNAK(&packet);
						break;
