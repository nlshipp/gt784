            ...
            if (cur_iface->skt < 0) {
                ...
                if(cur_iface->skt == -1) {
                    LOG(LOG_ERR, "couldn't create server "
                        ...
                        cur_iface->interface);
#if !defined(AEI_VDSL_CUSTOMER_CENTURYLINK) && !defined(AEI_VDSL_CUSTOMER_QWEST) //hk_qwest//hk_ctl
                        exit_server(0);
#else
...
#endif
#if defined(AEI_VDSL_CUSTOMER_TELUS)||defined(AEI_VDSL_CUSTOMER_MTS) //hk_telus
