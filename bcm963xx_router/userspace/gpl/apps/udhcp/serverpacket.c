...
int sendOffer(struct dhcpMessage *oldpacket)
{
	...
	if(!static_lease_ip) {
		   ...
		   if(lease !=NULL){
				...
				//cmsLog_error("the reserved ip is %s currently ip is %s", cur_iface->reservedIPAddress,inet_ntoa(temp));
		   }
		   if( lease !=NULL  && cur_iface->reservedIPAddress !=	NULL && !strstr(cur_iface->reservedIPAddress,inet_ntoa(temp)) ) {
#else 
