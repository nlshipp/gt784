...
void dalLan_getLanPageInfoEx(int index, WEB_NTWK_VAR *webVar);
...
#ifdef AEI_VDSL_CUSTOMER_NCS
/* set device icon */
CmsRet dalLan_setHostIcon(const WEB_NTWK_VAR *webVar);
#endif
...
#if  defined(AEI_VDSL_CUSTOMER_NCS)
CmsRet dalLan_StaticIPAdd(const char *brName, const char *static_ip, const char *mac, const char *manualflag);
#else
