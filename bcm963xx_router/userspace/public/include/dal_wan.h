...
#define ACT_DUMMY_PASSWORD "@2$[a#%]^&*b)(h3"
#endif
...
#ifdef AEI_VDSL_CUSTOMER_NCS
CmsRet fillWanPppconnectionByObj( WanPppConnObject *wanPppConnSrc, WanPppConnObject *wanPppConnDst,const WEB_NTWK_VAR *webVar);
CmsRet fillWanIpconnectionByObj( WanIpConnObject *wanIpConnSrc, WanIpConnObject *wanIpConnDst,const WEB_NTWK_VAR *webVar);
CmsRet addWanPppConnectionByObj(const WEB_NTWK_VAR *webVar,char *wanL2IfName, WanPppConnObject    *wanPppConn);
CmsRet addWanIpConnectionByObj(const WEB_NTWK_VAR *webVar,char *wanL2IfName, WanIpConnObject    *wanIpConn);
WanDslLinkCfgObject * getCurrentDslObjNew(void);
CmsRet deleteIpOrPppConnection(char *serviceIfcName, InstanceIdStack *wanConIid);
#endif
/** add a wan interface.
 *
