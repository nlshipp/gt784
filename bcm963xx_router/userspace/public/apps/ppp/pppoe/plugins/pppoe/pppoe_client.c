...
extern char servicename[BUFLEN_264]; /* service name from the connection,  defined in options.c */
	...
	FILE *fp;
	char acName[255] = {0};
	struct pppoe_tag *pt = NULL;

	pt = get_tag(p_in->hdr,PTT_AC_NAME);
	strncpy(acName, pt->tag_data, pt->tag_len);
	if((fp = fopen("/var/ppp/ac_name", "w")) != NULL)
	{	
		fprintf(fp, "%s", acName);
		fclose(fp);
	}
