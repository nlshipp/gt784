...
static int bcmxtmrt_sw_sched_init ( PBCMXTMRT_GLOBAL_INFO pGi ) ;
...
static void bcmxtmrt_swsched_recycle (XtmRtTxSchedContext *pXtmTxSchedCtxt, UINT32 ulCommand) ;
static void bcmxtmrt_swscheduler ( PBCMXTMRT_GLOBAL_INFO pGi ) ;
static void bcmxtmrt_swscheduler_command ( PBCMXTMRT_GLOBAL_INFO pGi ) ;
...
static void bcmxtmrt_swsched_force_disable_swdma (UINT32 ulPtmPrioIdx, BcmPktDma_XtmTxDma  *txSwdma);
static void bcmxtmrt_swsched_flush_swdma (UINT32 ulPtmPrioIdx, BcmPktDma_XtmTxDma  *txSwdma);
static void bcmxtmrt_swsched_force_flush_swdma (UINT32 ulPtmPrioIdx, BcmPktDma_XtmTxDma  *txSwdma);
static void bcmxtmrt_swsched_unset_swdma (UINT32 ulPtmPrioIdx, BcmPktDma_XtmTxDma  *txSwdma);
static void (*bcmxtmrt_swsched_timer1_int_cb) (PBCMXTMRT_GLOBAL_INFO data) ;
static PBCMXTMRT_GLOBAL_INFO bcmxtmrt_swsched_timer1_int_cb_data ;
static unsigned int bcmxtmrt_swsched_timer1_int(unsigned int i) ;
static int bcmxtmrt_compute_us_sched_timer_divpktsize (UINT32 usRate) ;
static int bcmxtmrt_init_swsched_timer1 (void (*cb)(PBCMXTMRT_GLOBAL_INFO cb_data), PBCMXTMRT_GLOBAL_INFO data,
                                         int timerInit) ;
static void bcmxtmrt_uninit_swsched_timer1 (void);
...
static int bcmxtmrt_ioctl(struct net_device *dev, struct ifreq *Req, int nCmd)
{
    ...
    case SIOCPORTMIRROR:
        ...
        else
        {
            ...
            {
                UpdateMirrorFlag(&(pDevCtx->usMirrorOutFlags),
                      mirrorCfg.szMirrorInterface, mirrorCfg.nStatus);
            }
#else
            if( mirrorCfg.nDirection == MIRROR_DIR_IN )
    ...
    case SIOCINETTRAFFICBLINK:
        if(copy_from_user((void*)&pDevCtx->inetTrafficBlinkEnable,data,sizeof(pDevCtx->inetTrafficBlinkEnable)))
            nRet=-EFAULT;
        break;
#endif
...
//spin_lock_bh(&pGi->xtmlock_tx);
        ...
        if( pDevCtx->ulTxQInfosSize )
        {
            ...
            if ((pDevCtx->Addr.ulTrafficType == TRAFFIC_TYPE_PTM) ||
               ...
               ulPtmPrioIdx = (uMark >> 4) & 0x1;

            txdma = pDevCtx->pTxPriorities[ulPtmPrioIdx][ulPort][uPriority];
            ...
            if (txdma && txdma->txEnabled == 1)
            {
                ...
                ulTxAvailable = bcmPktDma_XtmXmitAvailable(channel, TXDMATYPE(pDevCtx));
            }
            else
                ulTxAvailable = 0;
            ...
            if (ulTxAvailable && QueuePacket(pGi, txdma, isAtmCell))
            {
                ...
                if( pDevCtx->usMirrorOutFlags != 0 &&
                 ...
                 {
                     MultiMirrorPacket( pNBuffSkb, pDevCtx->usMirrorOutFlags );
                 }
#else
                ...
                if(IS_FKBUFF_PTR(pNBuff))
                {
                       ...
                       (pFkb->recycle_hook == (RecycleFuncP)enet_recycle_hook))
                    {
                        ...
                        bcmPktDma_XtmXmit(channel, pData, len, bufSource,
                                          dmaDesc.status, (uint32)key,
...
#else
   {
        ...
        if( pDevCtx->usMirrorInFlags != 0 &&
            (ulHdrType ==  HT_PTM ||
             ...
             ulHdrType ==  HT_VC_MUX_ETHERNET) )
        {
            MultiMirrorPacket( skb, pDevCtx->usMirrorInFlags );
        }
#endif
...
static void bcmxtmrt_timer( PBCMXTMRT_GLOBAL_INFO pGi )
{
    ...
    if ((pGi->bondConfig.sConfig.ptmBond == BC_PTM_BONDING_ENABLE)
       ...
       if (pGi->ptmBondInfo.tickCount >= MAX_TICK_COUNT) {
          ...
          bcmxtmrt_ptmbond_tick (&pGi->ptmBondInfo, 1) ;
          spin_unlock(&pGi->xtmlock_rx_bond);
          pGi->ptmBondInfo.tickCount = 1 ;
       }
       else
...
static int DoGlobInitReq( PXTMRT_GLOBAL_INIT_PARMS pGip )
{
   int ret ;
    ...
    if (pGi->bondConfig.sConfig.ptmBond == BC_PTM_BONDING_ENABLE) 
    {
      ...
      ret = bcmxtmrt_sw_sched_init (pGi) ;
    ...
    else
    {
       ...
       if (pGi->bondConfig.sConfig.ptmBond == BC_PTM_BONDING_ENABLE) 
       {
          /* Uninitialize the SW Scheduler timer */
           bcmxtmrt_uninit_swsched_timer1 () ;
        ...
        if (nRet == 0) 
        {
            ...
            for( i = 0; i < MAX_DEV_CTXS; i++ )
                if( pGi->pDevCtxs[i] == NULL )
                {
                    ...
                    bcmPktDma_XtmCreateDevice(i, pDevCtx->ulEncapType, delLen);
        ...
        for( i = 0; i < MAX_DEV_CTXS; i++ )
        {
            if( pGi->pDevCtxs[i] == pDevCtx )
            {
               ...
               if (pLsc->ulTrafficType == TRAFFIC_TYPE_PTM_BONDED) {
                ...
                {
                  if (pLsc->ulTrafficType == TRAFFIC_TYPE_PTM_BONDED) {
                     nRet = bcmxtmrt_init_swsched_timer1 (bcmxtmrt_swscheduler, pGi,
                ...
                else 
                {
                  ...
                  if (pLsc->ulTrafficType == TRAFFIC_TYPE_PTM_BONDED) {
                     bcmxtmrt_init_swsched_timer1 (bcmxtmrt_swscheduler, pGi, XTM_SW_SCHED_TIMER_REINIT) ;
                  }
                }
     ...
     PXTMRT_TOGGLE_PORT_DATA_STATUS_CHANGE pTpdsc )
{
	...
	for( i = 0; i < MAX_DEV_CTXS; i++ )
	{
		...
		if ((pDevCtx != NULL) && (pDevCtx->ulHdrType == HT_PTM)) {
			...
			//printk ("PDM2=%x\n", (unsigned int) pDevCtx->ulPortDataMask) ;
			spin_lock (&pGi->xtmlock_rx_bond) ;
			bcmxtmrt_ptmbond_handle_port_status_change (&pGi->ptmBondInfo,
					((pTpdsc->ulPortDataStatus == XTMRT_CMD_PORT_DATA_STATUS_ENABLED) ? LINK_UP :
					 LINK_DOWN),
					pDevCtx->ulPortDataMask) ;
			spin_unlock (&pGi->xtmlock_rx_bond) ;
    ...
    volatile DmaStateRam *pStRam)
{
    ...
    UINT32 ulOrigSwSchedTimerUSecs = swSchedTimerUsecs ;

    printk ("bcmxtmrt: Current DMA Q Size %u \n", (unsigned int) (txdma->ulQueueSize-txdma->txFreeBds)) ;
    ...
    bcmPktDma_XtmTxDisable(txdma->ulDmaIndex, TXDMATYPE(pDevCtx),
          bcmxtmrt_swsched_disable_swdma, ulPtmPrioIdx);

    if (TXDMATYPE (pDevCtx) == XTM_HW_DMA) {
       ...
       for (j = 0; (j < 1000) && ((txdma->txDma->cfg & DMA_ENABLE) == DMA_ENABLE); j++) {
          /* Blocking delays can not be used as we need to let the SW scheduler
           * to run for clean-up purposes.
           */
          for (k = 1000;  (k >= 0) && ((txdma->txDma->cfg & DMA_ENABLE) == DMA_ENABLE); k--)
             bcmxtmrt_swscheduler_command (pGi) ;
       }
    }

    if((txdma->txDma->cfg & DMA_ENABLE) == DMA_ENABLE) {
        printk("XTM Tx ch %d NOT disabled. ALARM\n", (int)txdma->ulDmaIndex);
        freeXmitPkts (pDevCtx, txdma, disableSuccess) ;
        printk ("bcmxtmrt: Current DMA Q Size %u \n", (unsigned int) (txdma->ulQueueSize-txdma->txFreeBds)) ;
        if (TXDMATYPE (pDevCtx) == XTM_SW_DMA) {
           disableSuccess = 0 ;
           bcmxtmrt_swsched_force_disable_swdma (ulPtmPrioIdx, (BcmPktDma_XtmTxDma *) txdma) ;
           bcmxtmrt_swscheduler_command (pGi) ;
           printk("XTM Tx ch %d FORCE disable. Success\n", (int)txdma->ulDmaIndex);
           freeXmitPkts (pDevCtx, txdma, disableSuccess) ;
           printk ("bcmxtmrt: Current DMA Q Size %u \n", (unsigned int) (txdma->ulQueueSize-txdma->txFreeBds)) ;
        }
    }
    ...
    if (TXDMATYPE (pDevCtx) == XTM_SW_DMA) {
       swSchedTimerUsecs = ulOrigSwSchedTimerUSecs ;
                          ...
                          int flushSuccess)
{
   ...
   {
      if (flushSuccess == 0x1) {
         ret = (bcmPktDma_XtmFreeXmitBufGet(txdma->ulDmaIndex,
                  (uint32 *)&nbuff_reclaim_p, &txSource, &txAddr, &rxChannel,
                  TXDMATYPE(pDevCtx), 0x0)) ;
      }
      else {
         ret = (bcmPktDma_XtmForceFreeXmitBufGet(txdma->ulDmaIndex,
                  (uint32 *)&nbuff_reclaim_p, &txSource, &txAddr, &rxChannel,
                  TXDMATYPE(pDevCtx), 0x0)) ;
      }
...
static void FlushdownTxQueue(PBCMXTMRT_DEV_CONTEXT pDevCtx, volatile BcmPktDma_XtmTxDma *txdma)
{
   ...
   if((txdma->txDma->cfg & DMA_ENABLE) == DMA_ENABLE) {
      printk("XTM Tx ch %d NOT flushed. ALARM\n", (int)txdma->ulDmaIndex);
      freeXmitPkts (pDevCtx, txdma, flushSuccess) ;
      printk ("bcmxtmrt: Current DMA Q Size %u \n", (unsigned int) (txdma->ulQueueSize-txdma->txFreeBds)) ;
      flushSuccess = 0 ;
      bcmxtmrt_swsched_force_flush_swdma (ulPtmPrioIdx, (BcmPktDma_XtmTxDma *) txdma) ;
      bcmxtmrt_swscheduler_command (pGi) ;
      printk("HOST XTM Tx ch %d FORCE Flush success\n", (int)txdma->ulDmaIndex);
      freeXmitPkts (pDevCtx, txdma, flushSuccess) ;
      printk ("bcmxtmrt: Current DMA Q Size %u \n", (unsigned int) (txdma->ulQueueSize-txdma->txFreeBds)) ;
   }
   else
   ...
   //txdma->ulNumTxBufsQdOne = 0; Managed from within PktDma Library.
   bcmPktDma_XtmTxEnable(txdma->ulDmaIndex, NULL, TXDMATYPE(pDevCtx)) ;
}
                         ...
                         PXTMRT_LINK_STATUS_CHANGE pLsc )
{
    ...
    if (pLsc->ulLinkDataMask == 0) {
       ...
       {
          txdma = pDevCtx->txdma[i];
          printk ("bcmxtmrt: LinkDownTx - Shutdown DMA Channel %d \n", (int) i) ;
       ...
       for (i = 0; i < pDevCtx->ulTxQInfosSize; i++)
       {
          if (TXDMATYPE(pDevCtx)==XTM_SW_DMA) {
            txdma = pDevCtx->txdma[i];
            printk ("bcmxtmrt: Data Flush Tx DMA Channel %d \n", (int) i) ;
    ...
    PXTMRT_TRANSMIT_QUEUE_ID pTxQId )
{
    ...
    bcmPktDma_XtmTxEnable(txdma->ulDmaIndex, &pDevCtx->devParams, TXDMATYPE(pDevCtx));
#endif
    ...
    PXTMRT_TRANSMIT_QUEUE_ID pTxQId )
{
    ...
    for (i = 0; i < pDevCtx->ulTxQInfosSize; i++)
    {
        ...
        if( txdma && pTxQId->ulQueueIndex == txdma->ulDmaIndex )
        {
            ...
            volatile DmaRegs *pDmaCtrl = TXDMACTRL(pDevCtx);
            volatile DmaStateRam *pStRam = pDmaCtrl->stram.s;
            UINT32 ulPtmPrioIdx = PTM_FLOW_PRI_LOW, ulTxQs;

            printk ("bcmxtmrt: DoUnsetTxQueue - Shutdown DMA Channel %d \n", (int) i) ;
            ShutdownTxQueue(pDevCtx, txdma, pStRam);
...
static int DoDeleteDeviceReq( PBCMXTMRT_DEV_CONTEXT pDevCtx )
{
    ...
    if ((pDevCtx != NULL) && (pDevCtx->ulTrafficType == TRAFFIC_TYPE_PTM_BONDED)) {
       spin_lock (&pGi->xtmlock_rx_bond);
       bcmxtmrt_ptmbond_tick (&pGi->ptmBondInfo, 0) ;
       spin_unlock (&pGi->xtmlock_rx_bond);
    }
...
static void bcmxtmrt_swsched_force_disable_swdma (UINT32 ulPtmPrioIdx, BcmPktDma_XtmTxDma  *txSwdma)
{
   ...
   *pu8SubPrioBitMap &= ~(0x1 << (txSwdma->ulSubPriority)) ;
   ...
   /* post deletion command. Ordering here is important. */
   pTxSchedCtxt->ulCommandParam = (ulPtmPrioIdx*MAX_SUB_PRIORITIES) + txSwdma->ulSubPriority ;
   pTxSchedCtxt->ulCommand = XTM_SW_DMA_FORCE_DELETE ;
}
...
static void bcmxtmrt_swsched_force_flush_swdma (UINT32 ulPtmPrioIdx, BcmPktDma_XtmTxDma  *txSwdma)
{
   ...
   /* post deletion command. Ordering here is important. */
   pTxSchedCtxt->ulCommandParam = (ulPtmPrioIdx*MAX_SUB_PRIORITIES) + txSwdma->ulSubPriority ;
   pTxSchedCtxt->ulCommand = XTM_SW_DMA_FORCE_FLUSH ;
}
...
static unsigned int bcmxtmrt_swsched_timer1_int(unsigned int i)
{
   if (swSchedTimerUsecs != 0) {
      TIMER->TimerInts |= TIMER1 ;
      (*bcmxtmrt_swsched_timer1_int_cb)(bcmxtmrt_swsched_timer1_int_cb_data);
      TIMER->TimerCtl1 = (TIMERENABLE | swSchedTimerUsecs);
      BcmHalInterruptEnable(INTERRUPT_ID_TIMER);
   }
  return (IRQ_HANDLED) ;
}
...
static int bcmxtmrt_compute_us_sched_timer_divpktsize (UINT32 usRate)
{
   ...
   if (usRate <= 10*oneMbps)
      divPktSize = 60 ;
   else if ((usRate > 10*oneMbps) && (usRate <= 20*oneMbps))
      divPktSize = 128 ;
...
static int bcmxtmrt_init_swsched_timer1 (void (*cb)(PBCMXTMRT_GLOBAL_INFO cb_data), 
       PBCMXTMRT_GLOBAL_INFO pGi, int timerInit)
{
  int k = 0, divPktSize ;
  UINT32 usTotalRate = 0, dsTotalRate = 0, timerValueinUSecs = 0;
  ...
  if ((timerInit == XTM_SW_SCHED_TIMER_INIT) && (TIMER->TimerCtl1 & TIMERENABLE) == 0) {

     /* Registering IRQ */
     k = BcmHalMapInterrupt((FN_HANDLER) bcmxtmrt_swsched_timer1_int, timerValueinUSecs , INTERRUPT_ID_TIMER);
     if (k == -1)
        goto _End ;

     bcmxtmrt_swsched_timer1_int_cb      = cb;
     bcmxtmrt_swsched_timer1_int_cb_data = pGi;
  }
  else if (timerInit == XTM_SW_SCHED_TIMER_REINIT) {

     swSchedTimerUsecs = 0 ;
     BcmHalInterruptDisable(INTERRUPT_ID_TIMER);
     TIMER->TimerCtl1 &= ~TIMERENABLE;
     ...
     if (usTotalRate != 0) {
        ...
        usTotalRate = usTotalRate+(usTotalRate*10/100) ;  /* extra credit of 10% times the total US rate, as the
                                                         ...
                                                         rate */
        divPktSize = bcmxtmrt_compute_us_sched_timer_divpktsize (usTotalRate) ;

        /* Enabling MIPS interrupt for timer 1 and clearing any previous interrupt */
        ...
        //printk ("usTotalRate = %d timervaluein USecs = %d \n", usTotalRate, timerValueinUSecs) ;
        swSchedTimerUsecs = timerValueinUSecs ;

        /* Setting the counter value and enabling timer 1 */
        TIMER->TimerCtl1 = TIMERENABLE | timerValueinUSecs ;

        BcmHalInterruptEnable(INTERRUPT_ID_TIMER);
     }
  }
  else {
     printk ("bcmxtmrt: SW Sched Timer (HW timer 1) is not available. Fatal !!! \n") ;
     k = -1 ;
  }
...
static void bcmxtmrt_uninit_swsched_timer1 ()
{
  BcmHalInterruptDisable(INTERRUPT_ID_TIMER);
  /* Stopping timer 1 */
  TIMER->TimerCtl1 &= ~TIMERENABLE;
...
/***************************************************************************
 * Function Name: bcmxtmrt_sw_sched_init
...
static int bcmxtmrt_sw_sched_init ( PBCMXTMRT_GLOBAL_INFO pGi )
{
   ...
   //printk ("bcmxtmrt: bcmxtmrt_sw_sched_init \n");
   ...
   nRet = bcmxtmrt_init_swsched_timer1 (bcmxtmrt_swscheduler, pGi, XTM_SW_SCHED_TIMER_INIT) ;
    ...
    PXTMRT_TRANSMIT_QUEUE_ID pTxQId )
{
   ...
   bcmPktDma_XtmInitTxChan (txdma->ulDmaIndex, txdma->ulQueueSize, (BcmPktDma_LocalXtmTxDma *)txdma, XTM_HW_DMA);
   bcmPktDma_XtmTxEnable(txdma->ulDmaIndex, NULL, XTM_HW_DMA);

   /* Increment channels per dev context */
   pTxSchedCtxt->ulHwTxQInfosSize++ ;
...
static int DoUnsetTxHwQueues ( )
{
    ...
    if (pTxSchedCtxt->ulSwTxQInfosSize == 0) {
       ...
       ulHwQueues = pTxSchedCtxt->ulHwTxQInfosSize ;
       pTxSchedCtxt->ulHwTxQInfosSize = 0 ;  /* Block the SW scheduler from doing anything */
       udelay (5000) ;
       ...
       for (i = 0; i < ulHwQueues; i++) {
          ...
          bcmPktDma_XtmTxDisable(txdma->ulDmaIndex, XTM_HW_DMA, NULL, 0) ;
...
#if !defined(CONFIG_BCM_FAP) && !defined(CONFIG_BCM_FAP_MODULE)
      bcmPktDma_XtmXmitSwdmaToIudma (txHwchannel, dmaDesc, key) ;
#endif
...
static void bcmxtmrt_swsched_recycle (XtmRtTxSchedContext *pXtmTxSchedCtxt, UINT32 ulCommand)
{
   ...
   /* Free packets that have been transmitted. */
   for (channel=pXtmTxSchedCtxt->ulHwTxQInfosSize-1; channel >= 0; channel--) {
      ...
      if (txdma->txFreeBds < txdma->ulQueueSize) {
         ...
         if ((ulCommand & XTM_SW_DMA_FORCED) != XTM_SW_DMA_FORCED) {

            while (bcmPktDma_XtmFreeXmitBufGet(txdma->ulDmaIndex,(uint32 *)&key,
            ...
            while (bcmPktDma_XtmForceFreeXmitBufGet(txdma->ulDmaIndex,(uint32 *)&key,
                     &txSource, &txAddr, &rxChannel, XTM_HW_DMA, 0x1) == TRUE)
            {
               txSwdmaIndex = XTM_SW_CHANNEL(key) ;
               txBdIndex    = XTM_SW_CHANNEL_INDEX(key) ;
...
static void bcmxtmrt_swscheduler ( PBCMXTMRT_GLOBAL_INFO pGi )
{
    ...
    * Strict Priority/WFQ.
    */
   bcmxtmrt_swsched_recycle (pXtmTxSchedCtxt, XTM_SW_DMA_NO_COMMAND) ;
...
/***************************************************************************
 * Function Name: bcmxtmrt_swscheduler_command
 * Description  : Services any commands to swscheduler, currently the admin/
 *                operative actions like Flush/Delete.
 *                Also included is Force Flush when the HW/Phy gets stuck.
 * Returns      : None.
 ***************************************************************************/
static void bcmxtmrt_swscheduler_command ( PBCMXTMRT_GLOBAL_INFO pGi )
{
   ...
   volatile BcmPktDma_XtmTxDma *txSwdma = NULL ;
   XtmRtTxSchedContext *pXtmTxSchedCtxt  = &pGi->txSchedCtxt ;
      UINT32 swdmaChannel = pXtmTxSchedCtxt->ulCommandParam ;

   /* Check for transmitted packet to HW queue(s) to free and then go for
    * scheduling from SW DMAs based on the priority scheme.
    * Strict Priority.
    */
   bcmxtmrt_swsched_recycle (pXtmTxSchedCtxt, pXtmTxSchedCtxt->ulCommand) ;
    ...
    int *eof, void *data)
{
    ...
    for( i = 0; i < MAX_RECEIVE_QUEUES; i++ )
    {
        ...
        if ( rxdma != NULL )
        {
           ...
           sz += sprintf(page + sz, "ch: %lu, Head index : %lu, Tail index %lu"
                 "numRxBds  %lu rxAssignedBds %lu\n", 
                 (long unsigned int) i, (long unsigned int) rxdma->rxHeadIndex, (long unsigned int) rxdma->rxTailIndex,
                 (long unsigned int) rxdma->numRxBds, (long unsigned int) rxdma->rxAssignedBds) ;

           sz += sprintf(page + sz, "DMA cfg: %lx, intstat : %lx, intmask  %lx\n",
                 rxdma->rxDma->cfg, 
                 rxdma->rxDma->intStat, rxdma->rxDma->intMask) ;
        }
    }
    ...
    int *eof, void *data)
{
    ...
    sz += sprintf(page + sz, "xtm tx sched: Hw Q -%ld Hw Tx DM0 %x\n",
                  pGi->txSchedCtxt.ulHwTxQInfosSize, (unsigned int) pGi->txSchedCtxt.txHwdma[0].txDma) ;
