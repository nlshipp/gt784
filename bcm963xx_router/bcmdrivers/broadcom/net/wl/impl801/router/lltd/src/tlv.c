...
write_etheraddr_t(int number, void *data, uint8_t *buf, int bytes_free, bool_t isHello, bool_t isLarge, size_t offset)
{
	...
	dbgprintf("write_etheraddr_t number = %d, bytes_free= %d, isHello = %d, isLarge = %d, offset = %d \n", number, bytes_free, (int)isHello, (int)isLarge, offset);
    END_TRACE
...
write_ucs2char_t(int number, void *data, uint8_t *buf, int bytes_free, bool_t isHello, bool_t isLarge, size_t offset)
{
	...
	IF_TRACED(TRC_TLVINFO)
            printf("write_ucs2char_t number = %d, bytes_free= %d, isHello = %d, isLarge = %d, offset = %d \n", number, bytes_free, (int)isHello, (int)isLarge, offset);
	END_TRACE
   ...
   IF_TRACED(TRC_TLVINFO)
            printf("write_ucs2char_t bytes_needed  = %d (while loop)\n", bytes_needed);
   END_TRACE
   ...
   IF_TRACED(TRC_TLVINFO)
            printf("write_ucs2char_t bytes_needed = %d, buf = %s\n", bytes_needed, buf);
   END_TRACE
...
write_assns_t(int number, void *data, uint8_t *buf, int bytes_free, bool_t isHello, bool_t isLarge, size_t offset)
{
...

    {
        ...
        {
        	IF_TRACED(TRC_TLVINFO)
        		printf("writing assn-list copy_cnt = %d \n", copy_cnt);
			printf("writing assn-list offset_index = %d \n", offset_index);
    		END_TRACE
...
write_comptbl_t(int number, void *data, uint8_t *buf, int bytes_free, bool_t isHello, bool_t isLarge, size_t offset)
{
    ...
    *buf++ = (uint8_t)0;
    ...
    bytes_used = (buf - length) - 2;		//sbtract 2 to fix a wired bug. The MAX size of component table is 23Bytes according to SPEC
	IF_TRACED(TRC_TLVINFO)
            printf("w-c-t: bytes_used= %d\n",bytes_used);
        END_TRACE
    g_short_reorder_buffer = htons(bytes_used);
...
static int
write_frucs2char_t(int number, void *data, uint8_t *buf, int bytes_free, bool_t isHello, bool_t isLarge, size_t offset)
{
    int         bytes_needed;
    uint16_t   *s = data;

	IF_TRACED(TRC_TLVINFO)
            printf("write_frucs2char_t number = %d, bytes_free= %d, isHello = %d, isLarge = %d, offset = %d \n", number, bytes_free, (int)isHello, (int)isLarge, offset);
	END_TRACE
