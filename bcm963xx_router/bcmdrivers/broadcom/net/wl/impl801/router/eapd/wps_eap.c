 ...
 *
 * $Id: wps_eap.c,v 1.1 2010/08/05 21:58:59 ywu Exp $
 */
...
#endif 
	ret = udp_bind(wps->appSocket, EAPD_WKSP_WPS_UDP_RPORT);
	if (ret < 0) {
...
wps_app_monitor_sendup(eapd_wksp_t *nwksp, uint8 *pData, int Len, char *from)
{
	...
	if (wps->appSocket >= 0) {
		...
		sentBytes = udp_write(wps->appSocket, (char *)pData, Len, &to);
