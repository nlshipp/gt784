 ...
 *
 * $Id: nas_eap.c,v 1.6 2011/04/20 01:24:41 slv Exp $
 */
...
#include <bcmwpa.h>
#if defined(AEI_VDSL_CUSTOMER_QWEST)  || defined(AEI_VDSL_CUSTOMER_TELUS) || defined(AEI_VDSL_CUSTOMER_CENTURYLINK) //hk_ctl//hk_wps_q2000
#include <bcmnvram.h>
#endif
...
#endif 
	ret = udp_bind(nas->appSocket, EAPD_WKSP_NAS_UDP_RPORT);
	if (ret < 0)
	{
...
nas_app_sendup(eapd_wksp_t *nwksp, uint8 *pData, int pLen, char *from)
{
	...
	if (nas->appSocket >= 0) {
		...
		sentBytes = udp_write(nas->appSocket, (char *)pData, pLen, &to);
...
if(1==atoi(nvram_safe_get("wps_proc_status"))) {
	sta = sta_lookup(nwksp, (struct ether_addr *)addr,
        ...
        sta = sta_lookup(nwksp, (struct ether_addr *)addr,
				...
				if (!nas_app_parse_ie(data, len, &wsec, &mode) &&
					CHECK_PSK(mode)) {
					/* set cient in PSK mode */
					sta->mode = EAPD_STA_MODE_NAS_PSK;
				}
#if defined(AEI_VDSL_CUSTOMER_NCS) //scott.lv fix bug23727
				else
