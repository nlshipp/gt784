 ...
 *
 * $Id: ses_eap.c,v 1.1 2010/08/05 21:58:59 ywu Exp $
 */
...
#endif 
	ret = udp_bind(ses->appSocket, EAPD_WKSP_SES_UDP_RPORT);
	if (ret < 0) {
...
ses_app_sendup(eapd_wksp_t *nwksp, uint8 *pData, int pLen, char *from)
{
	...
	if (ses->appSocket >= 0) {
		...
		sentBytes = udp_write(ses->appSocket, (char *)pData, pLen, &to);
