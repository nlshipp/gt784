 ...
 *
 * $Id: wai_eap.c,v 1.1 2010/08/05 21:58:59 ywu Exp $
 */
...
#endif 
	ret = udp_bind(wai->appSocket, EAPD_WKSP_WAI_UDP_RPORT);
	if (ret < 0)
	{
...
wai_app_sendup(eapd_wksp_t *nwksp, uint8 *pData, int pLen, char *from)
{
	...
	if (wai->appSocket >= 0) {
		...
		sentBytes = udp_write(wai->appSocket, pData, pLen, &to);
