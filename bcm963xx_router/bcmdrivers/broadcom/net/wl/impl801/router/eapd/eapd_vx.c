 ...
 *
 * $Id: eapd_vx.c,v 1.1 2010/08/05 21:58:59 ywu Exp $
 */
	...
	M_BLK_ID mbuf, LL_HDR_INFO *llhdr, void *spare)
{
	...
	if (muxobj->lbSocket >= 0) {
		...
		sentBytes = udp_write(muxobj->lbSocket, pkt, bytes, &to);
...
eapd_brcm_open(eapd_wksp_t *nwksp, eapd_brcm_socket_t *sock)
{
	...
	if (udp_bind(sock->drvSocket, lbport) < 0) {
