 ...
 *
 * $Id: eapd.c,v 1.1 2010/08/05 21:58:59 ywu Exp $
 */
...
#ifdef DSLCPE
// This is added by dslcpe, but without this this file won't pass compile when debug is on.
static char eabuf[6];
#endif
	...
	for (i = 0; i < EAPD_WKSP_MAX_NO_BRIDGE; i++) {
		...
		foreach(name, ifnames, next) {
			...
			if (wai_app_enabled(name))
				needStart |= eapd_add_interface(nwksp, name, EAPD_APP_WAI);
#endif /* BCMWAPI_WAI */
			...
			needStart |= eapd_add_interface(nwksp, name, EAPD_APP_EVT);
#endif
		}
	}
	...
	if (status > 0) {
		...
		for (i = 0; i < nwksp->brcmSocketCount; i++) {
			...
			if (brcmSocket->inuseCount > 0 &&
				...
				bytes = recv(brcmSocket->drvSocket, pkt, len, 0);
				if (bytes > 0)  {
		...
		while (cb) {
			...
			if (preauthSocket->drvSocket >= 0 &&
				...
				bytes = recv(preauthSocket->drvSocket, pkt, len, 0);
				if (bytes > 0) {
#ifdef BCMDBG
		...
		if (wai->appSocket >= 0 &&
			...
			if (bytes > ETHER_HDR_LEN) {
				...
				cb = eapd_wksp_find_cb(wai, ifname, eth->ether_shost);
				/* send message data out. */
				if (cb)
