 ...
 *
 * $Id: dcs_eap.c,v 1.1 2010/08/05 21:58:59 ywu Exp $
 */
...
#endif 
	ret = udp_bind(dcs->appSocket, EAPD_WKSP_DCS_UDP_RPORT);
	if (ret < 0)
	{
...
dcs_app_sendup(eapd_wksp_t *nwksp, uint8 *pData, int pLen, char *from)
{
	...
	if (dcs->appSocket >= 0) {
		...
		sentBytes = udp_write(dcs->appSocket, pData, pLen, &to);
