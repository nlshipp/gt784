 ...
 *
 * $Id: nas.c,v 1.1 2010/08/05 21:58:59 ywu Exp $
 */
...
#else
#define CHECK_EAPOL_KEY(key) ((key) == EAPOL_WPA_KEY)
#endif

static void eapol_canned(nas_t *nas, nas_sta_t *sta, unsigned char code,
                         unsigned char type);
static void eapol_dispatch_ex(nas_t *nas, eapol_header_t *eapol, int preauth);
...
void
eapol_dispatch(nas_t *nas, eapol_header_t *eapol)
{
	eapol_dispatch_ex(nas, eapol, 0);
}

#ifdef BCMWPA2
void
preauth_dispatch(nas_t *nas, eapol_header_t *eapol)
{
	if (!nas->disable_preauth)
		eapol_dispatch_ex(nas, eapol, 1);
}
#endif /* BCMWPA2 */

static void
eapol_dispatch_ex(nas_t *nas, eapol_header_t *eapol, int preauth)
{
	nas_sta_t *sta;
	eap_header_t *eap;
#ifdef BCMDBG
...
driver_message_dispatch(nas_t *nas, bcm_event_t *dpkt)
{
	...
	sta = lookup_sta(nas, (struct ether_addr *)addr,
	                 type == WLC_E_DISASSOC_IND ? SEARCH_ONLY : SEARCH_ENTER);
	...
	case WLC_E_MIC_ERROR:
		if ((ntohs(event->flags) & WLC_EVENT_MSG_GROUP) &&
			...
			dbg(nas, "GTK MIC error notification received by AP");
			return;
		}


		/*
			If an error is attributed to a STA we don't know about,
			just let it go.

			Otherwise, call wpa_mic_error for processing

		*/

		if (sta != NULL) {
			dbg(nas, "PTK MIC error notification received by AP");
			wpa_mic_error(nas->wpa, sta, TRUE);
		}
		else {
			dbg(nas,
			    "PTK MIC error notification received by AP but not from a known STA");
		}
