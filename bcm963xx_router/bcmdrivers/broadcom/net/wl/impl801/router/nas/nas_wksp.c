 ...
 *
 * $Id: nas_wksp.c,v 1.1 2010/08/05 21:58:59 ywu Exp $
 */
		...
		if (nwksp->eapd !=  NAS_WKSP_UNK_FILE_DESC && FD_ISSET(nwksp->eapd, &fdset)) {
			...
			if ((bytes = recv(nwksp->eapd, pkt, len, 0)) > 0) {
				...
				case ETHER_TYPE_802_1X: /* eapol */
					NASDBG("recv eapol packet from socket %d\n", nwksp->eapd);
				...
				case ETHER_TYPE_802_1X_PREAUTH: /* preauth */
					...
					preauth_dispatch(&nwcb->nas, eapol);
					break;
#endif /* BCMWPA2 */
				case ETHER_TYPE_BRCM: /* brcmevent */
					NASDBG("recv brcmevent packet from eapd Socket"
					       "%d\n", nwksp->eapd);
...
nas_get_wsec(nas_wksp_t *nwksp, uint8 *mac, char *osifname)
{
	...
	if ((ret = get_wsec(&info, (char *) mac, osifname)) != WLIFU_WSEC_SUCCESS) {
