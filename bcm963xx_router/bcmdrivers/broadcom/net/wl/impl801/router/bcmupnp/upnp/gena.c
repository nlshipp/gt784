 ...
 *
 * $Id: gena.c,v 1.1 2010/08/05 21:58:59 ywu Exp $
 */
#include <upnp.h>
...
notify_prop_change(UPNP_CONTEXT *context, UPNP_SUBSCRIBER *subscriber)
{
	...
	int len;
	int on;
	UPNP_DEVICE *device = context->focus_ifp->focus_devchain->device;
	int connect_reties = device->gena_connect_retries;
...
get_unique_id(char *unique_id, unsigned int size)
{
	 ...
	 * when size less than 17, the format will be %x,
	 * between 17 and 25, we use %x-%x,
	 * others with the foramt %x-%x-%x
	 */
	if (size < 17) {
		sprintf(unique_id, "%x", curr_time);
	}
	else if (size < 26) {
		sprintf(unique_id, "%x-%x", curr_time, pid);
	}
	else {
		sprintf(unique_id, "%x-%x-%x", curr_time, pid, (unsigned int)id);
	}
	...
	if (num != 0) {
		for (i = 0; i < num; i++) {
			...
			*value++ = 0;
			...
			strncpy(evalue->value.val.str, value, sizeof(evalue->value));
			if (convert_value(context, &evalue->value) != 0)
				continue;
