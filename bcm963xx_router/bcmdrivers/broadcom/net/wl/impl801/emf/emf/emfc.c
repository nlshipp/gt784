 ...
 *
 * $Id: emfc.c,v 1.1 2010/08/05 21:58:58 ywu Exp $
 */
#include <typedefs.h>
#include <bcmdefs.h>
#include <bcmendian.h>
	...
	if ((IPV4_PROT(iph) == IP_PROT_IGMP) && (emfc->snooper != NULL))
	{
		...
		switch (action)
		{
			...
			case EMF_CONVERT_QUERY:
			{
				...
				mgrp = emfc_mfdb_group_find(emfc, IPV4_MCADDR_ALLHOSTS);
					...
					ptr != mgrp->mi_head.prev; ptr = ptr->next)
				{
					...
					// PKTDUP() calls skb_clone(), which duplicates skb header,
					// zeros out skb->cb, and shares skb->data for both the new
					// and original skb's.
					// The function emfc->wrapper.forward_fn() (wmf_forward())
					// always modifies skb->data, therefore skb->data has to
					// be duplicated.  There are four instances of skb_unshare()
					// for this reason.

					if ((sdu_clone = PKTDUP(emfc->osh, sdu)) != NULL)
						sdu_clone = skb_unshare((struct sk_buff*)sdu_clone, GFP_ATOMIC);
