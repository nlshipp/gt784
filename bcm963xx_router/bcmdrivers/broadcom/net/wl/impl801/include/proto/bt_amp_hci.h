 ...
 *
 * $Id: bt_amp_hci.h,v 1.1 2010/08/05 21:58:59 ywu Exp $
*/
...
typedef struct flow_spec_mod_cmd_parms {
	uint8 llh[2];
	uint8 txflow[16];
	uint8 rxflow[16];
} PACKED flow_spec_mod_cmd_parms_t;
...
typedef struct ld_cmd_parms {
	uint8 ld_aware;
	uint8 ld[2];
	uint8 ld_opts;
	uint8 l_opts;
} PACKED ld_cmd_parms_t;
...
typedef struct disc_log_link_evt_parms {
	uint8 status;
	uint16 llh;
	uint8 reason;
} PACKED disc_log_link_evt_parms_t;

typedef struct log_link_cancel_evt_parms {
	uint8 status;
	uint8 plh;
	uint8 tx_fs_ID;
} PACKED log_link_cancel_evt_parms_t;

typedef struct flow_spec_mod_evt_parms {
	uint8 status;
	uint16 llh;
} PACKED flow_spec_mod_evt_parms_t;
...
typedef struct contact_counter_evt_parms {
	uint8 status;
	uint8 llh[2];
	uint16 counter;
} PACKED contact_counter_evt_parms_t;

typedef struct contact_counter_reset_evt_parms {
	uint8 status;
	uint8 llh[2];
} PACKED contact_counter_reset_evt_parms_t;
...
typedef struct ld_evt_parms {
	uint8 status;
	uint8 ld_aware;
	uint8 ld[2];
	uint8 ld_opts;
	uint8 l_opts;
} PACKED ld_evt_parms_t;
...
typedef struct local_version_info_evt_parms {
	uint8 status;
	uint8 hci_version;
	uint16 hci_revision;
	uint8 pal_version;
...
} PACKED local_version_info_evt_parms_t;

#define MAX_SUPPORTED_CMD_BYTE	64
typedef struct local_supported_cmd_evt_parms {
	uint8 status;
	uint8 cmd[MAX_SUPPORTED_CMD_BYTE];
} PACKED local_supported_cmd_evt_parms_t;
