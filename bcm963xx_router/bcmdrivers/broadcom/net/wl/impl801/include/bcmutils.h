 ...
 *
 * $Id: bcmutils.h,v 1.1 2010/08/05 21:58:58 ywu Exp $
 */
...
#include <bcmnet.h>
/* below are tunables:
   hacks to avoid that data coming to wlan
   from eth are queued up at wlan waiting,
   eth would not be able to receive more data.
   Don't want to gap too much or wlan would drop data.
*/
extern int kerSysGetSdramSize( void );
#define PKTQ_DROP_HIGH_WMARK    (NR_RX_BDS(kerSysGetSdramSize()))
#define PKTQ_DROP_LOW_WMARK     (PKTQ_DROP_HIGH_WMARK - 16)
#endif
