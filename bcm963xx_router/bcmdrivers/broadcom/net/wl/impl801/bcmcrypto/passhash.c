 ...
 *
 * $Id: passhash.c,v 1.1 2010/08/05 21:58:58 ywu Exp $
 */
 ...
 */
int
init_passhash(passhash_t *passhash,
              char *password, int passlen, unsigned char *ssid, int ssidlen)
{
	if (strlen(password) < 8 || strlen(password) > 63)
		return -1;

	bzero(passhash, sizeof(*passhash));
	passhash->count = 1;
	passhash->password = password;
	passhash->passlen = passlen;
	passhash->ssid = ssid;
	passhash->ssidlen = ssidlen;

	return 0;
}

int
do_passhash(passhash_t *passhash, int iterations)
{
	unsigned char *output;

	if (passhash->count > 2)
		return -1;
	output = passhash->output + SHA1HashSize * (passhash->count - 1);
	if (passhash->iters == 0) {
		init_F(passhash->password, passhash->passlen, passhash->ssid, passhash->ssidlen,
		       passhash->count, passhash->digest, output);
		passhash->iters = 1;
		iterations --;
	}
	if (passhash->iters + iterations > 4096)
		iterations = 4096 - passhash->iters;
	do_F(passhash->password, passhash->passlen, iterations, passhash->digest, output);
	passhash->iters += iterations;
	if (passhash->iters == 4096) {
		passhash->count ++;
		passhash->iters = 0;
		if (passhash->count > 2)
			return 0;
...
int
get_passhash(passhash_t *passhash, unsigned char *output, int outlen)
{
	if (passhash->count > 2 && outlen <= (int)sizeof(passhash->output)) {
		bcopy(passhash->output, output, outlen);
		return 0;
	}
	return -1;
}
