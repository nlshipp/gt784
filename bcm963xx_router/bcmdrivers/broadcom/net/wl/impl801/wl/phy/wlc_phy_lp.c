 ...
 *
 * $Id: wlc_phy_lp.c,v 1.1 2010/08/05 21:59:00 ywu Exp $
 */
...
#define wlc_lpphy_tx_gain_override_enabled(pi) \
	(0 != (read_phy_reg((pi), LPPHY_AfeCtrlOvr) & LPPHY_AfeCtrlOvr_dacattctrl_ovr_MASK))
...
#define wlc_lpphy_set_tx_pwr_npt(pi, npt) \
	mod_phy_reg(pi, LPPHY_TxPwrCtrlNnum, \
...
#define wlc_lpphy_set_target_tx_pwr(pi, target) \
	mod_phy_reg(pi, LPPHY_TxPwrCtrlTargetPwr, \
...
#define wlc_lpphy_get_target_tx_pwr(pi) \
	((read_phy_reg(pi, LPPHY_TxPwrCtrlTargetPwr) & \
		LPPHY_TxPwrCtrlTargetPwr_targetPwr0_MASK) >> \
...
static void wlc_lpphy_set_tx_gain_by_index(phy_info_t *pi, int index);
...
wlc_lpphy_set_tx_gain_override(phy_info_t *pi, bool bEnable)
{
	...
	if (LPREV_GE(pi->pubpi.phy_rev, 2)) {
		mod_phy_reg(pi, LPPHY_REV2_rfoverride2,
			LPPHY_REV2_rfoverride2_txgainctrl_ovr_MASK,
			bit << LPPHY_REV2_rfoverride2_txgainctrl_ovr_SHIFT);

		mod_phy_reg(pi, LPPHY_REV2_rfoverride2,
			LPPHY_REV2_rfoverride2_stxtxgainctrl_ovr_MASK,
			bit << LPPHY_REV2_rfoverride2_stxtxgainctrl_ovr_SHIFT);
	} else
		mod_phy_reg(pi, LPPHY_rfoverride2,
			LPPHY_rfoverride2_txgainctrl_ovr_MASK,
			bit << LPPHY_rfoverride2_txgainctrl_ovr_SHIFT);

	mod_phy_reg(pi, LPPHY_AfeCtrlOvr,
		LPPHY_AfeCtrlOvr_dacattctrl_ovr_MASK,
		bit << LPPHY_AfeCtrlOvr_dacattctrl_ovr_SHIFT);
}
...
wlc_phy_set_tx_pwr_ctrl_lpphy(phy_info_t *pi, uint16 mode)
{
	...
	if (old_mode != mode) {
		...
		if (pi_lp->lpphy_use_tx_pwr_ctrl_coeffs) {
			MOD_PHY_REG(pi, LPPHY, TxPwrCtrlCmd, use_txPwrCtrlCoefs,
				(LPPHY_TX_PWR_CTRL_HW == mode) ? 1 : 0);
		} else {
			MOD_PHY_REG(pi, LPPHY, TxPwrCtrlCmd, use_txPwrCtrlCoefs, 0);
		}

		/* Feed back RF power level to PAPD block */
		if (LPREV_GE(pi->pubpi.phy_rev, 2)) {
			MOD_PHY_REG(pi, LPPHY_REV2, papdctrl, rfpowerOverride,
...
wlc_lpphy_txpower_reset_npt(phy_info_t *pi)
{
	...
	else if (pi->sh->boardflags & BFL_HGPA)
...
{
	/* Set TR switch */
	mod_phy_reg(pi, LPPHY_RFOverrideVal0,
...
wlc_lpphy_set_pa_gain(phy_info_t *pi, uint16 gain)
{
	...
	mod_phy_reg(pi, LPPHY_REV2_txgainctrlovrval1,
		LPPHY_REV2_txgainctrlovrval1_pagain_ovr_val1_MASK,
		gain << LPPHY_REV2_txgainctrlovrval1_pagain_ovr_val1_SHIFT);
	mod_phy_reg(pi, LPPHY_REV2_stxtxgainctrlovrval1,
		LPPHY_REV2_stxtxgainctrlovrval1_pagain_ovr_val1_MASK,
		gain << LPPHY_REV2_stxtxgainctrlovrval1_pagain_ovr_val1_SHIFT);
...
wlc_lpphy_get_pa_gain(phy_info_t *pi)
{
	...
	pa_gain = (read_phy_reg(pi, LPPHY_REV2_txgainctrlovrval1) &
		LPPHY_REV2_txgainctrlovrval1_pagain_ovr_val1_MASK) >>
		LPPHY_REV2_txgainctrlovrval1_pagain_ovr_val1_SHIFT;
...
wlc_lpphy_set_dac_gain(phy_info_t *pi, uint16 dac_gain)
{
	...
	dac_ctrl =
		((read_phy_reg(pi, LPPHY_AfeDACCtrl) >> LPPHY_AfeDACCtrl_dac_ctrl_SHIFT) & 0xc7f) |
		(dac_gain << 7);
	mod_phy_reg(pi, LPPHY_AfeDACCtrl,
		LPPHY_AfeDACCtrl_dac_ctrl_MASK,
		dac_ctrl << LPPHY_AfeDACCtrl_dac_ctrl_SHIFT);
}
...
wlc_lpphy_set_tx_gain(phy_info_t *pi,  lpphy_txgains_t *target_gains)
{
	...
	if (LPREV_GE(pi->pubpi.phy_rev, 2)) {
		...
		mod_phy_reg(pi, LPPHY_REV2_txgainctrlovrval0,
			LPPHY_REV2_txgainctrlovrval0_txgainctrl_ovr_val0_MASK,
			((target_gains->gm_gain) | (target_gains->pga_gain << 8)) <<
			LPPHY_REV2_txgainctrlovrval0_txgainctrl_ovr_val0_SHIFT);
		mod_phy_reg(pi, LPPHY_REV2_txgainctrlovrval1,
			LPPHY_REV2_txgainctrlovrval1_txgainctrl_ovr_val1_MASK,
			((target_gains->pad_gain) | (pa_gain << 8)) <<
			LPPHY_REV2_txgainctrlovrval1_txgainctrl_ovr_val1_SHIFT);

		mod_phy_reg(pi, LPPHY_REV2_stxtxgainctrlovrval0,
			LPPHY_REV2_stxtxgainctrlovrval0_stxtxgainctrl_ovr_val0_MASK,
			((target_gains->gm_gain) | (target_gains->pga_gain << 8)) <<
			LPPHY_REV2_stxtxgainctrlovrval0_stxtxgainctrl_ovr_val0_SHIFT);
		mod_phy_reg(pi, LPPHY_REV2_stxtxgainctrlovrval1,
			LPPHY_REV2_stxtxgainctrlovrval1_stxtxgainctrl_ovr_val1_MASK,
			((target_gains->pad_gain) | (pa_gain << 8)) <<
			LPPHY_REV2_stxtxgainctrlovrval1_stxtxgainctrl_ovr_val1_SHIFT);
		...
		mod_phy_reg(pi, LPPHY_txgainctrlovrval,
			LPPHY_txgainctrlovrval_txgainctrl_ovr_val_MASK,
			rf_gain << LPPHY_txgainctrlovrval_txgainctrl_ovr_val_SHIFT);
	}
...
wlc_lpphy_get_tx_gain(phy_info_t *pi,  lpphy_txgains_t *gains)
{
	...
	dac_gain = read_phy_reg(pi, LPPHY_AfeDACCtrl) >>
		LPPHY_AfeDACCtrl_dac_ctrl_SHIFT;
	gains->dac_gain = (dac_gain & 0x380) >> 7;

	if (LPREV_LT(pi->pubpi.phy_rev, 2)) {
		uint16 rf_gain = (read_phy_reg(pi, LPPHY_txgainctrlovrval) &
			LPPHY_txgainctrlovrval_txgainctrl_ovr_val_MASK) >>
			LPPHY_txgainctrlovrval_txgainctrl_ovr_val_SHIFT;
		...
		rfgain0 = (read_phy_reg(pi, LPPHY_REV2_txgainctrlovrval0) &
			LPPHY_REV2_txgainctrlovrval0_txgainctrl_ovr_val0_MASK) >>
			LPPHY_REV2_txgainctrlovrval0_txgainctrl_ovr_val0_SHIFT;
		rfgain1 = (read_phy_reg(pi, LPPHY_REV2_txgainctrlovrval1) &
			LPPHY_REV2_txgainctrlovrval1_txgainctrl_ovr_val1_MASK) >>
			LPPHY_REV2_txgainctrlovrval1_txgainctrl_ovr_val1_SHIFT;
...
wlc_phy_set_tx_locc_ucode_lpphy(phy_info_t *pi, bool iscck, uint16 didq)
{
	...
	if (0 == (addr = 2*wlapi_bmac_read_shm(pi->sh->physhim, M_PHY_TX_FLT_PTR)))
		return;
...
wlc_phy_aci_init_lpphy(phy_info_t *pi, bool sys)
{
	...
	if (sys) {
		...
		while (p_aci_reg < p_aci_reg_end) {
			...
			*p_dflt_reg = (uint8)((read_phy_reg(pi, a) >> s) &
				((uint16)(1<<n)-1));
			p_aci_reg++;
			p_dflt_reg++;
		}
...
wlc_lpphy_aci_tweaks(phy_info_t *pi, bool on)
{
	...
	while (p_aci_reg < p_aci_reg_end) {
		...
		s = (uint8)((x >> LPPHY_ACI_R_S_S) & LPPHY_ACI_R_S_M);
		n = (uint8)((x >> LPPHY_ACI_R_N_S) & LPPHY_ACI_R_N_M);
		and_phy_reg(pi, a, ~(((uint16)(1<<n) - 1) << s));
		if (on)
			v = (uint8)((x >> LPPHY_ACI_R_V_S) & LPPHY_ACI_R_V_M);
		else
			v = *p_dflt_reg;
		or_phy_reg(pi, a, ((uint16)v << s));
		p_aci_reg++;
		p_dflt_reg++;
	}

	or_phy_reg(pi,  LPPHY_REV2_resetCtrl,  0x44);
	and_phy_reg(pi, LPPHY_REV2_resetCtrl, ~0x44);
...
wlc_lpphy_aci_scan(phy_info_t *pi)
{
	...
	/* Save settings */
	radar_en = read_phy_reg(pi, LPPHY_REV2_RadardetectEn);
	...
	while ((chan < ACI_LAST_CHAN) && !(chan_mask & aci_mask)) {
		...
		mod_phy_reg(pi, LPPHY_REV2_RadardetectEn,
			LPPHY_REV2_RadardetectEn_radar_detect_enable_MASK,
			1 << LPPHY_REV2_RadardetectEn_radar_detect_enable_SHIFT);
		write_phy_reg(pi, LPPHY_gpioSel, 9);
		write_phy_reg(pi, LPPHY_gpioOutEn, 0xffff);
		...
		for (i = 0, n = 0; i < pi_lp->lpphy_aci.chan_scan_cnt; i++) {
			...
			pwr_metric =
			  (read_phy_reg(pi, LPPHY_gpioOut) & 0xff);
			if (pwr_metric < 128) {
				pwr_metric = 0;
			} else {
	...
	/* Restore settings */
	write_phy_reg(pi, LPPHY_REV2_RadardetectEn, radar_en);
...
wlc_phy_rx_signal_power_lpphy(phy_info_t *pi, int32 gain_index)
{
	...
	nominal_power_db = read_phy_reg(pi, LPPHY_VeryLowGainDB) >>
	...
	input_power_offset_db = read_phy_reg(pi, LPPHY_InputPowerDB) & 0xFF;
...
wlc_lpphy_rx_gain_override_enable(phy_info_t *pi, bool enable)
{
	...
	mod_phy_reg(pi, LPPHY_RFOverride0,
		LPPHY_RFOverride0_trsw_rx_pu_ovr_MASK,
		ebit << LPPHY_RFOverride0_trsw_rx_pu_ovr_SHIFT);
	mod_phy_reg(pi, LPPHY_RFOverride0,
		LPPHY_RFOverride0_gmode_rx_pu_ovr_MASK,
		ebit << LPPHY_RFOverride0_gmode_rx_pu_ovr_SHIFT);
	mod_phy_reg(pi, LPPHY_RFOverride0,
		LPPHY_RFOverride0_amode_rx_pu_ovr_MASK,
		ebit << LPPHY_RFOverride0_amode_rx_pu_ovr_SHIFT);

	if (LPREV_GE(pi->pubpi.phy_rev, 2)) {
		mod_phy_reg(pi, LPPHY_REV2_rfoverride2,
			LPPHY_REV2_rfoverride2_rxgainctrl_ovr_MASK,
			ebit << LPPHY_REV2_rfoverride2_rxgainctrl_ovr_SHIFT);
		if (CHSPEC_IS2G(pi->radio_chanspec)) {
			mod_phy_reg(pi, LPPHY_REV2_rfoverride2,
				LPPHY_REV2_rfoverride2_slna_gain_ctrl_ovr_MASK,
				ebit << LPPHY_REV2_rfoverride2_slna_gain_ctrl_ovr_SHIFT);
			mod_phy_reg(pi, LPPHY_REV2_RFinputOverride,
				LPPHY_REV2_RFinputOverride_wlslnagainctrl_ovr_MASK,
				ebit << LPPHY_REV2_RFinputOverride_wlslnagainctrl_ovr_SHIFT);
		}
	} else {
		mod_phy_reg(pi, LPPHY_rfoverride2,
			LPPHY_rfoverride2_rxgainctrl_ovr_MASK,
			ebit << LPPHY_rfoverride2_rxgainctrl_ovr_SHIFT);
	}
}
...
wlc_lpphy_set_rx_gain(phy_info_t *pi, uint32 gain)
{
	if (LPREV_GE(pi->pubpi.phy_rev, 2)) {
		...
		mod_phy_reg(pi, LPPHY_REV2_RFOverrideVal0,
			LPPHY_REV2_RFOverrideVal0_trsw_rx_pu_ovr_val_MASK,
			trsw << LPPHY_REV2_RFOverrideVal0_trsw_rx_pu_ovr_val_SHIFT);
		mod_phy_reg(pi, LPPHY_REV2_rfoverride2val,
			LPPHY_REV2_rfoverride2val_gmode_ext_lna_gain_ovr_val_MASK,
			ext_lna << LPPHY_REV2_rfoverride2val_gmode_ext_lna_gain_ovr_val_SHIFT);
		mod_phy_reg(pi, LPPHY_REV2_rfoverride2val,
			LPPHY_REV2_rfoverride2val_amode_ext_lna_gain_ovr_val_MASK,
			ext_lna << LPPHY_REV2_rfoverride2val_amode_ext_lna_gain_ovr_val_SHIFT);
		mod_phy_reg(pi, LPPHY_REV2_rxgainctrl0ovrval,
			LPPHY_REV2_rxgainctrl0ovrval_rxgainctrl_ovr_val0_MASK,
			gain0_15 << LPPHY_REV2_rxgainctrl0ovrval_rxgainctrl_ovr_val0_SHIFT);
		mod_phy_reg(pi, LPPHY_REV2_rxlnaandgainctrl1ovrval,
			LPPHY_REV2_rxlnaandgainctrl1ovrval_rxgainctrl_ovr_val1_MASK,
			gain16_19 << LPPHY_REV2_rxlnaandgainctrl1ovrval_rxgainctrl_ovr_val1_SHIFT);

		if (CHSPEC_IS2G(pi->radio_chanspec)) {
			mod_phy_reg(pi, LPPHY_REV2_rfoverride2val,
				LPPHY_REV2_rfoverride2val_slna_gain_ctrl_ovr_val_MASK,
				lna1 << LPPHY_REV2_rfoverride2val_slna_gain_ctrl_ovr_val_SHIFT);
			mod_phy_reg(pi, LPPHY_REV2_RFinputOverrideVal,
				LPPHY_REV2_RFinputOverrideVal_wlslnagainctrl_ovr_val_MASK,
				lna1 << LPPHY_REV2_RFinputOverrideVal_wlslnagainctrl_ovr_val_SHIFT);
		}
	} else {
		...
		mod_phy_reg(pi, LPPHY_RFOverrideVal0,
			LPPHY_RFOverrideVal0_trsw_rx_pu_ovr_val_MASK,
			trsw << LPPHY_RFOverrideVal0_trsw_rx_pu_ovr_val_SHIFT);
		mod_phy_reg(pi, LPPHY_rfoverride2val,
			LPPHY_rfoverride2val_gmode_ext_lna_gain_ovr_val_MASK,
			ext_lna << LPPHY_rfoverride2val_gmode_ext_lna_gain_ovr_val_SHIFT);
		mod_phy_reg(pi, LPPHY_rfoverride2val,
			LPPHY_rfoverride2val_amode_ext_lna_gain_ovr_val_MASK,
			ext_lna << LPPHY_rfoverride2val_amode_ext_lna_gain_ovr_val_SHIFT);
		mod_phy_reg(pi, LPPHY_rxgainctrlovrval,
			LPPHY_rxgainctrlovrval_rxgainctrl_ovr_val_MASK,
			lna << LPPHY_rxgainctrlovrval_rxgainctrl_ovr_val_SHIFT);
	}
...
wlc_lpphy_set_tx_filter_bw(phy_info_t *pi, uint16 bw)
{
	if (LPREV_GE(pi->pubpi.phy_rev, 2)) {
		...
		mod_phy_reg(pi, LPPHY_REV2_lpfbwlutreg0,
			LPPHY_REV2_lpfbwlutreg0_lpfbwlut0_MASK,
			bw << LPPHY_REV2_lpfbwlutreg0_lpfbwlut0_SHIFT);
		/* ofdm setting */
		mod_phy_reg(pi, LPPHY_REV2_lpfbwlutreg1,
			LPPHY_REV2_lpfbwlutreg1_lpfbwlut5_MASK,
			bw << LPPHY_REV2_lpfbwlutreg1_lpfbwlut5_SHIFT);
...
static void
wlc_lpphy_set_rx_gain_by_index(phy_info_t *pi, uint16 index)
{
	lpphytbl_info_t tab;
	uint16 gain;
...
static void
wlc_lpphy_set_tx_gain_by_index(phy_info_t *pi, int index)
{
	lpphytbl_info_t tab;
	...
	tab.tbl_offset = LPPHY_TX_PWR_CTRL_IQ_OFFSET + index; /* iqCoefLuts */
	tab.tbl_ptr = &bbmultiqcomp; /* ptr to buf */
	wlc_phy_table_read_lpphy(pi,  &tab);

	/* Read index based tx gain from the table */
	tab.tbl_offset = LPPHY_TX_PWR_CTRL_GAIN_OFFSET + index; /* gainCtrlLuts */
	...
	if (pi_lp->lpphy_use_tx_pwr_ctrl_coeffs) {
		...
		tab.tbl_offset = LPPHY_TX_PWR_CTRL_LO_OFFSET + index; /* loftCoefLuts */
	...
	if (LPREV_GE(pi->pubpi.phy_rev, 2)) {
		tab.tbl_offset = LPPHY_TX_PWR_CTRL_PWR_OFFSET + index;
		tab.tbl_ptr = &rfpower; /* ptr to buf */
		wlc_phy_table_read_lpphy(pi,  &tab);

		MOD_PHY_REG(pi, LPPHY_REV2, papdrfpowerOvrVal, rfpowerOvrVal, rfpower);
	}
...
void
wlc_phy_set_tx_pwr_by_index_lpphy(phy_info_t *pi, int index)
{
	phy_info_lpphy_t *pi_lp = pi->u.pi_lpphy;

	ASSERT(index <= LPPHY_MAX_TX_POWER_INDEX);

	/* Save forced index */
	pi_lp->lpphy_tx_power_idx_override = (int8)index;

	/* Turn off automatic power control */
	if (wlc_lpphy_tssi_enabled(pi))
		wlc_phy_set_tx_pwr_ctrl_lpphy(pi, LPPHY_TX_PWR_CTRL_SW);

	wlc_lpphy_set_tx_gain_by_index(pi, index);
}
...
wlc_lpphy_set_rc_cap(phy_info_t *pi, uint8 rc_cap)
{
	...
	if (LPREV_IS(pi->pubpi.phy_rev, 1)) {
		if (CHSPEC_CHANNEL(pi->radio_chanspec) != 14) {
			...
			wide_filter = pi->japan_wide_filter;
		}
...
wlc_phy_get_tssi_lpphy(phy_info_t *pi, int8 *ofdm_pwr, int8 *cck_pwr)
{
	...
	if (wlc_lpphy_tssi_enabled(pi) &&
		((status = (read_phy_reg(pi, LPPHY_TxPwrCtrlStatus))) &
...
wlc_lpphy_set_tssi_mux(phy_info_t *pi, lpphy_tssi_mode_t pos)
{
	if (LPPHY_TSSI_EXT == pos) {
		...
		or_phy_reg(pi, LPPHY_REV2_extstxctrl1, 0x01 << 12);
		...
		if (LPPHY_TSSI_POST_PA == pos) {
			...
			mod_phy_reg(pi, LPPHY_REV2_AfeCtrlOvrVal,
				LPPHY_REV2_AfeCtrlOvrVal_rssi_muxsel_ovr_val_MASK,
				0x00 << LPPHY_REV2_AfeCtrlOvrVal_rssi_muxsel_ovr_val_SHIFT);
		} else {
			mod_radio_reg(pi, RADIO_2063_PA_SP_1, 0x01, 1);
			mod_phy_reg(pi, LPPHY_REV2_AfeCtrlOvrVal,
				LPPHY_REV2_AfeCtrlOvrVal_rssi_muxsel_ovr_val_MASK,
				0x04 << LPPHY_REV2_AfeCtrlOvrVal_rssi_muxsel_ovr_val_SHIFT);
		}
	}
}
...
wlc_lpphy_compute_idle_tssi(phy_info_t *pi, int save_restore_flag)
{
	...
	if ((!(NORADIO_ENAB(pi->pubpi))) && pi->hwpwrctrl_capable) {
		...
		*/
		MOD_PHY_REG(pi, LPPHY_REV2, TxPwrCtrlNnum, Ntssi_delay, 255);
		MOD_PHY_REG(pi, LPPHY_REV2, TxPwrCtrlNnum, Ntssi_intg_log2, 6);

		MOD_PHY_REG(pi, LPPHY_REV2, TxPwrCtrlIdleTssi, idleTssi0, 0x1f);

		if (LPREV_GE(pi->pubpi.phy_rev, 2)) {
			uint8 iqcal_ctrl2;
			/* save the aux adc settings */
			auxadc_ctrl_old = read_phy_reg(pi, LPPHY_REV2_auxadcCtrl);
			MOD_PHY_REG(pi, LPPHY_REV2, auxadcCtrl, rssifiltEn, 0);
			MOD_PHY_REG(pi, LPPHY_REV2, auxadcCtrl, rssiformatConvEn, 1);
			MOD_PHY_REG(pi, LPPHY_REV2, auxadcCtrl, txpwrctrlEn, 1);
			...
			/* Set TSSI mux */
			if (LPREV_IS(pi->pubpi.phy_rev, 1)) {
				mod_phy_reg(pi, LPPHY_REV1_AfeRSSISel,
					LPPHY_REV1_AfeRSSISel_grssi_sel_MASK |
					LPPHY_REV1_AfeRSSISel_grssi_sel_ovr_MASK,
					LPPHY_REV1_AfeRSSISel_grssi_sel_ovr_MASK);
			}
			MOD_PHY_REG(pi, LPPHY, lpphyCtrl, rssifiltEn, 0);
			MOD_PHY_REG(pi, LPPHY, lpphyCtrl, rssiFormatConvEn, 1);
		}

		/* This register field is used only here. So no need to save and restore */
		MOD_PHY_REG(pi, LPPHY, TxPwrCtrlIdleTssi, rawTssiOffsetBinFormat, 1);

		/* Step */
		/* This register is used only in this function. So no need to save and restore */
		write_phy_reg(pi, LPPHY_TxPwrCtrlDeltaPwrLimit, 10);
		...
		internalrftxpu_ovr_old = READ_PHY_REG(pi, LPPHY, RFOverride0, internalrftxpu_ovr);
		internalrftxpu_ovr_val_old = READ_PHY_REG(pi,
			LPPHY, RFOverrideVal0, internalrftxpu_ovr_val);
		MOD_PHY_REG(pi, LPPHY, RFOverride0, internalrftxpu_ovr, 1);
		MOD_PHY_REG(pi, LPPHY, RFOverrideVal0, internalrftxpu_ovr_val, 0);
		...
		if (pi_lp->lpphy_cck_papd_disabled) {
			MOD_PHY_REG(pi, LPPHY, TxPwrCtrlDeltaPwrLimit, cckPwrOffset, 6);
		} else {
			MOD_PHY_REG(pi, LPPHY, TxPwrCtrlDeltaPwrLimit, cckPwrOffset, 0);
		}
		...
		if (status & LPPHY_TxPwrCtrlStatus_estPwrValid_MASK) {
			...
			MOD_PHY_REG(pi, LPPHY, TxPwrCtrlIdleTssi, idleTssi0, tssi_val);
		...
		if (LPREV_GE(pi->pubpi.phy_rev, 2)) {
			MOD_PHY_REG(pi, LPPHY_REV2, RFOverride0, internalrftxpu_ovr, 0);
			wlc_lpphy_set_pa_gain(pi, save_pagain);
		}
		else {
			MOD_PHY_REG(pi, LPPHY, RFOverride0, internalrftxpu_ovr, 0);
		}
		...
		if (save_restore_flag) {
			if (LPREV_GE(pi->pubpi.phy_rev, 2)) {
				/* Restore ADC control */
				write_phy_reg(pi, LPPHY_REV2_auxadcCtrl, auxadc_ctrl_old);

				/* Restore tssi switch */
				write_radio_reg(pi, RADIO_2063_IQCAL_CTRL_2,
					(uint16)iqcal_ctrl2_old);
				MOD_PHY_REG(pi, LPPHY, RFOverride0, internalrftxpu_ovr,
					internalrftxpu_ovr_old);
				MOD_PHY_REG(pi, LPPHY, RFOverrideVal0, internalrftxpu_ovr_val,
					internalrftxpu_ovr_val_old);
			} else {
				MOD_PHY_REG(pi, LPPHY_REV2, RFOverride0, internalrftxpu_ovr,
					internalrftxpu_ovr_old);
				MOD_PHY_REG(pi, LPPHY_REV2, RFOverrideVal0, internalrftxpu_ovr_val,
					internalrftxpu_ovr_val_old);
			}
...
WLBANDINITFN(wlc_lpphy_rev2_baseband_init)(phy_info_t *pi)
{
	...
	write_phy_reg(pi, LPPHY_REV2_AfeDACCtrl, 0x50);
	write_phy_reg(pi, LPPHY_REV2_AfeCtrl, 0x8800);
	write_phy_reg(pi, LPPHY_REV2_AfeCtrlOvr, 0x0000);
	write_phy_reg(pi, LPPHY_REV2_AfeCtrlOvrVal, 0x0000);
	write_phy_reg(pi, LPPHY_REV2_RFOverride0, 0x0000);
	write_phy_reg(pi, LPPHY_REV2_rfoverride2, 0x0000);
	write_phy_reg(pi, LPPHY_REV2_rfoverride3, 0x0000);
	write_phy_reg(pi, LPPHY_REV2_swctrlOvr, 0x0000);

	/* Swap ADC output */
	mod_phy_reg(pi, LPPHY_REV2_adcCompCtrl,
	            LPPHY_REV2_adcCompCtrl_flipiq_adcswap_MASK,
	            !ISSIM_ENAB(pi->sh->sih) ?
	            (1 << LPPHY_REV2_adcCompCtrl_flipiq_adcswap_SHIFT) : 0);

	if (LPREV_GE(pi->pubpi.phy_rev, 2)) {
		MOD_PHY_REG(pi, LPPHY_REV2, crsgainCtrl, MRCEnable, 0);
		write_phy_reg(pi, LPPHY_REV2_lnaputable, 0x5555);
	}
	/* Disable aux table since it's not needed */
	and_phy_reg(pi, LPPHY_REV2_radioCtrl,
	...
	mod_phy_reg(pi, LPPHY_REV2_ofdmSyncThresh0,
		LPPHY_REV2_ofdmSyncThresh0_ofdmSyncThresh0_MASK,
		180 << LPPHY_REV2_ofdmSyncThresh0_ofdmSyncThresh0_SHIFT);
	mod_phy_reg(pi, LPPHY_REV2_DCOffsetTransient,
		LPPHY_REV2_DCOffsetTransient_dcOffsetTransientFreeCtr_MASK,
		2 << LPPHY_REV2_DCOffsetTransient_dcOffsetTransientFreeCtr_SHIFT);
	mod_phy_reg(pi, LPPHY_REV2_DCOffsetTransient,
		LPPHY_REV2_DCOffsetTransient_dcOffsetTransientThresh_MASK,
		127 << LPPHY_REV2_DCOffsetTransient_dcOffsetTransientThresh_SHIFT);
	mod_phy_reg(pi, LPPHY_REV2_gaindirectMismatch,
		LPPHY_REV2_gaindirectMismatch_LogainDirectMismatch_MASK,
		4 << LPPHY_REV2_gaindirectMismatch_LogainDirectMismatch_SHIFT);
	mod_phy_reg(pi, LPPHY_REV2_PreambleConfirmTimeout,
		LPPHY_REV2_PreambleConfirmTimeout_OFDMPreambleConfirmTimeout_MASK,
		2 << LPPHY_REV2_PreambleConfirmTimeout_OFDMPreambleConfirmTimeout_SHIFT);

	mod_phy_reg(pi, LPPHY_REV2_crsgainCtrl,
		LPPHY_REV2_crsgainCtrl_wlpriogainChangeEn_MASK,
		0 << LPPHY_REV2_crsgainCtrl_wlpriogainChangeEn_SHIFT);
	mod_phy_reg(pi, LPPHY_REV2_crsgainCtrl,
		LPPHY_REV2_crsgainCtrl_preferredAntEn_MASK,
		0 << LPPHY_REV2_crsgainCtrl_preferredAntEn_SHIFT);

	mod_phy_reg(pi, LPPHY_REV2_radioTRCtrl,
		LPPHY_REV2_radioTRCtrl_gainrequestTRAttnOnEn_MASK,
		1 << LPPHY_REV2_radioTRCtrl_gainrequestTRAttnOnEn_SHIFT);

	/* This is needed to fix per issue at high powers for rev1.8 boards. default 3 */
	mod_phy_reg(pi, LPPHY_REV2_ofdmSyncThresh1,
	            LPPHY_REV2_ofdmSyncThresh1_ofdmSyncThresh2_MASK,
	            3<<LPPHY_REV2_ofdmSyncThresh1_ofdmSyncThresh2_SHIFT);

	if (LPREV_LE(pi->pubpi.phy_rev, 3)) {
		if (BOARDTYPE(pi->sh->boardtype) == BCM94325SDABGWBA_BOARD) {
			/* x7 board */
			MOD_PHY_REG(pi, LPPHY_REV2, radioTRCtrl, gainrequestTRAttnOnEn, 0);
			...
			if (pi->sh->boardrev >= 0x18) {
				...
				MOD_PHY_REG(pi, LPPHY_REV2, radioTRCtrl,
	...
	mod_phy_reg(pi, LPPHY_REV2_gainidxoffset,
		LPPHY_REV2_gainidxoffset_ofdmgainidxtableoffset_MASK,
		0xf4 << LPPHY_REV2_gainidxoffset_ofdmgainidxtableoffset_SHIFT);
	mod_phy_reg(pi, LPPHY_REV2_gainidxoffset,
		LPPHY_REV2_gainidxoffset_dsssgainidxtableoffset_MASK,
		0xf1 << LPPHY_REV2_gainidxoffset_dsssgainidxtableoffset_SHIFT);

	/* AGC tweaks */
	write_phy_reg(pi, LPPHY_REV2_ClipThresh, 72);
	mod_phy_reg(pi, LPPHY_REV2_HiGainDB,
		LPPHY_REV2_HiGainDB_HiGainDB_MASK,
		70 << LPPHY_REV2_HiGainDB_HiGainDB_SHIFT);
	mod_phy_reg(pi, LPPHY_REV2_nftrAdj,
		LPPHY_REV2_nftrAdj_nftrAdj_MASK,
		16 << LPPHY_REV2_nftrAdj_nftrAdj_SHIFT);
	mod_phy_reg(pi, LPPHY_REV2_PwrThresh1,
		LPPHY_REV2_PwrThresh1_LargeGainMismatchThresh_MASK,
		9 << LPPHY_REV2_PwrThresh1_LargeGainMismatchThresh_SHIFT);
	mod_phy_reg(pi, LPPHY_REV2_gaindirectMismatch,
		LPPHY_REV2_gaindirectMismatch_MedHigainDirectMismatch_MASK,
		0 << LPPHY_REV2_gaindirectMismatch_MedHigainDirectMismatch_SHIFT);
	mod_phy_reg(pi, LPPHY_REV2_VeryLowGainDB,
		LPPHY_REV2_VeryLowGainDB_NominalPwrDB_MASK,
		85 << LPPHY_REV2_VeryLowGainDB_NominalPwrDB_SHIFT);
	mod_phy_reg(pi, LPPHY_REV2_ClipCtrThresh,
		LPPHY_REV2_ClipCtrThresh_clipCtrThreshLoGain_MASK,
		5 << LPPHY_REV2_ClipCtrThresh_clipCtrThreshLoGain_SHIFT);
	mod_phy_reg(pi, LPPHY_REV2_gaindirectMismatch,
		LPPHY_REV2_gaindirectMismatch_GainmisMatchMedGain_MASK,
		3 << LPPHY_REV2_gaindirectMismatch_GainmisMatchMedGain_SHIFT);
	mod_phy_reg(pi, LPPHY_REV2_HiGainDB,
		LPPHY_REV2_HiGainDB_MedHiGainDB_MASK,
		42 << LPPHY_REV2_HiGainDB_MedHiGainDB_SHIFT);

	mod_phy_reg(pi, LPPHY_REV2_LowGainDB,
	            LPPHY_REV2_LowGainDB_MedLowGainDB_MASK,
	            30 << LPPHY_REV2_LowGainDB_MedLowGainDB_SHIFT);
	mod_phy_reg(pi, LPPHY_REV2_VeryLowGainDB,
	            LPPHY_REV2_VeryLowGainDB_veryLowGainDB_MASK,
	            13 << LPPHY_REV2_VeryLowGainDB_veryLowGainDB_SHIFT);

	/* MED_GAIN_2 state is disabled b/c LargeGainMismatchThresh == MedHigainDirectMismatch */
	mod_phy_reg(pi, LPPHY_REV2_clipCtrThreshLowGainEx,
		LPPHY_REV2_clipCtrThreshLowGainEx_clipCtrThreshLoGain2_MASK,
		31 << LPPHY_REV2_clipCtrThreshLowGainEx_clipCtrThreshLoGain2_SHIFT);
	mod_phy_reg(pi, LPPHY_REV2_gainMismatchMedGainEx,
		LPPHY_REV2_gainMismatchMedGainEx_gainMismatchMedGain2_MASK,
		12 << LPPHY_REV2_gainMismatchMedGainEx_gainMismatchMedGain2_SHIFT);
	mod_phy_reg(pi, LPPHY_REV2_veryLowGainEx,
		LPPHY_REV2_veryLowGainEx_veryLowGain_2DB_MASK,
		25 << LPPHY_REV2_veryLowGainEx_veryLowGain_2DB_SHIFT);

	/* MED_GAIN_3 is disabled b/c  LargeGainMismatchThresh >= medHiGainDirectMismatchOFDMDet */
	mod_phy_reg(pi, LPPHY_REV2_gainMismatchMedGainEx,
		LPPHY_REV2_gainMismatchMedGainEx_medHiGainDirectMismatchOFDMDet_MASK,
		15 << LPPHY_REV2_gainMismatchMedGainEx_medHiGainDirectMismatchOFDMDet_SHIFT);
	mod_phy_reg(pi, LPPHY_REV2_clipCtrThreshLowGainEx,
		LPPHY_REV2_clipCtrThreshLowGainEx_clipCtrThreshLoGain3_MASK,
		31 << LPPHY_REV2_clipCtrThreshLowGainEx_clipCtrThreshLoGain3_SHIFT);
	mod_phy_reg(pi, LPPHY_REV2_gainMismatchMedGainEx,
		LPPHY_REV2_gainMismatchMedGainEx_gainMismatchMedGain2_MASK,
		12 << LPPHY_REV2_gainMismatchMedGainEx_gainMismatchMedGain2_SHIFT);
	mod_phy_reg(pi, LPPHY_REV2_veryLowGainEx,
		LPPHY_REV2_veryLowGainEx_veryLowGain_3DB_MASK,
		25 << LPPHY_REV2_veryLowGainEx_veryLowGain_3DB_SHIFT);

	mod_phy_reg(pi, LPPHY_REV2_ClipCtrThresh,
		LPPHY_REV2_ClipCtrThresh_clipCtrThresh_MASK,
		22 << LPPHY_REV2_ClipCtrThresh_clipCtrThresh_SHIFT);
	mod_phy_reg(pi, LPPHY_REV2_ClipCtrThresh,
		LPPHY_REV2_ClipCtrThresh_ClipCtrThreshHiGain_MASK,
		18 << LPPHY_REV2_ClipCtrThresh_ClipCtrThreshHiGain_SHIFT);
	mod_phy_reg(pi, LPPHY_REV2_gainMismatch,
		LPPHY_REV2_gainMismatch_GainmisMatchPktRx_MASK,
		5 << LPPHY_REV2_gainMismatch_GainmisMatchPktRx_SHIFT);

	/* Select phy band */
	if (CHSPEC_IS2G(pi->radio_chanspec)) {
		/* Enable DSS detector */
		mod_phy_reg(pi, LPPHY_REV2_crsgainCtrl,
			LPPHY_REV2_crsgainCtrl_DSSSDetectionEnable_MASK,
			0x1 << LPPHY_REV2_crsgainCtrl_DSSSDetectionEnable_SHIFT);
		/* Don't assert phycrs based on crsgain's DSSS detector */
		/* Wait for bphy to detect it as well */
		mod_phy_reg(pi, LPPHY_REV2_crsgainCtrl,
			LPPHY_REV2_crsgainCtrl_phycrsctrl_MASK,
			0xb << LPPHY_REV2_crsgainCtrl_phycrsctrl_SHIFT);
		/* Make bphy's detector less sensitive */
		mod_phy_reg(pi, LPPHY_REV2_SyncPeakCnt,
			LPPHY_REV2_SyncPeakCnt_MaxPeakCntM1_MASK,
			6 << LPPHY_REV2_SyncPeakCnt_MaxPeakCntM1_SHIFT);
		mod_phy_reg(pi, LPPHY_REV2_MinPwrLevel,
			LPPHY_REV2_MinPwrLevel_dsssMinPwrLevel_MASK,
			0x9d << LPPHY_REV2_MinPwrLevel_dsssMinPwrLevel_SHIFT);
		mod_phy_reg(pi, LPPHY_REV2_MinPwrLevel,
			LPPHY_REV2_MinPwrLevel_ofdmMinPwrLevel_MASK,
			0xa1 << LPPHY_REV2_MinPwrLevel_ofdmMinPwrLevel_SHIFT);

	} else
		mod_phy_reg(pi, LPPHY_REV2_crsgainCtrl,
			LPPHY_REV2_crsgainCtrl_DSSSDetectionEnable_MASK,
			0x0 << LPPHY_REV2_crsgainCtrl_DSSSDetectionEnable_SHIFT);

	mod_phy_reg(pi, LPPHY_REV2_crsedthresh,
		LPPHY_REV2_crsedthresh_edonthreshold_MASK,
		0xb3 << LPPHY_REV2_crsedthresh_edonthreshold_SHIFT);
	mod_phy_reg(pi, LPPHY_REV2_crsedthresh,
		LPPHY_REV2_crsedthresh_edoffthreshold_MASK,
		0xad << LPPHY_REV2_crsedthresh_edoffthreshold_SHIFT);

	{
		...
		mod_phy_reg(pi, LPPHY_REV2_InputPowerDB,
			LPPHY_REV2_InputPowerDB_inputpwroffsetdb_MASK,
			(uint16)lpphy_rx_power_offset <<
			LPPHY_REV2_InputPowerDB_inputpwroffsetdb_SHIFT);
	}

	/* Reset radio ctrl and crs gain */
	or_phy_reg(pi, LPPHY_REV2_resetCtrl, 0x44);
	write_phy_reg(pi, LPPHY_REV2_resetCtrl, 0x80);

	/* RSSI settings */
	write_phy_reg(pi, LPPHY_REV2_AfeRSSICtrl0, 0xA954);
	{
		uint8 lpphy_rssi_vf;
		uint8 lpphy_rssi_vc;
		uint8 lpphy_rssi_gs;
		...
		write_phy_reg(pi, LPPHY_REV2_AfeRSSICtrl1,
	...
	if ((LPREV_GE(pi->pubpi.phy_rev, 4)) ||
		...
		MOD_PHY_REG(pi, LPPHY_REV2, InputPowerDB, inputpwroffsetdb, -1);
		MOD_PHY_REG(pi, LPPHY_REV2, VeryLowGainDB, NominalPwrDB, 81);
		MOD_PHY_REG(pi, LPPHY_REV2, MinPwrLevel, ofdmMinPwrLevel, -93);
	}
	...
	if (CHIPID(pi->sh->chip) == BCM4325_CHIP_ID &&
		...
		MOD_PHY_REG(pi, LPPHY_REV2, radioCtrl, extlnaen, 1);
		MOD_PHY_REG(pi, LPPHY_REV2, extlnagainvalue0, extlnagain1, 12);
		MOD_PHY_REG(pi, LPPHY_REV2, extlnagainvalue0, extlnagain0, -15);
		...
		MOD_PHY_REG(pi, LPPHY_REV2, nftrAdj, nftrAdj, 48);

		MOD_PHY_REG(pi, LPPHY_REV2, radioTRCtrl, gainrequestTRAttnOnOffset, 10);
		tab.tbl_ptr = tabBuffer2;
		tab.tbl_len = 1;
		tab.tbl_offset = 64;
		wlc_phy_table_read_lpphy(pi, &tab);
		...
		if (RGain > 63)
			RGain = RGain - 128;
		temp32 = (read_phy_reg(pi, LPPHY_REV2_radioTRCtrl)
		          & LPPHY_REV2_radioTRCtrl_gainrequestTRAttnOnOffset_MASK)
		                  >> LPPHY_REV2_radioTRCtrl_gainrequestTRAttnOnOffset_SHIFT;
		...
		MOD_PHY_REG(pi, LPPHY_REV2, ofdmSyncThresh0, ofdmSyncThresh0, 160);
		MOD_PHY_REG(pi, LPPHY_REV2, ofdmSyncThresh0, ofdmSyncThresh1, 85);
		MOD_PHY_REG(pi, LPPHY_REV2, ofdmSyncThresh1, ofdmSyncThresh2, 2);
		MOD_PHY_REG(pi, LPPHY_REV2, ofdmSyncThresh1, ofdmSyncThresh3, 3);
		MOD_PHY_REG(pi, LPPHY_REV2, LTRNCtrl, crsLTRNOffset, 14);
		MOD_PHY_REG(pi, LPPHY_REV2, radioTRCtrl, gainrequestTRAttnOnEn, 1);
		MOD_PHY_REG(pi, LPPHY_REV2, InputPowerDB, inputpwroffsetdb, -1);
		MOD_PHY_REG(pi, LPPHY_REV2, VeryLowGainDB, veryLowGainDB, 14);
		MOD_PHY_REG(pi, LPPHY_REV2, LowGainDB, MedLowGainDB, 35);
		MOD_PHY_REG(pi, LPPHY_REV2, ClipCtrThresh, clipCtrThreshLoGain, 4);
		MOD_PHY_REG(pi, LPPHY_REV2, gaindirectMismatch, GainmisMatchMedGain, 3);
		MOD_PHY_REG(pi, LPPHY_REV2, ClipCtrThresh, clipCtrThresh, 22);
		MOD_PHY_REG(pi, LPPHY_REV2, ClipCtrThresh, ClipCtrThreshHiGain, 18);
		MOD_PHY_REG(pi, LPPHY_REV2, HiGainDB, HiGainDB, 73);
		MOD_PHY_REG(pi, LPPHY_REV2, PwrThresh1, LargeGainMismatchThresh, 9);
		MOD_PHY_REG(pi, LPPHY_REV2, gaindirectMismatch, MedHigainDirectMismatch, 0);
		MOD_PHY_REG(pi, LPPHY_REV2, DCBlankInterval, DCBlankIntervalLoGain, 14);
		MOD_PHY_REG(pi, LPPHY_REV2, ofdmSyncTimerCtrl, OFDMPreambleSyncTimeOut, 8);
		MOD_PHY_REG(pi, LPPHY_REV2, ofdmSyncTimerCtrl, ofdmSyncConfirmTime, 8);
		write_phy_reg(pi, LPPHY_REV2_ClipThresh, 64);

		tempu16 = READ_PHY_REG(pi, LPPHY_REV2, ClipCtrThresh, clipCtrThreshLoGain);
		MOD_PHY_REG(pi, LPPHY_REV2,
		            clipCtrThreshLowGainEx, clipCtrThreshLoGain2, tempu16);
		MOD_PHY_REG(pi, LPPHY_REV2,
		            clipCtrThreshLowGainEx, clipCtrThreshLoGain3, tempu16);
		tempu16 = READ_PHY_REG(pi, LPPHY_REV2,
		...
		tempu16 = READ_PHY_REG(pi, LPPHY_REV2, VeryLowGainDB, veryLowGainDB);
		MOD_PHY_REG(pi, LPPHY_REV2, veryLowGainEx, veryLowGain_2DB, tempu16);
		MOD_PHY_REG(pi, LPPHY_REV2, veryLowGainEx, veryLowGain_3DB, tempu16);
	}

	if (LPREV_GE(pi->pubpi.phy_rev, 2)) {
		/* reset the AGC */
		tempu16 = (read_phy_reg(pi, LPPHY_REV2_resetCtrl) |
		           (1 << LPPHY_REV2_resetCtrl_radioctrlSoftReset_SHIFT) |
		           (1 << LPPHY_REV2_resetCtrl_rxfrontendSoftReset_SHIFT));
		write_phy_reg(pi, LPPHY_REV2_resetCtrl, tempu16);
		write_phy_reg(pi, LPPHY_REV2_resetCtrl,
...
WLBANDINITFN(wlc_lpphy_rev0_baseband_init)(phy_info_t *pi)
{
	...
	/* Enable DAC/ADC */
	and_phy_reg(pi, LPPHY_AfeDACCtrl, ~0x0800);
	write_phy_reg(pi, LPPHY_AfeCtrl, 0);

	/* Clear overrides by default */
	write_phy_reg(pi, LPPHY_AfeCtrlOvr, 0);
	write_phy_reg(pi, LPPHY_RFOverride0, 0);
	write_phy_reg(pi, LPPHY_rfoverride2, 0);

	if (NORADIO_ENAB(pi->pubpi))
		return;

	or_phy_reg(pi, LPPHY_AfeDACCtrl, 0x04);

	/* in tot spreadsheet(revision 1.141) */
	mod_phy_reg(pi, LPPHY_ofdmSyncThresh0,
		LPPHY_ofdmSyncThresh0_ofdmSyncThresh0_MASK,
		0x78 <<LPPHY_ofdmSyncThresh0_ofdmSyncThresh0_SHIFT);
	mod_phy_reg(pi, LPPHY_ClipCtrThresh,
		LPPHY_ClipCtrThresh_clipCtrThresh_MASK,
		0x16 << LPPHY_ClipCtrThresh_clipCtrThresh_SHIFT);

	/* flip IQ */
	write_phy_reg(pi,  LPPHY_adcCompCtrl, 0x0016);

	/* Table in AFE AMS is wrong. Details provide the right value */
	/* tadj_iqadc_ref40u: default value is 4. */
	mod_phy_reg(pi, LPPHY_AfeADCCtrl0, 0x07, 0x04);

	/* Gain control */
	mod_phy_reg(pi, LPPHY_VeryLowGainDB,
		LPPHY_VeryLowGainDB_NominalPwrDB_MASK,
		0x54 << LPPHY_VeryLowGainDB_NominalPwrDB_SHIFT);
	mod_phy_reg(pi, LPPHY_HiGainDB,
		LPPHY_HiGainDB_MedHiGainDB_MASK,
		36 << LPPHY_HiGainDB_MedHiGainDB_SHIFT);
	mod_phy_reg(pi, LPPHY_LowGainDB,
		LPPHY_LowGainDB_MedLowGainDB_MASK,
		33 << LPPHY_LowGainDB_MedLowGainDB_SHIFT);
	mod_phy_reg(pi, LPPHY_VeryLowGainDB,
		LPPHY_VeryLowGainDB_veryLowGainDB_MASK,
		6 << LPPHY_VeryLowGainDB_veryLowGainDB_SHIFT);
	mod_phy_reg(pi, LPPHY_RxRadioControl,
		LPPHY_RxRadioControl_fine_gain_MASK,
		0 << LPPHY_RxRadioControl_fine_gain_SHIFT);


	mod_phy_reg(pi, LPPHY_ClipCtrThresh,
		LPPHY_ClipCtrThresh_ClipCtrThreshHiGain_MASK,
		5 << LPPHY_ClipCtrThresh_ClipCtrThreshHiGain_SHIFT);
	mod_phy_reg(pi, LPPHY_ClipCtrThresh,
		LPPHY_ClipCtrThresh_clipCtrThreshLoGain_MASK,
		12 << LPPHY_ClipCtrThresh_clipCtrThreshLoGain_SHIFT);
	mod_phy_reg(pi, LPPHY_ClipCtrThresh,
		LPPHY_ClipCtrThresh_clipCtrThresh_MASK,
		15 << LPPHY_ClipCtrThresh_clipCtrThresh_SHIFT);

	mod_phy_reg(pi, LPPHY_gaindirectMismatch,
		LPPHY_gaindirectMismatch_MedHigainDirectMismatch_MASK,
		5 << LPPHY_gaindirectMismatch_MedHigainDirectMismatch_SHIFT);
	mod_phy_reg(pi, LPPHY_gainMismatchLimit,
		LPPHY_gainMismatchLimit_gainmismatchlimit_MASK,
		26 << LPPHY_gainMismatchLimit_gainmismatchlimit_SHIFT);

	/* Setup energy detector */
	mod_phy_reg(pi, LPPHY_crsedthresh,
		LPPHY_crsedthresh_edonthreshold_MASK,
		0xb3 << LPPHY_crsedthresh_edonthreshold_SHIFT);
	mod_phy_reg(pi, LPPHY_crsedthresh,
		LPPHY_crsedthresh_edoffthreshold_MASK,
		0xad << LPPHY_crsedthresh_edoffthreshold_SHIFT);

	{
		...
		mod_phy_reg(pi, LPPHY_InputPowerDB,
			LPPHY_InputPowerDB_inputpwroffsetdb_MASK,
			(uint16)lpphy_rx_power_offset << LPPHY_InputPowerDB_inputpwroffsetdb_SHIFT);
	}

	if ((!(pi->sh->boardflags & BFL_FEM) &&
	     (CHSPEC_IS5G(pi->radio_chanspec))) ||
		(pi->sh->boardflags & BFL_PAREF)) {
		...
		if (LPREV_IS(pi->pubpi.phy_rev, 0))
			mod_phy_reg(pi, LPPHY_lprfsignallut,
				LPPHY_lprfsignallut_amode_tx_pu_lut_MASK,
				0x1 << LPPHY_lprfsignallut_amode_tx_pu_lut_SHIFT);
		...
		mod_phy_reg(pi, LPPHY_lprfsignallut,
			LPPHY_lprfsignallut_amode_tx_pu_lut_MASK,
			0x2 << LPPHY_lprfsignallut_amode_tx_pu_lut_SHIFT);
	...
	/* RSSI settings */
	{
		...
		write_phy_reg(pi, LPPHY_AfeRSSICtrl0,
			((uint16)lpphy_rssi_vf << 0) | /* selmid_rssi: RSSI Vmid fine */
			((uint16)lpphy_rssi_vc << 4) | /* selmid_rssi: RSSI Vmid coarse */
			(0x00 << 8) | /* selmid_rssi: default value from AMS */
			((uint16)lpphy_rssi_gs << 10) | /* selav_rssi: RSSI gain select */
	...
	rssi_ctrl = 0x2aa;
	if (pi->sh->boardflags & BFL_RSSIINV)
		rssi_ctrl |= 0x800;
	mod_phy_reg(pi, LPPHY_AfeRSSICtrl1, 0xfff, rssi_ctrl);
	...
	/* Board switch architecture */
	{
		...
		mod_phy_reg(pi, LPPHY_RxRadioControl,
			LPPHY_RxRadioControl_board_switch_arch_MASK,
			(uint16)lpphy_bx_arch << LPPHY_RxRadioControl_board_switch_arch_SHIFT);
	}

	/* TR switches */
	if (LPREV_IS(pi->pubpi.phy_rev, 1) && (pi->sh->boardflags & BFL_FEM_BT)) {
		mod_phy_reg(pi, LPPHY_TRLookup1, LPPHY_TRLookup1_TRLut0_MASK,
			0x9 << LPPHY_TRLookup1_TRLut0_SHIFT);
		mod_phy_reg(pi, LPPHY_TRLookup1, LPPHY_TRLookup1_TRLut1_MASK,
			0x9 << LPPHY_TRLookup1_TRLut1_SHIFT);
		mod_phy_reg(pi, LPPHY_TRLookup2, LPPHY_TRLookup2_TRLut2_MASK,
			0xa << LPPHY_TRLookup2_TRLut2_SHIFT);
		mod_phy_reg(pi, LPPHY_TRLookup2, LPPHY_TRLookup2_TRLut3_MASK,
			0xa << LPPHY_TRLookup2_TRLut3_SHIFT);
		mod_phy_reg(pi, LPPHY_TRLookup3, LPPHY_TRLookup3_TRLut4_MASK,
			0x9 << LPPHY_TRLookup3_TRLut4_SHIFT);
		mod_phy_reg(pi, LPPHY_TRLookup3, LPPHY_TRLookup3_TRLut5_MASK,
			0x9 << LPPHY_TRLookup3_TRLut5_SHIFT);
		mod_phy_reg(pi, LPPHY_TRLookup4, LPPHY_TRLookup4_TRLut6_MASK,
			0xa << LPPHY_TRLookup4_TRLut6_SHIFT);
		mod_phy_reg(pi, LPPHY_TRLookup4, LPPHY_TRLookup4_TRLut7_MASK,
			0xa << LPPHY_TRLookup4_TRLut7_SHIFT);
		mod_phy_reg(pi, LPPHY_REV1_TRLookup5, LPPHY_REV1_TRLookup5_TRLut8_MASK,
			0x5 << LPPHY_REV1_TRLookup5_TRLut8_SHIFT);
		mod_phy_reg(pi, LPPHY_REV1_TRLookup5, LPPHY_REV1_TRLookup5_TRLut9_MASK,
			0x5 << LPPHY_REV1_TRLookup5_TRLut9_SHIFT);
		mod_phy_reg(pi, LPPHY_REV1_TRLookup6, LPPHY_REV1_TRLookup6_TRLut10_MASK,
			0x6 << LPPHY_REV1_TRLookup6_TRLut10_SHIFT);
		mod_phy_reg(pi, LPPHY_REV1_TRLookup6, LPPHY_REV1_TRLookup6_TRLut11_MASK,
			0x6 << LPPHY_REV1_TRLookup6_TRLut11_SHIFT);
		mod_phy_reg(pi, LPPHY_REV1_TRLookup7, LPPHY_REV1_TRLookup7_TRLut12_MASK,
			0x5 << LPPHY_REV1_TRLookup7_TRLut12_SHIFT);
		mod_phy_reg(pi, LPPHY_REV1_TRLookup7, LPPHY_REV1_TRLookup7_TRLut13_MASK,
			0x5 << LPPHY_REV1_TRLookup7_TRLut13_SHIFT);
		mod_phy_reg(pi, LPPHY_REV1_TRLookup8, LPPHY_REV1_TRLookup8_TRLut14_MASK,
			0x6 << LPPHY_REV1_TRLookup8_TRLut14_SHIFT);
		mod_phy_reg(pi, LPPHY_REV1_TRLookup8, LPPHY_REV1_TRLookup8_TRLut15_MASK,
			0x6 << LPPHY_REV1_TRLookup8_TRLut15_SHIFT);
	} else if ((CHSPEC_IS5G(pi->radio_chanspec)) ||
	     (BOARDTYPE(pi->sh->boardtype) == BU4312_SSID) ||
	     (!LPREV_IS(pi->pubpi.phy_rev, 1) && (pi->sh->boardflags & BFL_FEM))) {
		mod_phy_reg(pi, LPPHY_TRLookup1, LPPHY_TRLookup1_TRLut0_MASK,
			0x1 << LPPHY_TRLookup1_TRLut0_SHIFT);
		mod_phy_reg(pi, LPPHY_TRLookup1, LPPHY_TRLookup1_TRLut1_MASK,
			0x4 << LPPHY_TRLookup1_TRLut1_SHIFT);
		mod_phy_reg(pi, LPPHY_TRLookup2, LPPHY_TRLookup2_TRLut2_MASK,
			0x1 << LPPHY_TRLookup2_TRLut2_SHIFT);
		mod_phy_reg(pi, LPPHY_TRLookup2, LPPHY_TRLookup2_TRLut3_MASK,
			0x5 << LPPHY_TRLookup2_TRLut3_SHIFT);
		mod_phy_reg(pi, LPPHY_TRLookup3, LPPHY_TRLookup3_TRLut4_MASK,
			0x2 << LPPHY_TRLookup3_TRLut4_SHIFT);
		mod_phy_reg(pi, LPPHY_TRLookup3, LPPHY_TRLookup3_TRLut5_MASK,
			0x8 << LPPHY_TRLookup3_TRLut5_SHIFT);
		mod_phy_reg(pi, LPPHY_TRLookup4, LPPHY_TRLookup4_TRLut6_MASK,
			0x2 << LPPHY_TRLookup4_TRLut6_SHIFT);
		mod_phy_reg(pi, LPPHY_TRLookup4, LPPHY_TRLookup4_TRLut7_MASK,
			0xa << LPPHY_TRLookup4_TRLut7_SHIFT);
	} else if (LPREV_IS(pi->pubpi.phy_rev, 1) && (pi->sh->boardflags & BFL_FEM)) {
		mod_phy_reg(pi, LPPHY_TRLookup1, LPPHY_TRLookup1_TRLut0_MASK,
			0x4 << LPPHY_TRLookup1_TRLut0_SHIFT);
		mod_phy_reg(pi, LPPHY_TRLookup1, LPPHY_TRLookup1_TRLut1_MASK,
			0x8 << LPPHY_TRLookup1_TRLut1_SHIFT);
		mod_phy_reg(pi, LPPHY_TRLookup2, LPPHY_TRLookup2_TRLut2_MASK,
			0x4 << LPPHY_TRLookup2_TRLut2_SHIFT);
		mod_phy_reg(pi, LPPHY_TRLookup2, LPPHY_TRLookup2_TRLut3_MASK,
			0xc << LPPHY_TRLookup2_TRLut3_SHIFT);
		mod_phy_reg(pi, LPPHY_TRLookup3, LPPHY_TRLookup3_TRLut4_MASK,
			0x2 << LPPHY_TRLookup3_TRLut4_SHIFT);
		mod_phy_reg(pi, LPPHY_TRLookup3, LPPHY_TRLookup3_TRLut5_MASK,
			0x1 << LPPHY_TRLookup3_TRLut5_SHIFT);
		mod_phy_reg(pi, LPPHY_TRLookup4, LPPHY_TRLookup4_TRLut6_MASK,
			0x2 << LPPHY_TRLookup4_TRLut6_SHIFT);
		mod_phy_reg(pi, LPPHY_TRLookup4, LPPHY_TRLookup4_TRLut7_MASK,
			0x3 << LPPHY_TRLookup4_TRLut7_SHIFT);
	} else {
		mod_phy_reg(pi, LPPHY_TRLookup1, LPPHY_TRLookup1_TRLut0_MASK,
			0xa << LPPHY_TRLookup1_TRLut0_SHIFT);
		mod_phy_reg(pi, LPPHY_TRLookup1, LPPHY_TRLookup1_TRLut1_MASK,
			0x9 << LPPHY_TRLookup1_TRLut1_SHIFT);
		mod_phy_reg(pi, LPPHY_TRLookup2, LPPHY_TRLookup2_TRLut2_MASK,
			0xa << LPPHY_TRLookup2_TRLut2_SHIFT);
		mod_phy_reg(pi, LPPHY_TRLookup2, LPPHY_TRLookup2_TRLut3_MASK,
			0xb << LPPHY_TRLookup2_TRLut3_SHIFT);
		mod_phy_reg(pi, LPPHY_TRLookup3, LPPHY_TRLookup3_TRLut4_MASK,
			0x6 << LPPHY_TRLookup3_TRLut4_SHIFT);
		mod_phy_reg(pi, LPPHY_TRLookup3, LPPHY_TRLookup3_TRLut5_MASK,
			0x5 << LPPHY_TRLookup3_TRLut5_SHIFT);
		mod_phy_reg(pi, LPPHY_TRLookup4, LPPHY_TRLookup4_TRLut6_MASK,
			0x6 << LPPHY_TRLookup4_TRLut6_SHIFT);
		mod_phy_reg(pi, LPPHY_TRLookup4, LPPHY_TRLookup4_TRLut7_MASK,
			0x7 << LPPHY_TRLookup4_TRLut7_SHIFT);
	}

	if (LPREV_IS(pi->pubpi.phy_rev, 1) && !(pi->sh->boardflags & BFL_FEM_BT)) {
		write_phy_reg(pi, LPPHY_REV1_TRLookup5, read_phy_reg(pi, LPPHY_REV1_TRLookup1));
		write_phy_reg(pi, LPPHY_REV1_TRLookup6, read_phy_reg(pi, LPPHY_REV1_TRLookup2));
		write_phy_reg(pi, LPPHY_REV1_TRLookup7, read_phy_reg(pi, LPPHY_REV1_TRLookup3));
		write_phy_reg(pi, LPPHY_REV1_TRLookup8, read_phy_reg(pi, LPPHY_REV1_TRLookup4));
	}

	/* Use Phase Shifter */
	if ((CHIPID(pi->sh->chip) == BCM5354_CHIP_ID) &&
	    (pi->sh->chippkg == BCM5354E_PKG_ID) &&
	    (pi->sh->boardflags & BFL_PHASESHIFT)) {
		/* Enable MRC */
		or_phy_reg(pi, LPPHY_crsgainCtrl,
			LPPHY_crsgainCtrl_DiversityChkEnable_MASK |
			LPPHY_crsgainCtrl_MRCEnable_MASK);

		write_phy_reg(pi, LPPHY_gpioSel, 5);
		write_phy_reg(pi, LPPHY_gpioOutEn, 0xffff);
	...
	/* Select phy band */
	if (CHSPEC_IS2G(pi->radio_chanspec)) {
		or_phy_reg(pi, LPPHY_lpphyCtrl, LPPHY_lpphyCtrl_muxGmode_MASK);
		or_phy_reg(pi, LPPHY_crsgainCtrl, LPPHY_crsgainCtrl_DSSSDetectionEnable_MASK);

		/* Raise the minimum DSSS detector power */
		mod_phy_reg(pi, LPPHY_MinPwrLevel,
			LPPHY_MinPwrLevel_dsssMinPwrLevel_MASK,
			0xa4 << LPPHY_MinPwrLevel_dsssMinPwrLevel_SHIFT);

		/* Don't assert phycrs based on crsgain's DSSS detector */
		/* Wait for bphy to detect it as well */
		mod_phy_reg(pi, LPPHY_crsgainCtrl,
			LPPHY_crsgainCtrl_phycrsctrl_MASK,
			0xb << LPPHY_crsgainCtrl_phycrsctrl_SHIFT);

		/* Make bphy's detector less sensitive */
		mod_phy_reg(pi, LPPHY_SyncPeakCnt,
			LPPHY_SyncPeakCnt_MaxPeakCntM1_MASK,
			7 << LPPHY_SyncPeakCnt_MaxPeakCntM1_SHIFT);
		mod_phy_reg(pi, LPPHY_DSSSConfirmCnt,
			LPPHY_DSSSConfirmCnt_DSSSConfirmCntHiGain_MASK,
			3 << LPPHY_DSSSConfirmCnt_DSSSConfirmCntHiGain_SHIFT);
		mod_phy_reg(pi, LPPHY_DSSSConfirmCnt,
			LPPHY_DSSSConfirmCnt_DSSSConfirmCntLoGain_MASK,
			4 << LPPHY_DSSSConfirmCnt_DSSSConfirmCntLoGain_SHIFT);

		/* Prevent bphy from dropping CRS after exiting rx state */
		mod_phy_reg(pi, LPPHY_IDLEafterPktRXTimeout,
			LPPHY_IDLEafterPktRXTimeout_BPHYIdleAfterPktRxTimeOut_MASK,
			0 << LPPHY_IDLEafterPktRXTimeout_BPHYIdleAfterPktRxTimeOut_SHIFT);
	} else {
		and_phy_reg(pi, LPPHY_lpphyCtrl, (uint16)~LPPHY_lpphyCtrl_muxGmode_MASK);
		and_phy_reg(pi, LPPHY_crsgainCtrl,
	...
	if (LPREV_IS(pi->pubpi.phy_rev, 1)) {
		...
		tmp = (read_phy_reg(pi, LPPHY_REV1_ClipCtrThresh)
			& LPPHY_REV1_ClipCtrThresh_clipCtrThreshLoGain_MASK)
			>> LPPHY_REV1_ClipCtrThresh_clipCtrThreshLoGain_SHIFT;
		val =
		...
		write_phy_reg(pi, LPPHY_REV1_clipCtrThreshLowGainEx, val);

		tmp = (read_phy_reg(pi, LPPHY_REV1_gaindirectMismatch)
			& LPPHY_REV1_gaindirectMismatch_GainmisMatchMedGain_MASK)
			>> LPPHY_REV1_gaindirectMismatch_GainmisMatchMedGain_SHIFT;
		val =
		...
		write_phy_reg(pi, LPPHY_REV1_gainMismatchMedGainEx, val);

	        tmp = (read_phy_reg(pi, LPPHY_REV1_VeryLowGainDB)
			& LPPHY_REV1_VeryLowGainDB_veryLowGainDB_MASK)
			>> LPPHY_REV1_VeryLowGainDB_veryLowGainDB_SHIFT;
		val =
			(tmp << LPPHY_REV1_veryLowGainEx_veryLowGain_2DB_SHIFT) |
			(tmp << LPPHY_REV1_veryLowGainEx_veryLowGain_3DB_SHIFT);
		write_phy_reg(pi, LPPHY_REV1_veryLowGainEx, val);
	}
}
...
WLBANDINITFN(wlc_lpphy_agc_temp_init)(phy_info_t *pi)
{
	...
	temp = read_phy_reg(pi, LPPHY_REV2_gainidxoffset);
...
BCMATTACHFN(wlc_phy_txpwr_srom_read_lpphy)(phy_info_t *pi)
{
	...
	if (PHY_GETVAR(pi, "parefldovoltage")) {
		pi->ldo_voltage = (uint8)PHY_GETINTVAR(pi, "parefldovoltage");
	} else {
		/* Default if not present in SROM */
		pi->ldo_voltage = 0x28;	 /* 2.9 V */
	}


	if (!PHY_GETVAR(pi, "japanwidefilter")) {
		/* Default if not present in SROM */
		pi->japan_wide_filter = TRUE;
	} else {
		pi->japan_wide_filter = (bool) PHY_GETINTVAR(pi, "japanwidefilter");
	}

	/* PAPD */
	if (!PHY_GETVAR(pi, "lpphyparecalmininterval")) {
		pi_lp->lpphy_papd_recal_min_interval = LPPHY_PAPD_RECAL_MIN_INTERVAL;
	} else {
		pi_lp->lpphy_papd_recal_min_interval = PHY_GETINTVAR(pi,
			"lpphyparecalmininterval");
	}
	if (!PHY_GETVAR(pi, "lpphyparecalmaxinterval")) {
		pi_lp->lpphy_papd_recal_max_interval = LPPHY_PAPD_RECAL_MAX_INTERVAL;
	} else {
		pi_lp->lpphy_papd_recal_max_interval = PHY_GETINTVAR(pi,
			"lpphyparecalmaxinterval");
	}
	if (!PHY_GETVAR(pi, "lpphyparecalgaindelta")) {
		pi_lp->lpphy_papd_recal_gain_delta = LPPHY_PAPD_RECAL_GAIN_DELTA;
	} else {
		pi_lp->lpphy_papd_recal_gain_delta = PHY_GETINTVAR(pi,
			"lpphyparecalgaindelta");
	}
	if (!PHY_GETVAR(pi, "lpphyparecalenable")) {
		pi_lp->lpphy_papd_recal_enable = LPPHY_PAPD_RECAL_ENABLE;
	} else {
		pi_lp->lpphy_papd_recal_enable = (bool)PHY_GETINTVAR(pi,
			"lpphyparecalenable");
	}

	pi->extlna_type = (uint16)PHY_GETINTVAR(pi, "extlnatype");
	if (pi->extlna_type > 1)
		pi->extlna_type = 0;
	...
	if (PHY_GETVAR(pi, "cckdigfilttype")) {
		...
		if ((temp >= 0) &&
			(temp < LPPHY_REV2_NUM_TX_DIG_FILTERS_CCK)) {
				pi_lp->lpphy_cck_dig_filt_type  = temp;
		}
	}
	pi_lp->lpphy_ofdm_dig_filt_type = -1;
	if (PHY_GETVAR(pi, "ofdmdigfilttype")) {
		int16 temp;
		temp = (int16) PHY_GETINTVAR(pi, "ofdmdigfilttype");
		if ((temp >= 0) &&
			(temp < LPPHY_REV2_NUM_TX_DIG_FILTERS_OFDM)) {
				pi_lp->lpphy_ofdm_dig_filt_type  = temp;
		}
	}
	if (PHY_GETVAR(pi, "txrf_sp_9")) {
		pi_lp->lpphy_txrf_sp_9_override = (int16) PHY_GETINTVAR(pi, "txrf_sp_9");
	} else {
		pi_lp->lpphy_txrf_sp_9_override = -1;
	}

	if (PHY_GETVAR(pi, "ofdmanalogfiltbw")) {
		pi->ofdm_analog_filt_bw_override =
			(int16) PHY_GETINTVAR(pi, "ofdmanalogfiltbw");
	} else {
		pi->ofdm_analog_filt_bw_override = -1;
	}
	if (PHY_GETVAR(pi, "cckanalogfiltbw")) {
		pi->cck_analog_filt_bw_override =
			(int16) PHY_GETINTVAR(pi, "cckanalogfiltbw");
	} else {
		pi->cck_analog_filt_bw_override = -1;
	}
	if (PHY_GETVAR(pi, "ofdmrccaloverride")) {
		pi->ofdm_rccal_override = (int16) PHY_GETINTVAR(pi, "ofdmrccaloverride");
	} else {
		pi->ofdm_rccal_override = -1;
	}
	if (PHY_GETVAR(pi, "cckrccaloverride")) {
		pi->cck_rccal_override = (int16) PHY_GETINTVAR(pi, "cckrccaloverride");
	} else {
		pi->cck_rccal_override = -1;
	}
...
WLBANDINITFN(wlc_lpphy_radio_init)(phy_info_t *pi)
{
	...
	/* Toggle radio reset */
	or_phy_reg(pi, LPPHY_fourwireControl, LPPHY_fourwireControl_radioReset_MASK);
	OSL_DELAY(1);
	and_phy_reg(pi, LPPHY_fourwireControl, ~LPPHY_fourwireControl_radioReset_MASK);
	OSL_DELAY(1);
	...
	if (BCM2063_ID == LPPHY_RADIO_ID(pi)) {
		...
		write_phy_reg(pi, LPPHY_REV2_extslnactrl0, 0x5f80);

		write_phy_reg(pi, LPPHY_REV2_extslnactrl1, 0x0);
...
static void
wlc_lpphy_save_papd_calibration_results(phy_info_t *pi)
{
	lpphytbl_info_t tab;
	phy_info_lpphy_t *pi_lp = pi->u.pi_lpphy;
	...
	pi_lp->lpphy_cal_results.papd_indexOffset =
		READ_PHY_REG(pi, LPPHY_REV2, papdScalar, indexOffset);

	pi_lp->lpphy_cal_results.papd_startstopindex =
		read_phy_reg(pi, LPPHY_REV2_papdstartstopIndex);
...
wlc_lpphy_restore_papd_calibration_results(phy_info_t *pi)
{
	...
	MOD_PHY_REG(pi, LPPHY_REV2, papdScalar, indexOffset,
		pi_lp->lpphy_cal_results.papd_indexOffset);

	/* Apply start stop index */
	write_phy_reg(pi, LPPHY_REV2_papdstartstopIndex,
		pi_lp->lpphy_cal_results.papd_startstopindex);

	/* Enable papd */
	MOD_PHY_REG(pi, LPPHY_REV2, papdctrl, papdcompEn, 1);
}

static void
wlc_lpphy_restore_calibration_results(phy_info_t *pi)
{
	phy_info_lpphy_t *pi_lp = pi->u.pi_lpphy;
	...
	write_phy_reg(pi, LPPHY_RxCompcoeff, pi_lp->lpphy_cal_results.rxiqcal_coeffs);
...
WLBANDINITFN(wlc_phy_init_lpphy)(phy_info_t *pi)
{
	...
	if (pi_lp->lpphy_full_cal_channel != CHSPEC_CHANNEL(pi->radio_chanspec)) {
		...
		wlc_lpphy_restore_calibration_results(pi);
	}
...
wlc_lpphy_iqcal_wait(phy_info_t *pi)
{
	...
	return (0 == wlc_lpphy_iqcal_active(pi));
}
...
wlc_phy_stop_tx_tone_lpphy(phy_info_t *pi)
{
	...
	and_phy_reg(pi, LPPHY_SampleplayCnt, (uint16)~LPPHY_SampleplayCnt_sampPlayCount_MASK);

	/* Wait for completion */
	while (read_phy_reg(pi, LPPHY_AphyControlAddr) & LPPHY_AphyControlAddr_phyloopbackEn_MASK) {
#if defined(DSLCPE_DELAY)
		OSL_YIELD_EXEC(pi->sh->osh, 100);
#else
		OSL_DELAY(100);
...
{
	mod_phy_reg(pi, LPPHY_SampplayBufControl,
		LPPHY_SampplayBufControl_sampPlaydepth_MASK,
		(num_samps - 1) << LPPHY_SampplayBufControl_sampPlaydepth_SHIFT);

	if (num_loops != 0xffff)
		num_loops--;
	mod_phy_reg(pi, LPPHY_SampleplayCnt,
		LPPHY_SampleplayCnt_sampPlayCount_MASK,
		num_loops << LPPHY_SampleplayCnt_sampPlayCount_SHIFT);

	mod_phy_reg(pi, LPPHY_SampplayBufControl,
		LPPHY_SampplayBufControl_sampPlayWait_MASK,
		wait << LPPHY_SampplayBufControl_sampPlayWait_SHIFT);

	mod_phy_reg(pi, LPPHY_AphyControlAddr,
		LPPHY_AphyControlAddr_phyloopbackEn_MASK,
		1 << LPPHY_AphyControlAddr_phyloopbackEn_SHIFT);
}
...
wlc_lpphy_rx_iq_est(phy_info_t *pi, uint16 num_samps, uint8 wait_time, lpphy_iq_est_t *iq_est)
{
	...
	mod_phy_reg(pi, LPPHY_crsgainCtrl,
		LPPHY_crsgainCtrl_APHYGatingEnable_MASK,
		0 << LPPHY_crsgainCtrl_APHYGatingEnable_SHIFT);

	mod_phy_reg(pi, LPPHY_IQNumSampsAddress,
		LPPHY_IQNumSampsAddress_numSamps_MASK,
		num_samps << LPPHY_IQNumSampsAddress_numSamps_SHIFT);
	mod_phy_reg(pi, LPPHY_IQEnableWaitTimeAddress,
		LPPHY_IQEnableWaitTimeAddress_waittimevalue_MASK,
		(uint16)wait_time << LPPHY_IQEnableWaitTimeAddress_waittimevalue_SHIFT);
	mod_phy_reg(pi, LPPHY_IQEnableWaitTimeAddress,
		LPPHY_IQEnableWaitTimeAddress_iqmode_MASK,
		0 << LPPHY_IQEnableWaitTimeAddress_iqmode_SHIFT);

	mod_phy_reg(pi, LPPHY_IQEnableWaitTimeAddress,
		LPPHY_IQEnableWaitTimeAddress_iqstart_MASK,
		1 << LPPHY_IQEnableWaitTimeAddress_iqstart_SHIFT);

	/* Wait for IQ estimation to complete */
	while (read_phy_reg(pi, LPPHY_IQEnableWaitTimeAddress) &
		LPPHY_IQEnableWaitTimeAddress_iqstart_MASK) {
		/* Check for timeout */
	...
	/* Save results */
	iq_est->iq_prod = ((uint32)read_phy_reg(pi, LPPHY_IQAccHiAddress) << 16) |
		(uint32)read_phy_reg(pi, LPPHY_IQAccLoAddress);
	iq_est->i_pwr = ((uint32)read_phy_reg(pi, LPPHY_IQIPWRAccHiAddress) << 16) |
		(uint32)read_phy_reg(pi, LPPHY_IQIPWRAccLoAddress);
	iq_est->q_pwr = ((uint32)read_phy_reg(pi, LPPHY_IQQPWRAccHiAddress) << 16) |
		(uint32)read_phy_reg(pi, LPPHY_IQQPWRAccLoAddress);
...
cleanup:
	mod_phy_reg(pi, LPPHY_crsgainCtrl,
		LPPHY_crsgainCtrl_APHYGatingEnable_MASK,
		1 << LPPHY_crsgainCtrl_APHYGatingEnable_SHIFT);
...
#define LPPHY_MIN_RXIQ_PWR 2
	...
	/* Save original c0 & c1 */
	c0_new = ((read_phy_reg(pi, LPPHY_RxCompcoeff) & LPPHY_RxCompcoeff_c0_MASK) >>
		LPPHY_RxCompcoeff_c0_SHIFT);
	c1_new = ((read_phy_reg(pi, LPPHY_RxCompcoeff) & LPPHY_RxCompcoeff_c1_MASK) >>
		LPPHY_RxCompcoeff_c1_SHIFT);

	/* Zero out comp coeffs and do "one-shot" calibration */
	mod_phy_reg(pi, LPPHY_RxCompcoeff,
		LPPHY_RxCompcoeff_c1_MASK,
		0xc0 << LPPHY_RxCompcoeff_c1_SHIFT);
	mod_phy_reg(pi, LPPHY_RxCompcoeff,
		LPPHY_RxCompcoeff_c0_MASK,
		0x00 << LPPHY_RxCompcoeff_c0_SHIFT);
...
cleanup:
	...
	mod_phy_reg(pi, LPPHY_RxCompcoeff,
		LPPHY_RxCompcoeff_c1_MASK,
		c1_new << LPPHY_RxCompcoeff_c1_SHIFT);
	mod_phy_reg(pi, LPPHY_RxCompcoeff,
		LPPHY_RxCompcoeff_c0_MASK,
		c0_new << LPPHY_RxCompcoeff_c0_SHIFT);
...
wlc_lpphy_disable_crs(phy_info_t *pi, bool user_flag)
{
	...
	mod_phy_reg(pi, LPPHY_RFOverrideVal0,
		LPPHY_RFOverrideVal0_ant_selp_ovr_val_MASK,
		0 << LPPHY_RFOverrideVal0_ant_selp_ovr_val_SHIFT);
	mod_phy_reg(pi, LPPHY_RFOverride0,
		LPPHY_RFOverride0_ant_selp_ovr_MASK,
		1 << LPPHY_RFOverride0_ant_selp_ovr_SHIFT);

	/* Power down the 2.4 GHz Tx */
	mod_phy_reg(pi, LPPHY_RFOverrideVal0,
		LPPHY_RFOverrideVal0_gmode_tx_pu_ovr_val_MASK,
		0 << LPPHY_RFOverrideVal0_gmode_tx_pu_ovr_val_SHIFT);
	mod_phy_reg(pi, LPPHY_RFOverride0,
		LPPHY_RFOverride0_gmode_tx_pu_ovr_MASK,
		1 << LPPHY_RFOverride0_gmode_tx_pu_ovr_SHIFT);

	/* Power up the 2.4 GHz Rx */
	mod_phy_reg(pi, LPPHY_RFOverrideVal0,
		LPPHY_RFOverrideVal0_gmode_rx_pu_ovr_val_MASK,
		1 << LPPHY_RFOverrideVal0_gmode_rx_pu_ovr_val_SHIFT);
	mod_phy_reg(pi, LPPHY_RFOverride0,
		LPPHY_RFOverride0_gmode_rx_pu_ovr_MASK,
		1 << LPPHY_RFOverride0_gmode_rx_pu_ovr_SHIFT);

	/* Power down the 5 GHz Tx */
	mod_phy_reg(pi, LPPHY_RFOverrideVal0,
		LPPHY_RFOverrideVal0_amode_tx_pu_ovr_val_MASK,
		0 << LPPHY_RFOverrideVal0_amode_tx_pu_ovr_val_SHIFT);
	mod_phy_reg(pi, LPPHY_RFOverride0,
		LPPHY_RFOverride0_amode_tx_pu_ovr_MASK,
		1 << LPPHY_RFOverride0_amode_tx_pu_ovr_SHIFT);

	/* Power down the 5 GHz Rx */
	mod_phy_reg(pi, LPPHY_RFOverrideVal0,
		LPPHY_RFOverrideVal0_amode_rx_pu_ovr_val_MASK,
		0 << LPPHY_RFOverrideVal0_amode_rx_pu_ovr_val_SHIFT);
	mod_phy_reg(pi, LPPHY_RFOverride0,
		LPPHY_RFOverride0_amode_rx_pu_ovr_MASK,
		1 << LPPHY_RFOverride0_amode_rx_pu_ovr_SHIFT);

	/* Force the HPFs to high BW */
	mod_phy_reg(pi, LPPHY_rfoverride2val,
		LPPHY_rfoverride2val_hpf1_ctrl_ovr_val_MASK,
		0x7 << LPPHY_rfoverride2val_hpf1_ctrl_ovr_val_SHIFT);
	mod_phy_reg(pi, LPPHY_rfoverride2val,
		LPPHY_rfoverride2val_hpf2_ctrl_ovr_val_MASK,
		0x7 << LPPHY_rfoverride2val_hpf2_ctrl_ovr_val_SHIFT);

	/* Force the LPF to low Q */
	mod_phy_reg(pi, LPPHY_rfoverride2val,
		LPPHY_rfoverride2val_lpf_lq_ovr_val_MASK,
		0x0 << LPPHY_rfoverride2val_lpf_lq_ovr_val_SHIFT);

	/* Power up LNA 1 and power down LNA 2 */
	mod_phy_reg(pi, LPPHY_rfoverride2val,
		LPPHY_rfoverride2val_lna1_pu_ovr_val_MASK,
		1 << LPPHY_rfoverride2val_lna1_pu_ovr_val_SHIFT);
	mod_phy_reg(pi, LPPHY_rfoverride2val,
		LPPHY_rfoverride2val_lna2_pu_ovr_val_MASK,
		0 << LPPHY_rfoverride2val_lna2_pu_ovr_val_SHIFT);

	/* Bypass phase shifter on signal from LNA1 and turn LNA 2 path off */
	write_phy_reg(pi, LPPHY_psctrlovrval0, 0x0000);
	write_phy_reg(pi, LPPHY_psctrlovrval1, 0x0001);
	write_phy_reg(pi, LPPHY_psctrlovrval2, 0x20);

	/* Force external LNA gain to 0 */
	mod_phy_reg(pi, LPPHY_rfoverride2val,
		LPPHY_rfoverride2val_gmode_ext_lna_gain_ovr_val_MASK,
		0 << LPPHY_rfoverride2val_gmode_ext_lna_gain_ovr_val_SHIFT);
	mod_phy_reg(pi, LPPHY_rfoverride2val,
		LPPHY_rfoverride2val_amode_ext_lna_gain_ovr_val_MASK,
		0 << LPPHY_rfoverride2val_amode_ext_lna_gain_ovr_val_SHIFT);

	/* Override tx and rx gains  */
	write_phy_reg(pi, LPPHY_txgainctrlovrval, 0x0);
	write_phy_reg(pi, LPPHY_rxgainctrlovrval, 0x45AF);

	/* Enable above overrides */
	write_phy_reg(pi, LPPHY_rfoverride2, 0x03ff);
}

static void
wlc_lpphy_stop_ddfs(phy_info_t *pi)
{
	mod_phy_reg(pi, LPPHY_afe_ddfs,
		LPPHY_afe_ddfs_playoutEn_MASK,
		0 << LPPHY_afe_ddfs_playoutEn_SHIFT);
	mod_phy_reg(pi, LPPHY_lpphyCtrl,
		LPPHY_lpphyCtrl_afe_ddfs_en_MASK,
		0 << LPPHY_lpphyCtrl_afe_ddfs_en_SHIFT);
}
	...
	int incr1, int incr2, int scale_index)
{
	...
	mod_phy_reg(pi, LPPHY_afe_ddfs_pointer_init,
		LPPHY_afe_ddfs_pointer_init_lutPointer1Init_MASK,
		0 << LPPHY_afe_ddfs_pointer_init_lutPointer1Init_SHIFT);
	mod_phy_reg(pi, LPPHY_afe_ddfs_pointer_init,
		LPPHY_afe_ddfs_pointer_init_lutPointer2Init_MASK,
		0 << LPPHY_afe_ddfs_pointer_init_lutPointer2Init_SHIFT);

	mod_phy_reg(pi, LPPHY_afe_ddfs_incr_init,
		LPPHY_afe_ddfs_incr_init_lutIncr1Init_MASK,
		incr1 << LPPHY_afe_ddfs_incr_init_lutIncr1Init_SHIFT);
	mod_phy_reg(pi, LPPHY_afe_ddfs_incr_init,
		LPPHY_afe_ddfs_incr_init_lutIncr2Init_MASK,
		incr2 << LPPHY_afe_ddfs_incr_init_lutIncr2Init_SHIFT);

	mod_phy_reg(pi, LPPHY_afe_ddfs,
		LPPHY_afe_ddfs_chanIEn_MASK,
		i_on << LPPHY_afe_ddfs_chanIEn_SHIFT);
	mod_phy_reg(pi, LPPHY_afe_ddfs,
		LPPHY_afe_ddfs_chanQEn_MASK,
		q_on << LPPHY_afe_ddfs_chanQEn_SHIFT);
	mod_phy_reg(pi, LPPHY_afe_ddfs,
		LPPHY_afe_ddfs_scaleIndex_MASK,
		scale_index << LPPHY_afe_ddfs_scaleIndex_SHIFT);

	/* Single tone */
	mod_phy_reg(pi, LPPHY_afe_ddfs,
		LPPHY_afe_ddfs_twoToneEn_MASK,
		0x0 << LPPHY_afe_ddfs_twoToneEn_SHIFT);
	mod_phy_reg(pi, LPPHY_afe_ddfs,
		LPPHY_afe_ddfs_playoutEn_MASK,
		0x1 << LPPHY_afe_ddfs_playoutEn_SHIFT);

	mod_phy_reg(pi, LPPHY_lpphyCtrl,
		LPPHY_lpphyCtrl_afe_ddfs_en_MASK,
		1 << LPPHY_lpphyCtrl_afe_ddfs_en_SHIFT);
}
...
wlc_lpphy_loopback(phy_info_t *pi)
{
	...
	mod_phy_reg(pi, LPPHY_AfeCtrlOvr,
		...
		1 << LPPHY_AfeCtrlOvr_pwdn_adc_ovr_SHIFT);
	mod_phy_reg(pi, LPPHY_AfeCtrlOvrVal,
		LPPHY_AfeCtrlOvrVal_pwdn_adc_ovr_val_MASK,
		0 << LPPHY_AfeCtrlOvrVal_pwdn_adc_ovr_val_SHIFT);

	/* Force RxPu on */
	mod_phy_reg(pi, LPPHY_RFOverride0,
		LPPHY_RFOverride0_internalrfrxpu_ovr_MASK,
		1 << LPPHY_RFOverride0_internalrfrxpu_ovr_SHIFT);
	mod_phy_reg(pi, LPPHY_RFOverrideVal0,
		LPPHY_RFOverrideVal0_internalrfrxpu_ovr_val_MASK,
		1 << LPPHY_RFOverrideVal0_internalrfrxpu_ovr_val_SHIFT);
	...
	mod_phy_reg(pi, LPPHY_RFOverrideVal0,
		LPPHY_RFOverrideVal0_gmode_tx_pu_ovr_val_MASK,
		1 << LPPHY_RFOverrideVal0_gmode_tx_pu_ovr_val_SHIFT);

	/* Set the Tx filter bandwidth to wide */
	write_radio_reg(pi, RADIO_2062_TX_CTRLA_NORTH, 0x80);
	mod_phy_reg(pi, LPPHY_RFOverride0,
		LPPHY_RFOverride0_lpf_bw_ovr_MASK,
		1 << LPPHY_RFOverride0_lpf_bw_ovr_SHIFT);
	mod_phy_reg(pi, LPPHY_RFOverrideVal0,
		LPPHY_RFOverrideVal0_lpf_bw_ovr_val_MASK,
		1 << LPPHY_RFOverrideVal0_lpf_bw_ovr_val_SHIFT);
...
wlc_lpphy_rc_cal_rev0(phy_info_t *pi)
{
	...
	/* Save overrides */
	old_rfovr0 = read_phy_reg(pi, LPPHY_RFOverride0);
	old_rfovr0val = read_phy_reg(pi, LPPHY_RFOverrideVal0);
	old_afeovr = read_phy_reg(pi, LPPHY_AfeCtrlOvr);
	old_afeovrval = read_phy_reg(pi, LPPHY_AfeCtrlOvrVal);
	old_rfovr2 = read_phy_reg(pi, LPPHY_rfoverride2);
	old_rfovr2val = read_phy_reg(pi, LPPHY_rfoverride2val);

	/* Save control settings */
	old_rx_ctrl = read_phy_reg(pi, LPPHY_lpphyCtrl);
	...
	*/
	if (-1 == (gain_index = wlc_lpphy_loopback(pi))) {
	...
	mod_phy_reg(pi, LPPHY_lpphyCtrl,
		LPPHY_lpphyCtrl_rx_filt_bypass_MASK,
		1 << LPPHY_lpphyCtrl_rx_filt_bypass_SHIFT);

	/* Fix the hpf and lpf values */
	mod_phy_reg(pi, LPPHY_rfoverride2val,
		LPPHY_rfoverride2val_hpf1_ctrl_ovr_val_MASK,
		0x01 << LPPHY_rfoverride2val_hpf1_ctrl_ovr_val_SHIFT);
	mod_phy_reg(pi, LPPHY_rfoverride2val,
		LPPHY_rfoverride2val_hpf2_ctrl_ovr_val_MASK,
		0x01 << LPPHY_rfoverride2val_hpf2_ctrl_ovr_val_SHIFT);
	mod_phy_reg(pi, LPPHY_rfoverride2val,
		LPPHY_rfoverride2val_lpf_lq_ovr_val_MASK,
		0x03 << LPPHY_rfoverride2val_lpf_lq_ovr_val_SHIFT);
...
cleanup:
	...
	/* Restore overrides */
	write_phy_reg(pi, LPPHY_RFOverrideVal0, old_rfovr0val);
	write_phy_reg(pi, LPPHY_RFOverride0, old_rfovr0);
	write_phy_reg(pi, LPPHY_AfeCtrlOvrVal, old_afeovrval);
	write_phy_reg(pi, LPPHY_AfeCtrlOvr, old_afeovr);
	write_phy_reg(pi, LPPHY_rfoverride2val, old_rfovr2val);
	write_phy_reg(pi, LPPHY_rfoverride2, old_rfovr2);

	/* Restore control settings */
	write_phy_reg(pi, LPPHY_lpphyCtrl, old_rx_ctrl);
	...
	lpphy_rx_iq_cal_data_for_temp_adj_t *p_temp_adj_data)
{
	    ...
	    (CHIPID(pi->sh->chip) != BCM4315_CHIP_ID))
	{
		...
		while (iqcomp_sz--) {
			if (iqcomp[iqcomp_sz].chan == CHSPEC_CHANNEL(pi->radio_chanspec)) {
				/* Apply new coeffs */
				MOD_PHY_REG(pi, LPPHY_REV2, RxCompcoeff, c1,
					(uint16)iqcomp[iqcomp_sz].c1);
				MOD_PHY_REG(pi, LPPHY_REV2, RxCompcoeff, c0,
		...
		/* Turn off papd */
		SAVE_papd_on = READ_PHY_REG(pi, LPPHY_REV2, papdctrl, papdcompEn);
		MOD_PHY_REG(pi, LPPHY_REV2, papdctrl, papdcompEn, 0);
		 ...
		 */
		if (CHSPEC_IS2G(pi->radio_chanspec)) {
			MOD_PHY_REG(pi, LPPHY_REV2, RFOverride0, gmode_tx_pu_ovr, 1);
			MOD_PHY_REG(pi, LPPHY_REV2, RFOverrideVal0, gmode_tx_pu_ovr_val, 1);
		} else {
			MOD_PHY_REG(pi, LPPHY_REV2, RFOverride0, amode_tx_pu_ovr, 1);
			MOD_PHY_REG(pi, LPPHY_REV2, RFOverrideVal0, amode_tx_pu_ovr_val, 1);
		}

		/* PA driver override
		 */
		MOD_PHY_REG(pi, LPPHY_REV2, rfoverride3_val, stxpadpu2g_ovr_val, 0);
		MOD_PHY_REG(pi, LPPHY_REV2, rfoverride3, stxpadpu2g_ovr, 1);

		/* PA override
		 */
		MOD_PHY_REG(pi, LPPHY_REV2, rfoverride3_val, stxpapu_ovr_val, 0);
		MOD_PHY_REG(pi, LPPHY_REV2, rfoverride3, stxpapu_ovr, 1);
		...
		write_phy_reg(pi, LPPHY_psctrlovrval0, (uint16)(ps & 0xffff));
		write_phy_reg(pi, LPPHY_psctrlovrval1, (uint16)((ps >> 16) & 0xff));

		if (CHSPEC_IS2G(pi->radio_chanspec)) {
			MOD_PHY_REG(pi, LPPHY_REV2, rfoverride2val, slna_pu_ovr_val, 0);
			MOD_PHY_REG(pi, LPPHY_REV2, RFinputOverrideVal, wlslnapu_ovr_val, 0);
			MOD_PHY_REG(pi, LPPHY_REV2, rxlnaandgainctrl1ovrval, lnapuovr_Val, 0x50);
		} else {
			MOD_PHY_REG(pi, LPPHY_REV2, rfoverride2val, slna_pu_ovr_val, 0);
			MOD_PHY_REG(pi, LPPHY_REV2, RFinputOverrideVal, wlslnapu_ovr_val, 0);
			MOD_PHY_REG(pi, LPPHY_REV2, rxlnaandgainctrl1ovrval, lnapuovr_Val, 0x0a);
		}

		MOD_PHY_REG(pi, LPPHY_REV2, RFinputOverride, wlslnapu_ovr, 0x1);
		MOD_PHY_REG(pi, LPPHY_REV2, rfoverride2, slna_pu_ovr, 1);
		MOD_PHY_REG(pi, LPPHY_REV2, rfoverride2, lna_pu_ovr, 1);
		MOD_PHY_REG(pi, LPPHY_REV2, rfoverride2, ps_ctrl_ovr, 1);

		if (CHSPEC_IS2G(pi->radio_chanspec)) {
			MOD_PHY_REG(pi, LPPHY_REV2, rxlnaandgainctrl1ovrval, lnapuovr_Val, 0x40);
			write_radio_reg(pi, RADIO_2063_TXRX_LOOPBACK_1, 0x4c);
			write_radio_reg(pi, RADIO_2063_TXRX_LOOPBACK_2, 0);
		} else {
			MOD_PHY_REG(pi, LPPHY_REV2, rxlnaandgainctrl1ovrval, lnapuovr_Val, 0x08);
		...
		gain = (7 << 8) | (3 << 4) | (3 << 6);
		if (CHSPEC_IS2G(pi->radio_chanspec)) {
			MOD_PHY_REG(pi, LPPHY_REV2, rfoverride2val, slna_gain_ctrl_ovr_val, 0);
			MOD_PHY_REG(pi, LPPHY_REV2, RFinputOverrideVal, wlslnagainctrl_ovr_val, 0);
		}
		MOD_PHY_REG(pi, LPPHY_REV2, rxgainctrl0ovrval, rxgainctrl_ovr_val0, gain);
		MOD_PHY_REG(pi, LPPHY_REV2, rxlnaandgainctrl1ovrval, rxgainctrl_ovr_val1, 0);
		wlc_lpphy_rx_gain_override_enable(pi, TRUE);
		MOD_PHY_REG(pi, LPPHY_REV2, rfoverride2, gmode_ext_lna_gain_ovr, 1);
		MOD_PHY_REG(pi, LPPHY_REV2, rfoverride2, amode_ext_lna_gain_ovr, 1);

		/* crs disable */
		wlc_phy_set_deaf_lpphy(pi, (bool)0);

		/* Set TR switch */
		MOD_PHY_REG(pi, LPPHY_REV2, RFOverride0, trsw_tx_pu_ovr, 1);
		MOD_PHY_REG(pi, LPPHY_REV2, RFOverrideVal0, trsw_tx_pu_ovr_val, tx_switch);
		MOD_PHY_REG(pi, LPPHY_REV2, RFOverride0, trsw_rx_pu_ovr, 1);
		MOD_PHY_REG(pi, LPPHY_REV2, RFOverrideVal0, trsw_rx_pu_ovr_val, rx_switch);

		/* Force ADC on */
		MOD_PHY_REG(pi, LPPHY_REV2, AfeCtrlOvr, pwdn_adc_ovr, 1);
		MOD_PHY_REG(pi, LPPHY_REV2, AfeCtrlOvrVal, pwdn_adc_ovr_val, 0);

		/* Force Rx on	 */
		MOD_PHY_REG(pi, LPPHY_REV2, RFOverride0, internalrfrxpu_ovr, 1);
		MOD_PHY_REG(pi, LPPHY_REV2, RFOverrideVal0, internalrfrxpu_ovr_val, 1);
		...
		/* Rx on/off */
		MOD_PHY_REG(pi, LPPHY_REV2, RFOverride0, internalrfrxpu_ovr, 0);

		/* ADC on/off */
		MOD_PHY_REG(pi, LPPHY_REV2, AfeCtrlOvr, pwdn_adc_ovr, 0);
	    ...
	    MOD_PHY_REG(pi, LPPHY_REV2, rfoverride2, gmode_ext_lna_gain_ovr, 0);
	    MOD_PHY_REG(pi, LPPHY_REV2, rfoverride2, amode_ext_lna_gain_ovr, 0);

		/* RX LNA PUs */
		MOD_PHY_REG(pi, LPPHY_REV2, RFinputOverride, wlslnapu_ovr, 0);
		MOD_PHY_REG(pi, LPPHY_REV2, rfoverride2, slna_pu_ovr, 0);
		MOD_PHY_REG(pi, LPPHY_REV2, rfoverride2, lna_pu_ovr, 0);
		MOD_PHY_REG(pi, LPPHY_REV2, rfoverride2, ps_ctrl_ovr, 0);

		write_radio_reg(pi, RADIO_2063_TXRX_LOOPBACK_1, 0);
		write_radio_reg(pi, RADIO_2063_TXRX_LOOPBACK_2, 0);

		/* restore PA */
		MOD_PHY_REG(pi, LPPHY_REV2, rfoverride3, stxpapu_ovr, 0);

		/* restore PA driver */
		MOD_PHY_REG(pi, LPPHY_REV2, rfoverride3, stxpadpu2g_ovr, 0);

		/* TX PU */
		MOD_PHY_REG(pi, LPPHY_REV2, RFOverride0, gmode_tx_pu_ovr, 0);
		MOD_PHY_REG(pi, LPPHY_REV2, RFOverride0, amode_tx_pu_ovr, 0);

		/* Restore Tx gain */
		wlc_lpphy_tx_pwr_state_restore(pi, &tx_pwr_state);

		/* restore papd */
		MOD_PHY_REG(pi, LPPHY_REV2, papdctrl, papdcompEn, (uint16)SAVE_papd_on);
	...
	pi_lp->lpphy_cal_results.rxiqcal_coeffs = read_phy_reg(pi, LPPHY_RxCompcoeff);

	PHY_INFORM(("wl%d: %s: Rx IQ cal complete, coeffs: A: %d, C: %d\n",
		pi->sh->unit, __FUNCTION__,
		(int8)((read_phy_reg(pi, LPPHY_RxCompcoeff) & LPPHY_RxCompcoeff_c1_MASK) >>
		LPPHY_RxCompcoeff_c1_SHIFT),
		(int8)((read_phy_reg(pi, LPPHY_RxCompcoeff) & LPPHY_RxCompcoeff_c0_MASK) >>
		LPPHY_RxCompcoeff_c0_SHIFT)));
...
wlc_lpphy_tx_iqlo_functional(phy_info_t *pi)
{
	...
	write_phy_reg(pi, LPPHY_iqloCalCmdNnum, 0x7877);
	write_phy_reg(pi, LPPHY_iqloCalCmdGctl, 0x0aa9);

	wlc_phy_set_deaf_lpphy(pi, (bool)0);
	wlc_phy_start_tx_tone_lpphy(pi, 2500, 100);
	or_phy_reg(pi, LPPHY_iqloCalCmdGctl, LPPHY_iqloCalCmdGctl_iqlo_cal_en_MASK);
	write_phy_reg(pi, LPPHY_iqloCalCmd, 0x4000);

	if (!wlc_lpphy_iqcal_wait(pi)) {
		PHY_ERROR(("wl%d: %s: iqcal did not complete\n", pi->sh->unit, __FUNCTION__));
	}

	and_phy_reg(pi, LPPHY_iqloCalCmdGctl, (uint16)~LPPHY_iqloCalCmdGctl_iqlo_cal_en_MASK);
...
wlc_lpphy_smooth_papd(phy_info_t *pi, uint32 winsz, uint32 start, uint32 end)
{
	...
	if (NULL == (buf = MALLOC(pi->sh->osh, 2 * sizeof(uint32) * LPPHY_PAPD_EPS_TBL_SIZE))) {
...
wlc_phy_tx_dig_filt_lpphy_rev2(phy_info_t *pi, bool bset, uint16 *coeffs)
{
	...
	for (j = 0; j < LPPHY_REV2_NUM_DIG_FILT_COEFFS; j++) {
		if (bset) {
			write_phy_reg(pi, addr[j], coeffs[j]);
		} else {
			coeffs[j] = read_phy_reg(pi, addr[j]);
		}
	}
}
...
wlc_phy_tx_dig_filt_cck_setup_lpphy(phy_info_t *pi, bool set_now)
{
	...
	} else if (pi->sh->boardflags & BFL_CCKFAVOREVM) {
		filt_index = 1;
	} else {
		filt_index = 0;
	}
...
wlc_lpphy_papd_cal_setup(phy_info_t *pi, papd_restore_state *state)
{
	...
	/* Force rx PU on */
	mod_phy_reg(pi, LPPHY_REV2_RFOverride0,
		LPPHY_REV2_RFOverride0_internalrfrxpu_ovr_MASK,
		1 << LPPHY_REV2_RFOverride0_internalrfrxpu_ovr_SHIFT);
	mod_phy_reg(pi, LPPHY_REV2_RFOverrideVal0,
		LPPHY_REV2_RFOverrideVal0_internalrfrxpu_ovr_val_MASK,
		1 << LPPHY_REV2_RFOverrideVal0_internalrfrxpu_ovr_val_SHIFT);
	...
	/* Rx HPF */
	MOD_PHY_REG(pi, LPPHY_REV2, rfoverride2val, hpf1_ctrl_ovr_val, 0x7);
	MOD_PHY_REG(pi, LPPHY_REV2, rfoverride2val, hpf2_ctrl_ovr_val, 0x7);
	MOD_PHY_REG(pi, LPPHY_REV2, rfoverride2, hpf1_ctrl_ovr, 1);
	MOD_PHY_REG(pi, LPPHY_REV2, rfoverride2, hpf2_ctrl_ovr, 1);
	...
	/* PAPD regs */
	MOD_PHY_REG(pi, LPPHY_REV2, papdctrl, papdcompEn, 1);
	MOD_PHY_REG(pi, LPPHY_REV2, papdScalar, indexScalar, 8);
}
...
wlc_lpphy_papd_cal_cleanup(phy_info_t *pi, papd_restore_state *state)
{
	...
	mod_phy_reg(pi, LPPHY_REV2_RFOverride0,
		LPPHY_REV2_RFOverride0_internalrfrxpu_ovr_MASK,
		0 << LPPHY_REV2_RFOverride0_internalrfrxpu_ovr_SHIFT);

	/* Clear rx gain override */
	wlc_lpphy_rx_gain_override_enable(pi, FALSE);

	/* Rx HPF */
	MOD_PHY_REG(pi, LPPHY_REV2, rfoverride2, hpf1_ctrl_ovr, 0);
	MOD_PHY_REG(pi, LPPHY_REV2, rfoverride2, hpf2_ctrl_ovr, 0);

	/* Clear ADC override */
	mod_phy_reg(pi, LPPHY_AfeCtrlOvr,
		LPPHY_AfeCtrlOvr_pwdn_adc_ovr_MASK,
		0 << LPPHY_AfeCtrlOvr_pwdn_adc_ovr_SHIFT);
...
wlc_lpphy_papd_cal(phy_info_t *pi, lpphy_txcalgains_t *txgains, phy_cal_mode_t cal_mode)
{
	...
	MOD_PHY_REG(pi, LPPHY_REV2, papdctrl, caltype, (swcal ? 1 : 0));
	MOD_PHY_REG(pi, LPPHY_REV2, papdstartstopIndex, startIndex, startindex);
	MOD_PHY_REG(pi, LPPHY_REV2, papdstartstopIndex, stopIndex, stopindex);
	if ((cal_mode == CAL_FULL) || (cal_mode == CAL_SOFT)) {
		rfpowerOvrVal = READ_PHY_REG(pi, LPPHY_REV2, papdrfpowerOvrVal, rfpowerOvrVal);
		MOD_PHY_REG(pi, LPPHY_REV2, papdScalar, indexOffset, -100 + (76 - rfpowerOvrVal)/2);
	}

	if (pi_lp->lpphy_papd_slow_cal) {
		write_phy_reg(pi, LPPHY_REV2_papdNSampMeasure, (32 * 256));
		MOD_PHY_REG(pi, LPPHY_REV2, papdrfpowerOvrVal, bitShiftCorrelator, 8);
		MOD_PHY_REG(pi, LPPHY_REV2, papditerpergain, iterpergain, 127);
		write_phy_reg(pi, LPPHY_REV2_papdNSampSettle, 512);
		MOD_PHY_REG(pi, LPPHY_REV2, papditerpergain, bitshiftepsCalc, 0);
	} else {
		write_phy_reg(pi, LPPHY_REV2_papdNSampMeasure, 128);
		MOD_PHY_REG(pi, LPPHY_REV2, papdrfpowerOvrVal, bitShiftCorrelator, 0);
		MOD_PHY_REG(pi, LPPHY_REV2, papditerpergain, iterpergain, 32);
		write_phy_reg(pi, LPPHY_REV2_papdNSampSettle, 16);
		MOD_PHY_REG(pi, LPPHY_REV2, papditerpergain, bitshiftepsCalc, 3);
	}
	if (cal_mode == CAL_GCTRL) {
		MOD_PHY_REG(pi, LPPHY_REV2, papditerpergain, iterpergain, 64);
	}

	if (!swcal) {
		MOD_PHY_REG(pi, LPPHY_REV2, rfoverride3_val, stxpapu_ovr_val, 1);
		MOD_PHY_REG(pi, LPPHY_REV2, papdctrl, runCal, 1);

		/* Wait for completion */
		SPINWAIT(
			read_phy_reg(pi, LPPHY_REV2_papdctrl) & LPPHY_REV2_papdctrl_runCal_MASK,
			10 * 1000 * 1000);
		...
		write_phy_reg(pi, LPPHY_REV2_papdNSampMeasure, 64);
		bs_eps_calc = 9;
		if (cal_mode == CAL_GCTRL)
			num_iter = 24;
		else
		...
		tab.tbl_offset = 32;
		wlc_phy_table_read_lpphy(pi, &tab);
		MOD_PHY_REG(pi, LPPHY_REV2, papdMult, papd_mult, val & 0xfff);
		MOD_PHY_REG(pi, LPPHY_REV2, papdCorrScalar, corrScalar, (val >> 12) & 0xfff);

		/* measure Y0 */
		MOD_PHY_REG(pi, LPPHY_REV2, papdctrl, epsilonOverride, 1);
		MOD_PHY_REG(pi, LPPHY_REV2, epsilonIOverVal, epsilonOverrideI, 0);
		MOD_PHY_REG(pi, LPPHY_REV2, epsilonQOverVal, epsilonOverrideQ, 0);

		write_phy_reg(pi, LPPHY_REV2_papdNSampSettle, 40);

		for (j = 0; j < 5; j++) {
			MOD_PHY_REG(pi, LPPHY_REV2, rfoverride3_val, stxpapu_ovr_val, 1);
			MOD_PHY_REG(pi, LPPHY_REV2, papdctrl, runCal, 1);
			while (READ_PHY_REG(pi, LPPHY_REV2, papdctrl, runCal))
				OSL_DELAY(1);
			MOD_PHY_REG(pi, LPPHY_REV2, rfoverride3_val, stxpapu_ovr_val, 0);
		...
		Y0i = READ_PHY_REG(pi, LPPHY_REV2, papdcorrResultI, corrI);
		Y0q = READ_PHY_REG(pi, LPPHY_REV2, papdcorrResultQ, corrQ);
		...
		for (papd_idx = startindex; papd_idx <= stopindex; papd_idx++) {
			...
			wlc_phy_table_read_lpphy(pi, &tab);
			MOD_PHY_REG(pi, LPPHY_REV2, papdMult, papd_mult, val & 0xfff);
			MOD_PHY_REG(pi, LPPHY_REV2, papdCorrScalar, corrScalar,
				(val >> 12) & 0xfff);
			...
			for (iter = 0; iter < num_iter; iter++) {
				...
				MOD_PHY_REG(pi, LPPHY_REV2, epsilonIOverVal,
					epsilonOverrideI, eps_i >> k_bs_epsilon);
				MOD_PHY_REG(pi, LPPHY_REV2, epsilonQOverVal,
				...
				Yi = READ_PHY_REG(pi, LPPHY_REV2, papdcorrResultI, corrI);
				Yq = READ_PHY_REG(pi, LPPHY_REV2, papdcorrResultQ, corrQ);
		...
		MOD_PHY_REG(pi, LPPHY_REV2, papdctrl, epsilonOverride, 0);
	...
	ASSERT(0 == (read_phy_reg(pi, LPPHY_REV2_papdctrl) & LPPHY_REV2_papdctrl_runCal_MASK));
	PHY_CAL(("wl%d: %s: PAPD cal completed: %d + j * %d\n",
		pi->sh->unit, __FUNCTION__,
		read_phy_reg(pi, LPPHY_REV2_papdcorrResultI),
		read_phy_reg(pi, LPPHY_REV2_papdcorrResultQ)));
...
wlc_phy_papd_cal_txpwr_lpphy(phy_info_t *pi, bool full_cal)
{
	...
	wlc_lpphy_save_papd_calibration_results(pi);
	...
	uchar keep_tone, bool apply_results)
{
	...
	/* Turn off ADC */
	MOD_PHY_REG(pi, LPPHY, AfeCtrlOvrVal, pwdn_adc_ovr_val, 0);
	MOD_PHY_REG(pi, LPPHY, AfeCtrlOvr, pwdn_adc_ovr, 1);
	...
	if (LPREV_LT(pi->pubpi.phy_rev, 2)) {
		...
		rssi_old = read_phy_reg(pi, LPPHY_AfeRSSICtrl0);
		mod_phy_reg(pi, LPPHY_AfeRSSICtrl0, 0xff, 0x59);
		...
		if (LPREV_IS(pi->pubpi.phy_rev, 1)) {
			...
			rssi_muxsel = rssi_muxsel_old = read_phy_reg(pi, LPPHY_REV1_AfeRSSISel);
			...
			write_phy_reg(pi, LPPHY_REV1_AfeRSSISel, rssi_muxsel);
		}
		...
		/* Temp sense */
		mod_phy_reg(pi, LPPHY_tempsenseCtrlAddr,
			LPPHY_tempsenseCtrlAddr_tempsenseCtrl_MASK,
			0x0001 << LPPHY_tempsenseCtrlAddr_tempsenseCtrl_SHIFT);
	} else {
		uint16 adc_mid;

		/* Adjust ADC common mode */
		rssi_old = read_phy_reg(pi, LPPHY_REV2_AfeRSSICtrl1);
		if (LPREV_GE(pi->pubpi.phy_rev, 3))
			adc_mid = 175;
			...
			adc_mid = 202;
		mod_phy_reg(pi, LPPHY_REV2_AfeRSSICtrl1, 0x3ff, adc_mid);
		...
		/* Turn off PAPD */
		papd_ctrl_old = read_phy_reg(pi, LPPHY_REV2_papdctrl);
		mod_phy_reg(pi, LPPHY_REV2_papdctrl,
			LPPHY_REV2_papdctrl_papdcompEn_MASK,
			0 << LPPHY_REV2_papdctrl_papdcompEn_SHIFT);

		auxadc_ctrl_old = read_phy_reg(pi, LPPHY_REV2_auxadcCtrl);
		mod_phy_reg(pi, LPPHY_REV2_auxadcCtrl,
			LPPHY_REV2_auxadcCtrl_iqlocalEn_MASK,
			1 << LPPHY_REV2_auxadcCtrl_iqlocalEn_SHIFT);
		mod_phy_reg(pi, LPPHY_REV2_auxadcCtrl,
			LPPHY_REV2_auxadcCtrl_rssiformatConvEn_MASK,
			0 << LPPHY_REV2_auxadcCtrl_rssiformatConvEn_SHIFT);
	...
	for (i = n_cal_start; i < n_cal_cmds; i++) {
		...
		write_phy_reg(pi, LPPHY_iqloCalCmdNnum, command_num);
		...
		/* Issue cal command */
		write_phy_reg(pi, LPPHY_iqloCalCmd, cal_cmds[i]);
...
cleanup:
	...
	if (keep_tone == 0) {
		...
		/* Stop sample buffer playback */
		and_phy_reg(pi, LPPHY_SampleplayCnt,
			(uint16)~LPPHY_SampleplayCnt_sampPlayCount_MASK);
	}

	/* Stop calibration */
	write_phy_reg(pi, LPPHY_iqloCalCmdGctl, 0);
	...
	if (LPREV_LT(pi->pubpi.phy_rev, 2)) {
		...
		/* Adjust ADC common mode */
		write_phy_reg(pi, LPPHY_AfeRSSICtrl0, rssi_old);

		/* Restore tssi switch */
		write_radio_reg(pi, RADIO_2062_IQCAL_NORTH, iqcal_old);
		if (LPREV_IS(pi->pubpi.phy_rev, 1))
			write_phy_reg(pi, LPPHY_REV1_AfeRSSISel, rssi_muxsel_old);
	} else {
		/* RSSI ADC selection */
		mod_phy_reg(pi, LPPHY_REV2_auxadcCtrl,
			LPPHY_REV2_auxadcCtrl_iqlocalEn_MASK,
		...
		/* Adjust ADC common mode */
		write_phy_reg(pi, LPPHY_REV2_AfeRSSICtrl1, rssi_old);

		/* Restore tssi switch */
		write_radio_reg(pi, RADIO_2063_IQCAL_CTRL_2, iqcal_old);

		/* Restore PAPD */
		write_phy_reg(pi, LPPHY_REV2_papdctrl, papd_ctrl_old);
	...
	/* RSSI on/off */
	and_phy_reg(pi, LPPHY_AfeCtrlOvr, (uint16)~LPPHY_AfeCtrlOvr_pwdn_rssi_ovr_MASK);

	/* Reenable ADC */
	MOD_PHY_REG(pi, LPPHY, AfeCtrlOvr, pwdn_adc_ovr, 0);
...
wlc_lpphy_txpwrtbl_iqlo_cal(phy_info_t *pi, phy_cal_mode_t cal_mode)
{
	...
	if (!pi_lp->lpphy_use_cck_dig_loft_coeffs) {
		...
		*/
		...
		save_ofdm_filt_bw = READ_PHY_REG(pi, LPPHY_REV2, lpfbwlutreg1, lpfbwlut5);
		cck_filt_bw = READ_PHY_REG(pi, LPPHY_REV2, lpfbwlutreg0, lpfbwlut0);
		MOD_PHY_REG(pi, LPPHY_REV2, lpfbwlutreg1, lpfbwlut5, cck_filt_bw);

		wlc_lpphy_tx_iqlo_cal(pi, &target_gains, CAL_DIGLO, 2, FALSE);

		/* restore filters */
		MOD_PHY_REG(pi, LPPHY_REV2, lpfbwlutreg1, lpfbwlut5, save_ofdm_filt_bw);
	...
	if (LPREV_GE(pi->pubpi.phy_rev, 2)) {
		...
		/* Restore PA */
		and_phy_reg(pi, LPPHY_RFOverride0, (uint16)~
			(LPPHY_RFOverride0_gmode_tx_pu_ovr_MASK |
			LPPHY_RFOverride0_amode_tx_pu_ovr_MASK));
	}
}
...
wlc_lpphy_tx_filter_init(phy_info_t *pi)
{
	...
	if (CHSPEC_CHANNEL(pi->radio_chanspec) != 14) {
		...
		japan_wide_filter = pi->japan_wide_filter;
	}

	if (LPREV_LT(pi->pubpi.phy_rev, 2)) {
		mod_phy_reg(pi, LPPHY_lpphyCtrl,
			LPPHY_lpphyCtrl_txfiltSelect_MASK,
			(japan_wide_filter ? 2 : 0) << LPPHY_lpphyCtrl_txfiltSelect_SHIFT);
		...
		mod_phy_reg(pi, LPPHY_lpphyCtrl,
			LPPHY_lpphyCtrl_txfiltSelect_MASK,
			2 << LPPHY_lpphyCtrl_txfiltSelect_SHIFT);
		...
		MOD_PHY_REG(pi, LPPHY_REV2, lpfbwlutreg0, lpfbwlut0,
			cck_filt_bw);
		MOD_PHY_REG(pi, LPPHY_REV2, lpfbwlutreg1, lpfbwlut5, ofdm_filt_bw);
...
wlc_lpphy_temp_adj_tables(phy_info_t *pi, int32 temp_adj)
{
	...
	tab.tbl_ptr = tableBuffer;	/* ptr to buf */
	tableBuffer[0] = pi->tr_R_gain_val + temp_adj;
	tableBuffer[1] = pi->tr_T_gain_val + temp_adj;
	tab.tbl_len = 2;			/* # values   */
	tab.tbl_id = 17;			/* gain_val_tbl_rev3 */
	tab.tbl_offset = 64;		/* tbl offset */
...
wlc_phy_periodic_cal_lpphy(phy_info_t *pi)
{
	...
	/*
	 * full_cal denote forced calibration. When full_cal is TRUE always calibration
	 * happens irrespective of temperature. When full_cal is FALSE and temperature
	 * change at least lpphy_cal_delta_temp from the previous calibration
	 * temperature then calibration will be done. Otherwise calibration will not be
	 * done.
	 */
	if (full_cal == 0) {
		if ((current_temperature <
			(pi_lp->lpphy_last_cal_temperature + pi_lp->lpphy_cal_delta_temp)) &&
			(current_temperature >
			(pi_lp->lpphy_last_cal_temperature - pi_lp->lpphy_cal_delta_temp))) {
			return;
		}
...
#define qone_by_numsamps 20	/* q format of one_by_numsamps */
	...
	if (LPREV_GE(pi->pubpi.phy_rev, 2)) {
		...
		if (!suspend)
		  ...
		  save_rssi_settings = read_phy_reg(pi, LPPHY_REV2_AfeRSSICtrl1);
		  save_rssiformat = read_phy_reg(pi, LPPHY_REV2_auxadcCtrl) &
		    LPPHY_REV2_auxadcCtrl_rssiformatConvEn_MASK;

		  /* set the "rssiformatConvEn" field in the auxadcCtrl to 1 */
		  mod_phy_reg(pi, LPPHY_REV2_auxadcCtrl,
		              LPPHY_REV2_auxadcCtrl_rssiformatConvEn_MASK,
		              1<<LPPHY_REV2_auxadcCtrl_rssiformatConvEn_SHIFT);

		  /* slpinv_rssi */
		  mod_phy_reg(pi, LPPHY_REV2_AfeRSSICtrl1, (1<<13), (0<<13));
		  mod_phy_reg(pi, LPPHY_REV2_AfeRSSICtrl1, (0xf<<0), (0<<0));
		  mod_phy_reg(pi, LPPHY_REV2_AfeRSSICtrl1, (0xf<<4), (11<<4));
		  mod_phy_reg(pi, LPPHY_REV2_AfeRSSICtrl1, (0x7<<10), (5<<10));

		  /* read the rcal value */
		  rcalvalue = read_radio_reg(pi, RADIO_2063_COMMON_13) & 0xf;

		  /* set powerdetector before PA and rssi mux to tempsense */
		  savemux = read_phy_reg(pi, LPPHY_REV2_AfeCtrlOvrVal) &
		          LPPHY_REV2_AfeCtrlOvrVal_rssi_muxsel_ovr_val_MASK;

		  mod_phy_reg(pi, LPPHY_REV2_AfeCtrlOvrVal,
		              LPPHY_REV2_AfeCtrlOvrVal_rssi_muxsel_ovr_val_MASK,
		              5<<LPPHY_REV2_AfeCtrlOvrVal_rssi_muxsel_ovr_val_SHIFT);

		  /* reset auxadc */
		  mod_phy_reg(pi, LPPHY_REV2_auxadcCtrl,
		              LPPHY_REV2_auxadcCtrl_auxadcreset_MASK,
		              1<<LPPHY_REV2_auxadcCtrl_auxadcreset_SHIFT);

		  mod_phy_reg(pi, LPPHY_REV2_auxadcCtrl,
		              LPPHY_REV2_auxadcCtrl_auxadcreset_MASK,
		              0<<LPPHY_REV2_auxadcCtrl_auxadcreset_SHIFT);
		  ...
		  /* reset auxadc */
		  mod_phy_reg(pi, LPPHY_REV2_auxadcCtrl, LPPHY_REV2_auxadcCtrl_auxadcreset_MASK,
		              1<<LPPHY_REV2_auxadcCtrl_auxadcreset_SHIFT);
		  mod_phy_reg(pi, LPPHY_REV2_auxadcCtrl, LPPHY_REV2_auxadcCtrl_auxadcreset_MASK,
		              0<<LPPHY_REV2_auxadcCtrl_auxadcreset_SHIFT);
		  ...
		  write_phy_reg(pi, (uint16)LPPHY_REV2_AfeRSSICtrl1, (uint16)save_rssi_settings);
		  mod_phy_reg(pi, (uint16)LPPHY_REV2_auxadcCtrl,
		              (uint16)LPPHY_REV2_auxadcCtrl_rssiformatConvEn_MASK,
		              (uint16)save_rssiformat);
		  mod_phy_reg(pi, (uint16)LPPHY_REV2_AfeCtrlOvrVal,
...
#define qc0 13									/* qformat of c0 */
	  ...
	  /* set the "rssiformatConvEn" field in the auxadcCtrl to 1 */
	  mod_phy_reg(pi, LPPHY_REV2_auxadcCtrl, LPPHY_REV2_auxadcCtrl_rssiformatConvEn_MASK,
	              1<<LPPHY_REV2_auxadcCtrl_rssiformatConvEn_SHIFT);

	  /* slpinv_rssi */
	  mod_phy_reg(pi, LPPHY_REV2_AfeRSSICtrl1, (1<<13), (0<<13));
	  mod_phy_reg(pi, LPPHY_REV2_AfeRSSICtrl1, (0xf<<0), (8<<0));
	  mod_phy_reg(pi, LPPHY_REV2_AfeRSSICtrl1, (0xf<<4), (10<<4));
	  mod_phy_reg(pi, LPPHY_REV2_AfeRSSICtrl1, (0x7<<10), (3<<10));
	  ...
	  mod_phy_reg(pi, LPPHY_REV2_AfeCtrlOvrVal,
	              LPPHY_REV2_AfeCtrlOvrVal_rssi_muxsel_ovr_val_MASK,
	              4<<LPPHY_REV2_AfeCtrlOvrVal_rssi_muxsel_ovr_val_SHIFT);
	...
	int32 *prod)
{
	...
	save_pwdn_rssi_ovr = read_phy_reg(pi, LPPHY_REV2_AfeCtrlOvr) &
		LPPHY_REV2_AfeCtrlOvr_pwdn_rssi_ovr_MASK;

	mod_phy_reg(pi, LPPHY_REV2_AfeCtrlOvr, LPPHY_REV2_AfeCtrlOvr_pwdn_rssi_ovr_MASK,
	1<<LPPHY_REV2_AfeCtrlOvr_pwdn_rssi_ovr_SHIFT);

	mod_phy_reg(pi, LPPHY_REV2_AfeCtrlOvrVal,
	LPPHY_REV2_AfeCtrlOvrVal_pwdn_rssi_ovr_val_MASK,
	0<<LPPHY_REV2_AfeCtrlOvrVal_pwdn_rssi_ovr_val_SHIFT);
	...
	mod_phy_reg(pi, LPPHY_REV2_NumrssiSamples,  LPPHY_REV2_NumrssiSamples_numrssisamples_MASK,
	numberOfSamples<<LPPHY_REV2_NumrssiSamples_numrssisamples_SHIFT);

	mod_phy_reg(pi, LPPHY_REV2_rssiwaittime, LPPHY_REV2_rssiwaittime_rssiwaittimeValue_MASK,
	waitTime<<LPPHY_REV2_rssiwaittime_rssiwaittimeValue_SHIFT);

	mod_phy_reg(pi, LPPHY_REV2_auxadcCtrl, LPPHY_REV2_auxadcCtrl_rssiestStart_MASK,
	1<<LPPHY_REV2_auxadcCtrl_rssiestStart_SHIFT);
	...
	if (done) {
		term0 = read_phy_reg(pi, LPPHY_REV2_rssiaccValResult0);
		term0 = (term0 & LPPHY_REV2_rssiaccValResult0_rssiaccResult0_MASK);
		term1 = read_phy_reg(pi, LPPHY_REV2_rssiaccValResult1);
		term1 = (term1 & LPPHY_REV2_rssiaccValResult1_rssiaccResult1_MASK);
		*sum = (term1 << 16) + term0;
		term0 = read_phy_reg(pi, LPPHY_REV2_rssiprodValResult0);
		term0 = (term0 & LPPHY_REV2_rssiprodValResult0_rssiProdResult0_MASK);
		term1 = read_phy_reg(pi, LPPHY_REV2_rssiprodValResult1);
	...
	/* restore result */
	mod_phy_reg(pi, (uint16)LPPHY_REV2_AfeCtrlOvr,
...
{
	return ((read_phy_reg(pi, LPPHY_TxPwrCtrlStatus) &
		LPPHY_TxPwrCtrlStatus_baseIndex_MASK) >> LPPHY_TxPwrCtrlStatus_baseIndex_SHIFT);
}
...
wlc_radio_2063_channel_tune(phy_info_t *pi, uint8 channel)
{
	...
	if (LPREV_GE(pi->pubpi.phy_rev, 3)) {
		 ...
		 */
		while (read_phy_reg(pi, LPPHY_REV2_extslnactrl0) != 0x5f80) {
			;
		}
		temp = read_phy_reg(pi, LPPHY_REV2_extslnactrl0);
		temp = (uint16)(temp & 0x0fff);
		temp = (uint16)(temp | (ci->RF_wrf_slna_RX_2G_1st_VT_STG1 << 12));
		write_phy_reg(pi, LPPHY_REV2_extslnactrl0, temp);
	}
	...
	qFxtal = wlc_phy_qdiv_roundup(pi->xtalfreq, PLL_2063_MHZ, 16);
...
WLBANDINITFN(wlc_radio_2062_init)(phy_info_t *pi)
{
	...
	/* Program PLL divider */
	if (pi->xtalfreq > (uint32)30000000) {
		pi->pdiv = 2;
	...
	pdiv_mhz = pi->pdiv * 1000000;

	/* Set KPD_SCALE */
	write_radio_reg(pi, RADIO_2062_RFPLL_CTRL07_SOUTH,
		(uint8)((((800 * pdiv_mhz) + pi->xtalfreq) / (2 * pi->xtalfreq)) - 8));
	...
	write_radio_reg(pi, RADIO_2062_RFPLL_CTRL18_SOUTH,
		(uint8)((((pi->xtalfreq * 100) + (pdiv_mhz << 4)) / (pdiv_mhz << 5)) - 1));
...
wlc_phy_init_test_lpphy(phy_info_t *pi)
{
	...
	/* Disable tx power control */
	wlc_phy_set_tx_pwr_ctrl_lpphy(pi, LPPHY_TX_PWR_CTRL_OFF);
	mod_phy_reg(pi, LPPHY_REV2_papdctrl, LPPHY_REV2_papdctrl_forcepapdClkOn_MASK,
		1 << LPPHY_REV2_papdctrl_forcepapdClkOn_SHIFT);
...
wlc_phy_tx_pu_lpphy(phy_info_t *pi, bool bEnable)
{
	if (!bEnable) {
		...
		mod_phy_reg(pi, LPPHY_AfeCtrlOvr,
			...
			1 << LPPHY_AfeCtrlOvr_pwdn_dac_ovr_SHIFT);

		/* Force on AFE */
		mod_phy_reg(pi, LPPHY_AfeCtrlOvrVal,
			LPPHY_AfeCtrlOvrVal_pwdn_dac_ovr_val_MASK,
			0 << LPPHY_AfeCtrlOvrVal_pwdn_dac_ovr_val_SHIFT);
		mod_phy_reg(pi, LPPHY_AfeCtrlOvr,
			LPPHY_AfeCtrlOvr_dac_clk_disable_ovr_MASK,
			1 << LPPHY_AfeCtrlOvr_dac_clk_disable_ovr_SHIFT);
		mod_phy_reg(pi, LPPHY_AfeCtrlOvrVal,
			LPPHY_AfeCtrlOvrVal_dac_clk_disable_ovr_val_MASK,
			0 << LPPHY_AfeCtrlOvrVal_dac_clk_disable_ovr_val_SHIFT);
		...
		if (LPREV_GE(pi->pubpi.phy_rev, 2)) {
			...
			mod_phy_reg(pi, LPPHY_REV2_RFOverride0,
				...
				1 << LPPHY_REV2_RFOverride0_internalrftxpu_ovr_SHIFT);
			mod_phy_reg(pi, LPPHY_REV2_RFOverrideVal0,
				LPPHY_REV2_RFOverrideVal0_internalrftxpu_ovr_val_MASK,
				1 << LPPHY_REV2_RFOverrideVal0_internalrftxpu_ovr_val_SHIFT);

			if (CHSPEC_IS2G(pi->radio_chanspec)) {
				/* PAP PU */
				mod_phy_reg(pi, LPPHY_REV2_rfoverride3,
					LPPHY_REV2_rfoverride3_stxpadpu2g_ovr_MASK,
					1 << LPPHY_REV2_rfoverride3_stxpadpu2g_ovr_SHIFT);
				mod_phy_reg(pi, LPPHY_REV2_rfoverride3_val,
					LPPHY_REV2_rfoverride3_val_stxpadpu2g_ovr_val_MASK,
					1 << LPPHY_REV2_rfoverride3_val_stxpadpu2g_ovr_val_SHIFT);
				/* PGA PU */
				mod_phy_reg(pi, LPPHY_REV2_rfoverride3,
					LPPHY_REV2_rfoverride3_stxpapu2g_ovr_MASK,
					1 << LPPHY_REV2_rfoverride3_stxpapu2g_ovr_SHIFT);
				mod_phy_reg(pi, LPPHY_REV2_rfoverride3_val,
					LPPHY_REV2_rfoverride3_val_stxpapu2g_ovr_val_MASK,
					1 << LPPHY_REV2_rfoverride3_val_stxpapu2g_ovr_val_SHIFT);
				/* PA PU */
				mod_phy_reg(pi, LPPHY_REV2_rfoverride3,
					LPPHY_REV2_rfoverride3_stxpapu_ovr_MASK,
					1 << LPPHY_REV2_rfoverride3_stxpapu_ovr_SHIFT);
				mod_phy_reg(pi, LPPHY_REV2_rfoverride3_val,
					LPPHY_REV2_rfoverride3_val_stxpapu_ovr_val_MASK,
					1 << LPPHY_REV2_rfoverride3_val_stxpapu_ovr_val_SHIFT);
			} else {
				/* PAP PU */
				mod_phy_reg(pi, LPPHY_REV2_rfoverride3,
					LPPHY_REV2_rfoverride3_stxpadpu2g_ovr_MASK,
				...
				mod_phy_reg(pi, LPPHY_REV2_rfoverride3_val,
					LPPHY_REV2_rfoverride3_val_stxpadpu2g_ovr_val_MASK,
					0 << LPPHY_REV2_rfoverride3_val_stxpadpu2g_ovr_val_SHIFT);
				/* PGA PU */
				mod_phy_reg(pi, LPPHY_REV2_rfoverride3,
					LPPHY_REV2_rfoverride3_stxpapu2g_ovr_MASK,
					1 << LPPHY_REV2_rfoverride3_stxpapu2g_ovr_SHIFT);
				mod_phy_reg(pi, LPPHY_REV2_rfoverride3_val,
					LPPHY_REV2_rfoverride3_val_stxpapu2g_ovr_val_MASK,
					0 << LPPHY_REV2_rfoverride3_val_stxpapu2g_ovr_val_SHIFT);
				/* PA PU */
				mod_phy_reg(pi, LPPHY_REV2_rfoverride3,
					LPPHY_REV2_rfoverride3_stxpapu_ovr_MASK,
					1 << LPPHY_REV2_rfoverride3_stxpapu_ovr_SHIFT);
				mod_phy_reg(pi, LPPHY_REV2_rfoverride3_val,
					LPPHY_REV2_rfoverride3_val_stxpapu_ovr_val_MASK,
					0 << LPPHY_REV2_rfoverride3_val_stxpapu_ovr_val_SHIFT);
			}
		} else {
			/* Force on the transmit chain */
			mod_phy_reg(pi, LPPHY_RFOverride0,
				LPPHY_RFOverride0_internalrftxpu_ovr_MASK,
				1 << LPPHY_RFOverride0_internalrftxpu_ovr_SHIFT);
			mod_phy_reg(pi, LPPHY_RFOverrideVal0,
				LPPHY_RFOverrideVal0_internalrftxpu_ovr_val_MASK,
				1 << LPPHY_RFOverrideVal0_internalrftxpu_ovr_val_SHIFT);
		}
	}
}
#endif 
