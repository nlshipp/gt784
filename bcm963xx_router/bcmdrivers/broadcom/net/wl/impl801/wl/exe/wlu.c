 ...
 *
 * $Id: wlu.c,v 1.1 2010/08/05 21:59:00 ywu Exp $
 */
...
static cmd_func_t wl_test_tssi, wl_test_tssi_offs, wl_rxiq;
...
cmd_t wl_cmds[] = {
	...
	"\tUsage: wl p2p_ifadd <MAC-address> go|client [chanspec]\n"
	...
	{ "p2p_noa", wl_p2p_noa, -1, WLC_SET_VAR,
	"setup WiFi NoA schedule\n"
	"\tUsage: wl p2p_noa <type> <action> <action-specific-options>\n"
	"\t\t<type>:\n"
	"\t\t\t0: Absence (on GO)\n"
	"\t\t\t1: Request Absence (on client)\n"
	"\t\t\t2: Request Presence (on client)\n"
	"\t\t<action>:\n"
	"\t\t\t-1: cancel the schedule\n"
	"\t\t\t0,1: <option> <schedule>\n"
	"\t\t<schedule>:\n"
	"\t\t\toption 0 (WL_P2P_SCHED_OPTION_NONE): <start> <interval> <duration> <count> ...\n"
	"\t\t\toption 1 (WL_P2P_SCHED_OPTION_BCNPCT): <percentage>\n"
	},
	{ "p2p_ops", wl_p2p_ops, -1, WLC_SET_VAR,
	"set WiFi OppPS and CTWindow\n"
...
{
	uint8 cbuf[HCI_CMD_PREAMBLE_SIZE + HCI_CMD_DATA_SIZE];
	amp_hci_cmd_t *cpkt = (amp_hci_cmd_t *)&cbuf[0];
	char *op;
	uint8 plen;
...
{
	uint8 dbuf[HCI_ACL_DATA_PREAMBLE_SIZE + 2048];
	amp_hci_ACL_data_t *dpkt = (amp_hci_ACL_data_t *)&dbuf[0];
	uint16 dlen;
...
wl_p2p_scan(void *wl, cmd_t *cmd, char **argv)
{
	...
	sparams = (wl_scan_params_t *)(params+1);
	sparams_size = malloc_size - sizeof(wl_p2p_scan_t);

	params->type = 'S';

	err = wl_scan_prep(wl, cmd, argv, sparams, &sparams_size);
...
wl_p2p_noa(void *wl, cmd_t *cmd, char **argv)
{
	 ...
	 */
		...
		normal:
			for (i = 0; i < count; i += 4) {
				noa->desc[i / 4].start = htod32(strtol(argv[i], &endptr, 0));
