 ...
 *
 * $Id: wlc_led.c,v 1.1 2010/08/05 21:59:02 ywu Exp $
 */
...
#define LED_300_SEC_COUNT ((1000 / LED_TIME) * 300) /* 5 * 300 */
...
static void wlc_timer_led(void *arg);
static void wlc_led_timer(led_info_t *ledh);
...
#ifdef DSLCPE
void dslcpe_setup_wlan_led(void *v, int led_idx, int pin, int func, int act_hi)
{
	struct led *led = (struct led *)v;
	led[led_idx].pin		= pin;
	led[led_idx].behavior	= func;
	led[led_idx].activehi	= act_hi? TRUE : FALSE;
}
#endif

led_info_t *
BCMATTACHFN(wlc_led_attach)(wlc_pub_t *pub, wlc_info_t *wlc)
{
	led_info_t *li;
	struct led *led = NULL;
	int i;
	...
	ledfunc[12] = wlc_led_assocact;
	ledfunc[13] = wlc_led_wi4;
	ledfunc[14] = wlc_led_wi5;
	ledfunc[15] = wlc_led_blinkslow;
	ledfunc[16] = wlc_led_blinkmed;
	ledfunc[17] = wlc_led_blinkfast;
	ledfunc[18] = wlc_led_blinkcustom;
	ledfunc[19] = wlc_led_blinkperiodic;
	ledfunc[20] = wlc_led_assoc_with_security;
	ledfunc[21] = wlc_led_off;
	...
	if (li->pub->sih->chip == BCM43224_CHIP_ID && li->pub->sih->chiprev == 0 &&
	    &li->led[4] != NULL) {
...
#ifdef DSLCPE
		wlc_dslcpe_led_attach(li->led, dslcpe_setup_wlan_led);
#endif
	}
	...
	/* register module */
	if (wlc_module_register(pub, led_iovars, "led",
		li, wlc_led_doiovar, NULL, NULL)) {
		WL_ERROR(("wl%d: led wlc_module_register() failed\n", pub->unit));
		goto fail;
	}
	if (!(li->led_timer = wl_init_timer(wlc->wl, wlc_timer_led, wlc, "led"))) {
	...
	void *params, uint p_len, void *arg, int len, int val_size, struct wlc_if *wlcif)
{
	...
	if ((err = wlc_iovar_check(li->pub, vi, arg, len, IOV_ISSET(actionid))) != 0)
		return err;
...
wlc_led_up(wlc_info_t *wlc)
{
	if (wlc->ledh->up)
		return;
	wl_add_timer(wlc->wl, wlc->ledh->led_timer, LED_TIME, TRUE);
	wlc->ledh->up = TRUE;
}
...
wlc_led_down(wlc_info_t *wlc)
{
	...
	if (!wl_del_timer(wlc->wl, wlc->ledh->led_timer))
		callbacks++;
