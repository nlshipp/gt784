 ...
 *
 * $Id: wlc_ap.c,v 1.2 2011/06/28 07:26:37 slv Exp $
 */
#ifndef AP
...
typedef struct wlc_radio_pwrsave {
	uint8  level;			/* No, Low, Medium or High Power Savings */
...
#ifdef RXCHAIN_PWRSAVE
	wlc_pwrsave_t rxchain_pwrsave;       /* rxchain reduction power save structure */
#endif
#ifdef RADIO_PWRSAVE
...
#endif /* WIN7 */
static bool wlc_scb_scan(wlc_ap_info_t *ap, uint32 flagmask, uint32 flagvalue);
static void wlc_ap_sta_probe(wlc_ap_info_t *ap, struct ether_addr *ea, struct scb *scb);
static void wlc_ap_sta_probe_complete(wlc_info_t *wlc, uint txstatus, void *scb_arg);
...
#ifdef WDS
static void wlc_ap_wds_probe(wlc_ap_info_t *ap, struct ether_addr *ea, struct scb *scb);
static void wlc_ap_wds_probe_complete(wlc_info_t *wlc, uint txstatus, void *ea);
#endif /* WDS */
...
BCMATTACHFN(wlc_ap_attach)(wlc_pub_t *pub, wlc_info_t *wlc)
{
	...
	wlc_module_register(pub, wlc_ap_iovars, "wlc_ap_iovars", appvt,
		wlc_ap_doiovar, wlc_ap_watchdog, NULL);
...
#ifdef RADAR
	if (BSS_WL11H_ENAB(wlc, bsscfg) && CHSPEC_IS5G(target_bss->chanspec) &&
...
wlc_update_ab_wds_timeout(wlc_ap_info_t *ap)
{
	 ...
	 *
	 */
	if (wlc->erp_ie_nonerp || wlc->protection->nonerp_ovlp_timeout || wlc->nonabcap_assoc) {
	...
	ASSERT((AB_WDS_TIMEOUT_MIN <= wlc->ab_wds_timeout) && (wlc->ab_wds_timeout <=
		AB_WDS_TIMEOUT_MAX));
...
wlc_wds_create_link_event(wlc_ap_info_t *ap, struct scb *scb)
{
	...
	wlc_event_t *e = wlc_event_alloc(wlc->eventq);
	if (e == NULL) {
	...
	e->bsscfgidx = WLC_BSSCFG_IDX(scb->bsscfg);
	e->event.event_type = WLC_E_LINK;
	e->event.flags = WLC_EVENT_MSG_LINK;
	bcopy(scb->ea.octet, e->event.addr.octet, ETHER_ADDR_LEN);
	e->addr = &(e->event.addr);

	/* Resolve correct sub-interface using bsscfg index */
	e->wlif = scb->wds->wlif;
	strncpy(e->event.ifname, wl_ifname(wlc->wl, scb->wds->wlif), sizeof(e->event.ifname) - 1);
	...
	const struct ether_addr *dst, wl_chan_switch_t *csa, uint8 action_id)
{
	...
	if (action_id == DOT11_SM_ACTION_CHANNEL_SWITCH) {
		cp = wlc_write_csa_ie(csa, action_hdr->data, (body_len - DOT11_ACTION_FRMHDR_LEN));
		if (ext_ie)
	...
	wlc_sendmgmt(wlc, p, NULL, 0, NULL, 0);
}
...
wlc_send_public_action_switch_channel(wlc_ap_info_pvt_t *appvt, wlc_bsscfg_t *bsscfg)
{
	...
	if ((p = wlc_frame_get_mgmt(wlc, FC_ACTION, &ether_bcast, &bsscfg->cur_etheraddr,
		&bsscfg->BSSID, body_len, &pbody)) == NULL) {
		return;
	}
	...
	cp = wlc_write_csa_body(&wlc->csa, action_hdr->data);
...
wlc_send_action_switch_channel(wlc_ap_info_t *ap, wlc_bsscfg_t *bsscfg)
{
	...
	wlc_send_action_switch_channel_ex(ap, bsscfg, &ether_bcast,
		&appvt->wlc->csa, DOT11_SM_ACTION_CHANNEL_SWITCH);
	if (N_ENAB(appvt->pub))
		wlc_send_action_switch_channel_ex(ap, bsscfg, &ether_bcast,
			&appvt->wlc->csa, DOT11_SM_ACTION_EXT_CSA);
	...
	uint8 *body, int body_len)
{
	...
	wlc_sendmgmt(wlc, outp, NULL, 0, NULL, 0);
}

/* return TRUE if there are any associated STAs with matching flag value, otherwise FALSE */
static bool
wlc_scb_scan(wlc_ap_info_t *ap, uint32 flagmask, uint32 flagvalue)
{
	wlc_ap_info_pvt_t *appvt = (wlc_ap_info_pvt_t *) ap;
	wlc_info_t *wlc = appvt->wlc;
	struct scb *scb;
	...
	FOREACHSCB(wlc->scbstate, &scbiter, scb) {
		if (SCB_ASSOCIATED(scb) && ((scb->flags & flagmask) == flagvalue))
			return (TRUE);
	}
...
wlc_ap_stas_timeout(wlc_ap_info_t *ap)
{
	...
	FOREACHSCB(wlc->scbstate, &scbiter, scb) {
		...
		if (SCB_ASSOCIATED(scb) &&
			...
			wlc_ap_sta_probe(ap, &scb->ea, scb);
		}
	...
	FOREACHSCB(wlc->scbstate, &scbiter, scb) {
		if (SCB_WDS(scb)) {
			...
			if ((pub->now - scb->used) >= wlc->scb_activity_time ||
				...
				wlc_ap_wds_probe(ap, &scb->ea, scb);
		}
	}
}
...
void
wlc_authresp_ap(wlc_ap_info_t *ap, wlc_bsscfg_t *bsscfg, d11rxhdr_t *rxh,
                struct dot11_management_header *hdr, void *body, uint len)
{
	...
	if (fc & FC_WEP) {
		 ...
		 * text
		 */
		key = WLC_BSSCFG_WEPKEY(bsscfg, key_index);
		if (key) {
	...
	btamp_sanity:
		...
		case DOT11_SHARED_KEY:
			...
			if (WLC_BSSCFG_AUTH(bsscfg) != DOT11_SHARED_KEY) {
				status = DOT11_SC_AUTH_MISMATCH;
				break;
			}
#endif
...
send_result:
	...
	wlc_sendauth(bsscfg, &hdr->sa, &bsscfg->BSSID, scb,
		auth_alg, auth_seq + 1, status, NULL, challenge,
		((rxh->PhyRxStatus_0 & PRXS0_SHORTH) != 0));
	...
	uint16 *auth, uint32 *wsec)
{
	...
	/* Update the potential crypto being used for the SCB */
	bcmwpa_cipher2wsec(mcast->oui, wsec);
...
#ifdef BCMWAPI_WAI
static int
wlc_check_wapiie(wlc_ap_info_t *ap, wlc_bsscfg_t *bsscfg, bcm_tlv_t *wapiie, uint16 *auth,
	int32 *wsec)
{
	wlc_ap_info_pvt_t *appvt = (wlc_ap_info_pvt_t *) ap;
	wlc_pub_t *pub;
	...
	*auth = (uint16)WPA_auth;
	...
	if (count != 1 ||
		...
		!wpa_cipher_enabled(ap, bsscfg, WAPI_CSE_WPI_2_CIPHER(ucast->list[0].type))) {
	...
	if (bcmp(mcast->oui, WAPI_OUI, DOT11_OUI_LEN) ||
		!wpa_cipher_enabled(ap, bsscfg, WAPI_CSE_WPI_2_CIPHER(mcast->type))) {
...
/*
 * Local copy from bcmwpa.c to resolve dongle rte build undefined
 * reference to bcm_find_wpsie problem
 */
static bool
wlc_ap_bcm_is_wps_ie(uint8 *ie, uint8 **tlvs, uint *tlvs_len)
{
...
wlc_ap_bcm_find_wpsie(uint8 *parse, uint len)
{
	bcm_tlv_t *ie;

	while ((ie = bcm_parse_tlvs(parse, len, DOT11_MNG_WPA_ID))) {
		if (wlc_ap_bcm_is_wps_ie((uint8*)ie, &parse, &len)) {
...
void
wlc_process_assocreq(wlc_ap_info_t *ap, wlc_bsscfg_t *bsscfg, d11rxhdr_t *rxh,
                     ...
                     struct scb *scb)
{
	...
	if (AP_ENAB(pub) && SCAN_IN_PROGRESS(wlc->scan)) {
	...
	*/
	...
	if ((bsscfg->WPA_auth != WPA_AUTH_DISABLED && WSEC_ENABLED(bsscfg->wsec)) ||
	    WSEC_SES_OW_ENABLED(bsscfg->wsec)) {
	        parse = tlvs;
	        parse_len = tlvs_len;
	        if ((wpaie = wlc_ap_bcm_find_wpsie(parse, parse_len)) != NULL) {
	            if (wpaie->length < 5) {
	                ...
	                akm_ie_included = TRUE;
	                /* check RSN AKM */
	                if (wlc_check_wpa2ie(ap, bsscfg, (bcm_tlv_t *)wpaie, &scb->WPA_auth,
	                                     &scb->wsec)) {
	                        WL_ERROR(("wl%d: wlc_process_assocreq: "
			...
			akm_ie_included = TRUE;
			/* check WAPI AKM */
			if (wlc_check_wapiie(ap, bsscfg, (bcm_tlv_t *)wpaie, &scb->WPA_auth,
				&scb->wsec)) {
				WL_ERROR(("wl%d: wlc_process_assocreq: "
	...
	if (N_ENAB(pub) && WSEC_ENABLED(bsscfg->wsec)) {
		/* When a HT STA using TKIP-only unicast chiper suite tries
	...
	if ((scb->flags & SCB_HT40INTOLERANT)) {
		...
		if (N_ENAB(pub) && COEX_ENAB(pub)) {
			...
			if (CHSPEC_IS40(current_bss->chanspec)) {
				...
				wlc_ht_coex_switch_bw(wlc, TRUE, COEX_DGRADE_40INTOL);
			}
		}
	}
#endif /* WL11N */
	...
	if ((scb->flags & SCB_APSDCAP) && ((scb->flags & SCB_HTCAP) == 0))
		wlc->non11n_apsd_assoc = TRUE;

	if (wlc->band->gmode)
	        wlc_update_gbss_modes(wlc);
	if (N_ENAB(pub))
	        wlc_update_n_protection(wlc);
	...
	*/
	...
	if ((bsscfg->WPA_auth == WPA_AUTH_DISABLED) && WSEC_WEP_ENABLED(bsscfg->wsec) &&
	    MBSS_ENAB(pub)) {
	        ...
	        if (defkey && ((defkey->algo == CRYPTO_ALGO_WEP1) ||
	...
	if (wlc->brcm_ie[TLV_LEN_OFF] > 0)
	        len += TLV_HDR_LEN + wlc->brcm_ie[TLV_LEN_OFF];
	...
	if (SCB_HT_CAP(scb)) {
		...
		if (COEX_ACTIVE(wlc)) {
			len += TLV_HDR_LEN + DOT11_EXTCAP_LEN;
	...
	if (SCB_HT_CAP(scb)) {
		...
		if (COEX_ACTIVE(wlc)) {
			...
			uint8 ext_cap_ie = DOT11_OBSS_COEX_MNG_SUPPORT;
			tlvs = wlc_write_info_elt(tlvs, DOT11_MNG_EXT_CAP,
				DOT11_EXTCAP_LEN, &ext_cap_ie);
	...
	if (wlc->brcm_ie[TLV_LEN_OFF] > 0)
		tlvs = wlc_copy_info_elt(tlvs, wlc->brcm_ie);
	...
	wlc_sendmgmt(wlc, p, scb, rxh->PhyRxStatus_0 & PRXS0_SHORTH, NULL, 0);
	...
	wlc_rateset_filter(&req_rates, &scb->rateset, FALSE, WLC_RATES_CCK_OFDM, RATE_MASK,
...
exit:
	...
	if (wapi_assocreq) {
		...
		wlc_wapi_station_event(wlc, bsscfg, &scb->ea, scb->wpaie, gsn,
		                       WAPI_WAI_REQUEST);
	}
#endif /* BCMWAPI_WAI */
#endif /* WIN7 */
...
wlc_reset_rxchain_pwrsave_mode(wlc_ap_info_t *ap)
{
	...
	if (appvt->rxchain_pwrsave.in_power_save)
		wlc_stf_rxchain_set(wlc, 0x3);
...
wlc_assoc_notify(wlc_ap_info_t *ap, struct ether_addr *sta, struct wlc_if *wlcif)
{
	...
	/* prepare 802.11f IAPP announce packet. This should look like a wl rx packet since it sent
	   ...
	   extra headroom (how much), but the alignement enforced by HWRXOFF must be preserved.
	*/
	if ((p = PKTGET(pub->osh, sizeof(pkt80211f) + BCMEXTRAHDROOM + HWRXOFF + ETHER_HDR_LEN,
	                FALSE)) == NULL)
		goto err;
	ASSERT(ISALIGNED((uintptr)PKTDATA(pub->osh, p), sizeof(uint32)));
	PKTPULL(pub->osh, p, BCMEXTRAHDROOM + HWRXOFF);
...
wlc_reassoc_notify(wlc_ap_info_t *ap, struct ether_addr *sta, struct wlc_if *wlcif)
{
	...
	len = sizeof(struct lu_reassoc_pkt);
	if ((p = PKTGET(osh, len + BCMEXTRAHDROOM + HWRXOFF + ETHER_HDR_LEN, FALSE)) == NULL)
		goto err;
	ASSERT(ISALIGNED((uintptr)PKTDATA(osh, p), sizeof(uint32)));
	PKTPULL(osh, p, BCMEXTRAHDROOM + HWRXOFF);
...
static void
wlc_ap_sta_probe(wlc_ap_info_t *ap, struct ether_addr *ea, struct scb *scb)
{
	wlc_ap_info_pvt_t *appvt = (wlc_ap_info_pvt_t *) ap;
	wlc_info_t *wlc = appvt->wlc;
	wlc_pub_t *pub;
	...
	if ((pkt == NULL) ||
	    wlc_pkt_callback_register(wlc, wlc_ap_sta_probe_complete, (void*)scb, pkt))
		WL_ERROR(("wl%d: wlc_ap_sta_probe: could not register callback\n", pub->unit));
}

static void
wlc_ap_sta_probe_complete(wlc_info_t *wlc, uint txstatus, void *scb_arg)
{
	struct scb *scb;
	struct scb_iter scbiter;
#if defined(BCMDBG) || defined(WLMSG_INFORM)
...
static void
wlc_ap_wds_probe(wlc_ap_info_t *ap, struct ether_addr *ea, struct scb *scb)
{
	wlc_ap_info_pvt_t* appvt = (wlc_ap_info_pvt_t*) ap;
	wlc_info_t* wlc = appvt->wlc;
	wlc_pub_t* pub;
	...
	if ((pkt == NULL) ||
		wlc_pkt_callback_register(wlc, wlc_ap_wds_probe_complete, (void*)ea, pkt))
		WL_ERROR(("wl%d: wlc_ap_wds_probe: could not register callback\n", pub->unit));
}

/*  Check for ack, if there is no ack, reset the rssi value */
static void
wlc_ap_wds_probe_complete(wlc_info_t *wlc, uint txstatus, void *ea)
{
	struct scb *scb;
#if defined(BCMDBG) || defined(WLMSG_INFORM)
	char eabuf[ETHER_ADDR_STR_LEN];
...
wlc_radar_detected(wlc_ap_info_t *ap)
{
	...
	if ((wlc->spect_state & RADAR_SIM) ||
		...
		wlc->spect_state &= ~RADAR_SIM; /* clear one-shot radar simulator */
		return TRUE;
	} else
		return FALSE;
}
...
wlc_dfs_cacstate_cac(wlc_ap_info_t *ap)
{
	...
	if (wlc_radar_detected(ap) == TRUE) {
		...
		wlc_do_chanswitch(wlc, appvt->dfs_cac.chanspec_next);
...
wlc_dfs_cacstate_ism(wlc_ap_info_t *ap)
{
	...
	if (!appvt->dfs_cac.chanspec_next) {
	        ...
	        wlc->csa.chspec = WLC_BAND_PI_RADIO_CHANSPEC;
	} else {
	        wlc->csa.chspec = appvt->dfs_cac.chanspec_next;
	}
	wlc->csa.mode = DOT11_CSA_MODE_NO_TX;
	wlc->csa.count = MAX((WLC_DFS_CSA_MSEC/cfg->current_bss->beacon_period),
	        WLC_DFS_CSA_BEACONS);

	wlc->csa.reg = wlc_get_regclass(wlc->cmi, wlc->csa.chspec);
	...
	wlc->spect_state |= (NEED_TO_UPDATE_BCN | NEED_TO_SWITCH_CHANNEL);
	...
	WL_DFS(("wl%d: dfs : state to %s channel current %d next %d at %dms, starting CSA"
		" process\n",
		pub->unit, wlc_dfs_cacstate_str[appvt->dfs_cac.status.state],
		CHSPEC_CHANNEL(WLC_BAND_PI_RADIO_CHANSPEC), CHSPEC_CHANNEL(wlc->csa.chspec),
		(appvt->dfs_cac.cactime -
			appvt->dfs_cac.duration)*WLC_DFS_RADAR_CHECK_INTERVAL));
...
wlc_dfs_cacstate_csa(wlc_ap_info_t *ap)
{
	...
	pub = appvt->pub;
	if ((wlc->spect_state & NEED_TO_SWITCH_CHANNEL) ||
...
wlc_dfs_cacstate_init(wlc_ap_info_t *ap)
{
	...
	wlc->quiet_cmd.count = wlc->csa.count = 0;
...
#ifdef DSLCPE
		if(wlc_is_EU_country(wlc) &&
		( (CHSPEC_CHANNEL(chanspec)==120) || (CHSPEC_CHANNEL(chanspec)==124) || (CHSPEC_CHANNEL(chanspec)==128)
			|| (chanspec == 0x1d76 /*ch116l aka ch118/40Mhz */) || 
			(chanspec == 0x1d7e /*ch124l aka ch126/40Mhz */) )) {

			appvt->dfs_cac.cactime_preism = 10*WLC_DFS_CAC_TIME_SEC_DEFAULT;
			appvt->dfs_cac.cactime_postism = 10*WLC_DFS_CAC_TIME_SEC_DEFAULT;

		} else {

			appvt->dfs_cac.cactime_preism = WLC_DFS_CAC_TIME_SEC_DEFAULT;
...
wlc_dump_rxchain_pwrsave(wlc_ap_info_t *ap, struct bcmstrbuf *b)
{
	...
	wlc_dump_pwrsave(&appvt->rxchain_pwrsave, b);
...
wlc_ap_watchdog(void *arg)
{
		...
		FOREACHSCB(wlc->scbstate, &scbiter, scb) {
			...
			if ((ap->rxchain_pwrsave_enable == RXCHAIN_PWRSAVE_ENAB_BRCM_NONHT) &&
				...
				if (SCB_ASSOCIATED(scb) && (!(scb->flags & SCB_BRCM) &&
					...
					if (appvt->rxchain_pwrsave.in_power_save) {
						wlc_stf_rxchain_set(wlc, 0x3);
						appvt->rxchain_pwrsave.in_power_save = FALSE;
					}
					appvt->rxchain_pwrsave.power_save_check = 0;
					done = 1;
				}
			}
#endif
...
#ifdef RXCHAIN_PWRSAVE
		if (!done)
			appvt->rxchain_pwrsave.power_save_check = ap->rxchain_pwrsave_enable;
#endif
#ifdef RADAR
	...
	/* part 2 */
		...
		if (wlc->ofdm_assoc && !wlc_scb_scan(ap, (SCB_HTCAP | SCB_NONERP), 0))
			wlc->ofdm_assoc = FALSE;

		if (wlc->ht20in40_assoc &&
		    !wlc_scb_scan(ap, (SCB_HTCAP | SCB_IS40), SCB_HTCAP))
			wlc->ht20in40_assoc = FALSE;

		if (wlc->non_gf_assoc && !wlc_scb_scan(ap, SCB_NONGF, SCB_NONGF))
			wlc->non_gf_assoc = FALSE;

		if (wlc->wme_apsd_assoc && !wlc_scb_scan(ap, SCB_APSDCAP, SCB_APSDCAP))
			wlc->wme_apsd_assoc = FALSE;

		if (wlc->ht40intolerant_assoc &&
		    !wlc_scb_scan(ap, SCB_HT40INTOLERANT, SCB_HT40INTOLERANT))
			wlc->ht40intolerant_assoc = FALSE;

		/* update non 11n APSD association status */
		if (wlc->non11n_apsd_assoc &&
		    !wlc_scb_scan(ap, (SCB_APSDCAP | SCB_HTCAP), SCB_APSDCAP))
			wlc->non11n_apsd_assoc = FALSE;

		if (wlc->band->gmode) {
			/* update our nonerp flag based on associated STAs */
			if (wlc->nonerp_assoc && !wlc_scb_scan(ap, SCB_NONERP, SCB_NONERP))
				wlc->nonerp_assoc = FALSE;

			/* update our longslot flag based on associated STAs */
			if (wlc->longslot_assoc && !wlc_scb_scan(ap, SCB_LONGSLOT, SCB_LONGSLOT))
				wlc->longslot_assoc = FALSE;

			/* update our long preamble flag based on associated STAs */
			if (wlc->longpre_assoc && !wlc_scb_scan(ap, SCB_SHORTPREAMBLE, 0))
				wlc->longpre_assoc = FALSE;
		}
	...
	/* part 3 */
		...
		wlc->nonabcap_assoc = wlc_nonabcap_assoc_upd(ap);
		...
		if (AP_ONLY(wlc->pub) && wlc->afterburner_override == AUTO) {
			...
			ab = (wlc->ab_wds_timeout > AB_WDS_TIMEOUT_MIN) ? FALSE : TRUE;
		...
		if (AP_ACTIVE(wlc) && wlc_update_brcm_ie(wlc)) {
			WL_APSTA_BCN(("wl%d: wlc_watchdog() calls wlc_update_beacon()\n",
				pub->unit));
			wlc_update_beacon(wlc);
			wlc_update_probe_resp(wlc, TRUE);
		}
		 ...
		 */
		if (COEX_ACTIVE(wlc) && CHSPEC_IS20(wlc->home_chanspec)) {
			uint32 delay;
			/* if any 40 intolerant still associated, don't upgrade */
			if (wlc->ht40intolerant_assoc)
				wlc_ht_coex_update_fid_time(wlc);
			else {
				delay = wlc->obss->ap_params.bss_widthscan_interval *
					wlc->obss->ap_params.chanwidth_transition_dly;
				if ((wlc->pub->now - wlc->obss->fid_time) > delay) {
					...
					wlc_ht_coex_switch_bw(wlc, FALSE, COEX_UPGRADE_TIMER);
				}
			}
		}
#endif /* WL11N */
...
#ifdef RXCHAIN_PWRSAVE
	if (appvt->rxchain_pwrsave.power_save_check)
		wlc_pwrsave_mode_check(ap, PWRSAVE_RXCHAIN);
	if (appvt->rxchain_pwrsave.in_power_save)
		appvt->rxchain_pwrsave.in_power_save_secs++;
#endif
...
wlc_ap_mbss16_tbtt(wlc_info_t *wlc, uint32 macintstatus)
{
...
#ifndef WLC_HIGH_ONLY
	{
		...
		delay = (R_REG(wlc->osh, &wlc->regs->tsf_timerlow) - wlc->last_tbtt_us);
		if (delay > wlc->ap->pre_tbtt_max_lat_us) {
			WLCNTINCR(wlc->pub->_cnt->late_tbtt_dpc);
...
#ifdef WL11N
void
wlc_ht_coex_switch_bw(wlc_info_t *wlc, bool downgrade, uint8 reason_code)
{
	...
	WL_COEX(("wl%d: %s: switching chanspec 0x%x to new chanspec 0x%x rc: %d\n",
		wlc->pub->unit, __FUNCTION__, chanspec, newchanspec, reason_code));

	wlc->csa.mode = DOT11_CSA_MODE_ADVISORY;
	...
	wlc->csa.chspec = newchanspec;
	wlc->csa.reg = wlc_get_regclass(wlc->cmi, wlc->csa.chspec);
	if (wlc->csa.count == 0) {
		wlc_do_chanswitch(wlc, wlc->csa.chspec);
	} else {
		/* dpc handles NEED_TO_UPDATE_BCN, NEED_TO_SWITCH_CHANNEL */
		wlc->spect_state |= (NEED_TO_UPDATE_BCN | NEED_TO_SWITCH_CHANNEL);
...
wlc_ht_ap_coex_tebc_proc(wlc_info_t *wlc)
{
	...
	if (CHSPEC_IS40(wlc->home_chanspec))
		wlc_ht_coex_switch_bw(wlc, TRUE, COEX_DGRADE_TEBC);
}
...
wlc_ap_ioctl(wlc_ap_info_t *ap, int cmd, void *arg, int len, struct wlc_if *wlcif)
{
	...
	case WLC_START_CHANNEL_SEL:
		...
		if (!pub->up || !AP_ONLY(pub) || pub->associated ||
	...
	void *params, uint p_len, void *arg, int len, int val_size, struct wlc_if *wlcif)
{
	...
	if ((err = wlc_iovar_check(pub, vi, arg, len, IOV_ISSET(actionid))) != 0)
		return err;
	...
	case IOV_SVAL(IOV_SSID): {
		...
		WL_OID(("wl%d: wlc_iovar_op: setting SSID of bsscfg %d to \"%s\"\n",
	...
	case IOV_SVAL(IOV_MBSS): {
		...
		if (rev16 && (wlc_ucodembss_hwcap(wlc) == FALSE)) {
			err = BCME_NORESOURCE;
			break;
		}
		...
		if (curstat) {
			...
			wlc->mbss_ucidx_mask = (uint8)((rev4 ? pub->tunables->maxucodebss4 :
			                        (int)wlc->max_ap_bss) - 1);
		}
#else
		err = BCME_UNSUPPORTED;
#endif	/* MBSS */
	...
	case IOV_SVAL(IOV_BSS): {
		...
		if (int_val2 > 0) {
			...
			if (BSSCFG_AP(bsscfg))
				...
				wlc_join(wlc, bsscfg, bsscfg->SSID, bsscfg->SSID_len, NULL, 0);
#endif
			if (err)
				break;
#ifdef RADAR
	...
	case IOV_SVAL(IOV_APSTA):
		...
		/* disable DPT if it was enabled; APSTA has higher priority */
		if (DPT_ENAB(wlc->pub) && bool_val)
			wlc_dpt_set(wlc->dpt, FALSE);
	...
	case IOV_SVAL(IOV_RXCHAIN_PWRSAVE_ENABLE):
		ap->rxchain_pwrsave_enable = appvt->rxchain_pwrsave.power_save_check = int_val;
		if (!int_val)
			wlc_reset_rxchain_pwrsave_mode(ap);
		break;
	case IOV_GVAL(IOV_RXCHAIN_PWRSAVE_QUIET_TIME):
	        *ret_int_ptr = appvt->rxchain_pwrsave.quiet_time;
		break;

	case IOV_SVAL(IOV_RXCHAIN_PWRSAVE_QUIET_TIME):
		appvt->rxchain_pwrsave.quiet_time = int_val;
		break;
	case IOV_GVAL(IOV_RXCHAIN_PWRSAVE_PPS):
	        *ret_int_ptr = appvt->rxchain_pwrsave.pps_threshold;
		break;

	case IOV_SVAL(IOV_RXCHAIN_PWRSAVE_PPS):
		appvt->rxchain_pwrsave.pps_threshold = int_val;
		break;
	case IOV_GVAL(IOV_RXCHAIN_PWRSAVE):
	        *ret_int_ptr = appvt->rxchain_pwrsave.in_power_save;
		break;
#endif /* RXCHAIN_PWRSAVE */
#ifdef RADIO_PWRSAVE
                      ...
                      uint8 mode, uint8 count, uint8 reg_class)
{
	...
	wlc->csa.mode = (mode != 0) ? 1 : 0;
	wlc->csa.count = count;
	wlc->csa.chspec = chanspec;

	if (reg_class != 0)
		wlc->csa.reg = reg_class;
	else
		wlc->csa.reg = wlc_get_regclass(wlc->cmi, wlc->csa.chspec);

	wlc->spect_state |= NEED_TO_SWITCH_CHANNEL;
...
wlc_radio_pwrsave_off_time_start(wlc_ap_info_t *ap)
{
	...
	off_time -= (wlc->ap->pre_tbtt_us + PHY_DISABLE_MAX_LATENCY_us);

	/* In power save state */
	appvt->radio_pwrsave.pwrsave_state = 3;
	appvt->radio_pwrsave.tbtt_skip = (off_time / DOT11_TU_TO_US / bcn_period);
	off_time /= 1000;

	WL_INFORM(("wl%d: add timer to enable phy after %d msec state %d, skip %d\n",
...
wlc_radio_pwrsave_update_quiet_ie(wlc_ap_info_t *ap, uint8 count)
{
	...
	duration -= (wlc->ap->pre_tbtt_us + PHY_ENABLE_MAX_LATENCY_us +
	             PHY_DISABLE_MAX_LATENCY_us);
	duration /= DOT11_TU_TO_US;

	/* Setup the quiet command */
	wlc->quiet_cmd.count = count;
	wlc->quiet_cmd.duration = (uint16)duration;
	wlc->quiet_cmd.offset = appvt->radio_pwrsave.on_time - wlc->cfg->current_bss->beacon_period;
	WL_INFORM(("wl%d: quiet cmd: count %d, dur %d, offset %d\n",
	            wlc->pub->unit, wlc->quiet_cmd.count,
	            wlc->quiet_cmd.duration, wlc->quiet_cmd.offset));
