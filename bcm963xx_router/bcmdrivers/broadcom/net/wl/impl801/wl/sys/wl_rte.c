 ...
 *
 * $Id: wl_rte.c,v 1.1 2010/08/05 21:59:01 ywu Exp $
 */
...
#ifdef WLLMAC
#define WLC_ATTACH	wlc_attach_lmac
#define WLC_DEV_UP(wlc)	wlc_up_lmac(wlc->lmac_info)
#else /* WLLMAC */
#define WLC_ATTACH	wlc_attach
#define WLC_DEV_UP	wlc_up
#endif /* WLLMAC */
...
/* Driver entry points */
static void *wl_probe(hndrte_dev_t *dev, void *regs, uint bus,
	uint16 devid, uint coreid, uint unit);
static INLINE void wl_free(wl_info_t *wl, osl_t *osh);
           ...
           void *p, uint plen, void *arg, int alen, int vsize, struct wlc_if *wlcif)
{
	...
	if ((err = wlc_iovar_check(wl->pub, vi, arg, alen, IOV_ISSET(actionid))) != 0)
		return err;
...
{
	if (wlif == NULL)
		return wl->dev->dev_fullname;
	else
		return wlif->dev->dev_fullname;
}
...
wl_alloc_if(wl_info_t *wl, int iftype, uint subunit, struct wlc_if *wlcif)
{
	...
	hndrte_dev_t *bus = wl->dev->dev_chained;
	uint len;
	int ifindex;
	wl_if_t **priv;
	...
	dev->dev_funcs = &wl_funcs;
	dev->dev_softc = wl;
	sprintf(dev->dev_fullname, "wl%d.%d", wl->pub->unit, subunit);

	*priv = wlif;

	/* use the return value as the i/f no. in the event to the host */
#ifdef DONGLEBUILD
	if ((ifindex = bus_binddev(bus, dev)) < 1) {
...
static
void wl_timer_main(hndrte_timer_t *t)
{
                      ...
                      uint coreid, uint unit)
{
	...
	sprintf(dev->dev_fullname, "wl%d", unit);

	/* print hello string */
	printf("wl%d: Broadcom BCM%04x 802.11 Wireless Controller %s\n", unit,
		wlc->hw->sih->chip, EPI_VERSION_STR);
	...
	/* register module */
	if (wlc_module_register(wlc->pub, wl_iovars, "wl", wl, wl_doiovar, NULL, NULL)) {
...
{
	wl_info_t *wl = (wl_info_t *) dev->dev_softc;
	bool dpc;
...
static void *
wl_pkt_header_push(wl_info_t *wl, void *p, uint8 *wl_hdr_sz)
{
	wl_header_t *h;
	osl_t *osh = wl->pub->osh;
...
wl_sendup(wl_info_t *wl, struct wl_if *wlif, void *p, int numpkt)
{
	...
	type = *(uint16 *)(buf + ETHER_TYPE_OFFSET);
	...
	if (chained) {
		...
		if ((p = wl_pkt_header_push(wl, p, &wl_hdr_len)) == NULL) {
			return;
		}

		PKTSETDATAOFFSET(p, wl_hdr_len);
		lb = PKTTONATIVE(wl->pub->osh, p);
		if (chained->dev_funcs->xmit(dev, chained, lb) != 0) {
...
{
	hndrte_dev_t *chained = wl->dev->dev_chained;

	/* sta mode must be chained */
	if (chained && chained->dev_funcs->txflowcontrol)
		chained->dev_funcs->txflowcontrol(chained, state, prio);
	else
		ASSERT(AP_ENAB(wl->pub));
}
...
/* Getting too many */
	...
	case WLC_E_MIC_ERROR: {
		...
		wlc_sup_mic_error(cfg,
		                  (e->event.flags&WLC_EVENT_MSG_GROUP) == WLC_EVENT_MSG_GROUP);
		break;
	}
#endif
	}
...
wl_ioctl(hndrte_dev_t *dev, uint32 cmd, void *buf, int len, int *used, int *needed, int set)
{
	...
	case RTEGALLMULTI:
		*ret_int_ptr = wl->pub->allmulti;
		break;
	case RTESALLMULTI:
		wl->pub->allmulti = *((uint32 *) buf);
		break;
#endif /* WLC_HIGH */
	case RTEGPROMISC:
		cmd = WLC_GET_PROMISC;
	...
	case RTESMULTILIST: {
		...
		wl->pub->nmulticast = len / ETHER_ADDR_LEN;
		for (i = 0; i < wl->pub->nmulticast; i++)
			wl->pub->multicast[i] = ((struct ether_addr *)buf)[i];
		break;
	}
#endif /* WLC_HIGH */
	case RTEGUP:
...
static int
wl_close(hndrte_dev_t *dev)
{
	wl_info_t *wl;
	wl = (wl_info_t *) dev->dev_softc;
