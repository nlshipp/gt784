 ...
 *
 * $Id: wlc_phy_shim.c,v 1.1 2010/08/05 21:59:02 ywu Exp $
 */
...
#ifdef DSLCPE_PERCAL_DELAY
struct wl_delayed_task *
wlapi_init_delayed_task(wlc_phy_shim_info_t *physhim, void (*fn)(void* arg), void *arg)
{
	return wl_init_delayed_task(physhim->wl, fn, arg);
}

void
wlapi_free_delayed_task(wlc_phy_shim_info_t *physhim, struct wl_delayed_task *task)
{
	wl_free_delayed_task(physhim->wl, (struct wl_delayed_task *) task);
}

void
wlapi_schedule_delayed_task(struct wl_delayed_task *task, uint delay)
{
	wl_schedule_delayed_task(task, delay);
}

bool
wlapi_cancel_delayed_task(struct wl_delayed_task *task)
{
	return wl_cancel_delayed_task(task);
}
#endif /* DSLCPE_PERCAL_DELAY */
...
{
	wlc_noise_cb(physhim->wlc, channel, noise_dbm);
}
...
void
wlapi_bmac_pktengtx(wlc_phy_shim_info_t *physhim, wl_pkteng_t *pkteng, uint8 rate,
	struct ether_addr *sa, uint32 wait_delay)
{
	wlc_bmac_pktengtx(physhim->wlc_hw, pkteng, rate, sa, wait_delay);
}
