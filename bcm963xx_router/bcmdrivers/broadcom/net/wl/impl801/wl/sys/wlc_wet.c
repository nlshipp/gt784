 ...
 *
 * $Id: wlc_wet.c,v 1.1 2010/08/05 21:59:02 ywu Exp $
 */
...
static int wet_eth_proc(wlc_wet_info_t *weth,
	uint8 *frame, int length, int send);
static int wet_vtag_proc(wlc_wet_info_t *weth,
	uint8 * eh, uint8 *vtag, int length, int send);
static int wet_ip_proc(wlc_wet_info_t *weth,
	uint8 * eh, uint8 *iph, int length, int send);
static int wet_arp_proc(wlc_wet_info_t *weth,
 ...
 */
typedef int (*prot_proc_t)(wlc_wet_info_t *weth, uint8 *eh,
 ...
 */
wlc_wet_info_t *
BCMATTACHFN(wlc_wet_attach)(wlc_pub_t *pub)
{
	wlc_wet_info_t *weth;
	int i;

	/* allocate wet private info struct */
	weth = MALLOC(pub->osh, sizeof(wlc_wet_info_t));
	if (!weth) {
		WL_ERROR(("wl%d: wlc_wet_attach: MALLOC failed, malloced %d bytes\n",
			pub->unit, MALLOCED(pub->osh)));
		return NULL;
	}

	/* init wet private info struct */
	bzero(weth, sizeof(wlc_wet_info_t));
	...
	/* register module */
	wlc_module_register(pub, wet_iovars, "wet", weth, wet_doiovar, NULL, NULL);
#ifdef BCMDBG
	...
	void *p, uint plen, void *a, int alen, int vsize, struct wlc_if *wlcif)
{
	...
	if ((err = wlc_iovar_check(weth->pub, vi, a, alen, IOV_ISSET(actionid))) != 0)
		return err;
...
*/
static int
wet_eth_proc(wlc_wet_info_t *weth, uint8 *frame, int length, int send)
{
...
*/
static int
wet_vtag_proc(wlc_wet_info_t *weth,
	uint8 * eh, uint8 *vtag, int length, int send)
{
	uint16 type;
	uint8 *pt;
	...
	for (phdlr = &prot_hash[WET_PROT_HASH(pt)];
		...
		if (phdlr->type != type || !phdlr->prot_proc)
			continue;
		return (phdlr->prot_proc)(weth, eh,
...
*/
static int
wet_ip_proc(wlc_wet_info_t *weth,
	uint8 *eh, uint8 *iph, int length, int send)
{
	uint8 type;
	int ihl;
...
*/
static int
wet_arp_proc(wlc_wet_info_t *weth,
	uint8 *eh, uint8 *arph, int length, int send)
{
	wet_sta_t *sta;
	uint8 *iaddr;
	...
	wet_sta_t **saddr)
{
	...
	if (!wet_sta_find_mac(weth, eaddr, &sta)) {
		if ((i = WET_STA_HASH_IP(sta->ip)) != WET_STA_HASH_IP(iaddr)) {
			wet_sta_t *sta2, **next;
...
wet_sta_remove_mac_entry(wlc_wet_info_t *weth, struct ether_addr *eaddr)
{
	...
	/* fix the list */
	if (prev == sta)
		weth->stahash_mac[i] = NULL; /* removing only entry in this bucket */
	else
...
wlc_wet_send_proc(wlc_wet_info_t *weth, void *sdu, void **new)
{
	...
	if (ETHER_ISMULTI(frame)) {
		...
		wlc_pkttag_info_move(weth->pub, sdu, pkt);
	...
	*new = pkt;
	...
	/* process frame */
	return wet_eth_proc(weth, frame, length, 1) < 0 ? -1 : 0;
}
...
{
	/* process frame */
	return wet_eth_proc(weth, PKTDATA(WLCOSH(weth), sdu),
