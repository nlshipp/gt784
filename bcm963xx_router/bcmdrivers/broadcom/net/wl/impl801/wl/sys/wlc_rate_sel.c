 ...
 *
 * $Id: wlc_rate_sel.c,v 1.1 2010/08/05 21:59:02 ywu Exp $
 */
...
static int mcs_sp_params[RATES_NUM_MCS][SPATIAL_PARAMS_NUM] = {
	{-1, 2, 16, 6}, /* mcs 32 and legacy rates */
	{ 8, 2, 16, 6}, /* mcs 0 */
	{ 8, 2, 16, 6},
	{ 9, 2, 17, 6},
	{ 9, 2, 17, 6},
 ...
 */
static int mcs_extsp_params[RATES_NUM_MCS][SPATIAL_PARAMS_NUM] = {
	{-1, 2, 16, 6}, /* mcs 32 */
	{ 8, 2, 16, 6}, /* mcs 0 */
	{ 8, 2, 16, 6},
	{ 9, 2, 17, 6},
	...
	void *p, uint plen, void *a, int alen, int val_size, struct wlc_if *wlcif)
{
	...
	if ((err = wlc_iovar_check(rsi->pub, vi, a, alen, IOV_ISSET(actionid))) != 0)
		return err;
...
static uint8
wlc_ratesel_find_startidx(scb_ratesel_t *state, uint8 start_rate, bool is40bw)
{
	uint8 rate_entry = 0;
	ratespec_t rspec;
	uint32 phy_rate;
...
wlc_rateset_filter_mcsset(uint8 src[], uint8 dst[], bool spatial_probing, uint8 nstreams)
{
	...
	/* always support single stream MCS 0-7 */
	dst[0] = src[0];
	...
	if (nstreams >= 2) {
		...
		/* No spatial probe between 2- and 3- stream rates */
		if (nstreams == 3)
	...
	*next_rateid = state->rateid;
	...
	if (incdec == +1) {
		...
		*next_rateid = state->rateid - 1;
		if (IS_MCS(rspec)) {
			uint8 mcs = RSPEC_RATE_MASK & rspec;
			if (SP_MODE(state) != SP_NONE && mcs == 8) {
				/* jump mcs 8 to 0 */
...
wlc_ratesel_filter(scb_ratesel_t *state, wlc_rateset_t *rateset, uint8 start_rate)
{
	...
	if (MPDU_MCSENAB(state)) {
		...
		if (isset(filter_mcs_bitarray, 32) && state->is40bw) {
			state->select_rates[state->active_rates_num++] =
	...
	for (i = 0; i < state->active_rates_num; i++) {
		uint8 fbr_id = LIMSUB(i, 0, 2);
#ifdef WL11N
		if (IS_MCS(known_rspec[state->select_rates[i]])) {
			switch (RSPEC_RATE_MASK & known_rspec[state->select_rates[i]]) {
			case 13: fbr_id = LIMSUB(i, 0, 9); break; /* mcs 4 */
			case 12: /* mcs 4 */
			case 11: /* mcs 3 */
			case 10: /* mcs 2 */
			case  9: /* mcs 1 */
			case  8: fbr_id = LIMSUB(i, 0, 8); break; /* mcs 0 */
			case  7: fbr_id = LIMSUB(i, 0, 3); break; /* mcs 4 */
			}
		}
#endif /* WL11N */
	...
	state->rateid = wlc_ratesel_find_startidx(state, start_rate, is40bw);
	state->gotpkts = 0;
	...
	for (i = 0; i < state->active_rates_num; i++) {
		RL_INFO(("%s: select_rates[%d] = %s%d rspec 0x%x prate %d fbrates[%d] = %s%d\n",
	...
	uint8 antselid_init, int32 ac)
{
	...
	if (BSS_N_ENAB(rsi->wlc, scb->bsscfg)) {
		...
		else if ((rateset->mcs[2] & 0xff) == 0xff)
	...
	if (rateset->mcs[0] || rateset->mcs[1]) {
...
#ifdef WL11N
	WL_RATE(("\tmcs_streams %d spmode %d\n", state->mcs_streams, state->spmode));
#endif
...
wlc_ratesel_load_params(scb_ratesel_t *state)
{
	...
	if (SP_MODE(state) == SP_NORMAL) {
		int mcs_sp = -1, mcs_sp_sel = -1;
		if (mcs != -1)
			mcs_sp = mcs_sp_params[tblidx][SPATIAL_MCS_COL];
		if (mcs_sp > 0) {
			mcs_sp_sel = wlc_ratesel_mcs2id(state, (uint8) mcs_sp);
		}

		state->mcs_sp_flgs = 0; /* clear all the flags */
		if (mcs_sp_sel > 0) {
			state->mcs_sp_id = (uint8) mcs_sp_sel;
		...
		if (mcs != -1)
			mcs_extsp_xpr = mcs_extsp_params[tblidx][SPATIAL_MCS_COL];
		if (mcs_extsp_xpr > 0)
		...
		state->extsp_K = (uint8) mcs_extsp_params[tblidx][SPATIAL_K_COL];
		state->extsp_Pmod = (uint16) mcs_extsp_params[tblidx][SPATIAL_PMOD_COL];
		state->extsp_Poff = (uint16) mcs_extsp_params[tblidx][SPATIAL_POFF_COL];
	}
...
make_decision:
	...
	if (decision) {
		...
		if (state->rsi->use_rssi) {
			if ((state->is40bw && prate_cur > 22 && prate_dn <= 22) ||
			     (!state->is40bw && prate_cur > 11 && prate_dn <= 11))
	...
	if (decision) {
		...
		if (IS_MCS(prv_rspec) && ((RSPEC_RATE_MASK & prv_rspec) == 12)) {
			if (SP_MODE(state) == SP_NORMAL) {
				...
				wlc_ratesel_clear_spstat(state); /* clear on way to mcs 13 */
			}
			else if (SP_MODE(state) == SP_EXT &&
...
#if defined(AP)
		if (state->rsi->use_rssi) {
			if ((state->is40bw && prate_cur <= 22 && prate_up > 22) ||
			     (!state->is40bw && prate_cur <= 11 && prate_up > 11))
...
wlc_ratesel_pick_rate(scb_ratesel_t *state, bool is_probe)
{
...
#endif /* WL11N */
	{
		...
		/* rate up */
		else if (wlc_ratesel_goup(state)) {
			change_rate = TRUE;
		}
