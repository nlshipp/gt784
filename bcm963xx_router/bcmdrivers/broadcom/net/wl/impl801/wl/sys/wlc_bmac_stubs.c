 ...
 *
 * $Id: wlc_bmac_stubs.c,v 1.1 2010/08/05 21:59:01 ywu Exp $
 */
...
wlc_rpc_high_dispatch(wlc_rpc_ctx_t *rpc_ctx, struct rpc_buf* buf)
{
	...
	case WLRPC_WLC_DOTXSTATUS_ID: {
		...
		err = bcm_xdr_unpack_uint32(&b, &rpc_txstatus.PAD_framelen);
		ASSERT(!err);

		err = bcm_xdr_unpack_uint32(&b, &rpc_txstatus.status_frameid);
		ASSERT(!err);

		err = bcm_xdr_unpack_uint32(&b, &rpc_txstatus.sequence_lasttxtime);
		ASSERT(!err);
	...
	case WLRPC_WLC_NOISE_CB_ID: {
		...
		wlc_noise_cb(rpc_ctx->wlc, channel, noise_dbm);
		break;
	}
	...
	case WLRPC_WLC_BMAC_PS_SWITCH_ID: {
		...
		wlc_apps_process_ps_switch(wlc, ea_p, ps_flags);
...
void
wlc_bmac_init(wlc_hw_info_t *wlc_hw, chanspec_t chanspec, bool mute)
{
	bcm_xdr_buf_t b;
	rpc_buf_t *rpc_buf;
	int err;
	...
	rpc_buf = wlc_rpc_buf_alloc(rpc, &b, 2 * sizeof(uint32), WLRPC_WLC_BMAC_INIT_ID);
	ASSERT(rpc_buf != NULL);
...
wlc_bmac_copyfrom_vars(wlc_hw_info_t *wlc_hw, char **buf_ptr, uint *len)
{
	...
	if (!(local_buf = MALLOC(wlc_hw->osh, retlen))) {
		*len = 0;
		*buf_ptr = NULL;
	} else {
...
void
wlc_enable_mac(struct wlc_info *wlc)
{
	bcm_xdr_buf_t b;
	rpc_buf_t *rpc_buf;
	int err;
...
wlc_bmac_set_rcmta_type(wlc_hw_info_t *wlc_hw, int idx, uint16 mask, uint16 val)
{
	...
	rpc_buf = wlc_rpc_buf_alloc(rpc, &b, 3 * sizeof(uint32),
...
void
wlc_suspend_mac_and_wait(struct wlc_info *wlc)
{
	bcm_xdr_buf_t b;
	rpc_buf_t *rpc_buf;
	int err;
...
static int
wlc_rpc_pack_txpwr_limits(bcm_xdr_buf_t *b, struct txpwr_limits *txpwr)
{
	int err;

	wlc_rpc_txpwr_limits(b, txpwr, pack, err);

	return bcm_xdr_pack_uint32(b, txpwr->mcs32);
}
	...
	struct txpwr_limits *txpwr)
{
	...
	rpc_buf = wlc_rpc_buf_alloc(rpc, &b, sizeof(uint32) * 2 + (TXPOWER_XDR_SZ),
...
wlc_phy_txpower_limit_set(wlc_phy_t *ppi, struct txpwr_limits *txpwr, chanspec_t chanspec)
{
	...
	rpc_buf = wlc_rpc_buf_alloc(rpc, &b, sizeof(uint32) + TXPOWER_XDR_SZ,
...
wlc_bmac_revinfo_get(wlc_hw_info_t *wlc_hw, wlc_bmac_revinfo_t *revinfo)
{
	...
	err = bcm_xdr_unpack_int32(&retb, &revinfo->btc_wire);
	if (err)
		goto fail;
...
wlc_bmac_btc_flags_set(wlc_hw_info_t *wlc_hw, int btc_flags)
{
	...
	rpc_buf = wlc_rpc_buf_alloc(rpc, &b, sizeof(int32), WLRPC_WLC_BMAC_BTC_FLAGS_SET_ID);
	ASSERT(rpc_buf != NULL);

	err = bcm_xdr_pack_uint32(&b, btc_flags);
	ASSERT(!err);
...
wlc_phy_txpower_ipa_ison(wlc_phy_t *pih)
{
	...
	rpc_buf = wlc_rpc_buf_alloc(rpc, &b, sizeof(uint32), WLRPC_WLC_PHY_TXPOWER_IPA_ISON);
	ASSERT(rpc_buf != NULL);
