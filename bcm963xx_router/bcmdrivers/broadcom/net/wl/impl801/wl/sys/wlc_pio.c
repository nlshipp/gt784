 ...
 *
 * $Id: wlc_pio.c,v 1.1 2010/08/05 21:59:02 ywu Exp $
 */
#ifndef WLPIO
...
static void *wlc_pioregs_offset(wlc_info_t *wlc, int txrx, int fifonum);
...
_wlc_pio_rx_war38778(pio_info_t *pt)
{
	...
	if (pt->wlc->PMenabled) {
		bool wake = pt->wlc->wake;
		pt->wlc->wake = TRUE;
		wlc_set_ps_ctrl(pt->wlc);
...
wlc_pioregs_offset(wlc_info_t *wlc, int txrx, int fifonum)
{
	...
	if (D11REV_LT(wlc->pub->corerev, 11)) {
		...
		if (D11REV_LT(wlc->pub->corerev, 8)) {
			if (txrx == PIO_TX)
				regs = (void *)&pioregs->b2.tx;
			else
				regs = (void *)&pioregs->b2.rx;
		}
		else {
			if (txrx == PIO_TX)
				regs = (void *)&pioregs->b4.tx;
			else
				regs = (void *)&pioregs->b4.rx;
		}
	}
	else {
		fifo64_t *fifo;
		...
		if (txrx == PIO_TX)
			regs = (void *)&fifo->piotx;
		else
			regs = (void *)&fifo->piorx;
