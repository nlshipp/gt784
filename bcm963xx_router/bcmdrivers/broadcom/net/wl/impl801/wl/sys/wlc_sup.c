 ...
 *
 * $Id: wlc_sup.c,v 1.1 2010/08/05 21:59:02 ywu Exp $
 */
...
wlc_wpa_sup_prepeapol(supplicant_t *sup, uint16 flags, wpa_msg_t msg)
{
	...
	case PMSG2:		/* pair-wise msg 2 */
		if (wpa->sup_wpaie == NULL)
			break;
		len += wpa->sup_wpaie_len;
		if ((p = wlc_eapol_pktget(sup->wlc, sup->bsscfg, &PEER_EA(sup), len)) == NULL)
			break;
		eapol_hdr = (eapol_header_t *) PKTDATA(osh, p);
		eapol_hdr->length =
			hton16(EAPOL_WPA_KEY_LEN+wpa->sup_wpaie_len);
		...
		wpa_key->data_len = hton16(wpa->sup_wpaie_len);
...
#ifdef BCMINTSUP
		wlc_mac_event(sup->wlc, WLC_E_PSK_SUP, NULL, status, reason, 0, 0, 0);
#else
		...
		if (!(key_info & WPA_KEY_MIC)) {
			...
			if (IS_WPA2_AUTH(wpa->WPA_auth)) {
				...
				if (!wpa2ie || wpa->auth_wpaie_len !=
					((uint16)TLV_HDR_LEN + (uint16)wpa2ie->len) ||
					(bcmp((char *)wpa->auth_wpaie, wpa2ie,
					wpa->auth_wpaie_len))) {
...
wlc_sup_wpa_passhash_timer(void *arg)
{
	...
	if (do_passhash(&info->passhash_states, 256) == 0) {
		...
		{
			wlc_join_attempt(sup->bsscfg);
			wlc_led_event(sup->wlc->ledh);
		}
		return;
	}
...
wlc_sup_hw_wowl_init(supplicant_t *sup)
{
	...
	if (sup->wpa->ucipher != CRYPTO_ALGO_AES_CCM)
		...
		wlc_write_shm(wlc, M_AESTABLES_PTR, T_TX_FIFO_TXRAM_BASE);

		wlc_write_template_ram(wlc, T_TX_FIFO_TXRAM_BASE,
