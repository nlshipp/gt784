 ...
 *
 * $Id: wlc_cac.c,v 1.1 2010/08/05 21:59:01 ywu Exp $
 */
...
BCMATTACHFN(wlc_cac_attach)(wlc_info_t *wlc)
{
	...
	if (!(cac = (wlc_cac_t *)MALLOC(wlc->osh, sizeof(wlc_cac_t)))) {
		WL_ERROR(("wl%d: wlc_cac_attach: out of mem, malloced %d bytes\n",
	...
	wlc_module_register(wlc->pub, cac_iovars, "cac", cac,
		wlc_cac_iovar, wlc_cac_watchdog, wlc_cac_down);
#ifdef BCMDBG
	...
	void *p, uint plen, void *a, int alen, int vsize, struct wlc_if *wlcif)
{
	...
	if ((err = wlc_iovar_check(cac->wlc->pub, vi, a, alen, IOV_ISSET(actionid))) != 0)
		return err;
		...
		case IOV_SVAL(IOV_CAC_ADDTS):
			{
				...
				bcopy((uint8*)a, (uint8*)&tspec_arg, sizeof(tspec_arg));
				err = wlc_cac_sta_addts(cac, &tspec_arg);
		...
		case IOV_SVAL(IOV_CAC_DELTS):
			{
				...
				if (!(scb = wlc_scbfind(cac->wlc, &cac->curr_bssid)))
					return BCME_NOTFOUND;

				bcopy((uint8*)a, (uint8*)&tspec_arg, sizeof(tspec_arg));
				err = wlc_cac_sta_delts(cac, &tspec_arg, scb, cac->tsentryq);
			}
			break;
	...
	uint8 dialog_token, int reason_code, void **pkt, struct scb *scb)
{
	...
	*pkt = NULL;
	...
	if (wlc_sendmgmt(wlc, *pkt, NULL, 0, NULL, 0))
		return 0;
...
wlc_cac_addts_req_send(wlc_cac_t *cac, tspec_t *ts, uint8 dialog_token)
{
	...
	if (!(scb = wlc_scbfind(cac->wlc, &cac->curr_bssid)) &&
		!(scb = wlc_scbfind(cac->wlc, &cac->wlc->cfg->BSSID))) {
		return BCME_NOTFOUND;
	}
#else
	if (!(scb = wlc_scbfind(cac->wlc, &cac->wlc->cfg->BSSID))) {
		return BCME_NOTFOUND;
	}
#endif /* MBSS */
...
wlc_cac_sta_addts(wlc_cac_t *cac, tspec_arg_t *tspec_arg)
{
	...
	if (WLC_CAC_GET_PSB(tspec_arg->tsinfo) && !cac->wlc->wme_apsd) {
	...
	if ((ts = wlc_cac_tsentry_find(cac->tsentryq, tid))) {
		...
		if (!(ts = (tsentry_t *)MALLOC(cac->wlc->osh, sizeof(tsentry_t)))) {
			WL_ERROR(("wl%d: wlc_cac_sta_addts: out of mem, malloced %d bytes\n",
				cac->wlc->pub->unit, MALLOCED(cac->wlc->osh)));
			return BCME_NORESOURCE;
		}

		tspec = &ts->tspec;
		bzero((char *)ts, sizeof(tsentry_t));
		bcopy(WME_OUI, tspec->oui, DOT11_OUI_LEN);
		tspec->type = WME_TYPE;
	...
	tsentry_t *tsentryq)
{
	...
	if (CAC_ENAB(cac->wlc->pub)) {
		/* Reset the power save to defaut settings */
...
wlc_cac_addts_resp(wlc_cac_t *cac, uint8 *body, uint body_len, struct scb *scb)
{
	...
	if (wlc_cac_ie_process(cac, body, body_len, &tid, &user_prio) == 0) {
		...
		if (tid == WLC_CAC_GET_TID(ts->tspec.tsinfo)) {
			...
			/* update the power save behaviour */
			if (CAC_ENAB(cac->wlc->pub)) {
				int ac;
				if (WLC_CAC_GET_PSB(ts->tspec.tsinfo) && cac->wlc->wme_apsd) {
					ac = WME_PRIO2AC(WLC_CAC_GET_USER_PRIO(ts->tspec.tsinfo));
...
wlc_cac_addts_timeout(void *arg)
{
	...
	if (!(scb = wlc_scbfind(wlc, &wlc->cac->curr_bssid)))
		return;
...
wlc_cac_tspec_state_reset(wlc_cac_t *cac)
{
	WL_CAC(("wl%d: wlc_cac_roam_tspec: change all roaming state to pending"
		" state\n", cac->wlc->pub->unit));
	/* change all TSPEC from ROAMING to PEDING */
...
wlc_cac_watchdog(void *hdl)
{
	...
	if (!AP_ENAB(cac->wlc->pub)) {
		if ((scb = wlc_scbfind(cac->wlc, &cac->curr_bssid)) && SCB_WME(scb)) {
			scb_acinfo = SCB_ACINFO(cac, scb);
			...
			for (ac = 0; ac < AC_COUNT; ac++) {
				...
				if (AC_BITMAP_TST(cac->flags, ac)) {
					...
					wlc_cac_txflowcontrol(cac, OFF, ac);
#endif	/* WMMAC_FLOWCONTROL */
				}
	...
	if (AP_ENAB(cac->wlc->pub)) {
		FOREACHSCB(cac->wlc->scbstate, &scbiter, scb) {
			...
			/* reset used time for each AC */
			for (ac = 0; ac < AC_COUNT; ac++) {
				if (!AC_BITMAP_TST(cac->wlc->wme_admctl, ac))
					continue;
				cac_ac = &scb_acinfo->cac_ac[ac];
...
wlc_cac_update_used_time(wlc_cac_t *cac, int ac, int dur, struct scb *scb)
{
	...
	if (!AC_BITMAP_TST(wlc->wme_admctl, ac))
		return FALSE;
 ...
 */
void
wlc_frameaction_cac(uint action_id, wlc_cac_t *cac,
	struct dot11_management_header *hdr, uint8 *body, int body_len)
{
	struct scb *scb;

	if (!(scb = wlc_scbfind(cac->wlc, &hdr->sa))) {
...
wlc_cac_process_addts_req(wlc_cac_t *cac, uint8 *body, uint body_len, struct scb *scb)
{
	...
	if (status == WME_ADMISSION_ACCEPTED) {
		...
		if (!(new_ts_entry = wlc_cac_tsentry_find(scb_acinfo->tsentryq, tid))) {
			...
			if (!(new_ts_entry =
				(tsentry_t *)MALLOC(cac->wlc->osh, sizeof(tsentry_t)))) {
				WL_ERROR(("wl%d: %s : out of memory for new tspec %d bytes\n",
		...
		/* update the power save behaviour */
		if (WLC_CAC_GET_PSB(ts->tsinfo) && cac->wlc->wme_apsd) {
 ...
 */
	...
	/* update the mgmt notification header feilds */
	mgmt_hdr = (struct dot11_management_notification *)body;
	mgmt_hdr->category = DOT11_ACTION_NOTIFICATION;
	mgmt_hdr->action = action;
	...
	if (wlc_sendmgmt(wlc, *pkt, scb, 0, NULL, 0)) {
		return 0;
	}
...
void
wlc_cac_on_join_bss(wlc_cac_t *cac, struct ether_addr *bssid, bool roam)
{
...
/* Manage flowcontrol */
void
wlc_cac_txflowcontrol(wlc_cac_t *cac, bool state, uint8 ac)
{
	switch (ac) {
		case AC_BK:
			wl_txflowcontrol(cac->wlc->wl, state, 1);
			wl_txflowcontrol(cac->wlc->wl, state, 2);
			break;
		case AC_BE:
			wl_txflowcontrol(cac->wlc->wl, state, 0);
			wl_txflowcontrol(cac->wlc->wl, state, 3);
			break;
		case AC_VI:
			wl_txflowcontrol(cac->wlc->wl, state, 4);
			wl_txflowcontrol(cac->wlc->wl, state, 5);
			break;
		case AC_VO:
			wl_txflowcontrol(cac->wlc->wl, state, 6);
			wl_txflowcontrol(cac->wlc->wl, state, 7);
			break;
	}
}
