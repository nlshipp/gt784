 ...
 *
 * $Id: wlc_assoc.c,v 1.1 2010/08/05 21:59:01 ywu Exp $
 */
...
#define WLC_SCAN_RECREATE_TIME 50
...
static void wlc_assoc_scan_prep(wlc_bsscfg_t *cfg, const struct ether_addr *bssid,
                                const chanspec_t* chanspec_list, int channel_num);
static void wlc_assoc_scan_start(wlc_bsscfg_t *cfg, const struct ether_addr *bssid,
	const chanspec_t* chanspec_list, int channel_num);
#ifdef WLSCANCACHE
...
static void wlc_roam_complete(wlc_bsscfg_t *cfg, uint status, struct ether_addr *addr,
	uint bss_type);
static void wlc_roam_set_env(wlc_bsscfg_t *cfg, uint entries);
static void wlc_roam_release_flow_cntrl(wlc_info_t *wlc);
...
static void wlc_join_adopt_bss(wlc_bsscfg_t *cfg, char *bcn, int len);
static void wlc_join_start(wlc_bsscfg_t *cfg, wl_assoc_params_t *assoc_params);
...
/* check if AS_LAST_STATE changed since the last time this function was updated */
#if AS_LAST_STATE != 17
#error "You need to add an assoc state name string for the new assoc state"
#endif
...
wlc_assoc_req_process_next(wlc_info_t *wlc)
{
	...
	WL_ASSOC(("wl%d.%d: process %s request in assoc_req list for SSID '%s'\n",
	          wlc->pub->unit, WLC_BSSCFG_IDX(cfg), WLCASTYPEN(cfg->assoc->type), ssidbuf));

	switch (cfg->assoc->type) {
	case AS_ASSOCIATION:
		if (cfg->assoc->state != AS_IDLE)
			wlc_assoc_abort(cfg);
		wlc_join_start(cfg, wlc_bsscfg_assoc_params(cfg));
		break;
	default:
		ASSERT(0);
		break;
...
{
	wlc_join(wlc, wlc->cfg, SSID, len, NULL, 0);
}
         ...
         wl_assoc_params_t *assoc_params, int assoc_params_len)
{
	...
	bsscfg->assoc->ess_retries = 0;
	bsscfg->assoc->type = AS_ASSOCIATION;
	...
	wlc_join_start(bsscfg, assoc_params);
}

/* start a join process. SSID is saved in bsscfg. */
static void
wlc_join_start(wlc_bsscfg_t *cfg, wl_assoc_params_t *assoc_params)
{
	...
	if (as->state == AS_WAIT_DISASSOC) {
		...
		if (cfg->associated &&
			...
			wlc_assoc_scan_prep(cfg, bssid, chanspec_list, chanspec_num);
		}
	}
#if defined(BCMDBG) || defined(WLMSG_ASSOC) || defined(WLMSG_WSEC)
	if (ssidbuf != NULL)
...
wlc_setssid_disassoc_complete(wlc_info_t *wlc, uint txstatus, void *arg)
{
	...
	if (cfg == NULL) {
		WL_ERROR(("wl%d: wlc_setssid_disassoc_complete: unable to find bsscfg by ID %p\n",
		          wlc->pub->unit, arg));
		return;
	}
...
wlc_cook_join_targets(wlc_bsscfg_t *cfg, bool for_roam, int cur_rssi)
{
	...
	if (!join_pref_score) {
		WL_ERROR(("wl%d: wlc_cook_join_targets: MALLOC failure\n",
			WLCWLUNIT(wlc)));
		return;
	}
...
wlc_reassoc(wlc_bsscfg_t *cfg, wl_reassoc_params_t *reassoc_params)
{
	...
	if (!BSSCFG_STA(cfg) || !cfg->BSS) {
		WL_ERROR(("wl%d: REASSOC: bad argument STA %d BSS %d\n",
		          wlc->pub->unit, BSSCFG_STA(cfg), cfg->BSS));
		return BCME_BADARG;
	}

	if (wlc_mac_request_entry(wlc, cfg, WLC_ACTION_REASSOC) != BCME_OK) {
		WL_ASSOC(("wl%d: REASSOC: Blocked by other activity\n",
		          wlc->pub->unit));
		return BCME_BUSY;
	}

	WL_ASSOC(("wl%d : REASSOC: Attempting association to %s\n",
	...
	wlc_assoc_scan_prep(cfg, bssid, chanspec_list, channel_num);
	return BCME_OK;
}

static void
wlc_assoc_scan_prep(wlc_bsscfg_t *cfg,
                    const struct ether_addr *bssid, const chanspec_t *chanspec_list,
                    int channel_num) {

	wlc_assoc_scan_start(cfg, bssid, chanspec_list, channel_num);
}
...
static void
wlc_assoc_scan_start(wlc_bsscfg_t *cfg,
	const struct ether_addr *bssid, const chanspec_t *chanspec_list, int channel_num)
{
...
#if defined(WLEXTLOG) || (defined(BCMDBG) || defined(WLMSG_ASSOC))
	...
	if (wlc->stas_associated > 0 &&
		...
		for (idx = 0; idx < channel_num; idx ++) {
			if (chanspec_list[idx] == wlc->home_chanspec)
				break;
		}
	...
	if (ASSOC_CACHE_ASSIST_ENAB(wlc) && wlc->scan_results->count > 0) {
		...
		wlc_assoc_scan_complete(cfg, WLC_E_STATUS_SUCCESS, NULL);
		...
		err = wlc_scan(wlc->scan, bss_type, bssid, 1, &ssid, -1, -1,
		               (as->flags & AS_F_SPEEDY_RECREATE) ? WLC_SCAN_RECREATE_TIME : -1,
		               passive_time, -1, chanspec_list, channel_num, 0, TRUE,
		               wlc_assoc_scan_complete, cfg, 0, FALSE, scan_suppress_ssid,
		               FALSE, cfg);
...
wlc_speedy_recreate_fail(wlc_bsscfg_t *cfg)
{
	...
	wlc_assoc_scan_start(cfg, NULL, NULL, 0);
}
#endif /* WL_ASSOC_RECREATE */
                     ...
                     wlc_bss_list_t *bss_list, chanspec_t **target_list, uint *target_num)
{
	...
	*target_num = 0;
	...
	for (i = 0; i < bss_list->count; i++) {
		 ...
		 * since this is where we should scan for the BSS.
		 */
		if (CHSPEC_IS40(chanspec)) {
			chanspec = (chanspec_t)(CHSPEC_CTL_CHAN(chanspec) | WL_CHANSPEC_BW_20 |
			                        WL_CHANSPEC_CTL_SB_NONE | CHSPEC_BAND(chanspec));
		}
...
{
	wlc_info_t *wlc = cfg->wlc;
	const wl_assoc_params_t *assoc_params;
	const struct ether_addr *bssid = NULL;
	const chanspec_t *chanspec_list = NULL;
	int chanspec_num = 0;
	...
	wlc_assoc_scan_start(cfg, bssid, chanspec_list, chanspec_num);
}
...
static void
wlc_assoc_scan_complete(void *arg, int status, wlc_bsscfg_t *dummy)
{
	...
	wlc_roam_t *roam = cfg->roam;
	wlc_bss_info_t *target_bss = cfg->target_bss;
	bool for_roam = (as->type != AS_ASSOCIATION);
#if defined(BCMDBG) || defined(WLMSG_ASSOC)
	char *ssidbuf;
	const char *ssidstr;
	const char *msg_name = for_roam ? "ROAM" : "JOIN";
	const char *msg_pref = !(ETHER_ISMULTI(&wlc->scan->bssid)) ? "Directed " : "";
	ssidbuf = (char *) MALLOC(wlc->osh, SSID_FMT_BUF_LEN);
	if (ssidbuf) {
...
#ifdef	BCMSUP_PSK
		if (cfg->sup_type == SUP_WPAPSK) {
			switch (wlc_sup_set_ssid(cfg->sup, target_bss->SSID,
			                         target_bss->SSID_len)) {
			case 1:
...
#endif /* BCMSUP_PSK */
		/* disable beacon and stop responding to probe requests */
		if (AP_ACTIVE(wlc)) {
			WL_ASSOC(("wl%d: %s: disable AP(s)\n", wlc->pub->unit, __FUNCTION__));
...
wlc_roam_scan(wlc_bsscfg_t *cfg, uint reason)
{
	...
	wlc_assoc_scan_prep(cfg, NULL, NULL, 0);
...
wlc_join_wsec_filter(wlc_info_t *wlc, wlc_bsscfg_t *cfg, wlc_bss_info_t *bi)
{
	...
	if (cfg->tk_cm_bt) {
		...
		if (wlc_clear_tkip_cm_bt(currentTime,
			cfg->tk_cm_bt_tmstmp) == TRUE) {
			cfg->tk_cm_bt = 0;
		}
	}
...
{
	wl_assoc_params_t *assoc_params = wlc_bsscfg_assoc_params(bsscfg);
	int i;
...
wlc_join_attempt(wlc_bsscfg_t *cfg)
{
	...
	for (; wlc->join_targets_last > 0; wlc->join_targets_last--) {
		...
		if (CHSPEC_IS40_UNCOND(chanspec) &&
		    (!N_ENAB(wlc->pub) ||
		     (wlc_channel_locale_flags_in_band(wlc->cmi, target_band->bandunit) &
		      WLC_NO_40MHZ) || !target_band->mimo_cap_40 ||
		     (BAND_2G(target_band->bandtype) && WLC_INTOL40_DET(wlc)))) {
		...
		if ((bi->capability & DOT11_CAP_PRIVACY) && !WSEC_ENABLED(cfg->wsec)) {
			WL_ASSOC(("wl%d: JOIN: Skipping BSSID %s, encryption mandatory in BSS, but"
			          " encryption off for us.\n", WLCWLUNIT(wlc),
			          bcm_ether_ntoa(&bi->BSSID, eabuf)));
			wlc_bss_mac_event(wlc, cfg, WLC_E_PRUNE, &bi->BSSID, 0,
				WLC_E_PRUNE_ENCR_MISMATCH,
				CHSPEC_CHANNEL(bi->chanspec), 0, 0);
		...
		if (cfg->macmode && (WIN7_OS(wlc->pub) || !APSTA_ENAB(wlc->pub))) {
		 ...
		 */
		if ((WL11H_ENAB(wlc) == SPECT_MNGMT_STRICT_11H) && bi->infra &&
		...
		if (WL11H_ENAB(wlc) && bi->infra &&
		    wlc_csa_quiet(wlc, (uint8 *)bi->bcn_prb, bi->bcn_prb_len)) {
		...
		if (APSTA_ENAB(wlc->pub) && (!WIN7_OS(wlc->pub) || AP_ACTIVE(wlc))) {
			struct scb *scb;
			scb = wlc_scbfind(wlc, &bi->BSSID);
			if ((scb == NULL) && (NBANDS(wlc) > 1))
				scb = wlc_scbfindband(wlc, &bi->BSSID, OTHERBANDUNIT(wlc));
	...
	if (wlc->join_targets_last > 0) {
		...
		if (as->type != AS_ASSOCIATION)
			wlc_bss_mac_event(wlc, cfg, WLC_E_ROAM_PREP, &bi->BSSID, 0,
				0, 0, NULL, 0);
	...
	wlc_roam_release_flow_cntrl(wlc);
}
...
wlc_join_bss_start(wlc_bsscfg_t *cfg)
{
	...
	if (as->type == AS_ROAM && wlc_bss_connected(cfg) &&
		...
		WL_ASSOC(("ROAM: waiting for %d tx packets drained out before roam\n",
			pktq_len(&wlc->txq) + TXPKTPENDTOT(wlc)));
	} else
...
wlc_join_BSS(wlc_bsscfg_t *cfg, wlc_bss_info_t *bi)
{
	...
	if (CHSPEC_IS40_UNCOND(chanspec) &&
	    (!N_ENAB(wlc->pub) ||
	     ...
	     !band->mimo_cap_40 ||
	     (BAND_2G(band->bandtype) && WLC_INTOL40_DET(wlc)))) {
		uint channel;
	...
	wlc_rateset_filter(&target_bss->rateset, &target_bss->rateset,
		...
		RATE_MASK_FULL, cck_only ? FALSE : bss_ht);
	...
	if (!wlc_rate_hwrs_filter_sort_validate(&target_bss->rateset, rs_hw, TRUE,
		...
		wlc_rateset_default(&target_bss->rateset, rs_hw, wlc->band->phytype,
			...
			cck_only ? FALSE : N_ENAB(wlc->pub),
			CHSPEC_WLC_BW(target_bss->chanspec),
			wlc->stf->txstreams);
	}
	...
	if (bi->infra == 0) {
		...
		/* enter the BSSID addr into the RXE match registers for STA w/o promisc beacon */
		wlc_set_addrmatch(wlc, RCM_BSSID_OFFSET, &bi->BSSID);
		/* set infra mode */
		cfg->BSS = FALSE;
		...
		if (!(scb = wlc_scblookup(wlc, &bi->BSSID))) {
			WL_ERROR(("wl%d: wlc_BSS_join: out of scbs\n", wlc->pub->unit));
			return;
		}
		...
		if (BSS_P2P_ENAB(wlc, cfg))
			wlc_rateset_filter(&target_bss->rateset, &scb->rateset, FALSE,
			                   WLC_RATES_OFDM, RATE_MASK_FULL, bss_ht);
		else
#endif
		wlc_rateset_filter(&target_bss->rateset, &scb->rateset, FALSE,
		                   WLC_RATES_CCK_OFDM, RATE_MASK, bss_ht);
				...
				for (idx = 0; idx < roam->cache_numentries; idx++) {
					if (!bcmp(&roam->cached_ap[idx].BSSID,
						...
						WL_ASSOC(("wl%d: %s: Marking current AP as "
						          "unavaible for reassociation for %d "
...
wlc_assoc_timeout(void *arg)
{
	...
	if (ASSOC_RECREATE_ENAB(wlc->pub) &&
		...
		/* allow 802.11 PS doze now that the association has been
		 * verified (well, not positive verification, but at least the AP has not
		 * sent us a disassoc/deauth in a timely manner)
		 */
		wlc_set_ps_ctrl(wlc);
		...
		wlc_join(wlc, cfg, ssid.SSID, ssid.SSID_len, NULL, 0);
...
wlc_join_recreate(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg)
{
	...
	if (bsscfg->BSS) {
		scb = wlc_scbfindband(wlc, &bi->BSSID, CHSPEC_WLCBANDUNIT(bi->chanspec));
	...
	if (bsscfg->BSS && scb != NULL) {
		...
		if (!(scb->flags & SCB_MYAP))
			WL_ERROR(("wl%d: JOIN recreate: SCB_MYAP 0x%x not set in flags 0x%x!\n",
			          WLCWLUNIT(wlc), SCB_MYAP, scb->flags));
		if ((scb->state & (AUTHENTICATED | ASSOCIATED)) != (AUTHENTICATED | ASSOCIATED))
			WL_ERROR(("wl%d: JOIN recreate: (AUTHENTICATED | ASSOCIATED) 0x%x "
				  "not set in scb->state 0x%x!\n",
				  WLCWLUNIT(wlc), (AUTHENTICATED | ASSOCIATED), scb->state));

		WL_ASSOC(("wl%d: JOIN: AID 0x%04x\n", WLCWLUNIT(wlc), wlc->AID));

		/*
		 * security setup
		 */
		if (scb->WPA_auth != bsscfg->WPA_auth)
			WL_ERROR(("wl%d: JOIN recreate: scb->WPA_auth 0x%x "
				  "does not match bsscfg->WPA_auth 0x%x!",
				  WLCWLUNIT(wlc), scb->WPA_auth, bsscfg->WPA_auth));
		...
		if (!bcn) {
			WL_ERROR(("wl%d: wlc_join_complete: out of memory, malloced %d bytes\n",
	...
	wlc_suspend_mac_and_wait(wlc);
	wlc_BSSinit(wlc, bi, bsscfg, bcn, BCN_TMPL_LEN, bsscfg->BSS ? WLC_BSS_JOIN : WLC_BSS_START);
	wlc_enable_mac(wlc);
	...
	if (WLCISNPHY(wlc->band) || WLCISHTPHY(wlc->band))
		wlc_phy_cal_perical(wlc->band->pi,
	...
	if (as->type == AS_RECREATE && as->state == AS_LOSS_ASSOC) {
		...
		/* start the roam scan */
		as->flags |= AS_F_SPEEDY_RECREATE;
		wlc_assoc_scan_prep(bsscfg, NULL, &bi->chanspec, 1);
	}
	else {
...
wlc_assoc_abort(wlc_bsscfg_t *cfg)
{
	...
	else if (as->state == AS_WAIT_PASSHASH)
	 ...
	 */
	if ((scb = wlc_scbfindband(wlc, &target_bss->BSSID, curr_bandunit)))
		/* Clear pending bits */
...
wlc_assoc_change_state(wlc_bsscfg_t *cfg, uint newstate)
{
	...
	if (newstate > AS_LAST_STATE) {
		WL_ERROR(("wl%d.%d: wlc_assoc_change_state: out of bounds assoc state %d\n",
		          WLCWLUNIT(wlc), WLC_BSSCFG_IDX(cfg), newstate));
		ASSERT(0);
	}
...
wlc_sradar_ap_update(wlc_info_t *wlc)
{
	wlc_bsscfg_t *cfg;
	uint rand_idx;
	int idx, listlen, noradar_listlen = 0, i;
	chanspec_t newchspec = 0, chanspec;
	...
	FOREACH_UP_AP(wlc, idx, cfg) {
		if (BSSCFG_SRADAR_ENAB(cfg) &&
			...
			if ((list = MALLOC(wlc->osh, listlen)) == NULL) {
				...
				list->count = 0;
				wlc_get_valid_chanspecs(wlc->cmi, list, bw20, ch2g, abbrev);
			}
			...
			if (list && list->count) {
				...
				if (noradar_list && noradar_list->count) {
					...
					rand_idx = R_REG(wlc->osh, &wlc->regs->tsf_random)
						% noradar_list->count;
	...
	FOREACH_UP_AP(wlc, idx, cfg) {
		if (BSSCFG_SRADAR_ENAB(cfg) &&
		    wlc_radar_chanspec(wlc->cmi, cfg->current_bss->chanspec)) {
			/* this will perform channel switch to non radar channel */
			/* and update all bsscfg */
			...
			wlc_do_chanswitch(wlc, newchspec);
			break;
		}
	}
}
...
#ifdef WLBDD
	wlc_bdd_state_upd(wlc->bdd, state);
#endif
...
/* update unaligned tbtt checking threshold and beacon loss triggerred roaming threshold */
void
wlc_roam_thresh_upd(wlc_bsscfg_t *cfg)
{
	wlc_info_t *wlc = cfg->wlc;
	wlc_roam_t *roam = cfg->roam;
	wlc_bss_info_t *current_bss = cfg->current_bss;

	if (current_bss->beacon_period == 0 || current_bss->dtim_period == 0) {
		roam->uatbtt_tbtt_thresh = WLC_UATBTT_TBTT_THRESH;
		roam->tbtt_thresh = WLC_ROAM_TBTT_THRESH;
		WL_ERROR(("wl%d: wrong beacon parms, use default tbtt threshholds\n",
		          wlc->pub->unit));
		return;
	}

	/* Honor wlc->pub->roam_time_thresh when it is set to non-zero value (in miliseconds).
	 * To do so we need to update
	 * the unaligned tbtt check threshold and beacon loss roam threshold to the
	 * minimum values based on the most ideal situation. For example, set unaligned tbtt
	 * threshold to 4 to anticipate to check unaligned tbtt in 1.2 seconds in PS mode
	 * (which is the number of tbtt indications that should happen in PS mode when AP's
	 * beacon interval is set to 100 ms and dtim interval is set to 3). If the d11 needs
	 * somehow to stay awake then tbtt indications will come more often, in which case
	 * the unaligned tbtt check will happen shorter than 1.2 seconds.
	 */
	if (wlc->pub->roam_time_thresh != 0) {
		/* Make unaligned tbtt check threshold 3/5 of total roam threshold
		 * in PM mode
		 */
		if (wlc->PM != PM_OFF && cfg->dtim_programmed) {
			uint16 uatbtt_time_thresh = wlc->pub->roam_time_thresh * 3 / 5;
			roam->uatbtt_tbtt_thresh = (uint8)
			        (uatbtt_time_thresh / current_bss->beacon_period /
			         current_bss->dtim_period);
			roam->tbtt_thresh = (uint8)
			        ((wlc->pub->roam_time_thresh -
			          roam->uatbtt_tbtt_thresh *
			          current_bss->dtim_period *
			          current_bss->beacon_period) /
			         current_bss->beacon_period +
			         roam->uatbtt_tbtt_thresh);
		}
		/* Make unaligned tbtt check threshold 1/2 of total roam threshold
		 * in non PM mode.
		 */
		else {
			roam->tbtt_thresh = (uint8)
			        (wlc->pub->roam_time_thresh / current_bss->beacon_period);
			roam->uatbtt_tbtt_thresh =  roam->tbtt_thresh / 2;
		}
	}
	/* Use uatbtt_tbtt_thresh and tbtt_thresh to decide when to perform
	 * unaligned tbtt check and roam. In this case we'll need to update the link down
	 * threshold due to beacon loss to a larger number to prevent to indicate link down
	 * before we start roaming or too close after roaming.
	 */
	else {
		roam->uatbtt_tbtt_thresh = WLC_UATBTT_TBTT_THRESH;
		roam->tbtt_thresh = WLC_ROAM_TBTT_THRESH;
		/* Make link down to happen 6 seconds after roaming */
		if (wlc->PM != PM_OFF && cfg->dtim_programmed) {
			uint bcn_timeout = roam->tbtt_thresh *
			        current_bss->dtim_period *
			        current_bss->beacon_period / 1000 + 6;
			if (bcn_timeout > WLC_BCN_TIMEOUT)
				roam->bcn_timeout = bcn_timeout;
		}
	}

	WL_INFORM(("wl%d: wlc_roam_thresh_upd: wlc->pub->roam_time_thresh %d, "
	          "dtim_programmed %d, PM %d, uatbtt_tbtt_thresh %d, "
	          "tbtt_thresh %d\n", WLCWLUNIT(wlc),
	          wlc->pub->roam_time_thresh, cfg->dtim_programmed,
	          wlc->PM, roam->uatbtt_tbtt_thresh, roam->tbtt_thresh));
}

static void
wlc_join_adopt_bss(wlc_bsscfg_t *cfg, char *bcn, int len)
{
	wlc_info_t *wlc = cfg->wlc;
	wlc_assoc_t *as = cfg->assoc;
	wlc_roam_t *roam = cfg->roam;
	...
	wlc_suspend_mac_and_wait(wlc);
	wlc_BSSinit(wlc, target_bss, cfg, bcn, len, WLC_BSS_JOIN);
	wlc_enable_mac(wlc);
	...
	else if (BTA_ENAB(wlc->pub))
		wlc_bta_assoc_complete(wlc->bta);
#endif /* WLBTAMP */

#if defined(BCMSUP_PSK) && defined(BCMINTSUP)
	if (cfg->sup_type != SUP_UNUSED) {
		bool sup_stat;
...
#endif /* FULL_LEGACY_B_RATE_CHECK */
...
void
wlc_join_recreate_complete(wlc_bsscfg_t *cfg, struct dot11_bcn_prb *bcn, int bcn_len)
{
	wlc_info_t *wlc = cfg->wlc;
	wlc_assoc_t *as = cfg->assoc;
	wlc_bss_info_t *current_bss = cfg->current_bss;
	...
	if (cfg->BSS) {
		 ...
		 * associated.
		 */
		pkt = wlc_sendnulldata(wlc, &cfg->current_bss->BSSID, 0, 0, PRIO_8021D_BE);
		if (pkt == NULL)
...
void
wlc_join_complete(wlc_bsscfg_t *cfg, struct dot11_bcn_prb *bcn, int bcn_len)
{
	wlc_info_t *wlc = cfg->wlc;
	wlc_assoc_t *as = cfg->assoc;
	wlc_bss_info_t *target_bss = cfg->target_bss;
	...
	wlc_roam_thresh_upd(cfg);

	if (target_bss->infra == 1) {
		/* illed AP, prevent going to power-save mode */
		if (!tim) {
			wlc->PM_override = TRUE;
		}
	} else {
		/* adopt the IBSS parameters */
		char *bcn_tmpl = (char *) MALLOC(wlc->osh, BCN_TMPL_LEN);
		if (!bcn_tmpl) {
			WL_ERROR(("wl%d: wlc_join_complete: out of memory, malloced %d bytes\n",
			wlc->pub->unit, MALLOCED(wlc->osh)));
			return;
		}
		wlc_join_adopt_bss(cfg, bcn_tmpl, BCN_TMPL_LEN);
		if (bcn_tmpl)
			MFREE(wlc->osh, (void *)bcn_tmpl, BCN_TMPL_LEN);
	 ...
	 */
	if ((BSS_WL11H_ENAB(wlc, cfg) || WL11D_ENAB(wlc)) && (target_bss->infra == 1)) {
		bcm_tlv_t *country_ie;
		country_ie = bcm_parse_tlvs(tlvs, tlvs_len, DOT11_MNG_COUNTRY_ID);
		if (country_ie) {
			WL_REGULATORY(("wl%d: %s: Adopting Country IE from associated AP\n",
			               wlc->pub->unit, __FUNCTION__));
			wlc_adopt_country_ie(wlc, country_ie);
		}
	}
#endif /* WL11H || WL11D */
	...
	if (PM2_RCV_DUR_ENAB(wlc) && wlc->PM == PM_FAST) {
		WL_RTDC(wlc, "wlc_join_complete: PMep=%02u AW=%02u",
			(wlc->PMenabled ? 10 : 0) | wlc->PMpending,
			(PS_ALLOWED(wlc) ? 10 : 0) | STAY_AWAKE(wlc));
		wlc_pm2_rcv_reset(wlc);
	}
	 ...
	 */
	if (PS_ALLOWED(wlc))
		wlc_set_pmstate(wlc, TRUE);
	else if (wlc->PM == PM_FAST) {
		WL_RTDC(wlc, "wlc_join_complete: start srtmr, PMep=%02u AW=%02u",
			(wlc->PMenabled ? 10 : 0) | wlc->PMpending,
			(PS_ALLOWED(wlc) ? 10 : 0) | STAY_AWAKE(wlc));
		wlc_pm2_sleep_ret_timer_start(wlc);
	}
...
#ifdef WLP2P
		if (BSS_P2P_ENAB(wlc, cfg))
			wlc_p2p_assoc_upd(wlc->p2p, cfg, TRUE);
#endif
	}
...
wlc_auth_tx_complete(wlc_info_t *wlc, uint txstatus, void *arg)
{
	...
	/* in case bsscfg is freed before this callback is invoked */
	if (cfg == NULL) {
		WL_ERROR(("wl%d: wlc_auth_tx_complete: unable to find bsscfg by ID %p\n",
		          wlc->pub->unit, arg));
		return;
	}
...
wlc_assocreq_complete(wlc_info_t *wlc, uint txstatus, void *arg)
{
	...
	/* in case bsscfg is freed before this callback is invoked */
	if (cfg == NULL) {
		WL_ERROR(("wl%d: wlc_assocreq_complete: unable to find bsscfg by ID %p\n",
		          wlc->pub->unit, arg));
		return;
	}
...
wlc_bss_info_dup(wlc_info_t *wlc, wlc_bss_info_t *bi)
{
	...
	if (!bss) {
		WL_ERROR(("wl%d: wlc_bss_info_dup: out of memory, malloced %d bytes\n",
...
void
wlc_assocresp_client(wlc_bsscfg_t *cfg, d11rxhdr_t *rxh,
	struct dot11_management_header *hdr, void *body, uint body_len, struct scb *scb)
{
	...
	if (!(as->state == AS_SENT_ASSOC || as->state == AS_REASSOC_RETRY) ||
		...
		WL_ERROR(("wl%d: unsolicited association response from %s\n",
			wlc->pub->unit, eabuf));
	...
	if (BSS_BTA_ENAB(wlc, cfg) && wlc_bta_qos(wlc->bta)) {
		...
		parse = (uint8 *)&assoc[1];
		parse_len = body_len - sizeof(struct dot11_assoc_resp);
...
#ifdef WLP2P
	if (BSS_P2P_ENAB(wlc, cfg)) {
		if (wlc_p2p_process_assocresp(wlc->p2p, scb, (uint8 *)&assoc[1],
		                body_len - sizeof(struct dot11_assoc_resp)) != BCME_OK) {
			return;
		}
	}
#endif
	...
	if (BSS_WME_ENAB(wlc, cfg)) {
		...
		parse = (uint8 *)&assoc[1];
		parse_len = body_len - sizeof(struct dot11_assoc_resp);
		while ((wme_ie = bcm_parse_tlvs(parse, parse_len, DOT11_MNG_PROPR_ID)) != NULL)
			if (wlc_is_wme_ie(wlc, (uint8 *)wme_ie, &parse, &parse_len))
				break;
		wlc_qosinfo_update(scb, 0);	/* Clear Qos Info by default */
		if (wme_ie) {
			wme_param_ie_t *wme_ie_body = (wme_param_ie_t *)&wme_ie->data;
			scb->flags |= SCB_WMECAP;
			/* save the new IE, or params IE which is superset of IE */
			bcopy(wme_ie_body, &wlc->wme_param_ie, wme_ie->len);
			 ...
			 */
			if (wlc->wme_param_ie.qosinfo & WME_QI_AP_APSD_MASK) {
				wlc_qosinfo_update(scb, wlc->apsd_sta_qosinfo);
				if (!AP_ENAB(wlc->pub)) {
					wlc->WME_PM_blocked = FALSE;
					if (wlc->PM == PM_MAX)
						wlc_set_pmstate(wlc, TRUE);
				}
			} else if (!AP_ENAB(wlc->pub) && WME_AUTO(wlc) &&
			           (scb->flags & SCB_BRCM)) {
				if (!(scb->flags & SCB_WMEPS)) {
					wlc->WME_PM_blocked = TRUE;
					WL_RTDC(wlc, "wlc_recvctl: exit PS", 0, 0);
					wlc_set_pmstate(wlc, FALSE);
				} else {
					wlc->WME_PM_blocked = FALSE;
					if (wlc->PM == PM_MAX)
						wlc_set_pmstate(wlc, TRUE);
...
void
wlc_authresp_client(wlc_bsscfg_t *cfg, d11rxhdr_t *rxh,
	struct dot11_management_header *hdr, void *body, uint body_len)
{
	...
	if (bcmp((char*)&hdr->sa, (char*)&target_bss->BSSID, ETHER_ADDR_LEN) ||
	    !(scb = wlc_scblookup(wlc, (struct ether_addr *) &hdr->sa)) ||
	    (as->state != AS_SENT_AUTH_1 && as->state != AS_SENT_AUTH_3)) {
		WL_ERROR(("wl%d: unsolicited authentication response from %s\n", wlc->pub->unit,
			sa));
	...
	/* invalid authentication algoritm number */
		cfg_auth_alg = cfg->auth;

	if (auth_alg != cfg_auth_alg && !cfg->openshared) {
		WL_ERROR(("wl%d: invalid authentication algoritm number, got %d, expected %d\n",
	...
	if (auth_alg == DOT11_SHARED_KEY && auth_seq == 2) {
		...
		pkt = wlc_sendauth(cfg, &scb->ea, &target_bss->BSSID, scb,
		            DOT11_SHARED_KEY, 3, DOT11_SC_SUCCESS,
		            (WSEC_SCB_KEY_VALID(scb) ? scb->key : NULL),
		            challenge, ((rxh->PhyRxStatus_0 & PRXS0_SHORTH) != 0));
		wl_del_timer(wlc->wl, as->timer);
		if (pkt != NULL)
			wlc_pkt_callback_register(wlc, wlc_auth_tx_complete,
			                          (void *)(uintptr)cfg->ID, pkt);
...
void
wlc_clear_hw_association(wlc_bsscfg_t *cfg)
{
	wlc_info_t *wlc = cfg->wlc;
...
wlc_join_start_ibss(wlc_bsscfg_t *cfg, const uint8 ssid[], int ssid_len)
{
	...
	*/
	...
	wlc_suspend_mac_and_wait(wlc);
	wlc_BSSinit(wlc, &bi, cfg, bcn, BCN_TMPL_LEN, WLC_BSS_START);
	wlc_enable_mac(wlc);
	...
	if (WLCISNPHY(wlc->band) || WLCISHTPHY(wlc->band))
		wlc_phy_cal_perical(wlc->band->pi, PHY_PERICAL_START_IBSS);
...
wlc_disassociate_client(wlc_bsscfg_t *cfg, bool send_disassociate, pkcb_fn_t fn, void *arg)
{
	...
	uint bsstype;
	wlc_bss_info_t *current_bss = cfg->current_bss;
	/* don't nuke the SSID, the callback should handle it in case it needs it */
	bool reset_ssid = (fn == NULL);
	...
	if (cfg->BSS) {
		...
		wlc_pspoll_timer_upd(wlc, FALSE);
		wlc_apsd_trigger_upd(wlc, FALSE);
		...
		if (send_disassociate) {
			if (ETHER_ISNULLADDR(cfg->BSSID.octet)) {
				 ...
				 * after this function is done it will be a quiet channel since
				 * we will no longer be associated
				 */
				WL_ASSOC(("wl%d: JOIN: skipping DISASSOC to %s since channel %d is"
					" quiet.\n", WLCWLUNIT(wlc), eabuf,
				...
				if (pkt == NULL)
					...
					if (wlc_pkt_callback_register(wlc, fn, arg, pkt)) {
						WL_ERROR(("wl%d: wlc_disassociate_client out of pkt"
						          " callbacks\n", wlc->pub->unit));
					} else {
...
#ifdef NCONF
		if (WLCISNPHY(wlc->band) && (D11REV_GE(wlc->pub->corerev, 16))) {
...
#ifdef WME
	/* Revert WME AC parameters to defaults */
	...
	if (!wlc->pub->associated) {
		...
		/* unassociated 11g protection mode */
		wlc_protection_g_sync(wlc);
#ifdef WL11N
		if (N_ENAB(wlc->pub))
			wlc_protection_ncfg_sync(wlc);
#endif
#ifdef WLAFTERBURNER
		/* afterburner fallback */
		if (wlc->afterburner_override == AUTO)
			wlc_afterburner(wlc, OFF);
#endif /* WLAFTERBURNER */
	}
	...
	wlc_rssi_reset_ma(cfg, WLC_RSSI_INVALID);
	wlc_rssi_event_update(cfg);
...
wlc_assoc_success(wlc_bsscfg_t *cfg, struct scb *scb)
{
	...
	if (cfg->BSS && cfg->associated) {
		prev_scb = wlc_scbfindband(wlc, &cfg->prev_BSSID,
	...
	uint auth_status, uint auth_type)
{
	...
	if (status == WLC_E_STATUS_SUCCESS) {
		...
		WL_ASSOC(("wl%d: JOIN: authentication failure, no ack from %s\n",
		    WLCWLUNIT(wlc), eabuf));
...
do_event:
	wlc_bss_mac_event(wlc, cfg, reassoc ? WLC_E_REASSOC : WLC_E_ASSOC, addr,
		status, assoc_status, bss_type, 0, 0);
...
wlc_set_ssid_complete(wlc_bsscfg_t *cfg, uint status, struct ether_addr *addr, uint bss_type)
{
	...
	/* don't nuke the SSID, the abort caller should handle it in case it needs it */
	bool reset_ssid = (status != WLC_E_STATUS_SUCCESS && status != WLC_E_STATUS_ABORT);
	...
	if (status != WLC_E_STATUS_SUCCESS) {
		WL_ASSOC(("wl%d: %s: failed status param %u\n",
	...
	if (BSS_BTA_ENAB(wlc, cfg) && status != WLC_E_STATUS_SUCCESS) {
		...
		scb = wlc_scbfind(wlc, &assoc_params->bssid);
		if (scb != NULL) {
			/* BT-AMP join post-processing:
			 * on failure
...
void wlc_disassoc_complete(wlc_bsscfg_t *cfg, uint status, struct ether_addr *addr,
...
#ifdef WLLMAC
	if (LMAC_ENAB(wlc->pub)) {
		wlc_lmac_roam_rssi_check(wlc->lmac_info);
		return;
	}
#endif
...
static void
wlc_roam_release_flow_cntrl(wlc_info_t *wlc)
{
	if (wlc->pub->txqstopped & TXQ_STOP_FOR_PKT_DRAIN) {
		wlc->pub->txqstopped &= ~TXQ_STOP_FOR_PKT_DRAIN;
		if (!wlc->pub->txqstopped)
			wl_txflowcontrol(wlc->wl, OFF, ALLPRIO);
		else if (wlc->pub->_priofc && !(wlc->pub->txqstopped & ~TXQ_STOP_FOR_PRIOFC_MASK)) {
			int prio;

			for (prio = MAXPRIO; prio >= 0; prio--) {
				if (!mboolisset(wlc->pub->txqstopped, NBITVAL(prio)))
					wl_txflowcontrol(wlc->wl, OFF, prio);
			}
		}
	}
}
...
wlc_assoc_roam(wlc_bsscfg_t *cfg)
{
	...
	if (BSSCFG_STA(cfg) &&
		...
		ssid = current_bss->SSID;
		ssid_len = current_bss->SSID_len;
		...
		if (ssid_cnt > 0) {
			...
			for (index = ssid_cnt; index < wlc->scan_results->count; index++) {
				bi = wlc->scan_results->ptrs[index];
				if (bi) {
					if (bi->bcn_prb)
						MFREE(wlc->osh, bi->bcn_prb, bi->bcn_prb_len);

					MFREE(wlc->osh, bi, sizeof(wlc_bss_info_t));
					wlc->scan_results->ptrs[index] = NULL;
				}
			}
...
wlc_roam_bcns_lost(wlc_bsscfg_t *cfg)
{
	...
	if (!roam->off && (cfg->assoc->state == AS_IDLE) && !SCAN_IN_PROGRESS(wlc->scan)) {
		WL_ASSOC(("wl%d.%d: ROAM: tbtt_since_bcn %d, "
		          "time_since_bcn %d, request roam scan\n",
		          WLCWLUNIT(wlc), WLC_BSSCFG_IDX(cfg),
		          roam->tbtt_since_bcn, roam->time_since_bcn));

		/* No longer need to track PSpoll since we may have lost our AP */
		wlc->PSpoll = FALSE;
	...
	bool pkt_max_retries)
{
	...
	if (pkt_sent && !wlc_ratesel_minrate(wlc->rsi, scb, txs))
		roam->txpass_cnt++;
	else
		roam->txpass_cnt = 0;
	...
	if (roam->minrate_txpass_thresh) {
		if (pkt_sent) {
			if (wlc_ratesel_minrate(wlc->rsi, scb, txs))
	...
	if (roam->minrate_txfail_thresh) {
		...
		else if (pkt_max_retries && wlc_ratesel_minrate(wlc->rsi, scb, txs)) {
...
wlc_mac_request_entry(wlc_info_t *wlc, wlc_bsscfg_t *cfg, int req)
{
	...
	case WLC_ACTION_ROAM:
		...
		else if (QUIET_STATE(wlc) & SILENCE) {
		...
		if (QUIET_STATE(wlc) & SILENCE) {
...
#endif /* WLRM */
		else if (req == WLC_ACTION_ISCAN) {
#ifdef WIFI_ACT_FRAME
			if (ACT_FRAME_IN_PROGRESS(wlc->scan)) {
				/* iscans preempt in-progress action frame */
				WL_INFORM(("e/iscan aborting action frame\n"));
			...
			if (SCAN_IN_PROGRESS(wlc->scan)) {
				if (ISCAN_IN_PROGRESS(wlc)) {
					...
					WL_INFORM(("iscan aborting iscan\n"));
					wlc_scan_abort(wlc->scan, WLC_E_STATUS_NEWSCAN);
				} else {
					/* other scans have precedence over iscans */
					WL_ERROR(("iscan blocked due to another kind of scan\n"));
					err = BCME_NOTREADY;
				}
			}
#ifdef WIFI_ACT_FRAME
		} else if (req == WLC_ACTION_ACTFRAME) {
			if (ACT_FRAME_IN_PROGRESS(wlc->scan) || SCAN_IN_PROGRESS(wlc->scan)) {
				/* scans have precedence over action frames */
				WL_ERROR(("action frame blocked due to scan in progress\n"));
				err = BCME_NOTREADY;
			}
		} else if (ACT_FRAME_IN_PROGRESS(wlc->scan)) {
				/* iscans preempt in-progress action frame */
				WL_INFORM(("scan aborting action frame\n"));
				wlc_scan_abort(wlc->scan, WLC_E_STATUS_NEWSCAN);
#endif
		} else if (ISCAN_IN_PROGRESS(wlc)) {
			/* regular scan aborts an iscan */
			WL_INFORM(("iscan aborting due to another kind of scan request\n"));
			wlc_scan_abort(wlc->scan, WLC_E_STATUS_NEWSCAN);
		} else
#endif /* STA */
		if (SCAN_IN_PROGRESS(wlc->scan)) {
			WL_ERROR(("wl%d: scan request blocked for scan in progress\n",
				wlc->pub->unit));
			err = BCME_NOTREADY;
		}
		break;
...
void
BCMINITFN(wlc_roam_defaults)(wlc_info_t *wlc, wlcband_t *band, int *roam_trigger, uint *roam_delta)
{
...
void
wlc_deauth_sendcomplete(wlc_info_t *wlc, uint txstatus, void *ea_arg)
{
	struct scb *scb;
#if defined(BCMDBG) || defined(WLMSG_WSEC)
	char eabuf[ETHER_ADDR_STR_LEN];
	...
	struct ether_addr *addr, uint disassoc_reason, uint bss_type)
{
	...
	wlc_bss_mac_event(wlc, bsscfg, WLC_E_DISASSOC_IND, addr, status, disassoc_reason,
		bss_type, 0, 0);
#ifdef BCMWAPI_WAI
	wlc_wapi_station_event(wlc, bsscfg, addr, NULL, NULL, WAPI_STA_AGING);
#endif /* BCMWAPI_WAI */
}
