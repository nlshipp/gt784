 ...
 *
 * $Id: wlc_apps.c,v 1.1 2010/08/05 21:59:01 ywu Exp $
 */
...
static void wlc_apps_scb_ps_on(wlc_info_t *wlc, struct scb *scb);
...
static void wlc_apps_psp_resp_complete(wlc_info_t *wlc, uint txstatus, void *ea_arg);
static void wlc_apps_ps_enq(void *ctx, struct scb *scb, void *pkt, uint prec);
static void wlc_apps_apsd_complete(wlc_info_t *wlc, uint txstatus, void *ea_arg);
...
uint wlc_acbitmap2precbitmap[16];
...
BCMATTACHFN(wlc_apps_attach)(wlc_info_t *wlc)
{
	...
	/* register module */
	wlc_module_register(wlc->pub, NULL, "apps", wlc, NULL, NULL, wlc_apps_down);
...
wlc_apps_ps_send(wlc_info_t *wlc, struct scb *scb, uint prec_bmp, uint32 extra_flags)
{
	 ...
	 */
	if (WLPKTTAG(pkt)->flags & WLF_MPDU) {
...
wlc_apps_ps_enq_resp(wlc_info_t *wlc, struct scb *scb, void *pkt, int prec)
{
	...
	WL_PS(("wl%d: ps_enq_resp %p mpdu %d apsd %d\n",
	       wlc->pub->unit, pkt,
	       (WLPKTTAG(pkt)->flags & WLF_MPDU) ? 1 : 0,
	       (WLPKTTAG(pkt)->flags & WLF_APSD) ? 1 : 0));

	/* Enqueue in order of precedence */
	if (!wlc_prec_enq(wlc, &wlc->txq, pkt, prec)) {
...
wlc_apps_tbtt_update(wlc_info_t *wlc)
{
	...
	FOREACHSCB(wlc->scbstate, &scbiter, scb) {
		if (!scb->permanent && SCB_PS(scb) &&
		    SCB_BSSCFG(scb) != NULL && BSSCFG_AP(SCB_BSSCFG(scb)))
                             ...
                             void *pkt, struct dot11_header *h, bool last_frag)
{
	...
	if (last_frag && !SCB_ISMULTI(scb)) {
		if (wlc_pkt_callback_register(wlc, wlc_apps_psp_resp_complete, (void *)&scb->ea,
		                              pkt)) {
			WL_ERROR(("wl%d: register PS-Poll response failed", wlc->pub->unit));
			scb_psinfo->psp_pending = FALSE;
		} else
...
static void
wlc_apps_psp_resp_complete(wlc_info_t *wlc, uint txstatus, void *ea_arg)
{
	struct scb *scb;
	struct ether_addr *ea = (struct ether_addr *)ea_arg;
	struct apps_scb_psinfo *scb_psinfo;

	/* Is this scb still around */
	if ((scb = wlc_scbfind(wlc, ea)) == NULL)
	    return;
...
wlc_apps_send_psp_response(wlc_info_t *wlc, struct scb *scb, uint16 fc)
{
	...
	if (pktq_empty(&scb_psinfo->psq) || wlc_apps_apsd_ndelv_count(wlc, scb) == 0) {
		...
		if ((pkt = wlc_sendnulldata(wlc, &scb->ea, 0, WLF_PSDONTQ,
			...
			scb_psinfo->psp_pending = FALSE;
		} else if (wlc_pkt_callback_register(wlc, wlc_apps_psp_resp_complete,
		                                     (void *)&scb->ea, pkt)) {
			WL_ERROR(("wl%d: %s: register PS-Poll response failed",
			          wlc->pub->unit, __FUNCTION__));
			scb_psinfo->psp_pending = FALSE;
		} else
			scb_psinfo->psp_pending = TRUE;
...
wlc_apps_suppr_frame_enq(wlc_info_t *wlc, void *pkt, tx_status_t *txs, bool last_frag)
{
...
#ifdef BCMDBG
	{
		...
		WL_PS(("SUPPRESSED packet %p - %s %s PS:%d \n", pkt,
		       (FC_TYPE(ltoh16(txh->MacFrameControl)) == FC_TYPE_DATA) ? "data" : "mgmt",
	...
	if (SCB_PS(scb) && !SCB_ISMULTI(scb)) {
		...
		if (wlc_prec_enq(wlc, &wlc->txq, pkt, WLC_PRIO_TO_HI_PREC(PKTPRIO(pkt))))
			return FALSE;
...
wlc_apps_apsd_trigger(wlc_info_t *wlc, struct scb *scb, int ac)
{
	...
	/* Determine number of frames to send in service period */
	delv = wlc_apps_apsd_delv_count(wlc, scb);

	scb_psinfo->apsd_cnt = MIN(delv, scb->apsd.maxsplen);

	WL_PS(("wl%d: wlc_apps_apsd_trigger: ac %d buffered %d delv %d cnt %d\n",
	       wlc->pub->unit, ac, pktq_plen(&scb_psinfo->psq, ac), delv, scb_psinfo->apsd_cnt));
	...
	if (scb_psinfo->apsd_cnt == 0) {
		...
		prio = (int)acbitmap2maxprio[scb->apsd.ac_delv];

		WL_PS(("wl%d: wlc_apps_apsd_trigger: send QoS Null prio=%d\n",
		       wlc->pub->unit, prio));
                      ...
                      struct dot11_header *h, bool last_frag)
{
	...
	if (last_frag && wlc_pkt_callback_register(wlc, wlc_apps_apsd_complete,
	                                           (void *)&scb->ea, pkt)) {
		WL_ERROR(("wl%d: could not set a callback for APSD frame %p",
		          wlc->pub->unit, pkt));
		scb_psinfo->apsd_usp = FALSE;
	}
}
...
static void
wlc_apps_apsd_complete(wlc_info_t *wlc, uint txstatus, void *ea_arg)
{
	struct scb *scb;
	struct ether_addr *ea = (struct ether_addr *)ea_arg;
	struct apps_scb_psinfo *scb_psinfo;
...
wlc_apps_bcmc_ps_enqueue(wlc_info_t *wlc, struct scb *bcmc_scb, void *pkt)
{
	...
	if (pktq_full(&scb_psinfo->psq) || pktq_pfull(&scb_psinfo->psq, MAXPRIO) || \
		osl_pktq_wmark_high_drop(wlc->pub->osh)) {
#else
	if (pktq_full(&scb_psinfo->psq) || pktq_pfull(&scb_psinfo->psq, MAXPRIO)) {
#endif /* DSLCPE */
