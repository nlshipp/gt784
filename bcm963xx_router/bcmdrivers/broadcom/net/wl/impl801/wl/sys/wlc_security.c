 ...
 *
 * $Id: wlc_security.c,v 1.1 2010/08/05 21:59:02 ywu Exp $
 */
...
wlc_wsec_isreplay(wlc_info_t *wlc, wsec_key_t *key, uint ividx, uint32 iv32, uint16 iv16)
{
	...
	/* It is a replay when the IV is beyond the trace window's lower bond,
	 ...
	 * the trace window bonds and initialize the trace entries.
	 */
	if (!in_tw) {
		wsec_iv_t old, new, adv;
		/* Update trace window upper bond */
		old = key->iv_tw[ividx].ub;
		key->iv_tw[ividx].ub.hi = iv32;
		key->iv_tw[ividx].ub.lo = iv16;
		/* Update trace window lower bond and trace entries */
		if (WSEC_IV_GE(key->iv_tw[ividx].ub.hi, key->iv_tw[ividx].ub.lo, 0, TWSIZE)) {
			/* Update trace window lower bond */
			...
			if (adv.lo > 1) {
				 ...
				 * each individual trace entries between the old upper bond
				 * the new upper bond so just do a big hammer.
				 */
				if (adv.lo >= TWSIZE)
					bzero(key->iv_tw[ividx].bmp, sizeof(key->iv_tw[ividx].bmp));
				/* IV has moved up but the trace entries between
				 * the old upper bond and the new upper bond are
				 ...
				 * the old upper bond and the new upper bond are now
				 * across different bytes in the trace window.
				 */
				else {
					/* The old upper bond points at an entry in the
					...
					/* The new upper bond points at an entry in the
					 * middle of a byte in the trace window. Clear
	...
	/* Update the trace entry for the currnet IV */
	key->iv_tw[ividx].bmp[off_tw >> 3] |= (1 << (off_tw & 7));
...
#else
#define wlc_wsec_isreplay(wlc, key, ividx, iv32, iv16) \
	_wlc_wsec_isreplay(iv32, iv16, (key)->rxiv[ividx].hi, (key)->rxiv[ividx].lo)
#endif	/* BRCMAPIVTW */
 ...
 */
const uint8 wlc_wai_hdr[] = {0xAA, 0xAA, 0x03, 0x00, 0x00, 0x00, 0x88, 0xb4};
#endif /* BCMWAPI_WAI */

#if defined(BCMCCMP)
static uint8 wlc_get_nonce_1st_byte(struct dot11_header *h, void *p, bool legacy);
#endif
	...
	struct wlc_frminfo *f, uint8 prio)
{
	...
	case CRYPTO_ALGO_TKIP:
		...
		if ((f->WPA_auth & WPA_AUTH_NONE) == 0 &&
		    wlc_wsec_isreplay(wlc, f->key, ividx, iv32, iv16)) {
	...
	if (WLC_SW_KEYS(wlc, bsscfg) ||
		...
		case CRYPTO_ALGO_AES_CCM:
...
#ifdef BCMCCMP
			{
				...
				nonce_1st_byte = wlc_get_nonce_1st_byte(f->h, f->p, FALSE);
	...
	if (f->key->algo == CRYPTO_ALGO_SMS4) {
		mic_len = SMS4_WPI_CBC_MAC_LEN;
#ifdef BCMWAPI_WAI
		/* Key receive cipher packets counter */
		f->key->wapi_rx++;
		if ((f->key->wapi_rx % WAPI_USK_REKEY_COUNT) == 0) {
			wlc_wapi_station_event(wlc, bsscfg, &scb->ea, NULL, NULL,
				WAPI_UNICAST_REKEY);
		}
#endif /* BCMWAPI_WAI */
	}
	...
	f->body_len -= f->key->iv_len + f->key->icv_len + mic_len;
	f->totlen -= f->key->iv_len + f->key->icv_len + mic_len;
...
#ifdef BCMWAPI_WAI
#define WAI_SNAP(pbody) \
	(bcmp(wlc_wai_hdr, (pbody), DOT11_LLC_SNAP_HDR_LEN) == 0)
#endif /* BCMWAPI_WAI */
	...
	struct scb *scb, wsec_key_t *key)
{
	...
	case CRYPTO_ALGO_AES_CCM:
		PKTSETLEN(osh, p, PKTLEN(osh, p) + key->icv_len);
			...
			nonce_1st_byte = wlc_get_nonce_1st_byte(h, p, FALSE);
			...
			if (BSSCFG_STA(bsscfg)) {
				...
				if (bsscfg->tk_cm_dt == 0) {
					...
					if (WIN7_OS(wlc->pub))
						...
						pktq_flush(wlc->osh, &wlc->txq, TRUE,
							wlc_ifpkt_chk_cb, WLC_BSSCFG_IDX(bsscfg));
					else
						/* clear common txq */
						pktq_flush(wlc->osh, &wlc->txq, TRUE, NULL, 0);
...
#if defined(BCMCCMP)
static uint8
wlc_get_nonce_1st_byte(struct dot11_header *h, void *p, bool legacy)
{
	uint16 fc, subtype;
	uint16 qc;
	bool wds, qos;
