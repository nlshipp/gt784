 ...
 *
 * $Id: wlc_ba.c,v 1.1 2010/08/05 21:59:01 ywu Exp $
 */
#ifndef WLBA
	...
	void *p, uint plen, void *a, int alen, int vsize, struct wlc_if *wlcif)
{
	...
	if ((err = wlc_iovar_check(ba->wlc->pub, vi, a, alen, IOV_ISSET(actionid))) != 0)
		return err;
...
wlc_ba_send_addba_req(ba_info_t *ba, struct scb *scb, uint8 tid)
{
	...
	p = wlc_frame_get_mgmt(ba->wlc, FC_ACTION, &scb->ea, &scb->bsscfg->cur_etheraddr,
	...
	/* count number of packets already released into txq to keep txpending in sync */
	wlc = ba->wlc;
	prec = WLC_PRIO_TO_PREC(tid);
	head_pkt = NULL;
	while (pktq_ppeek(&wlc->txq, prec) != head_pkt) {
		pkt = pktq_pdeq(&wlc->txq, prec);
		if (!head_pkt)
			head_pkt = pkt;
	...
	wlc_sendmgmt(ba->wlc, p, scb, 0, NULL, 0);
}
	...
	bool enq_only)
{
	...
	wlc_sendctl(ba->wlc, p, scb, TX_CTL_FIFO, 0, enq_only);
}
	...
	uint16 cf_policy)
{
	...
	wlc_sendctl(ba->wlc, p, scb, TX_CTL_FIFO, 0, FALSE);
}
