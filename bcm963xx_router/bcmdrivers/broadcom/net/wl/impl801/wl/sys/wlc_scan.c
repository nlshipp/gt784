 ...
 *
 * $Id: wlc_scan.c,v 1.1 2010/08/05 21:59:02 ywu Exp $
 */
...
#ifdef WLLMAC_ONLY
/* LMAC doesn't wouldn't have the support routines */
...
static void wlc_scan_channels(wlc_info_t *wlc,
	chanspec_t *chanspec_list, int *pchannel_num,
	int channel_max, chanspec_t chanspec_start, int channel_type);

#ifdef WLC_SCAN_IOVARS

static int
wlc_scan_doiovar(void *hdl, const bcm_iovar_t *vi, uint32 actionid,
...
struct scan_info {
	...
	chanspec_t	chanspec_return;	/* channel to return to when scan is done */
...
BCMATTACHFN(wlc_scan_attach)(wlc_info_t *wlc, void *wl, osl_t *osh, uint unit)
{
	...
	if (scan_info->timer == NULL) {
		WL_ERROR(("wl%d: wlc_scan_attach: wl_init_timer for scan timer failed\n", unit));
		goto error;
	}
	...
	if (scan_info->sdb == NULL) {
		WL_ERROR(("wl%d: wlc_scan_attach: wlc_create_scandb failed\n", unit));
		goto error;
	}
	...
	err = wlc_module_register(wlc->pub, iovars, "scan",
	                          scan_info, iovar_fn, watchdog_fn, wlc_scan_down);
	if (err) {
		WL_ERROR(("wl%d: wlc_scan_attach: wlc_module_register err=%d\n",
		          unit, err));
		goto error;
	}
...
BCMUNINITFN(wlc_scan_down)(void *hdl)
{
	...
	cbk = 0;
	if (!wl_del_timer(wlc->wl, scan_info->timer))
		cbk = 1;
...
wlc_scan(
	...
	bool save_prb,
	scancb_fn_t fn, void* arg, int away_channels_limit, bool extdscan,
	bool suppress_ssid,
	bool include_cache,
	wlc_bsscfg_t *cfg)
{
	...
	scan_timer_set = (scan_info->pass == WLC_SCAN_ABORT ||
		(scan_in_progress &&
		0 == (wlc_scan_info->state & (SCAN_STATE_WSUSPEND | SCAN_STATE_PSPEND))));
	...
	scan_info->bsscfg = cfg != NULL ? cfg : wlc->cfg;
	...
	if (wlc->pub->associated) {
		/* return to the current BSS channel if associated */
		scan_info->chanspec_return = wlc->home_chanspec;
	} else if (!scan_in_progress) {
		/* not associated and no scan in progress, so just return to the current channel */
		if (wlc_valid_chanspec_db(wlc->cmi, WLC_BAND_PI_RADIO_CHANSPEC)) {
			scan_info->chanspec_return = WLC_BAND_PI_RADIO_CHANSPEC;
		} else {
			scan_info->chanspec_return = wlc_next_chanspec(wlc->cmi,
				WLC_BAND_PI_RADIO_CHANSPEC, CHAN_TYPE_ANY, 1);
		}
	}
	/* else, we are interrupting a scan, but not associated, so use the original return channel
	 */

	if (!scan_timer_set) {
		/* call wlc_scantimer to get the scan state machine going */
		/* DUALBAND - Don't call wlc_scantimer() directly from DPC... */
		wl_add_timer(wlc->wl, scan_info->timer, 0, 0);
	} else if (ACT_FRAME_IN_PROGRESS(wlc->scan)) {
		/* send out AF as soon as possible to aid reliability of GON */
		wl_del_timer(wlc->wl, scan_info->timer);
		wl_add_timer(wlc->wl, scan_info->timer, 0, 0);
	}
...
wlc_scan_abort(wlc_scan_info_t *wlc_scan_info, int status)
{
	...
	/* TOT indicates this is required for Vista only */
	wlc_mac_event(wlc, WLC_E_SCAN_COMPLETE, NULL, WLC_E_STATUS_ABORT, 0, 0, NULL, 0);
}
...
wlc_scan_terminate(wlc_scan_info_t *wlc_scan_info, int status)
{
	 ...
	 */
	if (wlc->PM == PM_FAST) {
		if (wlc->ibss_bsscfgs == 0 && wlc->stas_connected > 0) {
...
wlc_scan_prepare_tx(wlc_info_t *wlc, wlc_scan_info_t *wlc_scan_info)
{
	...
	if (!wlc_tx_suspended(wlc)) {
		if ((wlc_scan_info->state & SCAN_STATE_PSPEND) == 0) {
...
wlc_scan_prepare_off_channel(wlc_info_t *wlc)
{
	...
	wlc_mhf(wlc, MHF2, MHF2_SKIP_CFP_UPDATE | MHF2_SKIP_ADJTSF,
	        MHF2_SKIP_CFP_UPDATE | MHF2_SKIP_ADJTSF, WLC_BAND_ALL);

	if (AP_ACTIVE(wlc)) {
		WL_SCAN(("wl%d: %s: disable AP(s)\n", wlc->pub->unit, __FUNCTION__));
...
wlc_scan_prepare_pm_mode(wlc_info_t *wlc, wlc_scan_info_t *wlc_scan_info)
{
	...
	if (wlc->stas_connected > 0 &&
	    !wlc->PMenabled &&
	    !wlc->exptime_cnt && !SCAN_TYPE_FOREGROUND(scan_info)) {
		...
		/* set PMblocked to prevent PS mode from being re-enabled */
		wlc->PMblocked = TRUE;
...
wlc_scantimer(void *arg)
{
	...
	if (DEVICEREMOVED(wlc)) {
		...
		if (SCAN_IN_PROGRESS(wlc_scan_info)) {
			...
			if (scan_cfg && !BSSCFG_HAS_NOIF(scan_cfg))
				wlc_bss_mac_event(wlc, scan_cfg, WLC_E_SCAN_COMPLETE, NULL,
				                  WLC_E_STATUS_ABORT,
				                  0, 0, NULL, 0);
			else
				wlc_mac_event(wlc, WLC_E_SCAN_COMPLETE, NULL, WLC_E_STATUS_ABORT,
				              0, 0, NULL, 0);
		}
			wl_down(wlc->wl);
		return;
...
#endif
	passes = wlc_quiet_chanspec(wlc->cmi, chanspec) ? 1 : scan_info->npasses;

	if (scan_info->pass > passes) {
		if (scan_info->pass == passes + 1) {
			/* scan passes complete for the current channel */
			WL_SCAN(("wl%d: %s: %sscanned channel %d, %d total responses\n",
			         wlc->pub->unit, __FUNCTION__,
		...
		if (scan_info->pass == (passes + 1) &&
			...
			wl_add_timer(wlc->wl, scan_info->timer, scan_info->home_time, 0);

			/* wlc_scantimer called back when scan_info->timer fires */
			return;
		}
	...
	if (scan_info->pass == WLC_SCAN_CHANNEL_PREP) {
		/* do off-home-channel setup if we are associated and on the home channel */
		if (!(wlc_scan_info->state & SCAN_STATE_READY) &&
		    (WLC_BAND_PI_RADIO_CHANSPEC == scan_info->chanspec_return)) {
#ifdef STA
	...
	if (scan_info->pass >= 1 && scan_info->pass <= passes) {
		...
		if (chanspec != WLC_BAND_PI_RADIO_CHANSPEC) {
			...
			wlc_enable_mac(wlc);
			WL_SCAN(("wl%d: %s: switched to channel %d\n", wlc->pub->unit, __FUNCTION__,
	 ...
	 */
	if (WL11D_ENAB(wlc) && wlc->awaiting_cntry_info)
		wlc_11d_scan_complete(wlc, WLC_E_STATUS_SUCCESS);
#endif /* WL11D */
#endif /* STA */
	...
	if (SCANCACHE_ENAB(wlc_scan_info) && !BSS_P2P_DISC_ENAB(wlc, scan_cfg)) {
	...
	/* scan completion event */
	{
		...
		if (scan_cfg && !BSSCFG_HAS_NOIF(scan_cfg))
			wlc_bss_mac_event(wlc, scan_cfg, WLC_E_SCAN_COMPLETE, NULL,
			WLC_E_STATUS_SUCCESS, 0, 0, NULL,
			0);
		else
			wlc_mac_event(wlc, WLC_E_SCAN_COMPLETE, NULL, WLC_E_STATUS_SUCCESS,
			0, 0, NULL, 0);
	}
#endif 
...
wlc_scan_return_home_channel(wlc_scan_info_t *wlc_scan_info)
{
	...
	wlc_suspend_mac_and_wait(wlc);
	if (wlc->chanspec != scan_info->chanspec_return) {
		wlc_set_chanspec(wlc, scan_info->chanspec_return);
		WL_SCAN(("wl%d: %s: switched to channel %d\n", wlc->pub->unit, __FUNCTION__,
		         wf_chspec_ctlchan(scan_info->chanspec_return)));
	} else {
		WL_SCAN(("wl%d: %s: already on return channel %d\n", wlc->pub->unit, __FUNCTION__,
		         wf_chspec_ctlchan(scan_info->chanspec_return)));
	}
	...
	wlc_mhf(wlc, MHF2, MHF2_SKIP_CFP_UPDATE | MHF2_SKIP_ADJTSF, 0, WLC_BAND_ALL);
#endif /* STA */
	...
	if (wlc->PMenabled) {
		...
		if (wlc->PM != PM_MAX || wlc->WME_PM_blocked) {
...
wlc_scan_radar_clear(wlc_scan_info_t *wlc_scan_info)
{
	...
	/* if there is not enough time remaining for a probe, do nothing */
	...
	wl_del_timer(wlc->wl, scan_info->timer);
	wl_add_timer(wlc->wl, scan_info->timer, active_time, 0);
	...
	int *channel_count)
{
	...
	if (wlc->pub->associated) {
		/* start the scan on the current BSS channel */
		chanspec_start = CH20MHZ_CHSPEC(wf_chspec_ctlchan(wlc->home_chanspec));
	} else if (SCAN_IN_PROGRESS(wlc_scan_info)) {
		/* interrupting a scan, so use the original return channel */
		chanspec_start = CH20MHZ_CHSPEC(wf_chspec_ctlchan(scan_info->chanspec_return));
	} else {
		/* not associated, so just start on the current channel */
		chanspec_start = CH20MHZ_CHSPEC(wf_chspec_ctlchan(WLC_BAND_PI_RADIO_CHANSPEC));
	}
	...
	chanspec_t chanspec_start, int channel_type)
{
	...
	ASSERT(CHSPEC_SB_NONE(chanspec_start));
...
wlc_scan_fifo_suspend_complete(wlc_scan_info_t *wlc_scan_info)
{
	...
	if (SCAN_IN_PROGRESS(wlc_scan_info) && (wlc_scan_info->state & SCAN_STATE_WSUSPEND)) {
		wlc_scan_info->state &= ~SCAN_STATE_WSUSPEND;
		wlc_scan_info->state |= SCAN_STATE_READY;
		wl_del_timer(wlc->wl, scan_info->timer);
		wl_add_timer(wlc->wl, scan_info->timer, 0, 0);
...
wlc_scan_pm_pending_complete(wlc_scan_info_t *wlc_scan_info)
{
	...
	if (SCAN_IN_PROGRESS(wlc_scan_info) && (wlc_scan_info->state & SCAN_STATE_PSPEND)) {
#if defined(EXTENDED_SCAN) && defined(BCMDBG)
		if (wlc->PMpending || scan_info->test_nopsack)
#else
		if (wlc->PMpending)
#endif
		{
...
wlc_scan_do_pass(scan_info_t *scan_info, chanspec_t chanspec)
{
	...
	if (passive_scan || wlc_quiet_chanspec(wlc->cmi, chanspec)) {
		...
		WL_SCAN(("wl%d: setting passive scantimer to %d ms, channel %d\n",
			wlc->pub->unit, channel_dwelltime, (CHSPEC_CHANNEL(chanspec))));

		wl_add_timer(wlc->wl, scan_info->timer, channel_dwelltime, 0);
		wlc_read_tsf(wlc, &scan_info->start_tsf, &dummy_tsf_h);
	}
	else {
		channel_dwelltime = CHANNEL_ACTIVE_DWELLTIME(scan_info)/scan_info->npasses;

		WL_SCAN(("wl%d: setting active scantimer to %d ms, channel %d\n",
			wlc->pub->unit, channel_dwelltime, (CHSPEC_CHANNEL(chanspec))));
#ifdef WIFI_ACT_FRAME
		if (WIFI_ACT_FRAME_ENAB(wlc->pub) && ACT_FRAME_IN_PROGRESS(wlc->scan)) {
			if (wlc->scan->action_frame) {
				WL_SCAN(("wl%d: sending action frame to %d ms, channel %d\n",
				wlc->pub->unit, channel_dwelltime, (CHSPEC_CHANNEL(chanspec))));
				wlc_tx_action_frame_now(wlc, scan_info->bsscfg,
					wlc->scan->action_frame);
				wlc->scan->action_frame = NULL;
			}
		}
		else
	...
	wlc_noise_sample_request(wlc, WLC_NOISE_REQUEST_SCAN, CHSPEC_CHANNEL(chanspec));
}
...
wlc_scan_ssid_match(wlc_scan_info_t *scan_pub, bcm_tlv_t *ssid_ie, bool filter)
{
	...
	if ((scan_pub->ccx_rm_scan_active) ||
...
bool
wlc_scan_parse_bcn_prb(wlc_scan_info_t *si, bool beacon, wlc_bss_info_t *bi,
	d11rxhdr_t *rxh, uint8 *body, int body_len)
{
	...
	if (P2P_ENAB(wlc->pub) &&
	    wlc_p2p_parse_bcn_prb(wlc->p2p, scan_info->bsscfg, beacon, &bi->rateset,
	                          rxh, body, body_len) != BCME_OK) {
...
wlc_ht_obss_scan_update(scan_info_t *scan_info, int status)
{
	...
	if ((status != WLC_SCAN_SUCCESS) && !wlc->obss->scan_countdown) {
		wlc->obss->scan_countdown = 5;
		return;
	}
	...
	if (wlc->stas_associated > 0 && COEX_ACTIVE(wlc)) {
		/* valid scan, analysis the result */
		wlc_ht_coex_filter_scanresult(wlc);
		wlc->obss->scan_countdown = params->bss_widthscan_interval;
...
wlc_scan_fill_cache(scan_info_t *scan_info, uint current_timestamp)
{
	...
	for (index = 0; index < scan_results->count; index++) {
		...
		if (data == NULL || bi_len > datalen) {
			...
			WL_TMP(("SCANCACHE: allocate a new buf sz %d for bi_len %d\n",
			        (int)datalen, (int)bi_len));
...
#ifdef WLLMAC
	if (LMAC_ENAB(wlc->pub)) {
		if (extdscan_params->nprobes &&
			!wlc->lmac_info->lmac_templates[TEMPLATE_PRBREQ_FRAME]->length)
		{
			WL_ERROR(("%s: %d\n", __FUNCTION__, __LINE__));
			bcmerror = BCME_ERROR;
			goto done;
		}
		/* convert the lmac rate to the rspec */
		if (extdscan_params->tx_rate == 0) {
			WL_ERROR(("txrate is specified as 0\n"));
			bcmerror = BCME_ERROR;
			goto done;
		}
	}
...
#ifdef WLLMAC
		if (LMAC_ENAB(wlc->pub)) {
			WL_ERROR(("Channel list is empty\n"));
			bcmerror = BCME_BADARG;
			goto done;
		}
		else
...
wlc_scan_dump(scan_info_t *si, struct bcmstrbuf *b)
{
	...
	bcm_format_flags(scan_flags, si->scan_pub->state, state_str, 64);

	if (si->scan_pub->bss_type == DOT11_BSSTYPE_INFRASTRUCTURE)
		bss_type_str = "Infra";
	else if (si->scan_pub->bss_type == DOT11_BSSTYPE_INDEPENDENT)
		bss_type_str = "IBSS";
	else
		bss_type_str = "any";

	bcm_bprintf(b, "in_progress %d SSID \"%s\" type %s BSSID %s state 0x%x %s\n",
	            si->scan_pub->in_progress, ssidbuf, bss_type_str,
	bcm_ether_ntoa(&si->scan_pub->bssid, eabuf),
	            si->scan_pub->state, state_str);

	bcm_bprintf(b, "rm_scan %d extdscan %d\n",
	            si->scan_pub->ccx_rm_scan_active, si->extdscan);

	bcm_bprintf(b, "away_channels_cnt %d pass %d scan_results %d\n",
	            si->away_channels_cnt, si->pass, si->wlc->scan_results->count);

	if (SCAN_IN_PROGRESS(si->scan_pub))
		bcm_bprintf(b, "chanspec_return: %x chanspec_current %x\n",
		            si->chanspec_return, si->chanspec_list[si->channel_idx]);
