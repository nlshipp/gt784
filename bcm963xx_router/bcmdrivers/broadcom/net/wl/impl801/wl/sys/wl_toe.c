 ...
 *
 * $Id: wl_toe.c,v 1.1 2010/08/05 21:59:01 ywu Exp $
 *
...
BCMATTACHFN(wl_toe_attach)(wlc_info_t *wlc)
{
	...
	toei = MALLOC(wlc->osh, sizeof(wl_toe_info_t));
	if (!toei) {
		WL_ERROR(("wl%d: wl_toe_attach: MALLOC failed, malloced %d bytes\n",
		          wlc->pub->unit, MALLOCED(wlc->osh)));
		return NULL;
	}

	/* init toe private info struct */
	bzero(toei, sizeof(wl_toe_info_t));
	toei->wlc = wlc;

	/* register module */
	wlc_module_register(wlc->pub, toe_iovars, "toe", toei, toe_doiovar, NULL, NULL);
            ...
            void *p, uint plen, void *a, int alen, int vsize, struct wlc_if *wlcif)
{
	...
	if ((err = wlc_iovar_check(toei->wlc->pub, vi, a, alen, IOV_ISSET(actionid))) != 0)
		return err;
                ...
                struct ipv4_hdr **iph_ptr, int *ipl_ptr)
{
	...
	if (length < ETHER_HDR_LEN) {
		WL_ERROR(("wl%d: wl_toe_parse_ip: short eth frame (%d)\n",
		          WLCUNIT(toei), length));
		return -1;
		...
		WL_INFORM(("wl%d: wl_toe_parse_ip: 802.3 LLC/SNAP\n", WLCUNIT(toei)));
		pt = frame + ETHER_HDR_LEN + SNAP_HDR_LEN;
	} else {
		WL_ERROR(("wl%d: wl_toe_parse_ip: non-SNAP 802.3 frame\n",
		          WLCUNIT(toei)));
		return -1;
	}
	...
	if (ethertype == ETHER_TYPE_8021Q) {
		...
		if (pt + ETHER_TYPE_LEN > frame + length) {
			WL_ERROR(("wl%d: wl_toe_parse_ip: short VLAN frame (%d)\n",
			          WLCUNIT(toei), length));
			return -1;
		}
	...
	if (ethertype != ETHER_TYPE_IP) {
		WL_ERROR(("wl%d: wl_toe_parse_ip: non-IP frame (ethertype 0x%x, length %d)\n",
		          WLCUNIT(toei), ethertype, length));
		return -1;
	}
	...
	WL_INFORM(("wl%d: wl_toe_parse_ip: IP len %d\n", WLCUNIT(toei), ipl));

	/* We support IPv4 only */
	if (ipl < IPV4_OPTIONS_OFFSET || (IP_VER(iph) != IP_VER_4)) {
		WL_ERROR(("wl%d: wl_toe_parse_ip: short frame (%d) or non-IPv4\n",
		          WLCUNIT(toei), ipl));
		return -1;
	}
	...
	if (ihl < IPV4_OPTIONS_OFFSET || ihl > ipl) {
		WL_ERROR(("wl%d: wl_toe_parse_ip: IP-header-len (%d) out of range (%d-%d)\n",
		          WLCUNIT(toei), ihl, IPV4_OPTIONS_OFFSET, ipl));
		return -1;
	}
	...
	if (ippktlen < ipl) {
		WL_ERROR(("wl%d: wl_toe_parse_ip: extra frame length ignored (%d)\n",
		          WLCUNIT(toei), ipl - ippktlen));
		ipl = ippktlen;
	} else if (ippktlen > ipl) {
		WL_ERROR(("wl%d: wl_toe_parse_ip: truncated IP packet (%d)\n",
		          WLCUNIT(toei), ippktlen - ipl));
		return -1;
	}
	...
	if ((iph_frag & IPV4_FRAG_MORE) || (iph_frag & IPV4_FRAG_OFFSET_MASK) != 0) {
		WL_INFORM(("wl%d: wl_toe_parse_ip: IP fragment not handled\n",
		           WLCUNIT(toei)));
		return -1;
	}
...
wl_toe_recv_proc(wl_toe_info_t *toei, void *sdu)
{
...
#ifdef BCMDBG
	if (WL_PRPKT_ON())
		prpkt("wl_toe_recv_proc: MPDU", WLCOSH(toei), sdu);
#endif
	...
	if (csum_inet(iph, ihl) != 0) {
		WL_ERROR(("wl%d: wl_toe_recv_proc: bad ip-hdr rx csum\n", WLCUNIT(toei)));
	...
	WL_INFORM(("wl%d: wl_toe_recv_proc: good ip-hdr rx csum\n", WLCUNIT(toei)));
	...
	if (prot == IP_PROT_TCP) {
		...
		if (csum != 0) {
			WL_ERROR(("wl%d: wl_toe_recv_proc: bad tcp csum 0x%x\n",
			          WLCUNIT(toei), csum));
			PKTSETSUMGOOD(sdu, FALSE);
		...
		WL_INFORM(("wl%d: wl_toe_recv_proc: good tcp-seg csum\n", WLCUNIT(toei)));
		...
		if (csum != 0) {
			WL_ERROR(("wl%d: wl_toe_recv_proc: bad udp csum 0x%x\n",
			          WLCUNIT(toei), csum));
			PKTSETSUMGOOD(sdu, FALSE);
		...
		WL_INFORM(("wl%d: wl_toe_recv_proc: good udp datagram csum\n", WLCUNIT(toei)));
		...
		if (csum_inet(icmph, (uint16)icmpl) != 0) {
			WL_ERROR(("wl%d: wl_toe_recv_proc: bad icmp csum\n", WLCUNIT(toei)));
			PKTSETSUMGOOD(sdu, FALSE);
		...
		WL_INFORM(("wl%d: wl_toe_recv_proc: good icmp csum\n", WLCUNIT(toei)));
	...
	WL_INFORM(("wl%d: wl_toe_recv_proc: sums verified\n", WLCUNIT(toei)));
