 ...
 *
 * $Id: wlc_rpctx.c,v 1.1 2010/08/05 21:59:02 ywu Exp $
 */
...
struct rpctx_info {
	...
	uint8 pktpend[NFIFO];		/* Maintain separate counter as AMPDU does
...
void
wlc_rpctx_txpktpenddec(rpctx_info_t *rpctx, uint fifo, uint8 val)
{
	rpctx->pktpend[fifo] -= val;
}
...
wlc_rpctx_getnexttxp(rpctx_info_t *rpctx, uint fifo)
{
	...
	p = pktdeq(&rpctx->txq[fifo]);

#if defined(BCM_RPC_NOCOPY) || defined(BCM_RPC_TXNOCOPY)
	/* Restore pkt length without padding for retransmission */
	PKTSETLEN(rpctx->pub->osh, p, WLPKTTAG(p)->unpadded_len);
	WLPKTTAG(p)->unpadded_len = 0;	/* clean up the used pkttag field */
...
#if defined(BCM_RPC_NOCOPY) || defined(BCM_RPC_TXNOCOPY)
	ASSERT(WLPKTTAG(p)->unpadded_len != 0);
#endif /* BCM_RPC_NOCOPY || BCM_RPC_TXNOCOPY */
...
wlc_rpctx_tx(rpctx_info_t *rpctx, uint fifo, void *p, bool commit, uint16 frameid, uint8 txpktpend)
{
...
#if defined(BCM_RPC_NOCOPY) || defined(BCM_RPC_TXNOCOPY)
	ASSERT(WLPKTTAG(p)->unpadded_len == 0);
	WLPKTTAG(p)->unpadded_len = totlen;
	...
	if (rpc_totlen - totlen) {
		memset(buf + totlen, 0, (rpc_totlen - totlen));
	}

	/* set the right length(do NOT include padding) before next PKTPUSH */
	PKTSETLEN(rpctx->pub->osh, p, totlen);
	...
	err = bcm_xdr_pack_uint32(&b, WLPKTTAG(p0)->flags); ASSERT(!err);
	err = bcm_xdr_pack_uint32(&b, WLPKTTAG(p0)->exptime); ASSERT(!err);
