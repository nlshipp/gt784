 ...
 *
 * $Id: wlc_scan.h,v 1.1 2010/08/05 21:59:02 ywu Exp $
 */
#ifndef _WLC_SCAN_H_
#define _WLC_SCAN_H_
...
#define WLC_SCAN_PASSIVE_TIME	250	/* ms to listen on a channel for beacons for passivescan */
...
struct wlc_scan_info {
	...
	bool		ccx_rm_scan_active;	/* rm state machine using scan */
	struct ether_addr bssid;
#ifdef WLSCANCACHE
	bool		_scancache;		/* scan cache enable */
#endif
#ifdef WIFI_ACT_FRAME
	int32 		act_frame_state;	/* to support action frame off home channel */
	void *		action_frame;	/* action frame for off channel */
#endif
};

#ifdef WIFI_ACT_FRAME
#define ACT_FRAME_DISABLED	0
#define ACT_FRAME_ENABLED	1
#define ACT_FRAME_IN_PROGRESS(scan_info) 	((scan_info)->act_frame_state == ACT_FRAME_ENABLED)
#else
#define ACT_FRAME_IN_PROGRESS(scan_info)  0
#endif

#define SCAN_IN_PROGRESS(scan_info)	((scan_info)->in_progress)
...
extern wlc_scan_info_t* BCMINITFN(wlc_scan_attach)(wlc_info_t *wlc, void *wl, osl_t *osh, uint);
extern void BCMUNINITFN(wlc_scan_detach)(wlc_scan_info_t *scan_ptr);
...
extern void wlc_scantimer(void *arg);
...
extern bool wlc_scan_parse_bcn_prb(wlc_scan_info_t *si, bool beacon, wlc_bss_info_t *bi,
