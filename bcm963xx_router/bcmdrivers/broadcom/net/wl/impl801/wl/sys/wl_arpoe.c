 ...
 *
 * $Id: wl_arpoe.c,v 1.1 2010/08/05 21:59:01 ywu Exp $
 */
	...
	for (i = 0; i < ARP_TABLE_SIZE; i++) {
		if (ARP_ENTRY_VALID()) {
			...
			if (arpi->arp_table[i].age == 0) {
				WL_INFORM(("wl%d: arp_watchdog: aged out ipaddr %s index %d\n",
				           WLCUNIT(arpi),
...
BCMATTACHFN(wl_arp_attach)(wlc_info_t *wlc)
{
	...
	if (!arpi) {
		WL_ERROR(("wl%d: wl_arp_attach: MALLOC failed; total mallocs %d bytes\n",
	...
	/* register module */
	wlc_module_register(wlc->pub, arp_iovars, "arp", arpi, arp_doiovar, arp_watchdog, NULL);
            ...
            void *p, uint plen, void *a, int alen, int vsize, struct wlc_if *wlcif)
{
	...
	if ((err = wlc_iovar_check(arpi->wlc->pub, vi, a, alen, IOV_ISSET(actionid))) != 0)
		return err;
...
savehostipmac(wl_arp_info_t *arpi, uint8 *ipa, uint8 *ea)
{
	...
	if (bcmp(&arpi->host_mac, ea, ETHER_ADDR_LEN) != 0) {
		WL_ERROR(("wl%d: savehostipmac: not adding host IP with different MAC\n",
		          WLCUNIT(arpi)));
	...
	WL_ERROR(("wl%d: savehostipmac: overflow\n", WLCUNIT(arpi)));
...
arp_reply_host(wl_arp_info_t *arpi, struct bcmarp *arp_req, struct ether_addr *dst_mac, bool snap)
{
	...
	if (!(pkt = PKTGET(WLCOSH(arpi), pktlen + ALIGN_ADJ_BUFLEN, FALSE))) {
	 ...
	 */
	PKTPULL(WLCOSH(arpi), pkt, ALIGN_ADJ_BUFLEN);
	...
	wl_sendup(arpi->wlc->wl, NULL, pkt, 1);
	arpi->fake_arp_reply = FALSE;
...
arp_reply_peer(wl_arp_info_t *arpi, struct bcmarp *arp_req, bool snap)
{
	...
	if (!(pkt = PKTGET(WLCOSH(arpi), pktlen, TRUE))) {
		WL_ERROR(("wl%d: arp_reply_peer: alloc failed; dropped\n",
		          WLCUNIT(arpi)));
		WLCNTINCR(arpi->wlc->pub->_cnt->rxnobuf);
		return -1;
	}
             ...
             struct bcmarp **arp_out, bool *snap_out)
{
	...
	if (length < ETHER_HDR_LEN) {
		WL_ERROR(("wl%d: wl_arp_parse: short eth frame (%d)\n",
		          WLCUNIT(arpi), length));
		return -1;
		...
		WL_ERROR(("wl%d: wl_arp_parse: non-SNAP 802.3 frame\n",
		          WLCUNIT(arpi)));
		return -1;
	}
	...
	if (ethertype == ETHER_TYPE_8021Q) {
		...
		if (pt + ETHER_TYPE_LEN > frame + length) {
			WL_ERROR(("wl%d: wl_arp_parse: short VLAN frame (%d)\n",
			          WLCUNIT(arpi), length));
			return -1;
		}
	...
	if (arplen > ARP_DATA_LEN) {
		WL_ERROR(("wl%d: wl_arp_parse: extra frame length ignored (%d)\n",
		          WLCUNIT(arpi), arplen - ARP_DATA_LEN));
		arplen = ARP_DATA_LEN;
	} else if (arplen < ARP_DATA_LEN) {
		WL_ERROR(("wl%d: wl_arp_parse: truncated ARP packet (%d)\n",
		          WLCUNIT(arpi), arplen));
		return -1;
	}
	...
	if (arp->oper == HTON16(ARP_OPC_REQUEST)) {
		...
		case 0:
			...
			if (savearptable(arpi, arp->dst_ip, arp->dst_eth) != 0)
				WL_ERROR(("wl%d: wl_arp_send_proc: savearptable unsuccessful!\n",
				          WLCUNIT(arpi)));
		...
		WL_ERROR(("wl%d: wl_arp_send_proc: unknown ARP oper 0x%x\n",
		          WLCUNIT(arpi), ntoh16(arp->oper)));
		return -1;
	}
...
#define ARP_ACCEPT_UNKNOWN	ARP_OL_SNOOP
		...
		WL_ERROR(("wl%d: wl_arp_recv_proc: unknown ARP oper 0x%x\n",
		          WLCUNIT(arpi), ntoh16(arp->oper)));
		return -1;
	}
