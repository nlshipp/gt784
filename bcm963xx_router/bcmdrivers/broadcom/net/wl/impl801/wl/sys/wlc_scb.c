 ...
 *
 * $Id: wlc_scb.c,v 1.1 2010/08/05 21:59:02 ywu Exp $
 */
...
#define SCB_MAX_CUBBY		12	/* max number of cubbies in container */
...
#if defined(BCMDBG)
/* Count for magic handle in scb for pkttag pointer validation */
static uint16 _scb_magic_cnt;
#endif
...
int
BCMINITFN(wlc_scb_cubby_reserve)(wlc_info_t *wlc, uint size, scb_cubby_init_t fn_init,
...
wlc_scbfree(wlc_info_t *wlc, struct scb *scbd)
{
...
#ifdef WLBTAMP_OFF
	/* need to cleanup user of this scb */
	wlc_bta_scb_cleanup(wlc->bta, scbd);
#endif
...
#if defined(BCMDBG)
	/* When freeing, invalidate the magic */
	scbd->_dbgid = 0;
#endif
	...
	if (scbd->wds != NULL) {
		...
		MFREE(wlc->osh, scbd->wds, sizeof(wlc_if_t));
		scbd->wds = NULL;
	}
	...
	if (!SCB_INTERNAL(scbd)) {
		if (!ETHER_ISMULTI(scbd->ea.octet)) {
			...
			scbinfo = (scbstate->scbhash[remove->band->bandunit][index] ?
				SCBINFO(scbstate->scbhash[remove->band->bandunit][index]) : NULL);
			ASSERT(scbinfo != NULL);
			...
			if (scbinfo == remove) {
				...
				scbstate->scbhash[remove->band->bandunit][index] =
...
wlc_scb_setstatebit(struct scb *scb, uint8 state)
{
	...
	if (state & AUTHORIZED)
	{
		ASSERT(((scb->state | state) & ASSOCIATED) || SCB_WDS(scb) ||
			SCB_IS_IBSS_PEER(scb) || BSSCFG_DPT_ENAB(scb->bsscfg));
	}
	scb->state |= state;
...
wlc_scb_ratesel_init(wlc_info_t *wlc, struct scb *scb)
{
	...
	is40 = ((scb->flags & SCB_IS40) ? TRUE : FALSE) && wlc->band->mimo_cap_40 &&
		(CHSPEC_WLC_BW(wlc->chanspec) == WLC_40_MHZ);
	...
	txc_ptr = wlc_scb_txcptr_to_invalidate_txc(wlc, scb);
	...
	for (ac = 0; ac < WME_MAX_AC(wlc, scb); ac++) {
		wlc_ratesel_init(wlc->rsi, scb, &scb->rateset, is40, sgi_tx,
			...
			wlc->afterburner,
#endif /* WLAFTERBURNER */
			txc_ptr, active_antcfg_num, antselid_init, ac);
	}
}
...
wlc_scb_set_bsscfg(struct scb *scb, wlc_bsscfg_t *cfg)
{
	...
	if (oldcfg == NULL || oldcfg != cfg) {
		...
		/* invalidate txc */
		if ((invtxc = wlc_scb_txcptr_to_invalidate_txc(wlc, scb)) != NULL)
			*invtxc = 0;
...
_wlc_scbfind(wlc_info_t *wlc, const struct ether_addr *ea, int bandunit)
{
	...
	scbinfo = (wlc->scbstate->scbhash[bandunit][index] ?
	           SCBINFO(wlc->scbstate->scbhash[bandunit][index]) : NULL);
	for (; scbinfo; scbinfo = scbinfo->hashnext) {
		SCBSANITYCHECK(scbinfo->scbpub);

		if (bcmp((const char*)ea, (const char*)&(scbinfo->scbpub->ea), ETHER_ADDR_LEN) == 0)
			break;
	}
...
_wlc_scblookup(wlc_info_t *wlc, const struct ether_addr *ea, int bandunit)
{
	...
	index = SCBHASHINDEX(ea->octet);
	scbinfo = (scbstate->scbhash[bandunit][index] ?
	           SCBINFO(scbstate->scbhash[bandunit][index]) : NULL);
...
wlc_dump_scb(wlc_info_t *wlc, struct bcmstrbuf *b)
{
	...
	FOREACHSCB(wlc->scbstate, &scbiter, scb) {
		...
		if (scb->bsscfg)
			bcm_bprintf(b, " Cfg:%d(%p)", WLC_BSSCFG_IDX(scb->bsscfg), scb->bsscfg);
		...
		if (BSSCFG_AP(scb->bsscfg)) {
			...
			bcm_bprintf(b, " BSS %d \"%s\"\n",
			            WLC_BSSCFG_IDX(scb->bsscfg), ssidbuf);
		}
#endif
...
wlc_wds_create(wlc_info_t *wlc, struct scb *scb, uint flags)
{
	...
	scb->wds = (wlc_if_t*)MALLOC(wlc->osh, sizeof(wlc_if_t));
	if (scb->wds == NULL) {
		WL_ERROR(("wl%d: wlc_wds_create: out of memory, malloced %d bytes\n",
		          wlc->pub->unit, MALLOCED(wlc->osh)));
		return BCME_NOMEM;
	}

	bzero(scb->wds, sizeof(wlc_if_t));
