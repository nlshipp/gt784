 ...
 *
 * $Id: wlc_cram.c,v 1.1 2010/08/05 21:59:02 ywu Exp $
 */
#ifndef CRAM
...
BCMATTACHFN(wlc_cram_detach)(cram_info_t *cram)
{
	...
	wlc_module_unregister(cram->wlc->pub, "cram", cram);
	MFREE(cram->wlc->osh, cram, sizeof(cram_info_t));
}
                 ...
                 void *p, uint plen, void *a, int alen, int vsize, struct wlc_if *wlcif)
{
	...
	if ((err = wlc_iovar_check(crami->wlc->pub, vi, a, alen, IOV_ISSET(actionid))) != 0)
		return err;
...
wlc_cram_open(cram_info_t *crami, struct ether_addr *ea)
{
	...
	*subtype = hton16(ETHER_BRCM_CRAM);
	...
	/* start timer */
	wlc_hwtimer_gptimer_set(wlc, WLC_CRAM_MAXTIME);
