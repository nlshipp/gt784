 ...
 *
 * $Id: wlc_bmac.c,v 1.1 2010/08/05 21:59:01 ywu Exp $
 */
...
#ifdef DSLCPE_DELAY
extern void wl_deferred_dpc(void *wl);
...
static uint16 xmtfifo_sz[][NFIFO] = {
	...
	{ 9, 58, 22, 14, 14, 5 },    /* corerev 22: 5120, 49152, 49152, 5376, 4352, 1280 */
#ifdef MACOSX
	/* Give more bandwidth to BK traffic as Apple wants to Aggregate */
	...
	{ 98, 255, 243, 21, 17, 5 },	/* corerev 26: 24990, 65280, 62208, 5376, 4352, 1280 */
	...
	{ 9, 58, 22, 14, 14, 5 }	/* corerev 30: 2304, 14848, 5632, 3584, 3584, 1280 */
};


/* corerev 26 host agg fifo size: 24990, 65280, 62208, 5376, 4352, 1280 */
static uint16 xmtfifo_sz_hostagg[] = { 98, 255, 243, 21, 17, 5 };
/* corerev 26 hw agg fifo size: 24990, 76880, 51200, 5120, 4352, 1280 */
static uint16 xmtfifo_sz_hwagg[] = { 98, 418, 80, 21, 17, 5 };
...
#ifdef DSLCPE_NAPI
static bool wlc_bmac_recv(wlc_hw_info_t *wlc_hw, uint fifo, bool bound, int work_todo, int *done);
#else
...
static void wlc_ucode_bsinit(wlc_info_t *wlc);
...
static uint16 wlc_read_ihr(wlc_hw_info_t *wlc_hw, uint offset);
static void wlc_write_ihr(wlc_hw_info_t *wlc_hw, uint offset, uint16 v);
#endif
static uint32 wlc_wlintrsoff(wlc_hw_info_t *wlc_hw);
static void wlc_wlintrsrestore(wlc_hw_info_t *wlc_hw, uint32 macintmask);
#ifdef BCMDBG
static bool wlc_intrs_enabled(wlc_hw_info_t *wlc);
#endif /* BCMDBG */
...
static void wlc_btcx_btcflag2ucflag(wlc_hw_info_t *wlc_hw);
...
static void
WLBANDINITFN(wlc_ucode_bsinit)(wlc_info_t *wlc)
{
	wlc_hw_info_t *wlc_hw = wlc->hw;
	...
	if (D11REV_IS(wlc_hw->corerev, 30)) {
	...
	} else if (D11REV_IS(wlc_hw->corerev, 26) || D11REV_IS(wlc_hw->corerev, 29)) {
		...
		if (WLCISSSLPNPHY(wlc_hw->band)) {
			...
			WL_ERROR(("wl%d: supported phy in corerev 21\n", wlc_hw->unit));
		}
		else
			WL_ERROR(("wl%d: unsupported phy in corerev 21\n", wlc_hw->unit));
	} else if (D11REV_IS(wlc_hw->corerev, 20) && WLCISSSLPNPHY(wlc->band)) {
...
void wlc_bmac_recv_task(void *arg)
{
...
#ifdef DSLCPE_NAPI
	int done;
	if (wlc_bmac_recv(wlc_hw, RX_FIFO, TRUE, RXBND, &done))
#else
	if (wlc_bmac_recv(wlc_hw, RX_FIFO, TRUE))
#endif
		wl_schedule_workqueue_task(wlc->bmac_rx_task);

	/* send any enq'd tx packets. Just makes sure to jump start tx */
	if (!pktq_empty(&wlc->txq))
		wlc_send_q(wlc, &wlc->txq);
	
	wlc->bmac_rx_task_dispatched = FALSE;
}
#endif  /* DSLCPE_RX_DELAY */
...
#ifdef DSLCPE_NAPI
wlc_bmac_recv(wlc_hw_info_t *wlc_hw, uint fifo, bool bound, int work_todo, int *done)
#else
...
#endif
{
	...
	uint bound_limit = bound ? (wlc_hw->wlc->rx_bound) : -1;
#else
...
#if (defined(WLC_LOW_ONLY) && defined(USB4322))
	uint32 mem_avail;
...
#endif /* (defined(WLC_LOW_ONLY) && defined(USB4322)) */

#ifdef DSLCPE_NAPI
	if (NAPI_ENAB(wlc_hw->wlc->pub))
		bound_limit = work_todo;
#endif
...
bool BCMFASTPATH
#ifdef DSLCPE_NAPI
wlc_dpc(wlc_info_t *wlc, bool bounded, int work_todo, int *done)
#else
...
#endif
{
	...
	*done = 0;
	...
	if (macintstatus & MI_BT_PRED_REQ)
		wlc_btcx_update_predictor(wlc, regs);
#endif
...
#ifdef AP
		if (wlc_bmac_processpmq(wlc->hw, bounded))
	...
	if (macintstatus & MI_TFS) {
		...
		#ifdef WLC_LOW_ONLY
		 /* rxf0ovfl is seen with rev22 when 40MHZ high throughput (512k window) */
			  if (D11REV_IS(wlc_hw->corerev, 22))
	...
	if (macintstatus & MI_DMAINT) {
		if ((BUSTYPE(wlc_hw->sih->bustype) == PCI_BUS) &&
			...
			dma_rxfill(wlc_hw->di[RX_FIFO]);
#ifndef DSLCPE_RX_DELAY
#ifdef DSLCPE_NAPI
		} else if (wlc_bmac_recv(wlc_hw, RX_FIFO, bounded, work_todo, done)) {
#else
		} else if (wlc_bmac_recv(wlc_hw, RX_FIFO, bounded)) {
#endif
#else
		} else if (wlc->enable_rx_delay==FALSE) {
#ifdef DSLCPE_NAPI
			if (wlc_bmac_recv(wlc_hw, RX_FIFO, bounded, work_todo, done)) {
#else
			...
			if ( wlc->bmac_rx_task_dispatched == FALSE) {
				wlc->bmac_rx_task_dispatched = TRUE;
				wl_schedule_workqueue_task(wlc->bmac_rx_task);
			}
		}
#endif /* DSLCPE_RX_DELAY */
	...
	if (macintstatus & MI_TXSTOP) {
		if (wlc_bmac_tx_fifo_suspended(wlc_hw, TX_DATA_FIFO)) {
			...
			if (wlc->rm_state->step == WLC_RM_WAIT_TX_SUSPEND)
				wl_add_timer(wlc->wl, wlc->rm_timer, 0, 0);
#endif
...
#ifdef WLC_HIGH
			bool health = wlc_dump_ucode_fatal(wlc);
#ifdef BCMDBG
			if (health)
				WL_ERROR(("Spurious watchdog, ucode looks clean!\n"));
			else
				WL_ERROR(("Genuine watchdog, ucode looks bad!\n"));
#endif /* BCMDBG */
#endif /* WLC_HIGH */

#ifndef BCMDBG
			WL_ERROR(("wl%d: PSM ucode watchdog fired at %d (seconds). Resetting.\n",
...
#ifdef WLC_HIGH
			if (!((wlc_hw->sih->chip == BCM4321_CHIP_ID) &&
				(wlc_hw->sih->chiprev == 0))) {
				WLC_EXTLOG(wlc, LOG_MODULE_COMMON, FMTSTR_REG_PRINT_ID,
					WL_LOG_LEVEL_ERR, 0, R_REG(wlc_hw->osh, &regs->psmdebug),
					"psmdebug");
				WLC_EXTLOG(wlc, LOG_MODULE_COMMON, FMTSTR_REG_PRINT_ID,
					WL_LOG_LEVEL_ERR, 0, R_REG(wlc_hw->osh, &regs->phydebug),
					"phydebug");
				WLC_EXTLOG(wlc, LOG_MODULE_COMMON, FMTSTR_REG_PRINT_ID,
					WL_LOG_LEVEL_ERR, 0, R_REG(wlc_hw->osh, &regs->psm_brc),
					"psm_brc");
...
#endif
#ifdef STA
		/* gptimer is used by both the AP CRAM and the STA PM2 features.
		 * However, the two uses are mutually exclusive because AP and
		 * APSTA builds should never enter power management mode PM2.
		 */
		if (wlc->PM == PM_FAST) {
			wlc_pm2_timeout(wlc);
		}
...
#if defined(DSLCPE) && (defined(CONFIG_BRCM_IKOS) || defined(DSLCPE_CANNEDTX))
	if (macintstatus & MI_TBTT) {	
		extern void wlc_emu_pkt_create(wlc_info_t *wlc);
		wlc_emu_pkt_create(wlc);		
	}
	...
	if (P2P_ENAB_HW(wlc_hw) &&
	     (macintstatus & MI_P2P))
		wlc_p2p_int_proc(wlc->p2p);
#endif

#ifdef DSLCPE_RX_DELAY
	if (!wlc->enable_rx_delay)
#endif /* DSLCPE_RX_DELAY */
	/* send any enq'd tx packets. Just makes sure to jump start tx */
	if (!pktq_empty(&wlc->txq))
		wlc_send_q(wlc, &wlc->txq);
...
#define MACINTMASK_BMAC \
	(MI_TFS | MI_ATIMWINEND |  MI_PHYTXERR | MI_DMAINT | MI_BG_NOISE | MI_CCA | MI_GP0 | MI_PMQ)
...
{
#ifdef WLC_LOW_ONLY
	bmac_pmq_entry_t * entry = wlc_hw->bmac_pmq->entry;
	...
	bool commit, uint16 frameid, uint8 txpktpend)
{
	...
	uint8 *plcp;
	uint16 phyctl;
	int16 mcs_adjustment = 0;
	int8 mcs_pwr_this_rate = 0;
	int8 legacy_pwr_this_rate = 0;
#endif
	...
	if (WLCISSSLPNPHY(wlc_hw->band)) {
		if (IS_SINGLE_STREAM(plcp[0] & MIMO_PLCP_MCS_MASK)) {
			...
			if (mcs_adjustment < -32)
				mcs_adjustment = -32;
			if (CHSPEC_IS40(wlc_hw->chanspec)) {
				mcs_adjustment = mcs_adjustment + 32;
			}
		} else {
			/* we need to be 40MHZ for not any MCS frames */
			if (CHSPEC_IS40(wlc_hw->chanspec))
				mcs_adjustment = 32;
		}
	...
	if (WLPKTTAG(p)->flags & WLF_AMPDU_MPDU) {
		...
		txh = (d11txh_t *)PKTDATA(wlc_hw->wlc->osh, p);
...
wlc_bmac_revinfo_get(wlc_hw_info_t *wlc_hw, wlc_bmac_revinfo_t *revinfo)
{
	...
	revinfo->btc_wire = wlc_hw->btc_wire;
...
BCMATTACHFN(wlc_bmac_attach_dmapio)(wlc_hw_info_t *wlc_hw, uint j, bool wme)
{
	...
	if (PIO_ENAB(wlc->pub)) {
		...
		wlc_hw->mem_required_lower = wlc_hw->mem_required_def -
			(wlc_hw->mem_required_def >> 2) + wlc_hw->wlc->pub->tunables->memreserved;
		wlc_hw->mem_required_least = (wlc_hw->mem_required_def >> 1) +
			wlc_hw->wlc->pub->tunables->memreserved;
		wlc_hw->mem_required_def += wlc_hw->wlc->pub->tunables->memreserved;
	...
	bool piomode, osl_t *osh, void *regsva, uint bustype, void *btparam)
{
	...
	ASSERT(sizeof(wlc_d11rxhdr_t) <= HWRXOFF);
	...
	for (j = 0; j < NBANDS_HW(wlc_hw); j++) {
		...
		WL_NONE(("wl%d: %s: bandunit %d bandtype %d coreidx %d\n", unit,
		         __FUNCTION__, wlc->band->bandunit, wlc->band->bandtype,
		         wlc->core->coreidx));
 ...
 */
void
BCMINITFN(wlc_bmac_info_init)(wlc_hw_info_t *wlc_hw)
{
	wlc_info_t *wlc = wlc_hw->wlc;
...
wlc_bmac_btc_mode_set(wlc_hw_info_t *wlc_hw, int btc_mode)
{
	...
	ASSERT((wlc_hw->btc_wire >= WL_BTC_2WIRE) || (wlc_hw->btc_wire <= WL_BTC_4WIRE));

	 /* Determine the default mode for the device */
	if (btc_mode == WL_BTC_DEFAULT) {
		if (BCMECICOEX_ENAB_BMAC(wlc_hw) || (wlc_hw->boardflags2 & BFL2_BTC3WIRE)) {
			btc_mode = WL_BTC_FULLTDM;
			if ((wlc_hw->boardflags2 & BFL2_BTC3WIRE) &&
				!BCMECICOEX_ENAB_BMAC(wlc_hw))
	...
	if (btc_mode != WL_BTC_DISABLE) {
		if ((wlc_hw->btc_wire >= WL_BTC_3WIRE) && D11REV_GE(wlc_hw->corerev, 13) &&
	...
	if (wlc_hw->up)
		ucode_up = (R_REG(wlc_hw->osh, &wlc_hw->regs->maccontrol) &
		            MCTL_EN_MAC);

	/* Disable BTC GPIOs if the BTC mode changed */
	if (wlc_hw->up &&
	    (btc_mode == WL_BTC_DISABLE) &&
	    (wlc_hw->btc_mode != WL_BTC_DISABLE))
		wlc_bmac_btc_gpio_disable(wlc_hw);

	if (btc_mode != WL_BTC_DISABLE) {
		btc_mhfs[MHF1] |= MHF1_BTCOEXIST;
		if (wlc_hw->btc_wire == WL_BTC_2WIRE) {
			...
			if (wlc_hw->boardflags & BFL_BTC2WIRE_ALTGPIO) {
				...
				wlc_hw->btc_gpio_mask =
				        BOARD_GPIO_BTCMOD_OUT | BOARD_GPIO_BTCMOD_IN;
				wlc_hw->btc_gpio_out = BOARD_GPIO_BTCMOD_OUT;
			} else {
				btc_mhfs[MHF2] &= ~MHF2_BTC2WIRE_ALTGPIO;
			}
		} else {
			if (D11REV_GE(wlc_hw->corerev, 13)) {
				wlc_hw->machwcap |= MCAP_BTCX;
				/* by default we use PS protection for non-hybrid mode. */
				if (btc_mode == WL_BTC_HYBRID)
					wlc_hw->btc_flags |= WL_BTC_FLAG_SIM_RSP;
				else if (btc_mode == WL_BTC_LITE)
					wlc_hw->btc_flags |= WL_BTC_FLAG_LIGHT;
				else if (btc_mode == WL_BTC_PARALLEL)
					wlc_hw->btc_flags |= WL_BTC_FLAG_PARALLEL;
				else
					wlc_hw->btc_flags |=
						(WL_BTC_FLAG_PS_PROTECT | WL_BTC_FLAG_ACTIVE_PROT);

				if (BCMECICOEX_ENAB_BMAC(wlc_hw)) {
					wlc_hw->btc_flags |= WL_BTC_FLAG_ECI;
				} else {
					uint32 gm = 0;
					if (wlc_hw->btc_wire == WL_BTC_4WIRE)
						btc_mhfs[MHF3] |= MHF3_BTCX_EXTRA_PRI;
					else
						wlc_hw->btc_flags |= WL_BTC_FLAG_PREMPT;
					...
					wlc_hw->btc_gpio_mask = wlc_hw->btc_gpio_out = gm;
				}
			} else { /* 3-wire over GPIO */
				wlc_hw->btc_flags |= (WL_BTC_FLAG_ACTIVE_PROT |
					WL_BTC_FLAG_SIM_RSP);
				wlc_hw->btc_gpio_mask = BOARD_GPIO_BTC3W_OUT | BOARD_GPIO_BTC3W_IN;
				wlc_hw->btc_gpio_out = BOARD_GPIO_BTC3W_OUT;
			}
		}
	} else {
		btc_mhfs[MHF1] &= ~MHF1_BTCOEXIST;
	...
	wlc_btcx_btcflag2ucflag(wlc_hw);

	if (wlc_hw->up) {
		/* Enable GPIOs if we are in 2G band, and BTC mode is enabled */
		if (BAND_2G(wlc_hw->band->bandtype) &&
		    wlc_hw->btc_mode != WL_BTC_DISABLE)
...
{
	return wlc_hw->btc_mode;
}

int
wlc_bmac_btc_wire_get(wlc_hw_info_t *wlc_hw)
{
	return wlc_hw->btc_wire;
}
...
{
	return wlc_hw->btc_flags;
}

int
wlc_bmac_btc_flags_set(wlc_hw_info_t *wlc_hw, int btc_flags)
{
	return BCME_OK;
}

/* Lower BTC GPIO through ChipCommon when BTC is OFF
 * or D11 MAC is in reset or on powerup
 */
void
wlc_bmac_btc_gpio_disable(wlc_hw_info_t *wlc_hw)
{
	...
	if (wlc_hw->btc_gpio_mask == 0)
		return;

	sih = wlc_hw->sih;

	gm = wlc_hw->btc_gpio_mask;
	go = wlc_hw->btc_gpio_out;
	...
	if (wlc_hw->clk)
		AND_REG(wlc_hw->osh, &wlc_hw->regs->psm_gpio_oe, ~wlc_hw->btc_gpio_out);
	...
	/* If the btc_mode has been disabled by now, then clear bitflags as well
	 * This has to be done here because GPIO clearing happens based on gpio_mask
	 */
	if (wlc_hw->btc_mode == WL_BTC_DISABLE) {
		wlc_hw->btc_gpio_mask = 0;
		wlc_hw->btc_gpio_out = 0;
	}
}
...
wlc_bmac_btc_gpio_enable(wlc_hw_info_t *wlc_hw)
{
	...
	gm = wlc_hw->btc_gpio_mask;
	go = wlc_hw->btc_gpio_out;
	gi = (~wlc_hw->btc_gpio_out) & wlc_hw->btc_gpio_mask;

	OR_REG(wlc_hw->osh, &wlc_hw->regs->psm_gpio_oe, wlc_hw->btc_gpio_out);
...
{
	uint8 idx;
	uint16 addr[] = {M_HOST_FLAGS1, M_HOST_FLAGS2, M_HOST_FLAGS3, M_HOST_FLAGS4, M_HOST_FLAGS5};
...
/* Write rcmta matching entry for multi-mac */
void
wlc_bmac_set_rcmta_type(wlc_hw_info_t *wlc_hw, int idx, uint16 mask, uint16 val)
{
	uint offset;
	uint16 orig_val;
	uint16 new_val;
	...
	offset = M_ADDR_BMP_BLK + (idx - P2P_ADDR_STRT_INDX) * 2;

	new_val = orig_val = wlc_bmac_read_shm(wlc_hw, offset);
	...
	WL_TMP(("%s %d id:%d offset 0x%x old: 0x%x new: 0x%x\n", __FUNCTION__, __LINE__,
	          idx, offset, orig_val, new_val));

	wlc_bmac_write_shm(wlc_hw, offset, new_val);
}
...
WLBANDINITFN(wlc_bmac_bsinit)(wlc_hw_info_t *wlc_hw, chanspec_t chanspec)
{
	...
	if (D11REV_IS(wlc_hw->corerev, 4) && WLCISAPHY(wlc->band))
...
static bool
BCMINITFN(wlc_isgoodchip)(wlc_hw_info_t *wlc_hw)
{
	/* reject some 4306 package/device combinations */
	if ((wlc_hw->sih->chip == BCM4306_CHIP_ID) && (wlc_hw->sih->chiprev > 2)) {
	...
	if ((wlc_hw->sih->chip == BCM4311_CHIP_ID) && (wlc_hw->sih->chiprev == 0)) {
...
static bool
BCMINITFN(wlc_validboardtype)(wlc_hw_info_t *wlc_hw)
{
	bool goodboard = TRUE;
...
BCMINITFN(wlc_get_macaddr)(wlc_hw_info_t *wlc_hw)
{
	 ...
	 * else:             il0macaddr
	 */
	if ((wlc_hw->sih->chip == BCM4306_CHIP_ID) &&
	    (wlc_hw->sih->chiprev == 2) && (NBANDS_HW(wlc_hw) > 1))
		varname = "il0macaddr";
...
BCMINITFN(wlc_corerev_fifofixup)(wlc_hw_info_t *wlc_hw)
{
	...
	if (D11REV_ISMBSS16(wlc_hw->corerev)) {
		...
		if (wlc_ucodembss_hwcap(wlc_hw->wlc))
	...
	if (D11REV_IS(wlc_hw->corerev, 26) && wlc_hw->sih->chiprev == 0) {
		if (AMPDU_HOST_ENAB(wlc_hw->wlc->pub))
...
static void
wlc_btcx_btcflag2ucflag(wlc_hw_info_t *wlc_hw)
{
	int index;
	int btc_flags = wlc_hw->btc_flags;
	/* update host flags based on btc_flags.  This is a very awkward implementation
	 * due to the fact that btc_flags are scattered across three hostflags 
	 */
	uint16 btc_mhf = (btc_flags & 1) ? MHF2_BTCPREMPT : 0;
	wlc_bmac_mhf(wlc_hw, MHF2, MHF2_BTCPREMPT, btc_mhf, WLC_BAND_2G);
	btc_mhf = 0;
	for (index = BTC_FLAGS_MHF3_START; index <= BTC_FLAGS_MHF3_END; index++)
		if (btc_flags & (1 << index))
			btc_mhf |= btc_ucode_flags[index].mask;
	/* ACTIVE_PROT and PS_PROTECT is dynamically configured in watchdog */
	btc_mhf &= ~(MHF3_BTCX_ACTIVE_PROT | MHF3_BTCX_PS_PROTECT);

	wlc_bmac_mhf(wlc_hw, MHF3, MHF3_BTCX_DEF_BT | MHF3_BTCX_SIM_RSP | MHF3_BTCX_ECI |
		MHF3_BTCX_SIM_TX_LP, btc_mhf, WLC_BAND_2G);
	wlc_bmac_mhf(wlc_hw, MHF3, MHF3_BTCX_ECI, btc_mhf & MHF3_BTCX_ECI, WLC_BAND_AUTO);

	btc_mhf = 0;
	for (index = BTC_FLAGS_MHF3_END + 1; index < BTC_FLAGS_SIZE; index++)
		if (btc_flags & (1 << index))
			btc_mhf |= btc_ucode_flags[index].mask;
...
static void
BCMINITFN(wlc_btcx_param_init)(wlc_hw_info_t *wlc_hw)
{
	wlc_info_t *wlc = wlc_hw->wlc;
	uint16 index;
	char   buf[15];
	...
	if (0 == (wlc->bt_shm_addr = 2 * wlc_bmac_read_shm(wlc_hw, M_BTCX_BLK_PTR)))
		return;
	/* go through all btc_params, if existed in nvram, overwrite shared memory */
	for (index = 0; index <= M_BTCX_MAX_INDEX; index++)
	{
		snprintf(buf, sizeof(buf), "btc_params%d", index);
		if (getvar(wlc_hw->vars, buf) != NULL)
			wlc_bmac_write_shm(wlc_hw, wlc->bt_shm_addr + index * 2,
	...
	{
		 wlc_hw->btc_flags = (uint16)getintvar(wlc_hw->vars, "btc_flags");
		 wlc_btcx_btcflag2ucflag(wlc_hw);
	}
}
...
BCMINITFN(wlc_coreinit)(wlc_hw_info_t *wlc_hw)
{
	...
	wlc_bmac_btc_mode_set(wlc_hw, wlc_hw->btc_mode);

	if ((CHIPID(wlc->pub->sih->chip) == BCM4313_CHIP_ID) &&
		(wlc_hw->boardflags2 & BFL2_BTC3WIRE))
		si_btc_enable_4313war(wlc_hw->sih);

	if (wlc_hw->boardflags & BFL_BTCOEX)
		si_seci_init(wlc_hw->sih, !(wlc_hw->boardflags2 & BFL2_BTC3WIRE));
	...
	if (wlc_hw->antsel_avail) {
		if ((wlc_hw->sih->chip == BCM43235_CHIP_ID) ||
	...
	if (D11REV_ISMBSS16(wlc_hw->corerev) && (wlc_ucodembss_hwcap(wlc) == TRUE)) {
		...
		for (idx = 0; idx < (uint) wlc->pub->tunables->maxucodebss; idx++) {
			...
			wlc_bmac_copyto_objmem(wlc->hw, start, &swplen,
...
BCMINITFN(wlc_gpio_init)(wlc_hw_info_t *wlc_hw)
{
	...
	if (wlc_hw->btc_gpio_out != 0 && !BAND_5G(wlc_hw->band->bandtype))
		wlc_bmac_btc_gpio_enable(wlc_hw);
	else
		wlc_bmac_btc_gpio_disable(wlc_hw);
	...
	if (((wlc_hw->sih->boardtype == BCM94322MC_SSID) ||
	    ...
	    ((wlc_hw->sih->chip == BCM4322_CHIP_ID) && (wlc_hw->sih->chiprev == 0))) {
...
BCMATTACHFN(wlc_ucode_download)(wlc_hw_info_t *wlc_hw)
{
	...
	if (P2P_ENAB_HW(wlc_hw)) {
		...
		uint nbytes = 0;
		if (WLCISNPHY(wlc_hw->band)) {
			if (D11REV_IS(wlc_hw->corerev, 25) || D11REV_IS(wlc_hw->corerev, 28)) {
				ucode32 = d11p2pucode25_mimo;
				nbytes = d11p2pucode25_mimosz;
			} else if (D11REV_IS(wlc_hw->corerev, 24)) {
				ucode32 = d11p2pucode24_mimo;
				nbytes = d11p2pucode24_mimosz;
			} else if (D11REV_IS(wlc_hw->corerev, 22)) {
				ucode32 = d11p2pucode22_mimo;
				nbytes = d11p2pucode22_mimosz;
			} else if (D11REV_GE(wlc_hw->corerev, 16)) {
				/* ucode only supports rev23(43224b0) with rev16 ucode */
				ucode32 = d11p2pucode16_mimo;
				nbytes = d11p2pucode16_mimosz;
			}
		} else if (WLCISLCNPHY(wlc_hw->band)) {
			if (D11REV_IS(wlc_hw->corerev, 25)) {
				ucode8 = d11p2pucode25_lcn;
				nbytes = d11p2pucode25_lcnsz;
			} else if (D11REV_IS(wlc_hw->corerev, 24)) {
				ucode8 = d11p2pucode24_lcn;
				nbytes = d11p2pucode24_lcnsz;
			}
		} else if (WLCISSSLPNPHY(wlc_hw->band)) {
			if (D11REV_GE(wlc_hw->corerev, 24)) {
				ucode8 = d11p2pucode20_sslpn;
				nbytes = d11p2pucode20_sslpnsz;
			} else if (D11REV_GE(wlc_hw->corerev, 16)) {
				ucode8 = d11p2pucode16_sslpn;
				nbytes = d11p2pucode16_sslpnsz;
			}
		} else if (WLCISLPPHY(wlc_hw->band)) {
			if (D11REV_GE(wlc_hw->corerev, 16)) {
				ucode32 = d11p2pucode16_lp;
				nbytes = d11p2pucode16_lpsz;
			}
		}
	...
	if (D11REV_IS(wlc_hw->corerev, 30)) {
		...
		if (WLCISSSLPNPHY(wlc_hw->band)) {
			wlc_ucode_write_byte(wlc_hw, d11ucode21_sslpn,
				d11ucode21_sslpnsz);
			WL_ERROR(("wl%d: supported NOBT phy in corerev 21\n", wlc_hw->unit));
		}
		else
			WL_ERROR(("wl%d: unsupported phy in corerev 21\n", wlc_hw->unit));
...
#ifdef BTC2WIRE
	else if (D11REV_IS(wlc_hw->corerev, 15) && (wlc_hw->btc_wire == WL_BTC_2WIRE))
...
#ifdef BTC2WIRE
	else if (D11REV_IS(wlc_hw->corerev, 13) && (wlc_hw->btc_wire == WL_BTC_2WIRE))
		wlc_ucode_write(wlc_hw, d11ucode_2w13, d11ucode_2w13sz);
#endif /* BTC2WIRE */
	else if (D11REV_IS(wlc_hw->corerev, 13))
		wlc_ucode_write(wlc_hw, d11ucode13, d11ucode13sz);
#ifdef BTC2WIRE
	else if (D11REV_GE(wlc_hw->corerev, 11) && (wlc_hw->btc_wire == WL_BTC_2WIRE))
...
static void
BCMATTACHFN(wlc_ucode_write)(wlc_hw_info_t *wlc_hw, const uint32 ucode[], const uint nbytes)
{
	osl_t *osh;
	d11regs_t *regs = wlc_hw->regs;
	uint i;
...
void
wlc_wakeucode_init(wlc_info_t *wlc, const uint32 ucode[], const uint nbytes,
                   const d11init_t *inits, const d11init_t *binits)
{
	wlc_hw_info_t *wlc_hw = wlc->hw;
	d11regs_t *regs;
	regs = wlc_hw->regs;
	...
	wlc_write_inits(wlc_hw, binits);

	/* tell the ucode the corerev */
	wlc_write_shm(wlc, M_MACHW_VER, (uint16)wlc_hw->corerev);

	/* Restore some of the additional shared memory */
#ifdef WLAFTERBURNER
	wlc_write_shm(wlc, M_MAX_ANTCNT, (wlc->afterburner? ANTCNT_AB : ANTCNT));
#else
	wlc_write_shm(wlc, M_MAX_ANTCNT, ANTCNT);
#endif /* WLAFTERBURNER */

	/* overwrite default long slot timing */
	if (wlc->shortslot)
		wlc_bmac_update_slot_timing(wlc_hw, wlc->shortslot);

	/* write rate fallback retry limits */
	wlc_write_shm(wlc, M_SFRMTXCNTFBRTHSD, wlc->SFBL);
	wlc_write_shm(wlc, M_LFRMTXCNTFBRTHSD, wlc->LFBL);

	/* restore shmem for txcore and spatial mapping */
	wlc_stf_wowl_upd(wlc, M_COREMASK_BLK);
	...
	/* Sync other states */
	wlc_write_shm(wlc, M_DOT11_DTIMPERIOD, wlc->cfg->current_bss->dtim_period);

	wlc_bmac_upd_synthpu(wlc_hw);

	wlc_pretbtt_set(wlc->cfg);
}
#endif /* WOWL */
...
int
wlc_bmac_xmtfifo_sz_set(wlc_hw_info_t *wlc_hw, uint fifo, uint blocks)
{
...
wlc_intstatus(wlc_info_t *wlc, bool in_isr)
{
	...
	if (macintstatus & MI_BT_RFACT_STUCK) {
		...
		wlc->btc_stuck_detected = TRUE;
		wlc_bmac_btc_mode_set(wlc_hw, WL_BTC_DISABLE);
		wlc_btc_stuck_war50943(wlc, FALSE);
	}
#endif
...
wlc_isr(wlc_info_t *wlc, bool *wantdpc)
{
	...
	*wantdpc = TRUE;
	 ...
	 */
	if ((macintstatus & (MI_DTIM_TBTT | MI_TBTT)) && MBSS_ENAB(wlc->pub)) {
...
wlc_bmac_txstatus_corerev4(wlc_hw_info_t *wlc_hw)
{
	...
	while (!fatal && (PIO_ENAB(wlc_hw->wlc->pub) ?
		...
		txs = (tx_status_t *)PKTDATA(osh, status_p);
...
#if defined(STA) && defined(WLRM)
static uint16
wlc_read_ihr(wlc_hw_info_t *wlc_hw, uint offset)
{
	d11regs_t *regs = wlc_hw->regs;
...
static void
wlc_write_ihr(wlc_hw_info_t *wlc_hw, uint offset, uint16 v)
{
	d11regs_t *regs = wlc_hw->regs;
	volatile uint16* objdata_lo = (volatile uint16*)(uintptr)&regs->objdata;
...
wlc_suspend_mac_and_wait(wlc_info_t *wlc)
{
	...
	if ((wlc_hw->btc_wire >= WL_BTC_3WIRE) &&
	    D11REV_LT(wlc_hw->corerev, 13)) {
		si_gpiocontrol(wlc_hw->sih, wlc_hw->btc_gpio_mask, 0, GPIO_DRV_PRIORITY);
	}
#endif /* STA */
...
#ifdef STA
	wlc_ucode_wake_override_clear(wlc->hw, WLC_WAKE_OVERRIDE_MACSUSPEND);

	if ((wlc_hw->btc_wire >= WL_BTC_3WIRE) &&
	    D11REV_LT(wlc_hw->corerev, 13)) {
		si_gpiocontrol(wlc_hw->sih, wlc_hw->btc_gpio_mask, wlc_hw->btc_gpio_mask,
		               GPIO_DRV_PRIORITY);
	}
#endif /* STA */
#ifdef WLC_HIGH
	/* BMAC_NOTE: cannot set either of these state items in higher level code.
	 * Maybe the prq fifo needs a functional interface at the high/low boundary
	 * instead of having the high level code directly access shm for this?
	 */
#ifdef MBSS
...
wlc_bmac_ifsctl_edcrs_set(wlc_hw_info_t *wlc_hw, bool abie, bool isht)
{
	...
	if (abie) {
		AND_REG(wlc_hw->osh, &wlc_hw->regs->ifs_ctl1, ~IFS_CTL1_EDCRS);
		return;
	}
#endif
	...
	if (WLCISNPHY(wlc_hw->band) && NREV_GE(wlc_hw->band->phyrev, 3)) {
		if (CHSPEC_IS20(wlc_hw->chanspec)) {
			...
			OR_REG(wlc_hw->osh, &wlc_hw->regs->ifs_ctl1, IFS_CTL1_EDCRS);
			AND_REG(wlc_hw->osh, &wlc_hw->regs->ifs_ctl1, ~IFS_CTL1_EDCRS_20L);
			AND_REG(wlc_hw->osh, &wlc_hw->regs->ifs_ctl1, ~IFS_CTL1_EDCRS_40);
		} else {
			/* 40 mhz, use 20U 20L and 40 ED */
			OR_REG(wlc_hw->osh, &wlc_hw->regs->ifs_ctl1, IFS_CTL1_EDCRS);
			OR_REG(wlc_hw->osh, &wlc_hw->regs->ifs_ctl1, IFS_CTL1_EDCRS_20L);
			OR_REG(wlc_hw->osh, &wlc_hw->regs->ifs_ctl1, IFS_CTL1_EDCRS_40);
		}
	}
...
bool
BCMINITFN(wlc_bmac_validate_chip_access)(wlc_hw_info_t *wlc_hw)
{
	d11regs_t *regs;
	uint32 w, val;
...
wlc_bmac_core_phypll_ctl(wlc_hw_info_t* wlc_hw, bool on)
{
	...
	WL_TMP(("%s: clk_ctl_st before phypll(%d) request 0x%x\n",
	        __FUNCTION__, on, R_REG(osh, &regs->clk_ctl_st)));

	if (on) {
		if ((wlc_hw->sih->chip == BCM4313_CHIP_ID))
			{
		OR_REG(osh, &regs->clk_ctl_st,
		       (CCS_ERSRC_REQ_HT | CCS_ERSRC_REQ_D11PLL | CCS_ERSRC_REQ_PHYPLL));
		...
		tmp = R_REG(osh, &regs->clk_ctl_st);
		if ((tmp & (CCS_ERSRC_AVAIL_D11PLL | CCS_ERSRC_AVAIL_PHYPLL)) !=
		    (CCS_ERSRC_AVAIL_D11PLL | CCS_ERSRC_AVAIL_PHYPLL)) {
			WL_ERROR(("%s: turn on PHY PLL failed\n", __FUNCTION__));
			ASSERT(0);
		}
	...
	WL_TMP(("%s: clk_ctl_st after phypll(%d) request 0x%x\n",
	        __FUNCTION__, on, tmp));
}
		...
		for (i = 0; i < NFIFO; i++)
			if (wlc_hw->di[i]) {
				...
				WL_TRACE(("wlc_flushqueues: ampdu_flags cleared, head %d tail %d\n",
				          wlc->hw->txstatus_ampdu_flags[i].head,
				          wlc->hw->txstatus_ampdu_flags[i].tail));
				wlc->hw->txstatus_ampdu_flags[i].head = 0;
				wlc->hw->txstatus_ampdu_flags[i].tail = 0;
#endif
			}
...
wlc_bmac_rm_cca_measure(wlc_hw_info_t *wlc_hw, uint32 us)
{
	...
	wlc_write_ihr(wlc_hw, TSF_GPT_2_STAT, TSF_GPT_USETSF);
	/* set GPT 2 to the measurement duration */
	wlc_write_ihr(wlc_hw, TSF_GPT_2_CTR_L, (gpt_ticks & 0xffff));
	wlc_write_ihr(wlc_hw, TSF_GPT_2_CTR_H, (gpt_ticks >> 16));
...
wlc_bmac_rm_cca_int(wlc_hw_info_t *wlc_hw)
{
	...
	gpt2_l = wlc_read_ihr(wlc_hw, TSF_GPT_2_VAL_L);
	gpt2_h = wlc_read_ihr(wlc_hw, TSF_GPT_2_VAL_H);
...
wlc_bmac_timer_led_blink(void *arg)
{
	...
	wlc_bmac_led_blink_timer(wlc->hw->ledh);
}
...
{
	bmac_led_t *led = &wlc_hw->ledh->led[index];
...
#if defined(WLTEST)
/* BMAC_NOTE, BMAC can't call high request return. the tx_testframe doesn't queue the frame
 * This should be integrated with the NPHY IOV_PKTENG flow
 */
...
void
wlc_bmac_pktengtx(wlc_hw_info_t *wlc_hw, wl_pkteng_t *pkteng, uint8 rate,
	struct ether_addr *sa, uint32 wait_delay)
{
	...
	suspend = (0 == (R_REG(wlc_hw->osh, &wlc->regs->maccontrol) & MCTL_EN_MAC));
	if (suspend)
		wlc_enable_mac(wlc);
	...
	if (pkteng->nframes) {
		...
		wlc_bmac_write_shm(wlc_hw, M_PKTENG_FRMCNT_LO, (pkteng->nframes & 0xffff));
		wlc_bmac_write_shm(wlc_hw, M_PKTENG_FRMCNT_HI, ((pkteng->nframes>>16) & 0xffff));
...
#if defined(WLTEST)
		wlc_tx_testframe(wlc, &pkteng->dest, sa, rate, pkteng->length);
#endif
	} else {
		/* CTS frame */
		val |= M_PKTENG_MODE_TX_CTS;
		wlc_bmac_write_shm(wlc_hw, M_PKTENG_IFS, (uint16)pkteng->delay);
		wlc_bmac_write_shm(wlc_hw, M_PKTENG_CTRL, val);

	}

	/* Wait for packets to finish */
	frame_cnt_check = wlc_bmac_read_shm(wlc_hw, M_PKTENG_FRMCNT_LO);
...
wlc_bmac_pkteng(wlc_hw_info_t *wlc_hw, wl_pkteng_t *pkteng, void* p)
{
	...
	case WL_PKTENG_PER_TX_WITH_ACK_START:
	{
		...
		if ((pkteng->delay < 30) || (pkteng->delay > 1000)) {
			err = BCME_RANGE;
			break;
		}
		...
		if (WLCISSSLPNPHY(wlc_hw->band) || WLCISLCNPHY(wlc_hw->band)) {
			...
			if (pkteng->nframes) {
				/* retry counter is used to replay the packet */
				if (pkteng->nframes > wlc_hw->SRL)
					pkteng->nframes -= wlc_hw->SRL;
...
bool
wlc_bmac_radio_hw(wlc_hw_info_t *wlc_hw, bool enable)
{
...
btc_flags_ucode_t btc_ucode_flags[BTC_FLAGS_SIZE] = {
	{MHF2, MHF2_BTCPREMPT},
