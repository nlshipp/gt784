 ...
 *
 * $Id: wlc_event.c,v 1.1 2010/08/05 21:59:02 ywu Exp $
 */
#include <typedefs.h>
#include <bcmutils.h>
#include <siutils.h>
...
/* Local prototypes */
#ifndef WLNOEIND
static void wlc_event_sendup(wlc_eventq_t *eq, struct ether_addr *ea,
...
wlc_eventq_handle_ind(wlc_eventq_t *eq, wlc_event_t *e)
{
	...
	/* BSSCFG could have been removed by the time the driver gets here due to async
	 * nature of event handling
	 */
	if (wlc->bsscfg[bsscfgidx] == NULL)
		return 0;

	if (wlc_eventq_test_ind(eq, e->event.event_type)) {
		wlc_event_sendup(eq, &wlc->bsscfg[bsscfgidx]->cur_etheraddr,
			e, e->data, e->event.datalen);
	}
	return 0;
}
...
static void
wlc_event_sendup(wlc_eventq_t *eq, struct ether_addr *ea,
	const wlc_event_t *e, uint8 *data, uint32 len)
{
	wlc_bsscfg_t *bsscfg;
	wlc_info_t *wlc = eq->wlc;
	...
	char *ptr;
	bcm_event_t *msg;
	struct ether_addr *dest_p;
	wlc_pub_t *pub = eq->pub;
	 ...
	 * nature of event handling
	 */
	if ((bsscfg = wlc->bsscfg[e->bsscfgidx]) == NULL)
		return;


	if ((p = PKTGET(pub->osh, pub->tunables->rxbufsz, FALSE)) == NULL) {
		WL_ERROR(("wl%d: wlc_event_sendup: failed to get a pkt\n", pub->unit));
		return;
	}

	WL_INFORM(("wlc_event_sendup START: bsscfgidx %d. ea %s ifname:%s\n", e->bsscfgidx,
	           bcm_ether_ntoa((struct ether_addr*)ea, eabuf), e->event.ifname));
	...
	msg = (bcm_event_t *) PKTDATA(pub->osh, p);
	dest_p = &bsscfg->cur_etheraddr;

	bcopy(dest_p, &msg->eth.ether_dhost, ETHER_ADDR_LEN);
	bcopy(ea, &msg->eth.ether_shost, ETHER_ADDR_LEN);
	...
	/* fixup lengths */
	msg->bcm_hdr.length  = hton16(sizeof(wl_event_msg_t) + ntoh16(msg->bcm_hdr.length));
	PKTSETLEN(pub->osh, p, (sizeof(bcm_event_t) + len + 2));
                       ...
                       uint8 *buf, uint16 buflen)
{
	...
	uint16 len;
	osl_t *osh = wlc->osh;
	hndrte_dev_t *busdev = wl_rtedev->dev_chained;
	...
	if (busdev->dev_funcs->xmit(NULL, busdev, lb) != 0) {
		lb_free(lb);
	}
}
