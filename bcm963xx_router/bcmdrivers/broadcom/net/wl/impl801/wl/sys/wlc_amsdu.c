 ...
 *
 * $Id: wlc_amsdu.c,v 1.1 2010/08/05 21:59:01 ywu Exp $
 */
...
BCMATTACHFN(wlc_amsdu_attach)(wlc_info_t *wlc)
{
	...
	if (wlc_module_register(ami->pub, amsdu_iovars, "amsdu",
		ami, wlc_amsdu_doiovar, NULL, wlc_amsdu_down)) {
...
BCMATTACHFN(wlc_amsdu_tx_attach)(amsdu_info_t *ami, wlc_info_t *wlc)
{
	...
	for (i = 0; i < NUMPRIO; i++) {
		 ...
		 */
		err = wlc_xmtfifo_sz_get(wlc, prio2fifo[i], &fifo_size);
		if (err) {
			WL_ERROR(("wl%d: wlc_amsdu_attach: wlc_xmtfifo_sz_get(%d) failed\n",
			          wlc->pub->unit, prio2fifo[i]));
			return -1;
		}
...
wlc_amsdu_mtu_init(amsdu_info_t *ami)
{
	...
	ami->amsdu_rxcap_big = ((ami->mac_rcvfifo_limit - HWRXOFF - 100) >= HT_MAX_AMSDU);
	...
	void *p, uint plen, void *a, int alen, int val_size, struct wlc_if *wlcif)
{
	...
	i = 0;
	if ((err = wlc_iovar_check(ami->pub, vi, a, alen, IOV_ISSET(actionid))) != 0)
		return err;
 ...
 */
void
wlc_recvamsdu(amsdu_info_t *ami, d11rxhdr_t *rxh, void *p)
{
	osl_t *osh;
	uint aggtype;

	osh = ami->pub->osh;
	aggtype = (rxh->RxStatus2 & RXS_AGGTYPE_MASK) >> RXS_AGGTYPE_SHIFT;
	...
	case RXS_AMSDU_INTERMEDIATE:
		...
		/* intermediate frames have no padding bytes due to HWRXOFF = 30 */
		ASSERT(!(rxh->RxStatus1 & RXS_PBPRES));
	...
	case RXS_AMSDU_LAST:
		...
		/* last frame have no padding bytes due to HWRXOFF = 30 */
		ASSERT(!(rxh->RxStatus1 & RXS_PBPRES));
	...
	if (ami->amsdu_deagg_state == WLC_AMSDU_DEAGG_LAST) {
		...
		wlc_recvdata(ami->wlc, ami->pub->osh, rxh, pp);
	}
