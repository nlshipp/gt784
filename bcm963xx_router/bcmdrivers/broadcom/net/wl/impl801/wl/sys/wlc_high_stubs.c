 ...
 *
 * $Id: wlc_high_stubs.c,v 1.1 2010/08/05 21:59:02 ywu Exp $
 */
...
static int
wlc_rpc_unpack_txpwr_limits(bcm_xdr_buf_t *b, struct txpwr_limits *txpwr)
{
	int err;
	uint32 mcs32;
                    ...
                    rpc_buf_t *buf, uint totlen)
{
	...
	WL_NONE(("PKTCONVERT: totlen %d p 0x%x len %d data 0x%x head 0x%x end 0x%x\n",
	         totlen, p, PKTLEN(osh, p), PKTDATA(osh, p), p->head, p->end));

	WL_NONE(("PKTCONVERT: b 0x%x b.buf 0x%x b.origbuf 0x%x b.size %d b.origsize %d\n",
	...
	while ((next_p = PKTNEXT(osh, iter_p)) != NULL) {
		...
		WL_NONE(("PKTCONVERT: resid %d p 0x%x len %d data 0x%x\n",
	...
	if (resid < PKTLEN(osh, iter_p)) {
		WL_NONE(("PKTCONVERT: trim to resid %d p 0x%x len %d data 0x%x\n",
		         resid, p, PKTLEN(osh, p), PKTDATA(osh, p)));
		if (PKTLEN(osh, iter_p) - resid >= 4) {
			printf("PKTCONVERT: ??? trim to resid %d p 0x%x len %d data 0x%x\n",
				resid, p, PKTLEN(osh, p), PKTDATA(osh, p));
		}
...
wlc_rpc_bmac_dispatch(wlc_rpc_ctx_t *rpc_ctx, struct rpc_buf* buf)
{
	...
	case WLRPC_WLC_BMAC_INIT_ID: {
		...
		wlc_bmac_init(wlc_hw, (chanspec_t)chanspec, (bool)mute);
		break;
	}
	...
	case WLRPC_WLC_PHY_DOIOVAR_ID: {
		...
		ret = wlc_bmac_phy_iovar_dispatch(wlc_hw, actionid, 0, p, plen, a, alen, vsize);

		rpc_buf = wlc_rpc_buf_alloc(rpc, &retb, sizeof(int32) +
			ROUNDUP(alen, sizeof(uint32)), WLRPC_WLC_PHY_DOIOVAR_ID);
	...
	case WLRPC_WLC_PHY_HOLD_UPD_ID: {
		...
		wlc_phy_hold_upd(((wlc_info_t *)wlc)->band->pi, id, set);
		break;
	}
	...
	case WLRPC_WLC_PHY_MUTE_UPD_ID: {
		...
		wlc_phy_mute_upd(((wlc_info_t *)wlc)->band->pi, set, flags);
		break;
	}

	case WLRPC_WLC_PHY_CLEAR_TSSI_ID:
		wlc_phy_clear_tssi(((wlc_info_t *)wlc)->band->pi);
		break;

	case WLRPC_WLC_PHY_ANT_RXDIV_GET_ID: {
		uint8 pval;
		bcm_xdr_buf_t retb;
		rpc_buf_t *rpc_buf;
		...
		wlc_phy_ant_rxdiv_get(((wlc_info_t *)wlc)->band->pi, &pval);
	...
	case WLRPC_WLC_PHY_ANT_RXDIV_SET_ID: {
		uint32 ret;
		uint8 val;
		...
		wlc_phy_ant_rxdiv_set(((wlc_info_t *)wlc)->band->pi, val);
		break;
	}
	...
	case WLRPC_WLC_PHY_PREAMBLE_SET_ID: {
		...
		wlc_phy_preamble_override_set(((wlc_info_t *)wlc)->band->pi, val);
		break;
	}

	case WLRPC_WLC_PHY_FREQTRACK_END_ID:
		wlc_phy_freqtrack_end(((wlc_info_t *)wlc)->band->pi);
		break;

	case WLRPC_WLC_PHY_FREQTRACK_START_ID:
		wlc_phy_freqtrack_start(((wlc_info_t *)wlc)->band->pi);
		break;

	case WLRPC_WLC_PHY_IOCTL_ID: {
		int32 ret;
		int cmd;
		uint len;
		void *arg = NULL;
		...
		err = bcm_xdr_unpack_int32(&b, &cmd);
		ASSERT(!err);

		err = bcm_xdr_unpack_opaque_varlen(&b, &len, &arg);
		ASSERT(!err);

		ret = wlc_phy_ioctl(wlc_hw->band->pi, cmd, len, arg, &ta_ok);

		rpc_buf = wlc_rpc_buf_alloc(rpc, &retb,
		                            3*sizeof(int32) + ROUNDUP(len, sizeof(uint32)),
		                            WLRPC_WLC_PHY_IOCTL_ID);
		ASSERT(rpc_buf != NULL);

		err = bcm_xdr_pack_opaque_varlen(&retb, len, arg);
		ASSERT(!err);

		err = bcm_xdr_pack_int32(&retb, ta_ok);
		ASSERT(!err);
	...
	case WLRPC_WLC_PHY_NOISE_SAMPLE_REQUEST_ID:
		wlc_phy_noise_sample_request_external(((wlc_info_t *)rpc_ctx->wlc)->band->pi);
		break;
	...
	case WLRPC_WLC_PHY_CAL_PERICAL_ID: {
		...
		wlc_phy_cal_perical(((wlc_info_t *)wlc)->band->pi, reason);
		break;
	};
	...
	case WLRPC_WLC_PHY_TXPOWER_GET_ID: {
		...
		wlc_phy_txpower_get(((wlc_info_t*)wlc)->band->pi, &qdbm, &override);
	...
	case WLRPC_WLC_PHY_ACIM_NOISEM_RESET_NPHY_ID: {
		...
		wlc_phy_acimode_noisemode_reset(((wlc_info_t *)wlc)->band->pi,
			channel, clear_aci_state, clear_noise_state, disassoc);
		break;
	}
	...
	case WLRPC_WLC_PHY_INTERFER_SET_NPHY_ID: {
		...
		wlc_phy_interference_set(((wlc_info_t *)wlc)->band->pi, init);
		break;
	}
	...
	case WLRPC_WLC_PHY_TXPOWER_SET_ID: {
		...
		ret = wlc_phy_txpower_set(((wlc_info_t*)wlc)->band->pi, qdbm, override);
	...
	case WLRPC_WLC_PHY_TXPOWER_SROMLIMIT_ID: {
		...
		err = bcm_xdr_unpack_uint32(&b, &chan);
		ASSERT(!err);

		err = bcm_xdr_unpack_int32(&b, &ret);
		ASSERT(!err);

		txp_rate_idx = (int)ret;

		wlc_phy_txpower_sromlimit(((wlc_info_t *)wlc)->band->pi, chan,
		                          &min_pwr, &max_pwr, txp_rate_idx);

		err = bcm_xdr_pack_uint32(&retb, min_pwr);
		ASSERT(!err);

		err = bcm_xdr_pack_uint32(&retb, max_pwr);
		ASSERT(!err);
	...
	case WLRPC_WLC_PHY_RADAR_DETECT_ENABLE_ID: {
		...
		wlc_phy_radar_detect_enable(((wlc_info_t *)wlc)->band->pi, on);
#endif
		break;
	}
...
#if defined(AP) && defined(RADAR)
		ret = wlc_phy_radar_detect_run(((wlc_info_t *)wlc)->band->pi);
#else
		ret = 0;
#endif
		err = bcm_xdr_pack_int32(&retb, ret);
	...
	case WLRPC_WLC_PHY_RADAR_DETECT_MODE_SET_ID: {
		...
		wlc_phy_radar_detect_mode_set(wlc_hw->band->pi, mode);
#endif
		break;
	}
	...
	case WLRPC_WLC_PHY_TEST_ISON_ID :{
		...
		phytest_on = wlc_phy_test_ison(wlc->band->pi);
	...
	case WLRPC_WLC_BMAC_COPYFROM_VARS_ID: {
		...
		wlc_bmac_copyfrom_vars(wlc_hw, &buf, &len);
		...
		err = bcm_xdr_pack_opaque_varlen(&retb, len, buf);
		ASSERT(!err);
	...
	case WLRPC_WLC_BMAC_SET_ADDRMATCH_ID: {
		...
		wlc_bmac_set_addrmatch(wlc_hw, match_reg_offset, &addr);
		break;
	}
	...
	case WLRPC_WLC_BMAC_SET_RCMTA_TYPE_ID: {
		int idx;
		uint32 mask, val;
		...
		wlc_bmac_set_rcmta_type(wlc_hw, idx, mask, val);
		break;
	}
	...
	case WLRPC_WLC_BMAC_WRITE_TEMPLATE_RAM_ID: {
		...
		err = bcm_xdr_unpack_opaque_varlen(&b, &len, &buf);
		ASSERT(!err);

		wlc_bmac_write_template_ram(wlc_hw, offset, (int)len, buf);
		break;
	}
	...
	case WLRPC_WLC_BMAC_CHANSPEC_SET_ID: {
		...
		wlc_bmac_set_chanspec(wlc_hw, chanspec, mute, &txpwr);
		break;
	}
...
#define HANDLE_BOUNDARY(b, p, prev_p)						\
	do {									\
		if (b.size == 0) {						\
			...
			WL_NONE(("TXFIFO: next p 0x%x len %d data 0x%x\n",	\
		...
		WL_NONE(("TXFIFO: totlen %d p 0x%x len %d data 0x%x\n",
		 ...
		 */
		if (prev_p) {
			WL_NONE(("TXFIFO: unlinking prev_p 0x%x from p 0x%x\n", prev_p, p));
			PKTSETNEXT(osh, prev_p, NULL);
		} else {
			WL_NONE(("TXFIFO: buf 0x%x is p 0x%x, so don't free\n", buf, p));
			free_buf = FALSE;
		}
		...
		/* Flags are needed for AMPDU */
		WLPKTTAG(p)->flags = flags;

		/* Exptime needed for RF awareness */
		WLPKTTAG(p)->exptime = exptime;

		WL_TRACE(("%s fifo:%d txpktpend:%d totlen:%d flags:0x%x p:%p\n",
		          __FUNCTION__, fifo, txpktpend, totlen, flags, p));

		if (commit && fifo == TX_DATA_FIFO) {
			int txpktpend = TXPKTPENDGET(wlc, fifo);

			if (txpktpend < HIST_SIZE)
				histogram_txfifo[txpktpend]++;
			else {
				WL_TRACE(("wl%d:TXPKTPENDGET: %d\n",
				          wlc_hw->unit, txpktpend));
	...
	case WLRPC_WLC_PHY_TXPOWER_GET_CURRENT_ID: {
		...
		rpc_buf = wlc_rpc_buf_alloc(rpc, &retb, ROUNDUP(sizeof(tx_power_t), sizeof(uint32)),
		                            WLRPC_WLC_PHY_TXPOWER_GET_CURRENT_ID);
		ASSERT(rpc_buf != NULL);

		err = bcm_xdr_unpack_uint32(&b, &channel);
		ASSERT(!err);

		power = (tx_power_t *)MALLOC(wlc_hw->osh, sizeof(tx_power_t));
		if (power == NULL) {
			ASSERT(0);
			break;
		}
		err = bcm_xdr_unpack_opaque_cpy(&b, sizeof(tx_power_t), power);
		ASSERT(!err);

		wlc_phy_txpower_get_current(((wlc_info_t *)wlc)->band->pi, power, channel);

		err = bcm_xdr_pack_opaque(&retb, sizeof(tx_power_t), power);
		ASSERT(!err);

		err = wlc_rpc_call_return(rpc, rpc_buf);
		ASSERT(!err);

		MFREE(wlc_hw->osh, power, sizeof(tx_power_t));
	...
	case WLRPC_WLC_PHY_BAND_FIRST_CHANSPEC_ID: {
		...
		pi = ((wlc_info_t *)wlc)->bandstate[band]->pi;
		ret = (uint32)wlc_phy_chanspec_band_firstch(pi, band);
	...
	case WLRPC_WLC_PHY_TXPOWER_HW_CTRL_GET_ID :{
		...
		hwpwrctrl = wlc_phy_txpower_hw_ctrl_get(((wlc_info_t *)wlc)->band->pi);
	...
	case WLRPC_WLC_PHY_TXPOWER_HW_CTRL_SET_ID: {
		...
		wlc_phy_txpower_hw_ctrl_set(((wlc_info_t *)wlc)->band->pi, hwpwrctrl);
	...
	case WLRPC_WLC_PHY_TXPOWER_LIMIT_SET_ID: {
		...
		wlc_phy_txpower_limit_set(((wlc_info_t *)wlc)->band->pi, &txpwr, chanspec);
		break;
	}
	...
	case WLRPC_WLC_PHY_BSSINIT_ID: {
		...
		err = bcm_xdr_unpack_int32(&b, &rssi);
		ASSERT(!err);

		wlc_phy_BSSinit(((wlc_info_t *)wlc)->band->pi, bonlyap, rssi);
	...
	case WLRPC_WLC_PHY_BAND_CHANNELS_ID: {
		...
		wlc_phy_chanspec_band_validch(((wlc_info_t *)wlc)->band->pi, band, &channels);
	...
	case WLRPC_WLC_BMAC_REVINFO_GET_ID: {
		...
		for (idx = 0; idx < revinfo.nbands; idx++) {
			...
			err = bcm_xdr_pack_uint32(&retb, (uint32)revinfo.band[idx].abgphy_encore);
			ASSERT(!err);
		}

		err = bcm_xdr_pack_int32(&retb, revinfo.btc_wire);
		ASSERT(!err);
	...
	case WLRPC_WLC_PHY_SET_CHANNEL_14_WIDE_FILTER_ID: {
		...
		wlc_phy_chanspec_ch14_widefilter_set(((wlc_info_t *)wlc)->band->pi,
		                                   wide_filter);
	...
	case WLRPC_WLC_PHY_NOISE_AVG_ID: {
		...
		noise = wlc_phy_noise_avg(((wlc_info_t *)wlc)->band->pi);
	...
	case WLRPC_WLC_BMAC_SET_UCODE_LOADED : {
		bool ucode_loaded;
		uint32 uret;
		...
		ucode_loaded = (bool)uret;
		wlc_bmac_set_ucode_loaded(wlc_hw, ucode_loaded);
		break;
	}

	case WLRPC_WLC_BMAC_P2P_SET : {
		bool enable;
		uint32 uret;
		...
		enable = (bool)uret;
		wlc_bmac_set_ucode_loaded(wlc_hw, enable);
		break;
	}
	...
	case WLRPC_WLC_BMAC_BTC_FLAGS_SET_ID: {
		bcm_xdr_buf_t retb;
		rpc_buf_t *rpc_buf;
		int btc_flags;
		int ret;

		err = bcm_xdr_unpack_int32(&b, &btc_flags);
		ASSERT(!err);

		ret = wlc_bmac_btc_flags_set(wlc_hw, btc_flags);
	...
	case WLRPC_WLC_PHY_TXPOWER_GET_TARGET_MIN_ID: {
		...
		ret = wlc_phy_txpower_get_target_min(wlc->band->pi);
	...
	case WLRPC_WLC_PHY_TXPOWER_GET_TARGET_MAX_ID: {
		...
		ret = wlc_phy_txpower_get_target_max(wlc->band->pi);
	...
	case WLRPC_WLC_BMAC_DOIOVARS_ID: {
		uint32 actionid;
		void *p, *a;
		uint plen;
		int alen, vsize, ret;
		...
		ret = wlc_bmac_iovars_dispatch(wlc_hw, actionid, 0,
		                             p, plen, a, alen, vsize);

		rpc_buf = wlc_rpc_buf_alloc(rpc, &retb,
		                            sizeof(int32) + ROUNDUP(alen, sizeof(uint32)),
	...
	case WLRPC_WLC_BMAC_DUMP_ID: {
		...
		err = bcm_xdr_unpack_uint32(&b, &dump_id);
		ASSERT(!err);
		...
		/* wlc_bmac_dump() will pack the string directly into rpc_buf */
		wlc_bmac_dump(wlc_hw, &strb, dump_id);
	...
	case WLRPC_WLC_BMAC_PKTENG_ID: {
		...
		if (totlen > 0) {
			...
			p = wlc_rpc_bmac_txfifo_buf_to_txpkt(rpc, wlc_hw->osh, &b, buf, totlen);
...
#if defined(WLC_LOW) && defined(AP)
		wlc_bmac_process_ps_switch(wlc_hw, ea_p, ps_flags);
#endif
	...
	case WLRPC_WLC_PHY_CAL_CACHE_INIT_ID: {
		...
		ret = wlc_phy_cache_cals_nphy_init(((wlc_info_t *)wlc)->band->pi);
	...
	case WLRPC_WLC_PHY_CAL_CACHE_DEINIT_ID: {
		...
		wlc_phy_cache_cals_nphy_deinit(((wlc_info_t *)wlc)->band->pi);
		break;
	}
	...
	case WLRPC_WLC_BMAC_IFSCTL_EDCRS_SET_ID: {
		...
		err = bcm_xdr_unpack_uint32(&b, &abie);
		ASSERT(!err);

		err = bcm_xdr_unpack_uint32(&b, &isht);
		ASSERT(!err);

		wlc_bmac_ifsctl_edcrs_set(wlc_hw, (bool)abie, (bool)isht);
	...
	case WLRPC_WLC_PHY_LDPC_SET_ID: {
		...
		wlc_phy_ldpc_override_set(((wlc_info_t *)wlc)->band->pi, (bool)ldpc);
		break;
	}
...
void
wlc_noise_cb(wlc_info_t *wlc, uint8 channel, int8 noise_dbm)
{
	bcm_xdr_buf_t b;
	rpc_buf_t *rpc_buf;
	int err;
...
wlc_rxpkt_to_rpc_buf(wlc_hw_info_t *wlc_hw, rpc_info_t *rpc, void *p)
{
	...
	ASSERT(PKTHEADROOM(wlc_hw->osh, p) >=
		(sizeof(uint32) * 2 + bcm_rpc_buf_header_len(rpc)));

	PKTSETLEN(wlc_hw->osh, p, pktlen);

	/* Make room for RPC ID + length field */
	PKTPUSH(wlc_hw->osh, p, sizeof(uint32) * 2);
	...
	bcm_xdr_buf_init(&b, bcm_rpc_buf_data(bcm_rpc_tp_get(rpc), rpc_buf), sizeof(uint32) * 2);
	...
	if (rpc_buf == NULL) {
		printf("%s %d : Insufficient memory %d(%dK) to receive Requested:%d\n",
		       __FUNCTION__,
		       __LINE__, hndrte_memavail(), hndrte_memavail()/1000,
...
wlc_dotxstatus(wlc_info_t *wlc, tx_status_t *txs, uint32 frm_tx2)
{
	...
	/* validate Frame ID */
	fid_offset = OFFSETOF(d11txh_t, TxFrameID);
	if (PKTLEN(wlc->osh, p) > fid_offset) {
		txh = (d11txh_t *)PKTDATA(wlc->osh, p);
		frameid = htol16(txh->TxFrameID);
	} else {
		/* advance next packet in chain */
		rpc_buf_t *next_p;
		uint16 *fid_p;

		next_p = (rpc_buf_t*)PKTNEXT(wlc_hw->osh, p);
		ASSERT(next_p != NULL);
		fid_p = (uint16 *)((uint8*)PKTDATA(wlc->osh, next_p) +
			(fid_offset - PKTLEN(wlc->osh, p)));
		frameid = htol16(*fid_p);
	}
	ASSERT(txs->frameid == frameid);

	WL_TRACE(("p:%p p->flags 0x%x AMPDU:0x%x \n", p, WLPKTTAG(p)->flags,
	...
	err = bcm_xdr_pack_uint32(&b, rpc_txstatus.PAD_framelen);
	ASSERT(!err);

	err = bcm_xdr_pack_uint32(&b, rpc_txstatus.status_frameid);
	ASSERT(!err);

	err = bcm_xdr_pack_uint32(&b, rpc_txstatus.sequence_lasttxtime);
	ASSERT(!err);
...
#ifndef DMA_TX_FREE
	if ((WLPKTTAG(p)->flags & WLF_AMPDU_MPDU) != WLF_AMPDU_MPDU) {
		PKTFREE(wlc->hw->osh, p, TRUE);
		TXPKTPENDDEC(wlc, queue, 1);
		...
		/* If AMPDU txstatus does not handle, then append a NULL id */
		if (!wlc_high_stubs_ampdu_txstatus_complete(wlc, p, txs, queue, &b)) {
			err = bcm_xdr_pack_uint32(&b, WLRPC_NULL_ID);
			ASSERT(!err);
		}
	}
#else
	tail_idx = wlc->hw->txstatus_ampdu_flags[queue].tail;
	wlc->hw->txstatus_ampdu_flags[queue].tail = (tail_idx + 1) % WLC_TXSTATUS_SZ;

	if (isset(wlc->hw->txstatus_ampdu_flags[queue].vec, tail_idx)) {
		wlc_high_stubs_ampdu_txstatus_complete(wlc, NULL, txs, queue, &b);
	} else {
		TXPKTPENDDEC(wlc, queue, 1);
		WL_TRACE(("wlc_dotxstatus, pktpend dec 1 to %d\n", TXPKTPENDGET(wlc, queue)));
		err = bcm_xdr_pack_uint32(&b, WLRPC_NULL_ID);
...
wlc_high_dpc(wlc_info_t *wlc, uint32 macintstatus)
{
	...
	rpc_buf = wlc_rpc_buf_alloc(rpc, &b, 2 * sizeof(uint32), WLRPC_WLC_HIGH_DPC_ID);
	ASSERT(rpc_buf != NULL);
...
bcm_assert_log(char *str)
{
	...
	rpc_buf_t *rpc_buf = NULL;
	int err;
	rpc_info_t *rpc = (rpc_info_t *)g_assert_hdl;

	rpc_buf = wlc_rpc_buf_alloc(rpc, &b, MAX_ASSRTSTR_LEN, WLRPC_BCM_ASSERT_LOG_ID);

	ASSERT(rpc_buf != NULL);

	if (str)
		err = bcm_xdr_pack_opaque_varlen(&b, MIN(MAX_ASSRTSTR_LEN,
			strlen(str)), (void*)str);
	else
		err = bcm_xdr_pack_opaque_varlen(&b, 0, NULL);
	ASSERT(!err);

	WL_TRACE(("%s(): str=%s\n", __FUNCTION__, str ? str : NULL));
