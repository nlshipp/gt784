...
#define IsChipA0()	((PERF->RevID & 0xFF) == 0xa0)
static uint32 fakewrap_core_base(void) 
{
    if (IsChipA0()) /* a0b0compatible, see all these below */
    	return 0x10007028;
    else
    	return 0x10007008;	
}

#define FAKEWRAP_ADDR_CORE_BASE	(fakewrap_core_base()) /* a0b0compatible */	
...
{	
	...
	if (!IsChipA0())
		...
		return (((R_REG(sii->osh, &ai->ioctrl) & (SICF_FGC | SICF_CLOCK_EN)) == SICF_CLOCK_EN) &&
	        	((R_REG(sii->osh, &WLAN_SHIM->a0.ShimMisc) & WLAN_SHIM_MACRO_SOFT_RESET) == 0));
}
...
ub_core_disable(si_t *sih, uint32 bits)
{
	...
	if (!IsChipA0()) {
		...
		if (R_REG(sii->osh, &WLAN_SHIM->a0.ShimMisc) & WLAN_SHIM_MACRO_SOFT_RESET)
			return;	
	}
	...
	if (!IsChipA0()) {
		...
		if(sii->curidx == 1) {					
			/* reset operate on both cores, so need to deal with both cores */
			WLAN_SHIM->a0.ShimMisc = WLAN_SHIM_MACRO_SOFT_RESET;
		}						
	}	
