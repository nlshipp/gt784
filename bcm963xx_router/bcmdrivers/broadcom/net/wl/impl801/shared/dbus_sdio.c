 ...
 *
 * $Id: dbus_sdio.c,v 1.1 2010/08/05 21:59:00 ywu Exp $
 */
...
dbus_sdio_probe_cb(void *handle, const char *desc, uint32 bustype, uint32 hdrlen)
{
	...
	if (handle == &g_probe_info) {
		...
		vi = bcm_iovar_lookup(dbus_sdio_iovars, (char *) desc);
		if (vi == NULL) {
			DBUSERR(("Unsupported probe iovar: %s\n", desc));
			return NULL;
		}
		actionid = IOV_SVAL(vi->varid);

		/* Handle pre-attach() requests */
		switch (actionid) {
			case IOV_SVAL(IOV_DOWNLOAD):
				dbus_sdio_alpclk(g_probe_info.sdh);
				dbus_sdio_download_state(&g_probe_info, (bool) parm2);
				g_probe_info.dl_addr = 0;
			break;
			case IOV_SVAL(IOV_MEMBYTES): { /* Write Bytes: */
				char *buf = parm1;
				int count = parm2;

				dbus_sdio_membytes(&g_probe_info, TRUE, g_probe_info.dl_addr,
					(uint8 *) buf, count);
