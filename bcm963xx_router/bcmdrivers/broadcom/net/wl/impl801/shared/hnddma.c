 ...
 *
 * $Id: hnddma.c,v 1.1 2010/08/05 21:59:00 ywu Exp $
 */
...
#if defined(__mips__) && defined(IL_BIGENDIAN) && !defined(DSLCPE)
...
_dma_rxfill(dma_info_t *di)
{
	...
	for (i = 0; i < n; i++) {
		...
		*/
		if ((p = PKTGET(di->osh, di->rxbufsize + extra_offset + alignment_req - 1, FALSE))
			== NULL) {
