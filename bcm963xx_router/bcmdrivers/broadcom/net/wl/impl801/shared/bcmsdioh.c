 ...
 *
 * $Id: bcmsdioh.c,v 1.1 2010/08/05 21:59:00 ywu Exp $
 */
...
#define BYTEMODE_MAX_LENGTH		(52)	/* WAR for PR14862, 64 maximum */
...
/* XXX Should not be needed (should be based on CIS) */
/* XXX Should F0 size be 32?  (The 64 was cmd53_blk_size default...) */
...
sdioh_init_hostdma(sdioh_info_t *sdioh)
{
	...
	ASSERT(ISALIGNED((uint32)(sdioh->dma_xmt_buf), sizeof(uint32)));
	ASSERT(ISALIGNED((uint32)(sdioh->dma_xmt_desp), sizeof(uint32)));
	ASSERT(ISALIGNED((uint32)(sdioh->dma_rcv_buf), sizeof(uint32)));
	ASSERT(ISALIGNED((uint32)(sdioh->dma_rcv_desp), sizeof(uint32)));
#endif /* BCMDRIVER */
...
#if 0
	/* use word by word 4 bytes access */
	SDIOH_ASSERT((tot_bytes % 4) == 0);
	SDIOH_ASSERT(dma == SDIOH_MODE_PIO);
	for (i = 0; i < tot_bytes; i += 4) {
		if (SDIOH_CMDPASS != sdio_cmd53(sdioh, SDIOH_MODE_PIO, SD_IO_BYTE_MODE, rw, fnc,
		           inc, addr, 4, (base + i))) {
			SDIOH_ERROR(("ERROR!!! buf_pkt failed 0x%x!!\n", addr));
			return SDIOH_FAIL;
		}
	}
...
sdioh_attach(osl_t *osh, void *cfghdl, uint irq)
{
	...
	/* XXX We drive only broadcom hnd devices: read chipid from chipcommon */
...
extern SDIOH_API_RC
sdioh_query_device(sdioh_info_t *sdioh)
{
	return sdioh->target_dev;
}
...
sdioh_cfg_write(sdioh_info_t *sdioh, uint fnc_num, uint32 addr, uint8 *data)
{
	...
	/* PR15618, protect function 0 CIS */
...
#if 0
	if (SDIOH_CMDPASS != sdioh_request_buffer(sdioh, SDIOH_DATA_PIO, SDIOH_DATA_INC, SDIOH_READ,
	                                          SDIO_FUNC_0, addr, 1, length, cis)) {
               ...
               bool abort, bool notpolling)
{
	...
	/* XXX On Normal command completion, don't turn off device interrupts! */
	/* XXX Lifted the set of bits used here from wsu */
	if (cmd_type == SDIOH_CMDTYPE_NORMAL) {
		if (notpolling == FALSE) {
           ...
           uint8 *rspdata)
{
	...
	/* XXX Disable SDIO interrupts during command execution */
...
diag_sdio_sb_buf_loopback(sdioh_info_t *sdioh, bool inc, uint addr, uint tot_bytes)
{
	...
	if ((sdioh->data_buf_tx == NULL) || (sdioh->data_buf_rx == NULL)) {
		...
		ASSERT(ISALIGNED((uint32)(sdioh->data_buf_tx), sizeof(uint32)));
		ASSERT(ISALIGNED((uint32)(sdioh->data_buf_rx), sizeof(uint32)));
...
const bcm_iovar_t sdioh_iovars[] = {
	...
	{"sd_clientreg", IOV_CLIENTREG, 0,	IOVT_BUFFER,	sizeof(sdreg_t)	},
	{NULL, 0, 0, 0, 0 }
};
