...
brcm_wpscli_status wpscli_wlan_connect_vista(const char *ssid, uint wsec)
{
	...
	TUTRACE((TUTRACE_INFO, "Enter wpscli_connect_vista(ssid=%s, wsec=%d)\n", ssid, wsec));
	...
	TUTRACE((TUTRACE_INFO, "Exit wpscli_connect_vista(ssid=%s, wsec=%d)\n", ssid, wsec));
...
brcm_wpscli_status wpscli_wlan_disconnect_vista()
{
	...
	TUTRACE((TUTRACE_INFO, "Enter wpscli_disconnect_vista\n"));
	...
	TUTRACE((TUTRACE_INFO, "Exit wpscli_disconnect_vista\n"));
...
static brcm_wpscli_status scan_list_acm_to_wl(wl_scan_results_t *pWlScanList, uint32 buf_size, PWLAN_BSS_LIST const pBssidList)
{
	...
	TUTRACE((TUTRACE_INFO, "scan_list_acm_to_wl: copy %d bssinfo, bufferSize=%d, sizeNeeded=%d\n", pBssidList->dwNumberOfItems, buf_size, wl_list_length));
	...
	for (i=0; i < pBssidList->dwNumberOfItems; i++)
	{
		...
		TUTRACE((TUTRACE_INFO, "scan_list_acm_to_wl: entry %d converted, ielength=%d, ssid=%s, \n", i, ielength, pWlBssInfo->SSID));
