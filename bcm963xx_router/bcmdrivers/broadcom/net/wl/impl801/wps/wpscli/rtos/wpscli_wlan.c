...
int join_network(char* ssid, uint8 wsec)
{
	...
	*/
	...
	if ((ret = wpscli_wlh_ioctl_set(WLC_SET_INFRA, (const char *)&infra, sizeof(int))) < 0)
		return ret;

	/* set authentication mode */
	if ((ret = wpscli_wlh_ioctl_set(WLC_SET_AUTH, (const char *)&auth, sizeof(int))) < 0)
		return ret;

	/* set wsec mode */
	if ((ret = wpscli_wlh_ioctl_set(WLC_SET_WSEC, (const char *)&wsec, sizeof(int))) < 0)
		return ret;

	/* set WPA_auth mode */
	if ((ret = wpscli_wlh_ioctl_set(WLC_SET_WPA_AUTH, (const char *)&wpa_auth,
		sizeof(wpa_auth))) < 0)
		return ret;

	/* set ssid */
	if ((ret = wpscli_wlh_ioctl_set(WLC_SET_SSID, (const char *)&ssid_t, sizeof(wlc_ssid_t)))
		== 0) {
		...
		for (retry = 0; retry < WLAN_CONNECTION_TIMEOUT; retry++) {
			...
			if (retry != 0 && retry % 10 == 0) {
				if ((ret = wpscli_wlh_ioctl_set(WLC_SET_SSID, (const char *)&ssid_t,
					sizeof(wlc_ssid_t))) < 0) {
...
int join_network_with_bssid(const char* ssid, uint8 wsec, const char *bssid)
{
	...
	*/
	...
	/* set ssid */
	if ((ret = wpscli_wlh_ioctl_set(WLC_SET_SSID, (const char *)&join_params,
		...
		for (retry = 0; retry < WLAN_CONNECTION_TIMEOUT; retry++) {
			...
			if (retry != 0 && retry % 10 == 0) {
				if ((ret = wpscli_wlh_ioctl_set(WLC_SET_SSID,
					(const char *)&join_params,
					sizeof(wl_join_params_t))) < 0) {
					TUTRACE((TUTRACE_INFO,
						"join_network_with_bssid: Exiting. ret=%d\n", ret));
					return ret;
				}
			}
		}
