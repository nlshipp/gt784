...
/*
		// When STA wps connects to the SoftAP, the ll.sll_addr contains the virtual interface address, while peer_mac is really
		// the physical interface address. They won't match any more and we don't need them to be the same either in this case
		if (memcmp(peer_mac, &(ll.sll_addr), 6)) {
			printf("received frame from wrong AP %s\n",
				(char *)ether_ntoa((struct ether_addr *)(&ll.sll_addr)));
			TUTRACE((TUTRACE_ERR, "wpscli_pktdisp_wait_for_packet: Exiting. status=WPS_STATUS_PKTD_NOT_EAP_PKT\n"));
			return WPS_STATUS_PKTD_NOT_EAP_PKT;
		}
*/
