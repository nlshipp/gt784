 ...
 *
 * $Id: wpscli_api.h,v 1.1 2010/08/05 21:59:03 ywu Exp $
 *
...
brcm_wpscli_status brcm_wpscli_open(const char *dev_if_name,
								    ...
								    brcm_wpscli_status_cb_data status_cb);
...
// SoftAP starts WPS registration to enrollee STA device. Return after the WPS process is completed.
...
brcm_wpscli_status brcm_wpscli_softap_is_wps_window_open(void);

// Start WPS EAP monitor and respond as enrollee
brcm_wpscli_status brcm_wpscli_softap_start_eap_monitoring(const brcm_wpscli_nw_settings *nw_conf, 
														   const char* dev_pin);

// Request to abort WPS EAP monitoring procedure. This should be called before openning WPS window
brcm_wpscli_status brcm_wpscli_softap_abort_eap_monitoring(void);
