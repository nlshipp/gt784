 ...
 *
 * $Id: reg_proto_utils.c,v 1.1 2010/08/05 21:59:03 ywu Exp $
 */
...
reg_proto_generate_dhkeypair(DH **DHKeyPair, BufferObj *pubKey)
{
	...
	TUTRACE((TUTRACE_ERR, "DH generate 1\n"));

	*DHKeyPair = DH_new();

	TUTRACE((TUTRACE_ERR, "DH generate 2\n"));
	if (*DHKeyPair == NULL) {
		TUTRACE((TUTRACE_ERR, "RPROTO: DH_new failed\n"));
		return RPROT_ERR_CRYPTO;
	}
	...
	(*DHKeyPair)->p = BN_new();
	(*DHKeyPair)->g = BN_new();
	TUTRACE((TUTRACE_ERR, "DH generate 3\n"));
	...
	TUTRACE((TUTRACE_ERR, "DH generate 4\n"));
	/* 4. generate the DH key */
	if (WPS_DH_GENERATE_KEY(NULL, *DHKeyPair) == 0) {
		return RPROT_ERR_CRYPTO;
	}

	TUTRACE((TUTRACE_ERR, "DH generate 5\n"));
	/* 5. extract the DH public key */
	len = BN_bn2bin((*DHKeyPair)->pub_key, temp);
	if (0 == len) {
		return RPROT_ERR_CRYPTO;
