 ...
 *
 * $Id: ap_api.c,v 1.2 2010/11/29 05:09:59 khuang Exp $
 */
...
static int get_wps_EncrFlags()
{
    FILE *fp=NULL;
    int val=-1;

    fp=fopen("/var/wps_tmp","r");
    if(fp==NULL)
        return -1;
    fscanf(fp,"%d", &val);
    fclose(fp);
...
static uint32
wps_initReg(void *mc_dev, IN EMode e_currMode,
	IN char *devPwd, IN uint16 devPwdId)
{
	uint32 ret = WPS_SUCCESS;
	...
	case EModeUnconfAp:
	{
		...
		/* encrType */
#ifndef AEI_VDSL_CUSTOMER_QWEST  //hk_wps_qwest          // ACTION_TEC change (to fix WPS issues with Intel Proset clients
		if (devcfg_getAuth(g_mc->mp_info))
...
#else
	        // If both TKIP and AES are enabled then we'll pick up AES [Praveen]
                int val = get_wps_EncrFlags();
        	data16 = WPS_ENCRTYPE_NONE;
        	if(val != -1){
            		if(val & 0x1)       //WEP
                		data16 = WPS_ENCRTYPE_WEP;
            		if(val & 0x2)       //tkip
                		data16 = WPS_ENCRTYPE_TKIP;
            		if(val & 0x4)       //aes
                		data16 = WPS_ENCRTYPE_AES;
        	}
#endif
...
wps_setBeaconIE(WPSAPI_T *g_mc, IN bool b_configured, IN bool b_selRegistrar,
	IN uint16 devPwdId, IN uint16 selRegCfgMethods)
{
	uint32 ret;
	...
	if (data8 == 3) {
		...
		tlv_serialize(WPS_ID_UUID_E, bufObj, p_data8, 16); /* change 16 to SIZE_UUID */
...
wps_createM8AP(WPSAPI_T *g_mc, IN char *cp_ssid)
{
	...
	/* encrType */
#ifndef AEI_VDSL_CUSTOMER_QWEST  //hk_wps_qwest
	if (devcfg_getAuth(g_mc->mp_info))
		data16 = WPS_ENCRTYPE_WEP;
	else if (data16 == WPS_AUTHTYPE_OPEN) {
		if (devcfg_getWep(g_mc->mp_info))
...
wps_createM8Sta(WPSAPI_T *g_mc)
{
	...
	/* nwKey */
	p_nwKey = devcfg_getNwKey(g_mc->mp_info, &nwKeyLen);
	tlv_set(&p_tlvCred->nwKey, WPS_ID_NW_KEY, p_nwKey,
		nwKeyLen);
	/* macAddr */
	p_macAddr = devcfg_getMacAddr(g_mc->mp_info);
	data16 = SIZE_MAC_ADDR;
...
int
wps_upnpDevSSR(WPSAPI_T *g_mc, void *p_cbData, uint32 length, TRANSPORT_TYPE m_transportType)
{
	 ...
	 */
	if (EModeApProxy == devcfg_getConfiguredMode(g_mc->mp_info)) {
		CTlvVersion version;
		...
		tlv_dserialize(&version, WPS_ID_VERSION, bufObj, 0, 0);
		/* Selected Registrar */
		tlv_dserialize(&selReg,
			WPS_ID_SEL_REGISTRAR, bufObj, 0, 0);
		/* Device Password ID */
		tlv_dserialize(&devPwdId,
			WPS_ID_DEVICE_PWD_ID, bufObj, 0, 0);
		/* Selected Registrar Config Methods */

		tlv_dserialize(&selRegCfgMethods,
		               WPS_ID_SEL_REG_CFG_METHODS, bufObj, 0, 0);

		/* UUID-E */
		/* Per 1.0b spec, SetSelectedRegistrar no longer includes the UUID
		 * CTlvUuid uuid = CTlvUuid(WPS_ID_UUID_E, bufObj);
		...
		switch (r->m_sm->m_transportType) {
			case TRANSPORT_TYPE_EAP:
				...
				if (msgType != WPS_ID_MESSAGE_ACK) {
					...
					err = wps_sendMsg(r->g_mc, trType, (char *)inbuffer,
						in_len);
				}
			...
			default:
				...
				if (wps_getUpnpDevGetDeviceInfo(r->g_mc)) {
					...
					err = reg_proto_create_devinforsp(r->m_sm->mps_regData,
						outMsg);
					if (WPS_SUCCESS != err) {
...
out:
	if (inMsg) buffobj_del(inMsg);
	if (outMsg) buffobj_del(outMsg);
