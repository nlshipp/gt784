 ...
 *
 * $Id: reg_sm.c,v 1.1 2010/08/05 21:59:03 ywu Exp $
 */
...
static void* reg_sm_copyEsM7Ap2NewM8Ap(void *esM8Ap, void *esM7Ap, uint32 *err);
...
reg_sm_handleMessage(RegSM *r, BufferObj *msg, BufferObj *outmsg)
{
	...
	case M2:
		...
		if (WPS_SUCCESS != err) {
			...
			retVal = state_machine_send_nack(r->m_sm, WPS_MESSAGE_PROCESSING_ERROR,
				outmsg);
			goto out;
		}
		...
		if (WPS_ID_MESSAGE_M5 != msgType) {
			...
			if (regInfo->p_registrarInfo->primDeviceCategory == 1) {
				if (WPS_ID_MESSAGE_NACK == msgType) {
					uint16 configError = 0xffff;
					err = reg_proto_process_nack(r->m_sm->mps_regData,
						msg, &configError);
					if (configError != WPS_ERROR_NO_ERROR) {
	...
	case M6:
		...
		if (r->m_sm->mps_regData->p_enrolleeInfo->b_ap) {
			...
			if (esM8Ap && esM8Ap->ssid.tlvbase.m_len == 0) {
				/* get settings from M7Ap to new M8Ap */
				if ((newEsM8Ap = reg_sm_copyEsM7Ap2NewM8Ap(
					esM8Ap, encrSettings, &err))) {
					esM8Ap = newEsM8Ap;
				}
			}
...
static void *
reg_sm_copyEsM7Ap2NewM8Ap(void *esM8Ap, void *esM7Ap, uint32 *err)
{
	CTlvEsM8Ap *pM8_tlvEncr = (CTlvEsM8Ap *)esM8Ap;
	CTlvEsM7Ap *pM7_tlvEncr = (CTlvEsM7Ap *)esM7Ap;
	CTlvEsM8Ap *newEsM8Ap;
	CTlvNwKey *nwKey, *p_tlvKey;
	char *cp_data;
	uint16 data16;

	if (!pM8_tlvEncr || !pM7_tlvEncr) {
		*err = WPS_ERR_INVALID_PARAMETERS;
		return NULL;
	}

	newEsM8Ap = reg_msg_m8ap_new();
	if (newEsM8Ap == NULL) {
		*err = WPS_ERR_OUTOFMEMORY;
		return NULL;
	}

	/* copy M7Ap settings to new M8Ap */
	/* ssid */
	cp_data = (char *)(pM7_tlvEncr->ssid.m_data);
	data16 = pM7_tlvEncr->ssid.tlvbase.m_len;
	tlv_set(&newEsM8Ap->ssid, WPS_ID_SSID, (uint8 *)cp_data, data16);

	/* keyMgmt */
	data16 = pM7_tlvEncr->authType.m_data;
	tlv_set(&newEsM8Ap->authType, WPS_ID_AUTH_TYPE, (void *)((uint32)data16), 0);

	/* encrType */
	data16 = pM7_tlvEncr->encrType.m_data;
	tlv_set(&newEsM8Ap->encrType, WPS_ID_ENCR_TYPE, (void *)((uint32)data16), 0);
	...
	nwKey = (CTlvNwKey *)(list_getFirst(pM7_tlvEncr->nwKey));
	if (nwKey == NULL) {
		*err = WPS_ERR_SYSTEM;
		reg_msg_m8ap_del(newEsM8Ap, 0);
		return NULL;
	...
	tlv_set(p_tlvKey, WPS_ID_NW_KEY, nwKey->m_data, data16);
	if (!list_addItem(newEsM8Ap->nwKey, p_tlvKey)) {
		*err = WPS_ERR_OUTOFMEMORY;
	...
	tlv_set(&newEsM8Ap->macAddr, WPS_ID_MAC_ADDR, pM8_tlvEncr->macAddr.m_data, data16);
...
reg_proto_process_m1(RegData *regInfo, BufferObj *msg)
{
	...
	tlv_dserialize(&m.uuid, WPS_ID_UUID_E, msg, SIZE_UUID, 0);
	tlv_dserialize(&m.macAddr, WPS_ID_MAC_ADDR, msg, SIZE_MAC_ADDR, 0);
	tlv_dserialize(&m.enrolleeNonce, WPS_ID_ENROLLEE_NONCE, msg, SIZE_128_BITS, 0);
	tlv_dserialize(&m.publicKey, WPS_ID_PUBLIC_KEY, msg, SIZE_PUB_KEY, 0);
	...
	tlv_dserialize(&m.scState, WPS_ID_SC_STATE, msg, 0, 0);
	...
	tlv_dserialize(&m.devPwdId, WPS_ID_DEVICE_PWD_ID, msg, 0, 0);
	...
	wps_setStaDevName((unsigned char*)m.deviceName.m_data);
	...
	memcpy(regInfo->p_enrolleeInfo->macAddr, m.macAddr.m_data,
		m.macAddr.tlvbase.m_len);
...
reg_proto_create_m2(RegData *regInfo, BufferObj *msg, void *encrSettings)
{
	...
	if (!regInfo->DHSecret) {
		reg_proto_generate_dhkeypair(&regInfo->DHSecret, buf1);
	}

	/* extract the DH public key */
	len = BN_bn2bin(regInfo->DHSecret->pub_key, regInfo->pkr);
	if (0 == len) {
	...
	tlv_serialize(WPS_ID_DEVICE_NAME, msg, regInfo->p_registrarInfo->deviceName,
		strlen(regInfo->p_registrarInfo->deviceName));
