 ...
 *
 * $Id: wps_linux_cmd.c,v 1.1 2010/08/05 21:59:02 ywu Exp $
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
usage(char *prog)
{
	fprintf(stderr,
		...
		"	%s addenrollee [ifname] sta_pin=[pin]\n"
...
static int
wps_addenrollee(char *ifname, char *method)
{
	int uilen;
	char uibuf[256];
	char pin[9] = {0};
...
main(int argc, char *argv[])
{
	...
	/* Do "wps addenrollee [pin|sta_pin=[pin]]" */
	if (!strcmp(command, "addenrollee")) {
		if (wps_addenrollee(ifname, method) != 0)
			goto out;
