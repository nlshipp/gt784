 ...
 *
 * $Id: wps_enr_core.c,v 1.1 2010/08/05 21:59:02 ywu Exp $
 *
 */
...
wpsenr_process(void *app_wksp, char *buf, int len, int msgtype)
{
	...
	if (retVal == WPS_SUCCESS) {
		...
		msg_type = wps_get_ap_msg_type(buf, buf_len);
...
wpsenr_check_timeout(void *app_wksp)
{
	...
	case WPS_ASSOC_STATE_FINDINGAP:
		...
		if (find_pbc == PBC_OVERLAP || find_pbc == PBC_TIMEOUT) {
			...
			rem_wps_ie(NULL, 0);
			wpsenr_osl_proc_states(find_pbc);
			return REG_FAILURE;
		}
	...
	case WPS_ASSOC_STATE_ASSOCIATING:
		...
		if ((now - assoc_state_time) > 10 /* WPS_ENR_SCAN_JOIN_MAX_WAIT_SEC */) {
			...
			/* start wps */
			if (enroll_again && do_enrollement_again())
				return REG_FAILURE;
...
config_init()
{
	...
	info.configMethods = 0x0001;
	info.authTypeFlags = 0x0001;
	info.encrTypeFlags = 0x0004;
