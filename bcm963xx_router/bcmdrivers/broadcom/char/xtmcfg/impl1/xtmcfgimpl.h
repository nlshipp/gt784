...
inline UINT32 PORT_LOG_TO_PHYS(UINT32 LP)
{
    UINT32 ulRet = LP;

    if( XtmOsChipRev() == CHIP_REV_BCM6368B0 )
    {
        UINT32 ulPhysPorts[] = {0, 2, 3, 1};
        ulRet = ulPhysPorts[LP];
    }

    return( ulRet );
}

inline UINT32 PORT_PHYS_TO_LOG(UINT32 PP)
{
    UINT32 ulRet = PP;

    if( XtmOsChipRev() == CHIP_REV_BCM6368B0 )
    {
        UINT32 ulLogPorts[]  = {0, 3, 1, 2};
        ulRet = ulLogPorts[PP];
    }
...
inline UINT32 PORT_PHYS_TX_TO_PHYS_RX(UINT32 TXP)
{
    UINT32 ulRet = TXP;

    if( XtmOsChipRev() == CHIP_REV_BCM6368B0 )
    {
        UINT32 ulPhysTxToPhysRx[]  = {0, 2, 1, 3};
        ulRet = ulPhysTxToPhysRx[TXP];
    }
...
#define DATA_STATUS_ENABLED   	0
#define DATA_STATUS_DISABLED  	1
...
public:
    ...
    BCMXTM_STATUS SetLinkInfo( UINT32 ulLinkMask, UINT32 ulLogicalPort, PXTM_INTERFACE_LINK_INFO pLinkInfo,
...
public:
    ...
    BCMXTM_STATUS SetLinkInfo( PXTM_INTERFACE_LINK_INFO pLinkInfo, UINT8 rxConfig, UINT8 linkInfoConfig,
	 ...
	 BCMXTM_STATUS GetLinkErrorStatus (void) ;
...
#if defined(CONFIG_BCM96368)
	 void SetLinkDataStatus (UINT32 status)
		  { (status == DATA_STATUS_ENABLED) ?
			 XP_REGS->ulTxSarCfg &= ~(1 << (m_ulTxPhysPort+TXSARA_BOND_PORT_DIS_SHIFT-1))
					 :
			 XP_REGS->ulTxSarCfg |= (1 << (m_ulTxPhysPort+TXSARA_BOND_PORT_DIS_SHIFT-1)) ;}
...
#define TIME_BETWEEN_CPE_POLL 150000
#define XTM_BOND_DSL_MONITOR_TIMEOUT_MS 100
#define XTM_BOND_DSL_ERR_DURATION_TIMEOUT_MS 2500 /* multiple of dsl_monitor_timeout */
...
#define BND_RUNNING     1
#define BND_FORCE_RESET 2
#define BND_IDLE        0  /* complete mapping is not yet known -- we should not send
                            * any asm */
...
#ifdef AEI_ADSL_BONDING_NONBONDING_AUTOSWITCH
		BOOL  m_linkup;
		unsigned long  m_lastAsmTimestamp;
#endif
	private:
		FN_XTMRT_REQ	m_pfnXtmrtReq;
		UINT32 			m_ulRegistered;
		...
		UINT32 UpdateResetFsm(int wrongCheck, int newMsgType);
		...
		/* crc-32 routines */
		static UINT32 swapWord(UINT32 word);
		static UINT32 AAL5_crc32_common(UINT32 *payload, UINT32 size, UINT32 remainder);
		static UINT32 AAL5_crc32(UINT32 *payload, UINT32 size_in_UINT32s);
		static void make_crc_table_rev ();
		static UINT32  getCrc32_rev32Bit (UINT32 *pdata, UINT32 size, UINT32 crc);
};
