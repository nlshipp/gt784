 ...
 * Returns      : XTMSTS_SUCCESS if successful or error status.
 ***************************************************************************/
BCMXTM_STATUS XTM_CONNECTION::CreateNetworkDevice( char *pszNetDeviceName )
{
...
BCMXTM_STATUS XTM_CONNECTION::DeleteNetworkDevice( void )
{
    ...
    if( m_ulConnected )
    {
        ...
        SetLinkInfo( 0x0, 0, &LinkInfo, 0, NULL );
    }
...
BCMXTM_STATUS XTM_CONNECTION::ReserveVcidEntries( void )
{
    ...
    for(ulPort = PHY_PORTID_0; ulPort < MAX_INTERFACES && bxStatus==XTMSTS_SUCCESS; ulPort++)
    {
    ...
    pTransmitQParms, UINT32 ulTransmitQParmsSize)
{
         ...
         i++, pTxQParms++ )
    {
        ...
        pTxQId->ulWeightAlg     = pTxQParms->ulWeightAlg;
        pTxQId->ulWeightValue   = pTxQParms->ulWeightValue;
        pTxQId->ulSubPriority   = pTxQParms->ulSubPriority;
        pTxQId->ulQueueSize     = pTxQParms->ulSize;
           ...
           if( pTxQId->ulPortId == ulPortId ) {
              ...
              if ((*pulSstCtrl & SST_MP_EN) == 0) {
                 ...
                 *pulSstBt &= ~(SST_WEIGHT_VALUE_MASK | SST_ALG_WEIGHT_MASK);
                 ...
                 if (m_Cfg.ulWeightAlg == WA_DISABLED)
                 {
                    /* For SP connection, set WGHT_ALG of the SST BT register to CWRR
                     * with WGHT_VAL=1.
                     */
...
void XTM_CONNECTION::ConfigureHwShaper( PXTM_TRANSMIT_QUEUE_PARMS pTxQParms )
{
   ...
   for (ulPhysPort = 0 ; ulPhysPort < MAX_BONDED_LINES ; ulPhysPort++) {
      ...
      if( (*pulSstCtrl & SST_EN) == 0 ) {
         ...
         if (m_Cfg.ulWeightAlg == WA_DISABLED)
            /* For SP connection, pTxQParms->ulSubPriority carries the
             * subpriority value of this queue. It is also used as the
             * queue ID to uniquely identify this queue in the connection.
             */ 
            ulSstCtrl |= (pTxQParms->ulSubPriority << SST_SUB_PRIORITY_SHIFT);
         else
            /* For WFQ connection, each queue has the same subpriority
             * specified in m_Cfg.ulGrpPriority. The value carried by
             * pTxQParms->ulSubPriority is the queue ID to uniquely
             * identify this queue in the connection.
             */
            ulSstCtrl |= (m_Cfg.ulGrpPriority << SST_SUB_PRIORITY_SHIFT);
    ...
    PXTM_TRANSMIT_QUEUE_PARMS pTxQParms )
{
    ...
    if( bxStatus == XTMSTS_SUCCESS )
    {
         ...
         * is rounded up.
         */
        UINT32 ulScaledPcr = ((ulUt * ulPcr) / 10);
        UINT32 ulIp = (ulPcr) ? (ulUsPerSec+(ulScaledPcr/2)) / ulScaledPcr : 0;
        UINT32 ulScaledScr = ((ulUt * ulScr) / 10);
        UINT32 ulIs = (ulScr) ? (ulUsPerSec+(ulScaledScr/2)) / ulScaledScr : 0;
        UINT32 ulScaledMcr = ((ulUt * ulMcr) / 10);
        UINT32 ulIm = (ulMcr && ulMcrEn)
            ? (ulUsPerSec+(ulScaledMcr/2)) / ulScaledMcr : 0;
        UINT32 ulBt = (ulMbs - 1) * (ulIs - ulIp);
...
void XTM_CONNECTION::CheckTransmitQueues( void )
{
    ...
    for( i = 0, pTxQId = m_TxQIds; i < m_ulTxQIdsSize; i++, pTxQId++ )
    {
        ...
        {
            if( pTxQId->ulPortId      == pTxQParms->ulPortId &&
                pTxQId->ulSubPriority == pTxQParms->ulSubPriority &&
                pTxQId->ulPtmPriority == pTxQParms->ulPtmPriority &&
                pTxQId->ulQueueSize   == pTxQParms->ulSize )
            {
              ...
              switch (m_ulVcidsCount[ulVcid])
              {
                 ...
                 case 1:
                    for (j = 0, pTxQId2 = m_TxQIds; j < m_ulTxQIdsSize; j++, pTxQId2++)
                    {
                       ...
                       if (pTxQId2->ulPortId == pTxQId->ulPortId &&
                             (*pulSstCtrl & SST_MP_EN))
                       {
...
#ifdef AEI_VDSL_CUSTOMER_NCS
   /* zero out these values before using or else tx sometimes wont work */
   for (i=0;i<MAX_SHAPERS;i++)
   {
      ulSPP_SstCtrl[i]=0;
      ulSPP_SstBt[i]=0;	
   }
