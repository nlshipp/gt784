 ...
 * $Revision: 1.6 $
 *
 * $Id: SoftDsl.h,v 1.6 2011/07/07 09:10:52 lzhang Exp $
 *
 * $Log: SoftDsl.h,v $
 * Revision 1.6  2011/07/07 09:10:52  lzhang
 * ATU-R system vendor ID field reports "ACTI"  which is not unique to this product.
 *
 * Revision 1.5  2011/06/24 01:52:40  dkhoo
...
#error userspace vectoring not enabled while phy vectoring support is enabled
#endif

#if (!defined(VECTORING) && defined(SUPPORT_VECTORINGD))
#error userspace vectoring enabled but phy does not support vectoring
#endif
#endif
...
#define kG993ImpDetMaxNumBands                  3
...
#define kDslDrvConfigGetIntLDGpio	(8)
#define kDslDrvConfigGetExtLDGpio	(9)
...
typedef struct {
	...
	union {
		...
		struct {
			/* (ulong)-1 means not configured in boardparam */
			/* Otherwise, a ushort with the most significant bit indicating active low when set.*/
			ulong	bpGpioPwr;
			ulong	bpGpioMode;
		};
	};
...
#define kDslInteropCarryOverMask            (kDslInteropRetrainWithVDSL2Only|kDslInteropRetrainFor6314Power)
#else
...
#define kDslVendorIDRegisterLength			10
#else
...
#define kDslEnableRNC                           0x04000000 /* Enables RNC in bonding builds when set */
...
#define kPhyCfg1EnableImpFiltInArqMode         0x00000001  /* enable training impulse gating in Arq mode */
#define kPhyCfg1EnableImpFiltInFecMode         0x00000002  /* enable training impulse gating in FEC mode */
...
#ifdef BONDING_G994_DISCOVERY
    uchar                           pmeId;
#endif
	ushort							maxSNRMargin, coFeature;
	int								QLNMonitoring;
	int                 DslTestECResp;
