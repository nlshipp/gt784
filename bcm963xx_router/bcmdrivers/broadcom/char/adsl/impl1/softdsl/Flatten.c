 ...
 * $Revision: 1.4 $
 *
 * $Id: Flatten.c,v 1.4 2011/05/11 08:28:58 ydu Exp $
 *
 * $Log: Flatten.c,v $
	...
	*dstPtr++ = cmd->command;
	switch (DSL_COMMAND_CODE(cmd->command)) {
...
#ifdef FLATTEN_ADDR_ADJUST
				*dstPtr++ = (long)SDRAM_ADDR_TO_ADSL(cmd->param.dslModeSpec.capabilities.carrierInfoG993p2AnnexA);
#else
				*dstPtr++ =	(long)cmd->param.dslModeSpec.capabilities.carrierInfoG993p2AnnexA;
#endif
			}
#endif
			break;
		...
		case kDslDiagSetupBufDesc:
			...
			*dstPtr++ =	(long)cmd->param.dslDiagBufDesc.descBufAddr;
			*dstPtr++ =	(long)cmd->param.dslDiagBufDesc.bufCnt;
			break;
		...
		case kDslAfeTestCmd1:
			...
			*dstPtr++ = (ulong) cmd->param.dslAfeTestSpec1.type;
			*dstPtr++ = (ulong) cmd->param.dslAfeTestSpec1.param1;
			*dstPtr++ = (ulong) cmd->param.dslAfeTestSpec1.param2;
			break;
			...
			*dstPtr++ = cmd->param.dslPlnSpec.plnCmd;
			if (kDslPLNControlStart == cmd->param.dslPlnSpec.plnCmd) {
				...
				*dstPtr++ = (ulong) cmd->param.dslPlnSpec.itaBinPtr;
			        ...
			        *dstPtr++ = (ulong) cmd->param.dslPlnSpec.inmContinueConfig;
				*dstPtr++ = (ulong) cmd->param.dslPlnSpec.inmInpEqMode;
			}
			else if (kDslINMConfigParams==cmd->param.dslPlnSpec.plnCmd) {
			        *dstPtr++ = (ulong) cmd->param.dslPlnSpec.inmContinueConfig;
				*dstPtr++ = (ulong) cmd->param.dslPlnSpec.inmInpEqMode;
				*dstPtr++ = (ulong) cmd->param.dslPlnSpec.inmIATO;
				*dstPtr++ = (ulong) cmd->param.dslPlnSpec.inmIATS;
			}
			  break;
...
#else
			status->param.dslConnectInfo.buffPtr = (void *) srcPtr[2];
#endif
			srcPtr += 3;
#ifndef DEVICENAME
...
#endif
		case kDslAfeTestStatus:
			status->param.dslAfeTestStatus.type =  *srcPtr++;
			status->param.dslAfeTestStatus.param1 =  *srcPtr++;	
			status->param.dslAfeTestStatus.param2 =  *srcPtr++;				
			break;
...
Public int	FlattenBufferStatusRead(stretchBufferStruct *fBuf, dslStatusStruct *status)
{
	...
	/* Type conversion in CircBufferGetReadPtr() makes it unsuitable for an L value. GCC 4.x catches that problem so we need to change it. */
