...
phy_write_mem(char *addr, char* val, int size)
{
  ...
  for (i = 0; i < size; i++) {
    gdb_mbox->param0 = (long)addr & 0xbfffffff;
    addr++;
    gdb_mbox->param1 = val[i];
...
phy_read_regs(void)
{
  ...
  //printk("phy_read_regs::orig=0x%08lx conv=0x%08lx\n", gdb_mbox->param0, (long)ADSL_ADDR_TO_HOST(gdb_mbox->param0));
  return (char *)gdb_mbox->param0;
}

/* Read some PHY CP0 regs */
static char *
phy_read_cp0_regs(void)
{
  gdb_mbox->gdbCmd = PHY_STUB_READ_CP0_REG;
  while (gdb_mbox->gdbCmd != PHY_STUB_ACK);
  //printk("phy_read_cp0_regs::orig=0x%08lx conv=0x%08lx\n", gdb_mbox->param0, (long)ADSL_ADDR_TO_HOST(gdb_mbox->param0));
  return (char *)gdb_mbox->param0;
}
...
single_step(void)
{
  ...
  long *gp_regs = (long *)phy_read_regs();
  long *cp0_regs = (long *)phy_read_cp0_regs();
  unsigned long cp0_epc = *(long *)(ADSL_ADDR_TO_HOST(cp0_regs + EPC_REG_OFFSET));

  //printk("single_step::cp0_epc=0x%lx\n", cp0_epc);
  //printk("single_step::PhyStack=0x%08lx\n", (long)gp_regs);

  targ = cp0_epc;
  insn.word = *(unsigned int *)(ADSL_ADDR_TO_HOST(targ));
  //printk("single_step::inst=0x%lx\n", insn.word);
  is_branch = is_cond = 0;
  ...
  case spec_op:
    ...
    case jr_op:
      ...
      targ = *(unsigned long *)(ADSL_ADDR_TO_HOST(gp_regs + insn.r_format.rs - 1));
      is_branch = 1;
      break;
    }
    break;
  ...
  if (is_branch) {
    ...
    if (is_cond && targ != (cp0_epc + 8)) {
      ...
      step_bp[i++].val = *(unsigned *)(ADSL_ADDR_TO_HOST(cp0_epc + 8));
      //*(unsigned *)(regs->cp0_epc + 8) = BP;
      phy_write_mem((char *)(cp0_epc + 8), (char *)&BP, sizeof(long));
    }
    step_bp[i].addr = targ;
    step_bp[i].val  = *(unsigned *)(ADSL_ADDR_TO_HOST(targ));
    //*(unsigned *)targ = BP;
    phy_write_mem((char *)targ, (char *)&BP, sizeof(long));
  } else {
    step_bp[0].addr = cp0_epc + 4;
    step_bp[0].val  = *(unsigned *)(ADSL_ADDR_TO_HOST(cp0_epc + 4));
    //printk("single_step::0x%lx: 0x%lx\n", step_bp[0].addr, step_bp[0].val);
    //*(unsigned *)(regs->cp0_epc + 4) = BP;
    phy_write_mem((char *)(cp0_epc + 4), (char *)&BP, sizeof(long));
  }
}
...
send_trap_packet(int first_break)
{
  ...
  long cp0_cause = *(long *)(ADSL_ADDR_TO_HOST(cp0_regs + CAUSE_REG_OFFSET));
  long cp0_epc = *(long *)(ADSL_ADDR_TO_HOST(cp0_regs + EPC_REG_OFFSET));
  long reg30 = *(long *)(ADSL_ADDR_TO_HOST(gp_regs + 29));
  long reg29 = *(long *)(ADSL_ADDR_TO_HOST(gp_regs + 28));
  ...
  if (step_bp[0].addr) {
    ...
    phy_write_mem((char *)step_bp[0].addr, (char *)&step_bp[0].val, sizeof(step_bp[0].val)); 
    step_bp[0].addr = 0;

    if (step_bp[1].addr) {
      //*(unsigned *)step_bp[1].addr = step_bp[1].val;
     phy_write_mem((char *)step_bp[1].addr, (char *)&step_bp[1].val, sizeof(step_bp[1].val)); 
     step_bp[1].addr = 0;
    }
  }
...
parse_protocol(void)
{
  ...
  switch (input_buffer[0])
	...
	gp_regs = (char *)ADSL_ADDR_TO_HOST((long *)phy_read_regs());
	//printk("***PhyIntStack=0x%08lx 0x%08lx\n", (long)gp_regs, (long)phy_read_regs());
	cp0_regs = (char *)ADSL_ADDR_TO_HOST((long *)phy_read_cp0_regs());
	ptr = output_buffer;      
    ...
    case 'G':
      {
	gp_regs = phy_read_regs();
	//printk("WriteRegs::PhyStack=0x%08lx\n", (long)gp_regs);
	cp0_regs = phy_read_cp0_regs();
    ...
    case 'm':
      ...
      if (hexToLong(&ptr, &addr) && *ptr++ == ',' && hexToInt(&ptr, &length)) {
		if(addr & 0x80000000) {	/* Host addresses */
			...
			long mapped_addr;
			addr &= ~0x40000000;	/* Clear WriteBack Cache addresses */
    ...
    case 'c':
      ...
      ptr = &input_buffer[1];
      if (hexToLong(&ptr, &addr)) {
	phy_write_mem(cp0_regs + EPC_REG_OFFSET*sizeof(long), (char *)&addr, sizeof(long));
	phy_write_cp0_reg(EPC_REG_OFFSET);
      }
      gdb_mbox->gdbCmd = PHY_STUB_CONTINUE;
      goto finish_kgdb;
...
BcmAdslCoreGdbTask(void)
{
  ...
  if (gdb_mbox->gdbOn == 1) {
    if (gdb_mbox->gdbCmd == PHY_STUB_TRAP) {
      phy_in_trap = 1;
      send_trap_packet(gdb_mbox->param0);
    }
    else if (gdb_mbox->gdbCmd == PHY_STUB_STOP) { /* Phy is dead, command PHY_STUB_STOP is not cleared */
      printk("*** PHY does not respond. GDB in post-morten mode.\n");
      BcmAdslCoreDiagWriteStatusString(0, "*** PHY does not respond. GDB in post-morten mode.");
      gdb_mbox->gdbCmd = PHY_STUB_TRAP;
      output_buffer[0] = 'S';
      output_buffer[1] = hexchars[9 >> 4];
      output_buffer[2] = hexchars[9 & 0xf];
      output_buffer[3] = 0;
