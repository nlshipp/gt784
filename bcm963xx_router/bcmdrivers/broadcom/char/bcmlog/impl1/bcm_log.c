               ...
               cmdValid = 0;
                ...
                if (!ishex(arg[3])) {
                    ...
                    if ((length = str2val(arg[4])) > 32) {
                        ...
                        if (SPI_STATUS_OK != BcmSpiReserveSlave(busnum, chipsel, clkspeed))
                        {
                            bcmPrint ("Spi device already reserved, clkspeed parameter ignored\n");
                        }
                        BcmSpiSyncTrans(txbuf, rxbuf, 0, length, busnum, chipsel);
                        bcmPrint ("Transmitted:\n");
                        dumpHexData((void *)txbuf, length, UNIT_SIZE_BYTES);
                        bcmPrint ("Received:\n");
                        dumpHexData((void *)rxbuf, length, UNIT_SIZE_BYTES);
                    }
                }
            }
            ...
            else
            {
                ...
                else
                {
                    ...
                    case 'b':
                      if ( 4 == argc )
                      {
                         kerSysBcmSpiSlaveWrite(addr, val, 1);
                      }
                      else
                      {
                         kerSysBcmSpiSlaveRead(addr, &val, 1);
                         bcmPrint("SPI Register read 0x%08lx = 0x%02x\n", addr, (uint8_t)val);
                      }
                      break;
                    case 'h':
                      if ( 4 == argc )
                      {
                         kerSysBcmSpiSlaveWrite(addr, val, 2);
                      }
                      else
                      {
                         kerSysBcmSpiSlaveRead(addr, &val, 2);
                         bcmPrint("SPI Register read 0x%08lx = 0x%04x\n", addr, (uint16_t)val);
                      }
                      break;
                    case 'w':
                      if ( 4 == argc )
                      {
                         kerSysBcmSpiSlaveWrite(addr, val, 4);
                      }
                      else
                      {
                         kerSysBcmSpiSlaveRead(addr, &val, 4);
                         bcmPrint("SPI Register read 0x%08lx = 0x%08x\n", addr, (unsigned int)val);
                      }
                      break;
                    default:
                      BCM_LOG_ERROR(BCM_LOG_ID_LOG, "Incorrect unit size '%s', must be 'b', 'h' or 'w'\n", arg[2]);
...
static int __init bcm_logInit(void) {
...
static void __exit bcm_logExit(void) {
...
#if defined(__KERNEL__)
MODULE_DESCRIPTION("Broadcom Logging Module");
MODULE_LICENSE("Proprietary");

module_init(bcm_logInit);
module_exit(bcm_logExit);
#endif  /* __KERNEL__ */
