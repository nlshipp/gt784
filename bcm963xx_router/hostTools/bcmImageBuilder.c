...
#ifdef AEI_VDSL_CUSTOMER_NCS
static void build_image(int littleendian, char *chipid, char *board, char *release, unsigned int blksize, 
#else
    ...
    char *outputfile, char *cfefile, char *rootfsfile, char *kernelfile, int includecfe)
{
...
#ifdef AEI_VDSL_CUSTOMER_NCS    
    strncpy(tag.signiture_2, release, SIG_LEN_2 -1);
#else      
    ...
    if (!stream_rootfs || !stream_kernel) 
    {
        sprintf(tag.rootfsAddress,"%lu",(uint32_t ) 0);
    ...
    {
        sprintf(tag.rootfsAddress,"%lu",(uint32_t ) (fskerneladdr+TAG_LEN));
        sprintf(tag.rootfsLen,"%d",rootfs_size);
...
#ifdef AEI_VDSL_CUSTOMER_NCS             
          else if (strcmp(longopts[option_index].name, "release") == 0)
             strncpy(release, optarg, RELEASE_LEN);
#endif                
...
#ifdef AEI_VDSL_CUSTOMER_NCS      
      fputs ("  --release <firmware version>Firmware version\n", stderr);
#endif       
...
#ifdef AEI_VDSL_CUSTOMER_NCS
  build_image(littleendian, chipid, boardid, release, blksize, outputfile, cfefile, rootfsfile, kernelfile, includecfe);
#else
