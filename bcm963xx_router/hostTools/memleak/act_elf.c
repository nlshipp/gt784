...
{
    sbs_func_status_bind(MQ_SERVER_PORT);
    return NULL;
}
...
int main(int argc, char *argv[])
{
    ...
    while((ch=getchar()) != EOF){
        ...
        case 'r':
            fclose(g_log_fd);
            g_log_fd = NULL;
            act_memleak_send_mq_op(ACT_MEMLEAK_MQ_OP_CLR, NULL, 0);
            break;
            
        case 'd':
            act_memleak_send_mq_op(ACT_MEMLEAK_MQ_OP_DISP, NULL, 0);
            break;
            
        case 'u':
            act_memleak_send_mq_op(ACT_MEMLEAK_MQ_OP_DISP_UNFREE, NULL, 0);
            break;

        case 'b':
            act_memleak_send_mq_op(ACT_MEMLEAK_MQ_OP_DISP_DBFREE, NULL, 0);
            break;
        ...
        case 'o':
            ...
            act_memleak_send_mq_op(ACT_MEMLEAK_MQ_OP_PROFILE_DISP, NULL, 0);
            break;

        case 'g':
            act_memleak_send_mq_op(ACT_MEMLEAK_MQ_OP_PROFILE_GEO, NULL, 0);
            break;

        case 'q':
            act_memleak_send_mq_op(ACT_MEMLEAK_MQ_OP_QUIT, NULL, 0);
            goto end;
